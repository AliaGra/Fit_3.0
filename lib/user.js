  ...
  if (hasMinData) {
    try {
      ...
      if (goalsRow) {
        ...
        const analysis = bodyGoals.analyzeGoalsVsCurrentState(goalsRow, existing, current, 'measurement_update');
        const prev = goalsRow.goals_analysis || null;
        const notification = bodyGoals.determineNotificationLevel(prev, analysis);
        ...
        if (notification && notification.level === 'notify' && existing.coachId) {
          const text = bodyGoals.buildCoachNotificationText(existing.firstName, notification);
          if (text) await Helpers.safeSend(existing.coachId, text);
        }
      }
    } catch (e) {
...
module.exports = { getByChatId, getStudentsByCoach, getCities, createUser, parseBirthDate, calculateAge, updateField, updateMeasurements, updateActivityProfile, activateInvite, linkCoachByInviteCode, createStudentByInvite };
