...
async function updateMeasurements(chatId, measurements) {
  ...
  if (measurements.weight != null || measurements.waist != null) {
    try {
      const bodyAnalysisAI = require('./ai/bodyAnalysis');
      await bodyAnalysisAI.generateAndSave(chatId, 'measurement_update', measurements);
    } catch (e) {
    ...
    try {
      ...
      if (goalsRow) {
        ...
        if (notification && notification.level === 'notify' && existing.coachId) {
          const text = bodyGoalsModule.buildCoachNotificationText(existing.firstName, notification);
