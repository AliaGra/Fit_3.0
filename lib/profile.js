...
async function askProfileInstagram(chatId) {
  const user = await User.getByChatId(chatId);
  const cur =
    user && user.instagram
      ? '\n\nЗараз: ' + Helpers.escapeHtml(String(user.instagram))
      : '\n\nЗараз: не вказано';
  await State.set(chatId, { step: CONSTANTS.FSM_STATES.PROFILE_EDIT_INSTAGRAM });
  const keyboard = [
...
async function askProfileCycleLength(chatId) {
  ...
  const k = CONSTANTS.CALLBACK_PREFIXES.PROFILE_CYCLE_LEN;
  const keyboard = [
    [
  ...
  await Helpers.sendKeyboard(
    chatId,
    '📅 Середня довжина циклу (днів від початку місячних до очікуваного наступного).',
    keyboard
  );
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.PROFILE_CYCLE_LEN && param) {
    const st = await State.get(chatId);
    if (!st || st.step !== CONSTANTS.FSM_STATES.PROFILE_CYCLE_LEN) return false;
    ...
    await Helpers.safeSend(chatId, '✅ Довжину циклу збережено: ' + n + ' дн.');
    await showProfileCycleMenu(chatId);
    return true;
  }
