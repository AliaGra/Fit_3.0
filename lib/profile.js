...
const COACH_TRAINING_HINT =
  '💡 Якщо обираєш «Групові заняття», далі потрібно буде вказати: які саме групові ти проводиш, у якому закладі (якщо тренажерних клубів кілька) і в який час.\n\n';

function previewGroupDetails(text) {
...
function formatProfileMessage(user) {
  ...
  if (user.role === CONSTANTS.ROLES.COACH) {
    ...
    const gt = Array.isArray(user.coachTrainingTypes) && user.coachTrainingTypes.includes('group');
    if (gt && user.coachGroupTrainingDetails && String(user.coachGroupTrainingDetails).trim()) {
      msg += 'Групові (уточнення): ' + previewGroupDetails(user.coachGroupTrainingDetails) + '\n';
    }
  }
  return msg;
}
...
async function show(chatId) {
  try {
    ...
    try {
      if (user.role === CONSTANTS.ROLES.COACH) {
        const cv = await supabase.getCoachVenues(chatId);
        if (cv && cv.length) message += '\n🏢 Де треную: ' + cv.map((v) => v.nameUa).join(', ') + '\n';
      } else {
        const uvs = await supabase.getUserVenues(chatId);
        if (uvs && uvs.length) message += '\n🏢 Заклад: ' + uvs.map((v) => v.nameUa).join(', ') + '\n';
      }
    } catch (_) {}
    const keyboard = [
    ...
    if (user.role === CONSTANTS.ROLES.COACH) {
      ...
      keyboard.push([{ text: '🏢 Де треную (заклади)', callback_data: CONSTANTS.CALLBACKS.PROFILE_COACH_VENUES }]);
...
async function showProfileGroupTrainingDetailsScreen(chatId) {
  const user = await User.getByChatId(chatId);
  if (!user || user.role !== CONSTANTS.ROLES.COACH) {
    await Helpers.safeSend(chatId, '❌ Доступ тільки для тренера.');
    await show(chatId);
  ...
  const hasGroup = Array.isArray(user.coachTrainingTypes) && user.coachTrainingTypes.includes('group');
  if (!hasGroup) {
    await Helpers.safeSend(chatId, '⚠️ Спочатку обери «Групові заняття» у типах тренувань.');
    await showProfileCoachTrainingTypesEditor(chatId);
    return;
  }
  const cur = String(user.coachGroupTrainingDetails || '').trim();
  const curEsc = cur ? Helpers.escapeHtml(previewGroupDetails(cur)) : '';
  const body =
    '👥 <b>Групові заняття — уточнення</b>\n\n' +
    COACH_TRAINING_HINT +
    (cur ? '<b>Зараз зазначено:</b>\n' + curEsc + '\n\n' : '') +
    'Натисни «Вказати / змінити» і надішли <b>одним повідомленням</b>: які групові заняття проводиш, у якому закладі (якщо клубів кілька) і коли.';
  const keyboard = [
    [{ text: '✏️ Вказати / змінити', callback_data: CONSTANTS.CALLBACKS.PROFILE_GROUP_TRAINING_START_TEXT }],
    [{ text: '⏭️ Пропустити', callback_data: CONSTANTS.CALLBACKS.PROFILE_GROUP_TRAINING_SKIP }],
    [{ text: CONSTANTS.EMOJI.BACK + ' До типів тренувань', callback_data: CONSTANTS.CALLBACKS.PROFILE_EDIT_TRAINING_TYPES }]
  ];
...
async function showProfileCoachTrainingTypesEditor(chatId) {
  ...
  if (hasGroup) {
    keyboard.push([{ text: '👥 Групові — деталі (що / де / коли)', callback_data: CONSTANTS.CALLBACKS.PROFILE_GROUP_TRAINING_OPEN }]);
  }
  ...
  const body =
    ...
    COACH_TRAINING_HINT +
    'Обери один або кілька варіантів. Після вибору «Групові заняття» відкрий розділ «Групові — деталі», щоб описати формати, заклади та час.';
  await Helpers.sendKeyboard(chatId, body, keyboard, { parse_mode: 'HTML' });
}
...
async function askProfileCity(chatId) {
  ...
  await Helpers.safeSend(chatId, '🗺️ Введи область (від 2 літер):');
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.PROFILE_GROUP_TRAINING_OPEN) {
    await showProfileGroupTrainingDetailsScreen(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.PROFILE_GROUP_TRAINING_SKIP) {
    await showProfileCoachTrainingTypesEditor(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.PROFILE_GROUP_TRAINING_START_TEXT) {
    const user = await User.getByChatId(chatId);
    if (!user || user.role !== CONSTANTS.ROLES.COACH) {
      await Helpers.safeSend(chatId, '❌ Доступ тільки для тренера.');
      await show(chatId);
    ...
    if (!Array.isArray(user.coachTrainingTypes) || !user.coachTrainingTypes.includes('group')) {
      await Helpers.safeSend(chatId, '⚠️ Спочатку обери «Групові заняття».');
      await showProfileCoachTrainingTypesEditor(chatId);
      return true;
    }
    await State.set(chatId, { step: CONSTANTS.FSM_STATES.PROFILE_GROUP_TRAINING_DETAILS });
    await Helpers.safeSend(
      chatId,
      '✍️ Надішли <b>одним повідомленням</b>:\n' +
        '• які групові заняття ти проводиш;\n' +
        '• у якому закладі (якщо тренажерних клубів кілька — вкажи для кожного);\n' +
        '• у який час (діні/години).\n\n' +
        'Щоб скасувати ввід — натисни «⬅️ Назад» у меню розділу.',
      { parse_mode: 'HTML' }
    );
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.PROFILE_EDIT_TRAINING_TYPES_TOGGLE && param) {
    ...
    if (key === 'group' && hadGroup && !next.includes('group')) {
      updates.coachGroupTrainingDetails = null;
    }
    ...
    if (addedGroup) await showProfileGroupTrainingDetailsScreen(chatId);
...
async function handleTextMessage(chatId, text) {
  ...
  if (step === CONSTANTS.FSM_STATES.PROFILE_GROUP_TRAINING_DETAILS) {
    const raw = String(text || '').trim();
    if (raw.length > 3500) {
      await Helpers.safeSend(chatId, '⚠️ Занадто довгий текст. Скороти повідомлення (до ~3500 символів).');
      return true;
    }
    const ok = await supabase.updateUser(chatId, { coachGroupTrainingDetails: raw || null });
    if (!ok) await Helpers.safeSend(chatId, '❌ Не вдалося зберегти.');
    else await Helpers.safeSend(chatId, '✅ Збережено.');
    await State.clear(chatId);
    await showProfileGroupTrainingDetailsScreen(chatId);
    return true;
  }
...
module.exports = { show, showEditMenu, handleCallback, handleTextMessage, handleFileMessage, formatProfileMessage };
