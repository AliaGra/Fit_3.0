...
async function buildEditKeyboard(chatId) {
  ...
  rows.push(
    [{ text: '🌸 Цикл (підтвердити початок)', callback_data: CONSTANTS.CALLBACKS.PROFILE_EDIT_CYCLE }],
...
async function showCoachGroupVenueMenu(chatId) {
  ...
  if (!venues.length) {
    await Helpers.sendKeyboard(
      chatId,
      '⚠️ У тебе поки немає привʼязаних закладів.\n\nСпочатку додай заклад у меню «🏢 Де треную (заклади)».',
...
async function showProfileCycleMenu(chatId) {
  ...
  const lines = [
    '🌸 Цикл — ручне підтвердження початку',
    '',
    ...
    '',
    'Підтверджуйте дату початку місячних вручну — це підвищує точність плану.',
    '',
    'Статуси:',
    '• Перименопауза: нерегулярний цикл, адаптивне навантаження.',
    '• Менопауза: точка 12 міс без місячних.',
    '• Постменопауза: лінійніші навантаження + контроль симптомів.'
  ];
  const keyboard = [
    [{ text: '🧬 Змінити статус', callback_data: CONSTANTS.CALLBACKS.PROFILE_CYCLE_STATUS }],
    [{ text: '✅ Підтвердити початок сьогодні', callback_data: CONSTANTS.CALLBACKS.PROFILE_CYCLE_CONFIRM_START }],
    [{ text: '📝 Ввести дату вручну', callback_data: CONSTANTS.CALLBACKS.PROFILE_CYCLE_ENTER_DATE }],
