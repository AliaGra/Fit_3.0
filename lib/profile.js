...
function formatProfileMessage(user) {
  ...
  if (user.role === CONSTANTS.ROLES.COACH) {
    msg += '\nТип тренувань: ' + fmtCoachTrainingTypes(user.coachTrainingTypes) + '\n';
  }
  return msg;
}
...
function buildEditKeyboard(user) {
  const keyboard = [
  ...
  if (user && user.role === CONSTANTS.ROLES.COACH) {
    keyboard.splice(7, 0, [{ text: '💪 Тип тренувань', callback_data: CONSTANTS.CALLBACKS.PROFILE_EDIT_TRAINING_TYPES }]);
  }
  return keyboard;
}
...
async function showEditMenu(chatId) {
  const user = await User.getByChatId(chatId);
  await Helpers.sendKeyboard(chatId, '✏️ Що хочеш змінити?', buildEditKeyboard(user));
}
...
async function showProfileCoachTrainingTypesEditor(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: '💾 Зберегти', callback_data: CONSTANTS.CALLBACKS.PROFILE_EDIT_TRAINING_TYPES_SAVE }],
    [{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: CONSTANTS.CALLBACKS.PROFILE_EDIT_DATA }]
  ];
  await Helpers.sendKeyboard(
    chatId,
    '💪 Тип тренувань\n\nОбери один або кілька варіантів і натисни "Зберегти".',
    keyboard
  );
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.PROFILE_EDIT_TRAINING_TYPES) {
  ...
  if (action === CONSTANTS.CALLBACKS.PROFILE_EDIT_TRAINING_TYPES_TOGGLE && param) {
    ...
    const ok = await supabase.updateUser(chatId, { coachTrainingTypes: Array.from(cur) });
