...
async function adminGetExerciseCycleFlagSummary() {
  try {
    const { data: rows, error } = await getClient()
      ...
      .select('id, is_inversion, is_high_impact');
    if (error) throw error;
    const arr = rows || [];
    let inversion = 0;
    let highImpact = 0;
    ...
    return {
      ...
      highImpact,
      flaggedAny,
      unflagged: Math.max(0, arr.length - flaggedAny)
    };
  } catch (e) {
    console.error('Supabase adminGetExerciseCycleFlagSummary', e.message);
    return { total: 0, inversion: 0, highImpact: 0, flaggedAny: 0, unflagged: 0 };
  }
}
...
async function adminGetExerciseCycleFlagList(kind = 'all', offset = 0, limit = 20) {
  try {
    ...
    let q = getClient()
      .from('exercise_library')
      .select('id, group_level1, group_level2, group_level3, name_ua, name_ru, is_inversion, is_high_impact')
    ...
    else if (k === 'unflagged') q = q.eq('is_inversion', false).eq('is_high_impact', false);
