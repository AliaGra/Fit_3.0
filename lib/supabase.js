...
async function getExercisesByGroup(groupLevel1, groupLevel2) {
  try {
    let q = getClient()
...
async function getSubgroups(groupLevel1) {
  try {
    let q = getClient()
      .from('exercise_library')
      .select('group_level2')
      .eq('group_level1', String(groupLevel1 || ''))
      .not('group_level2', 'is', null);
    q = activeFilter(q);
    const { data: rows, error } = await q;
    if (error) throw error;
    if (!rows || !rows.length) return [];
    ...
    const result = [];
    for (const r of rows) {
      const v = (r.group_level2 || '').toString().trim();
      if (v && !seen[v]) {
        seen[v] = true;
        result.push(v);
      }
