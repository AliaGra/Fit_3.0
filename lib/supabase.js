...
/** Адмін-бот: service role обходить RLS (anon у dashboard/SQL Editor дані бачить, бот — ні). */
function getAdminClient() {
  const serviceKey = (process.env.SUPABASE_SERVICE_ROLE_KEY || '').trim();
  if (serviceKey && SUPABASE_URL) {
    if (!_adminClient) _adminClient = createClient(SUPABASE_URL, serviceKey);
    return _adminClient;
  }
  if (!_adminClientWarned) {
    console.warn(
      'Supabase admin: SUPABASE_SERVICE_ROLE_KEY not set — admin lists use anon key; empty «Користувачі» often means RLS'
    );
    _adminClientWarned = true;
  }
  return getClient();
}
...
function userFromRow(r) {
  ...
  return {
    ...
    phone: r.phone || '',
    telegramUsername: r.telegram_username != null ? String(r.telegram_username) : '',
    ...
    coachServiceModes: Array.isArray(r.coach_service_modes)
      ? r.coach_service_modes.map((x) => String(x))
      : (r.coach_service_modes ? [].concat(r.coach_service_modes).map((x) => String(x)) : []),
...
function userToRow(u) {
  const row = {
    ...
    phone: u.phone || '',
    telegram_username: u.telegramUsername != null && String(u.telegramUsername).trim() !== '' ? String(u.telegramUsername).trim() : null,
    ...
    ads_opt_in: u.adsOptIn !== false
  };
  if (u.coachServiceModes !== undefined) {
    row.coach_service_modes = Array.isArray(u.coachServiceModes)
      ? u.coachServiceModes.map((x) => String(x))
      : u.coachServiceModes
        ? [].concat(u.coachServiceModes).map((x) => String(x))
        : [];
  }
  return row;
}
...
async function insertUser(userData) {
  try {
    ...
    const detail = e && (e.details || e.hint) ? ` (${e.details || e.hint})` : '';
    console.error('Supabase insertUser', e.message + detail);
    return false;
  }
}

async function getWebCabinetOnboarding(tokenHash) {
  try {
    const { data, error } = await getAdminClient()
      .from('web_cabinet_onboarding')
      .select('*')
      .eq('token_hash', String(tokenHash || ''))
      .limit(1);
    if (error) throw error;
    return data && data[0] ? data[0] : null;
  } catch (e) {
    console.error('Supabase getWebCabinetOnboarding', e.message);
    return null;
  }
}

async function confirmWebCabinetOnboarding(tokenHash, chatId, phone) {
  try {
    const { data, error } = await getAdminClient()
      .from('web_cabinet_onboarding')
      .update({
    ...
    console.error('Supabase confirmWebCabinetOnboarding', e.message);
    return null;
  }
}

async function completeWebCabinetOnboarding(tokenHash, chatId, phone) {
  try {
    const updates = {
      chat_id: String(chatId),
      status: 'completed',
    ...
    const { data, error } = await getAdminClient()
      ...
      .in('status', ['contact_confirmed', 'completed'])
      .select('*');
    if (error) throw error;
    return data && data[0] ? data[0] : null;
  } catch (e) {
    console.error('Supabase completeWebCabinetOnboarding', e.message);
    return null;
  }
}

/** Мінімальний рядок INVITE_* (help-bot / універсальний інвайт) — без зайвих колонок. */
function universalInviteUserRow(u) {
  const row = {
    created_at: (u.createdAt instanceof Date) ? u.createdAt.toISOString() : (u.createdAt || new Date()).toISOString(),
    user_id: u.userId || u.chatId || '',
    chat_id: String(u.chatId || ''),
    first_name: u.firstName || 'Інвайт',
    last_name: u.lastName || '',
    city: u.city || '',
    role: u.role || 'student',
    coach_id: u.coachId != null && String(u.coachId).trim() !== '' ? String(u.coachId) : null
  };
  const ob = u.oblast != null ? String(u.oblast).trim() : '';
  if (ob) row.oblast = ob;
  return row;
}

async function insertUniversalInviteUser(userData) {
  try {
    const row = universalInviteUserRow(userData);
    if (!row.chat_id || !row.first_name || !row.role) throw new Error('Missing required: chatId, firstName, role');
    const { error } = await getAdminClient().from('users').insert(row);
    if (error) throw error;
    return { ok: true };
  } catch (e) {
    const detail = e && (e.details || e.hint) ? String(e.details || e.hint) : '';
    console.error('Supabase insertUniversalInviteUser', e.message, detail);
    return { ok: false, error: [e.message, detail].filter(Boolean).join(' ') };
  }
}
...
/** Telegram chat_id (числовий), не технічний код інвайту. */
function isRealTelegramChatId(chatId) {
  return /^\d{5,20}$/.test(String(chatId || '').trim());
}

/** Заготовка інвайту в users (user_id/chat_id INVITE_* або USED_INVITE_*). */
function isInviteTechnicalUserRow(r) {
  if (!r) return false;
  const cid = String(r.chat_id != null ? r.chat_id : r.chatId || '').trim();
  const cUp = cid.toUpperCase();
  if (cUp.startsWith('INVITE_') || cUp.startsWith('USED_INVITE_')) return true;
  // PK = chat_id: реальний Telegram id — завжди показувати в адмінці, навіть якщо user_id лишився INVITE_*.
  if (isRealTelegramChatId(cid)) return false;
  const uid = String((r.user_id != null ? r.user_id : r.userId) || '')
    .trim()
    .toUpperCase();
  return uid.startsWith('INVITE_');
}

/** Запит users для адмін-бота: лише реальні акаунти (PK chat_id = Telegram, не INVITE_*). */
function adminRealUsersQuery() {
  return getAdminClient()
    .from('users')
    .filter('chat_id', 'match', '^[0-9]{5,20}$');
}

/** Список «Користувачі» + лічильник порожнього списку: усі рядки `users` (як раніше; можливі дублікати INVITE + Telegram). */
function adminAllUsersListQuery() {
  return getAdminClient().from('users');
}

function escapeIlikeFragment(s) {
  return String(s || '').replace(/\\/g, '\\\\').replace(/%/g, '\\%').replace(/_/g, '\\_');
}

/** Пошук за ім’ям: «Ім’я Прізвище» або «Прізвище Ім’я». */
async function adminQueryUsersByName(name, limit = 30) {
  const parts = String(name || '')
    .trim()
    .split(/\s+/)
    .filter(Boolean);
  const client = getAdminClient();
  const lim = Math.min(Math.max(1, parseInt(limit, 10) || 30), 30);
  const hits = [];
  const seen = new Set();
  ...
  if (parts.length >= 2) {
    const a = escapeIlikeFragment(parts[0]);
    const b = escapeIlikeFragment(parts.slice(1).join(' '));
    for (const [fn, ln] of [
      [a, b],
      [b, a]
    ]) {
    ...
    const q1 = escapeIlikeFragment(parts[0] || name);
    const { data, error } = await client
      .from('users')
      .select('*')
      .or(`first_name.ilike.%${q1}%,last_name.ilike.%${q1}%`)
      .limit(lim);
    if (error) throw error;
    addRows(data);
  }
...
async function adminGetUserRowByChatId(chatId) {
  const id = String(chatId || '').trim();
  if (!id) return null;
  try {
    const { data: rows, error } = await getAdminClient().from('users').select('*').eq('chat_id', id).limit(1);
    if (error) throw error;
    return rows && rows.length ? rows[0] : null;
  } catch (e) {
    console.error('Supabase adminGetUserRowByChatId', e.message);
    return null;
  }
}

async function adminGetUserRowByUserId(userId) {
  const id = String(userId || '').trim();
  if (!id) return null;
  try {
    const { data: rows, error } = await getAdminClient().from('users').select('*').eq('user_id', id).limit(1);
    if (error) throw error;
    return rows && rows.length ? rows[0] : null;
  } catch (e) {
    console.error('Supabase adminGetUserRowByUserId', e.message);
    return null;
  }
}

/** Після invite/реєстрації: user_id має збігатися з chat_id (для адмінки та пошуку). */
async function syncUserIdToChatId(chatId) {
  const id = String(chatId || '').trim();
  if (!isRealTelegramChatId(id)) return false;
  try {
    const { error } = await getClient().from('users').update({ user_id: id }).eq('chat_id', id);
    if (error) throw error;
    return true;
  } catch (e) {
    console.error('Supabase syncUserIdToChatId', e.message);
    return false;
  }
}
...
async function replaceInviteWithChatId(inviteCode, realChatId) {
  ...
  if (!existing) {
    ...
    const patch = {};
    const pfFirst = String(inviteUser.firstName || '').trim();
    const pfLast = String(inviteUser.lastName || '').trim();
    const pfCity = String(inviteUser.city || '').trim();
    const pfOblast = inviteUser.oblast != null ? String(inviteUser.oblast).trim() : '';
    const techFirst = pfFirst.toLowerCase() === 'інвайт' || pfFirst.toLowerCase() === 'invite';
    const exFirst = String(existing.firstName || '').trim();
    const exTech = exFirst.toLowerCase() === 'інвайт' || exFirst.toLowerCase() === 'invite';
    if (pfFirst && !techFirst && (!exFirst || exTech)) patch.firstName = pfFirst;
    if (pfLast && !String(existing.lastName || '').trim()) patch.lastName = pfLast;
    if (pfCity && !String(existing.city || '').trim()) patch.city = pfCity;
    if (pfOblast && !String(existing.oblast || '').trim()) patch.oblast = pfOblast;
    if (Object.keys(patch).length) {
      const ok = await updateUser(realId, patch);
      if (!ok) console.error('Supabase replaceInviteWithChatId merge prefill', realId);
    }
    await touchUserInviteRegistrationStarted(realId, activatedAt);
  }
  await syncUserIdToChatId(realId);
...
async function updateUser(chatId, updates) {
  try {
    ...
    if (updates.phone !== undefined) row.phone = updates.phone != null ? String(updates.phone).trim() : null;
    ...
    if (updates.telegramUsername !== undefined) {
      row.telegram_username =
        updates.telegramUsername != null && String(updates.telegramUsername).trim() !== ''
          ? String(updates.telegramUsername).trim()
          : null;
    }
    ...
    if (updates.coachServiceModes !== undefined) {
      row.coach_service_modes = Array.isArray(updates.coachServiceModes)
        ? updates.coachServiceModes.map((x) => String(x))
        : (updates.coachServiceModes ? [].concat(updates.coachServiceModes).map((x) => String(x)) : []);
    }
    ...
    if (updates.adsOptIn !== undefined) row.ads_opt_in = updates.adsOptIn !== false;
    if (Object.keys(row).length === 0) return true;
    const id = String(chatId);
    const { data: byChat, error: errChat } = await getClient().from('users').update(row).eq('chat_id', id).select('chat_id');
    if (errChat) throw errChat;
    if (byChat && byChat.length > 0) return true;
    const { data: byUser, error: errUser } = await getClient()
      .from('users')
      .update({ ...row, chat_id: id })
      .eq('user_id', id)
      .select('chat_id');
    if (errUser) throw errUser;
    if (byUser && byUser.length > 0) return true;
    console.error('Supabase updateUser: no row matched chat_id or user_id', id);
    return false;
  } catch (e) {
...
async function adminInsertLog(row) {
  try {
    ...
    const { error } = await getAdminClient().from('admin_log').insert(payload);
    if (error) throw error;
    return true;
  } catch (e) {
...
async function adminFetchUsersInInviteRegistration() {
  try {
    let stateRows = null;
    const { data: d1, error: e1 } = await getAdminClient().from('bot_state').select('chat_id, data').contains('data', { inviteOnboarding: true });
    if (!e1) stateRows = d1;
    else {
      const { data: d2, error: e2 } = await getAdminClient()
        .from('bot_state')
        .select('chat_id, data')
    ...
    const { data: rows, error } = await adminAllUsersListQuery().select(ADMIN_USER_LIST_FIELDS).in('chat_id', ids);
    if (error) throw error;
...
async function adminGetInviteOnboardingChatIds(chatIds) {
  ...
  try {
    const { data, error } = await getAdminClient().from('bot_state').select('chat_id, data').in('chat_id', ids);
    if (error) throw error;
...
function normalizeInviteCode(raw) {
  ...
  if (!c) return '';
  if (c.startsWith('INVITE_')) return c;
  if (/^[A-F0-9]{4,12}$/.test(c)) return 'INVITE_' + c;
  return '';
}

/**
 * Пошук у БД за інвайт-кодом і/або ім’ям (без обмежень adminRealUsersQuery).
 * Повертає технічні рядки INVITE_* і реальних користувачів.
 */
async function adminLookupUsers({ inviteCode, nameQuery, limit = 25 } = {}) {
  const out = [];
  const seen = new Set();
  const addRow = (row, kind, note) => {
    if (!row) return;
    const cid = String(row.chat_id != null ? row.chat_id : row.chatId || '');
    const key = kind + ':' + cid;
    if (!cid || seen.has(key)) return;
    seen.add(key);
    ...
    out.push({
      ...
      _adminKind: kind,
      _adminNote: note || '',
      _inAdminUsersList: kind === 'real_user' && isRealTelegramChatId(cid) && !isInviteTechnicalUserRow(row)
    });
  };

  const code = normalizeInviteCode(inviteCode);
  const name = String(nameQuery || '').trim();
  const rawQuery = String(inviteCode || nameQuery || '').trim();

  try {
    if (/^\d{5,20}$/.test(rawQuery)) {
      const rowChat = await adminGetUserRowByChatId(rawQuery);
      if (rowChat) addRow(rowChat, 'real_user', 'За chat_id');
    }

    if (code) {
      const invRow = await adminGetUserRowByUserId(code);
      const inv = invRow ? userFromRow(invRow) : null;
      if (inv) {
        const invCid = String(inv.chatId || '');
        if (invCid === code) {
          addRow(
            { chat_id: inv.chatId, user_id: inv.userId, first_name: inv.firstName, last_name: inv.lastName, role: inv.role, coach_id: inv.coachId, created_at: inv.createdAt },
            'active_invite',
            'Код не активовано — див. «🔑 Інвайти», не «Користувачі»'
          );
        } else if (invCid.toUpperCase().startsWith('USED_INVITE_')) {
          addRow(
            { chat_id: inv.chatId, user_id: inv.userId, first_name: inv.firstName, last_name: inv.lastName, role: inv.role, coach_id: inv.coachId, created_at: inv.createdAt },
            'used_invite',
            'Інвайт використано'
          );
          const fn = String(inv.firstName || '').trim();
          const ln = String(inv.lastName || '').trim();
          if (fn || ln) {
            const nameHits = await adminQueryUsersByName([fn, ln].filter(Boolean).join(' '), Math.min(limit, 20));
            for (const r of nameHits) {
              if (isRealTelegramChatId(r.chat_id)) {
                addRow(r, 'real_user', 'Ймовірний акаунт після активації (за ім’ям з інвайту)');
              }
            }
          }
        }
    ...
    if (name.length >= 2) {
      const data = await adminQueryUsersByName(name, limit);
      for (const r of data) {
        const cid = String(r.chat_id || '');
        if (isRealTelegramChatId(cid)) addRow(r, 'real_user', 'За іменем');
        else if (cid.toUpperCase() === code) addRow(r, 'active_invite', 'Активний інвайт');
        else if (cid.toUpperCase().startsWith('USED_INVITE_')) addRow(r, 'used_invite', 'Використаний інвайт');
        else if (cid.toUpperCase().startsWith('INVITE_')) addRow(r, 'active_invite', 'Технічний інвайт');
      }
    }
  } catch (e) {
    console.error('Supabase adminLookupUsers', e.message);
  }
  return out;
}

/** Вирівняти user_id і підняти в списку (після hotfix 43cfc73 для старих рядків). */
async function adminEnsureUserVisibleInList(chatId) {
  const id = String(chatId || '').trim();
  if (!isRealTelegramChatId(id)) return { ok: false, reason: 'not_telegram_chat_id' };
  const row = await adminGetUserRowByChatId(id);
  if (!row) return { ok: false, reason: 'user_not_found' };
  const u = userFromRow(row);
  try {
    const { error } = await getAdminClient().from('users').update({ user_id: id }).eq('chat_id', id);
    if (error) throw error;
  } catch (e) {
    console.error('Supabase adminEnsureUserVisibleInList sync', e.message);
    return { ok: false, reason: 'sync_failed' };
  }
  await touchUserInviteRegistrationStarted(id, new Date());
...
async function adminGetUsersPage(offset = 0, limit = 20) {
  try {
    ...
    if (off === 0) {
      // Не заповнювати всю сторінку лише «реєстрація» з bot_state — інакше щойно завершені
      // (напр. chat_id 8525662901) не потрапляють у перші 10 слотів.
      const maxPinnedInReg = 3;
      const inRegAll = await adminFetchUsersInInviteRegistration();
      const inReg = inRegAll.slice(0, maxPinnedInReg);
      const pinnedIds = new Set(inReg.map((u) => String(u.chatId)));

      const { data: rows, error } = await adminAllUsersListQuery()
        .select(ADMIN_USER_LIST_FIELDS)
        .order('created_at', { ascending: false })
        .limit(Math.max(lim + inReg.length, 15));
      if (error) throw error;
      const rest = mapAdminUserRows(rows).filter((u) => !pinnedIds.has(String(u.chatId)));

      const merged = [];
      const seen = new Set();
      for (const u of inReg) {
        if (merged.length >= lim) break;
        const id = String(u.chatId);
        if (seen.has(id)) continue;
        seen.add(id);
        merged.push({ ...u, registrationInProgress: true });
      }
      for (const u of rest) {
        if (merged.length >= lim) break;
        const id = String(u.chatId);
      ...
      return merged.map((u) => ({
        ...u,
        registrationInProgress: !!u.registrationInProgress || flags.has(String(u.chatId))
      }));
    }
    ...
    console.error('Supabase adminGetUsersPage', e.message, e.code || '', e.details || '');
    return [];
  }
}

async function adminGetUserByChatId(chatId) {
  const row = await adminGetUserRowByChatId(chatId);
  return row ? userFromRow(row) : null;
}

async function adminCountUsers() {
  try {
    const { count, error } = await adminAllUsersListQuery().select('*', { count: 'exact', head: true });
    if (error) throw error;
    return count || 0;
  } catch (e) {
...
/** Підрахунок рядків users з user_id INVITE_* для блоку «Статистика». */
async function adminGetInviteInventoryCounts() {
  try {
    const { data: rows, error } = await getAdminClient()
      .from('users')
      .select('user_id, chat_id')
      .ilike('user_id', 'INVITE_%');
    if (error) throw error;
    let activeUnused = 0;
    let usedStubs = 0;
    let other = 0;
    for (const r of rows || []) {
      ...
      if (!uid.toUpperCase().startsWith('INVITE_')) continue;
      if (cid === uid) activeUnused += 1;
      else if (cid.toUpperCase().startsWith('USED_')) usedStubs += 1;
      else other += 1;
    }
    return { activeUnused, usedStubs, other };
  } catch (e) {
    console.error('Supabase adminGetInviteInventoryCounts', e.message);
    return { activeUnused: 0, usedStubs: 0, other: 0 };
  }
}

/**
 * Список для адмінки «🔑 Інвайти»: невикористані (chat_id = user_id = INVITE_*)
 * та технічні заготовки після активації (chat_id = USED_*…, user_id = INVITE_*).
 * Раніше LIKE по chat_id INVITE_% ховав USED_INVITE_* — їх не можна було видалити з бота.
 */
  ...
  try {
    ...
    const mapped = (rows || [])
      .map((r) => {
        const uid = String(r.user_id || '').trim();
        const cid = String(r.chat_id || '').trim();
        if (!uid.toUpperCase().startsWith('INVITE_')) return null;
        const activeUnused = cid === uid;
        const usedStub = cid.toUpperCase().startsWith('USED_') && cid !== uid;
        if (!activeUnused && !usedStub) return null;
        return {
          chatId: cid,
          userId: uid,
          usedStub,
      ...
      .filter(Boolean);
    mapped.sort((a, b) => {
      if (a.usedStub !== b.usedStub) return a.usedStub ? 1 : -1;
      const ta = a.createdAt instanceof Date ? a.createdAt.getTime() : 0;
      const tb = b.createdAt instanceof Date ? b.createdAt.getTime() : 0;
      return tb - ta;
    });
    return mapped.slice(off, off + lim);
  } catch (e) {
...
async function adminSetExerciseCycleFlags(exerciseId, payload = {}) {
  try {
    ...
    const { error } = await getAdminClient().from('exercise_library').update(updates).eq('id', id);
    if (error) throw error;
    return true;
  } catch (e) {
...
async function deleteScheduleSlot(slotId) {
  try {
    ...
    if (!id) return false;
    const { error } = await getClient().from('workout_schedule').delete().eq('id', id);
    if (error) throw error;
    return true;
  } catch (e) {
    console.error('Supabase deleteScheduleSlot', e.message);
    return false;
  }
}
...
async function wasReminderSent(slotId, kind = '2h', slotKey = '') {
  try {
    let q = getClient()
      .from('reminders_sent')
      .select('slot_id')
      .eq('slot_id', String(slotId))
      .eq('kind', String(kind || '2h'));
    if (slotKey) q = q.eq('slot_key', String(slotKey));
...
async function insertReminderSent(slotId, kind = '2h', slotKey = '') {
  try {
    const row = {
    ...
    const { error } = await getClient()
      .from('reminders_sent')
      .upsert(row, { onConflict: 'slot_id,kind,slot_key' });
    if (error) throw error;
    return true;
  } catch (e) {
...
async function supportGenerateUniversalInvite({ operatorChatId, prefill } = {}) {
  ...
  try {
    ...
    while (attempts < MAX_ATTEMPTS) {
      ...
      if (!exists) {
        const inserted = await insertUniversalInviteUser({
          chatId: code,
          userId: code,
          role: 'student',
        ...
        if (!inserted.ok) {
          lastError = inserted.error || 'Failed to insert invite user row';
    ...
    return { error: 'Cannot generate unique invite code' };
  } catch (e) {
    const msg = e && e.message ? e.message : String(e);
    console.error('Supabase supportGenerateUniversalInvite', msg);
    return { error: lastError || msg };
  }
}
...
function buildVenueUpdateRow(patch = {}) {
...
async function updateVenue(venueId, patch = {}) {
  try {
    const row = buildVenueUpdateRow(patch);
...
/** Оновлення контактів закладу власником (service role + перевірка venue_managers). */
async function updateVenueByManager(managerChatId, venueId, patch = {}) {
  try {
    if (!(await isVenueManager(managerChatId, venueId))) return false;
    const row = buildVenueUpdateRow(patch);
    const { error } = await getAdminClient().from('venues').update(row).eq('id', String(venueId));
    if (error) throw error;
    return true;
  } catch (e) {
    console.error('Supabase updateVenueByManager', e.message);
    return false;
  }
}

async function replaceVenueFacetsWithClient(venueId, facets, client) {
  const vid = String(venueId);
  const { error: delErr } = await client.from('venue_facets').delete().eq('venue_id', vid);
  ...
  const { error } = await client.from('venue_facets').insert(rows);
  if (error) throw error;
  return true;
}
...
async function replaceVenueFacets(venueId, facets = []) {
  try {
    await replaceVenueFacetsWithClient(venueId, facets, getClient());
    return true;
  } catch (e) {
...
async function replaceVenueFacetsByManager(managerChatId, venueId, facets = []) {
  try {
    if (!(await isVenueManager(managerChatId, venueId))) return false;
    await replaceVenueFacetsWithClient(venueId, facets, getAdminClient());
    return true;
  } catch (e) {
    console.error('Supabase replaceVenueFacetsByManager', e.message);
    return false;
  }
}
...
function normalizeCoachVenueLinkOpts(opts) {
  if (typeof opts === 'boolean') {
    return { isPrimary: opts, teachesHere: true, trainsHere: undefined };
  }
  const o = opts && typeof opts === 'object' ? opts : {};
  return {
    isPrimary: !!o.isPrimary,
    teachesHere: o.teachesHere,
    trainsHere: o.trainsHere
  };
}

async function getCoachVenueLinkRow(coachChatId, venueId) {
  try {
    const { data, error } = await getClient()
      .from('coach_venues')
      .select('venue_id, is_primary, teaches_here, trains_here')
    ...
    console.error('Supabase getCoachVenueLinkRow', e.message);
    return null;
  }
}

async function linkCoachVenue(coachChatId, venueId, opts = false) {
  try {
    const { isPrimary, teachesHere, trainsHere } = normalizeCoachVenueLinkOpts(opts);
    const existing = await getCoachVenueLinkRow(coachChatId, venueId);
    const row = {
      coach_chat_id: String(coachChatId),
      venue_id: String(venueId),
      is_primary: isPrimary || !!(existing && existing.is_primary),
      teaches_here:
        teachesHere !== undefined ? !!teachesHere : existing ? !!existing.teaches_here : true,
      trains_here: trainsHere !== undefined ? !!trainsHere : existing ? !!existing.trains_here : false
    };
...
async function unlinkCoachVenue(coachChatId, venueId, opts = {}) {
  try {
    const o = opts && typeof opts === 'object' ? opts : {};
    const clearTeach = !!o.teachesHere;
    const clearTrain = !!o.trainsHere;
    const existing = await getCoachVenueLinkRow(coachChatId, venueId);
    if (!existing) return true;
    const nextTeach = clearTeach ? false : !!existing.teaches_here;
    const nextTrain = clearTrain ? false : !!existing.trains_here;
    if (!nextTeach && !nextTrain) {
      const { error } = await getClient()
        .from('coach_venues')
        .delete()
        .eq('coach_chat_id', String(coachChatId))
    ...
    const row = {
      ...
      teaches_here: nextTeach,
      trains_here: nextTrain
    };
    const { error } = await getClient().from('coach_venues').upsert(row, { onConflict: 'coach_chat_id,venue_id' });
    if (error) throw error;
    return true;
...
async function getCoachVenues(coachChatId, filter = {}) {
  try {
    ...
    if (error) throw error;
    let filtered = links || [];
    if (filter.teachesOnly) filtered = filtered.filter((l) => !!l.teaches_here);
    if (filter.trainsOnly) filtered = filtered.filter((l) => !!l.trains_here);
    const ids = filtered.map((l) => l.venue_id);
    if (!ids.length) return [];
    ...
    const meta = new Map(
      filtered.map((l) => [
        String(l.venue_id),
        {
          ...
          teachesHere: !!l.teaches_here,
          trainsHere: !!l.trains_here
        }
      ])
    );
    return (rows || []).map((r) => {
      ...
      v.isPrimary = m.isPrimary || false;
      v.teachesHere = !!m.teachesHere;
      v.trainsHere = !!m.trainsHere;
      return v;
    });
  } catch (e) {
...
function getCoachVenuesWhereTeach(coachChatId) {
  return getCoachVenues(coachChatId, { teachesOnly: true });
}

function getCoachVenuesWhereTrain(coachChatId) {
  return getCoachVenues(coachChatId, { trainsOnly: true });
}
...
async function listVenuesForManager(chatId) {
  try {
    const id = String(chatId || '').trim();
    if (!id) return [];
    const { data, error } = await getAdminClient()
      .from('venue_managers')
      .select('venue_id')
      .eq('manager_chat_id', id);
    if (error) throw error;
    const ids = [...new Set((data || []).map((r) => String(r.venue_id || '').trim()).filter(Boolean))];
    if (!ids.length) return [];
    const { data: rows, error: e2 } = await getClient().from('venues').select('*').in('id', ids);
    if (e2) throw e2;
    return (rows || []).map((r) => venueFromRow(r, []));
  } catch (e) {
    console.error('Supabase listVenuesForManager', e.message);
    return [];
  }
}

async function getVenueIdForManager(chatId) {
  try {
    const id = String(chatId || '').trim();
    if (!id) return null;
    const { data, error } = await getAdminClient()
      ...
      .maybeSingle();
    if (error) throw error;
    return data && data.venue_id ? String(data.venue_id) : null;
  } catch (e) {
    console.error('Supabase getVenueIdForManager', e.message);
    return null;
  }
}

async function isVenueManager(chatId, venueId) {
  try {
    const mid = String(chatId || '').trim();
    const vid = String(venueId || '').trim();
    if (!mid || !vid) return false;
    const { data, error } = await getAdminClient()
      .from('venue_managers')
      .select('manager_chat_id')
      .eq('manager_chat_id', mid)
      .eq('venue_id', vid)
      .limit(1)
      .maybeSingle();
    if (error) throw error;
    ...
    console.error('Supabase isVenueManager', e.message);
    return false;
  }
}

async function assignVenueManager(managerChatId, venueId, role = 'owner') {
  try {
    const mid = String(managerChatId || '').trim();
    const vid = String(venueId || '').trim();
    if (!mid || !vid) return false;
    const { error } = await getAdminClient()
      .from('venue_managers')
      .upsert(
        { manager_chat_id: mid, venue_id: vid, role: role === 'staff' ? 'staff' : 'owner' },
        { onConflict: 'manager_chat_id,venue_id' }
      );
    if (error) throw error;
    return true;
  } catch (e) {
    console.error('Supabase assignVenueManager', e.message);
    return false;
  }
}

/** Список менеджерів закладу (за замовч. role=owner). */
async function listVenueManagers(venueId, roleFilter = 'owner') {
  try {
    const vid = String(venueId || '').trim();
    if (!vid) return [];
    let q = getAdminClient().from('venue_managers').select('manager_chat_id, venue_id, role, created_at').eq('venue_id', vid);
    if (roleFilter) q = q.eq('role', roleFilter);
    const { data, error } = await q.order('created_at', { ascending: true });
    if (error) throw error;
    return (data || []).map((r) => ({
      managerChatId: String(r.manager_chat_id || ''),
      venueId: String(r.venue_id || ''),
      role: String(r.role || 'owner'),
      createdAt: r.created_at ? new Date(r.created_at) : null
    }));
  } catch (e) {
    console.error('Supabase listVenueManagers', e.message);
    return [];
  }
}

async function countVenueManagerLinks(managerChatId) {
  try {
    const mid = String(managerChatId || '').trim();
    if (!mid) return 0;
    const { count, error } = await getAdminClient()
      .from('venue_managers')
      .select('manager_chat_id', { count: 'exact', head: true })
      .eq('manager_chat_id', mid);
    if (error) throw error;
    return count || 0;
  } catch (e) {
    console.error('Supabase countVenueManagerLinks', e.message);
    return 0;
  }
}

/** Зняти привʼязку менеджера до закладу (заклад не видаляється). */
async function removeVenueManager(managerChatId, venueId) {
  try {
    const mid = String(managerChatId || '').trim();
    const vid = String(venueId || '').trim();
    if (!mid || !vid) return false;
    ...
    console.error('Supabase removeVenueManager', e.message);
    return false;
  }
}

/**
 * Адмін: зняти власника з закладу. Якщо інших закладів немає — роль → student.
 * @returns {{ ok: boolean, roleDemoted?: boolean }}
 */
async function adminUnassignVenueOwner(managerChatId, venueId) {
  const mid = String(managerChatId || '').trim();
  const vid = String(venueId || '').trim();
  if (!mid || !vid) return { ok: false };
  const removed = await removeVenueManager(mid, vid);
  if (!removed) return { ok: false };
  const remaining = await countVenueManagerLinks(mid);
  let roleDemoted = false;
  if (remaining === 0) {
    const u = await getUserByChatId(mid);
    if (u && String(u.role || '').toLowerCase() === 'venue_owner') {
      const okRole = await updateUser(mid, { role: 'student' });
      roleDemoted = !!okRole;
    }
  }
  return { ok: true, roleDemoted };
}

async function setCoachVenueListingVisible(coachChatId, venueId, visible) {
  try {
    const existing = await getCoachVenueLinkRow(coachChatId, venueId);
    if (!existing || !existing.teaches_here) return false;
    const row = {
      coach_chat_id: String(coachChatId),
      venue_id: String(venueId),
      is_primary: !!existing.is_primary,
      teaches_here: true,
      trains_here: !!existing.trains_here,
      listing_visible: !!visible
    };
    const { error } = await getAdminClient().from('coach_venues').upsert(row, { onConflict: 'coach_chat_id,venue_id' });
    if (error) throw error;
    return true;
  } catch (e) {
    console.error('Supabase setCoachVenueListingVisible', e.message);
    return false;
  }
}

function mapVenueCoachUsers(links, users) {
  const isPrimary = new Map((links || []).map((x) => [String(x.coach_chat_id), !!x.is_primary]));
  const listingVisible = new Map((links || []).map((x) => [String(x.coach_chat_id), x.listing_visible !== false]));
  return (users || [])
    ...
    .map((u) => ({
      ...
      coachTrainingTypes: Array.isArray(u.coach_training_types)
        ...
        : u.coach_training_types
          ? [].concat(u.coach_training_types).map((x) => String(x))
          : [],
      isPrimary: isPrimary.get(String(u.chat_id || '')) === true,
      listingVisible: listingVisible.get(String(u.chat_id || '')) !== false
    }));
}

/** Групові заняття тренерів закладу (лише listing_visible + teaches_here). */
async function listVenueCoachGroupClassLinks(venueId) {
  try {
    const coaches = await listVenueCoaches(venueId);
    if (!coaches.length) return [];
    const byId = new Map(coaches.map((c) => [String(c.chatId), c]));
    const ids = [...byId.keys()];
    const vid = String(venueId || '').trim();
    const { data, error } = await getClient()
      .from('coach_group_classes')
      .select('coach_chat_id, group_class_code, label_ua')
      .eq('venue_id', vid)
      .in('coach_chat_id', ids)
      .order('group_class_code', { ascending: true });
    if (error) throw error;
    return (data || []).map((r) => {
      const cid = String(r.coach_chat_id || '');
    ...
    console.error('Supabase listVenueCoachGroupClassLinks', e.message);
    return [];
  }
}
...
async function listVenueCoaches(venueId) {
  try {
    ...
    const { data: links, error } = await getClient()
      .from('coach_venues')
      .select('coach_chat_id, is_primary, listing_visible')
      .eq('venue_id', vid)
      .eq('teaches_here', true)
    ...
    const { data: users, error: e2 } = await getClient()
      ...
      .in('chat_id', ids);
    if (e2) throw e2;
    return mapVenueCoachUsers(links, users);
  } catch (e) {
...
/** Усі тренери з teaches_here (для панелі власника закладу). */
async function listVenueCoachesForManager(venueId) {
  try {
    const vid = String(venueId || '').trim();
    if (!vid) return [];
    const { data: links, error } = await getAdminClient()
      .from('coach_venues')
      .select('coach_chat_id, is_primary, listing_visible')
      .eq('venue_id', vid)
      .eq('teaches_here', true);
    ...
    const ids = (links || []).map((x) => String(x.coach_chat_id || '')).filter(Boolean);
    if (!ids.length) return [];
    const { data: users, error: e2 } = await getAdminClient()
      .from('users')
      .select('chat_id, first_name, last_name, instagram, role, coach_training_types')
      .in('chat_id', ids);
    if (e2) throw e2;
    ...
    console.error('Supabase listVenueCoachesForManager', e.message);
    return [];
  }
}
...
function isNumericTelegramChatId(id) {
...
async function listStudentIdsWithUpcomingBookings(studentChatIds) {
  const ids = (studentChatIds || []).map((x) => String(x).trim()).filter(isNumericTelegramChatId);
  if (!ids.length) return [];
  ...
  try {
    for (const part of chunkIds(ids, 100)) {
      const { data, error } = await getClient()
        ...
        .in('student_id', part)
        .gte('date', today)
        .in('status', ['REQUESTED', 'BOOKED', 'RESERVED']);
      if (error) throw error;
      for (const r of data || []) {
    ...
    console.error('Supabase listStudentIdsWithUpcomingBookings', e.message);
  }
  return [...found];
}

async function listStudentIdsWithRecentWorkouts(studentChatIds, days = 30) {
  const ids = (studentChatIds || []).map((x) => String(x).trim()).filter(isNumericTelegramChatId);
  if (!ids.length) return [];
  const from = new Date(Date.now() - Number(days) * 24 * 60 * 60 * 1000);
  const fromStr = from.toISOString();
  const fromDate = new Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/Kyiv' }).format(from);
  const found = new Set();
  try {
    for (const part of chunkIds(ids, 100)) {
      const { data: logs, error: e1 } = await getClient()
        .from('bot_training_data')
        .select('chat_id')
        .in('chat_id', part)
        .gte('date', fromStr);
      ...
      const { data: slots, error: e2 } = await getClient()
        .from('workout_schedule')
        .select('student_id')
        .in('student_id', part)
        .eq('status', 'COMPLETED')
    ...
    console.error('Supabase listStudentIdsWithRecentWorkouts', e.message);
  }
  return [...found];
}

/**
 * Потенційні отримувачі розсилки (учні в місті).
 * @param {{ city: string, excludeChatId?: string, excludeCoachId?: string, requireNoActiveCoach?: boolean, excludeVenueId?: string, adsOnly?: boolean }} p
 */
async function listBroadcastPotentialChatIds(p = {}) {
  const city = String(p.city || '').trim();
  if (!city) return [];
  const excludeChatId = String(p.excludeChatId || '').trim();
  const excludeCoachId = String(p.excludeCoachId || '').trim();
  const excludeVenueId = String(p.excludeVenueId || '').trim();
  const requireNoActiveCoach = p.requireNoActiveCoach === true;
  const adsOnly = p.adsOnly !== false;
  try {
    let q = getClient()
      .from('users')
      .select('chat_id, role, coach_id, is_archived, ads_opt_in, city')
      .eq('role', 'student')
      .eq('city', city)
      .not('chat_id', 'like', 'INVITE_%')
      .or('is_blocked.is.null,is_blocked.eq.false')
    ...
    if (error) throw error;
    let venueLinked = new Set();
    if (excludeVenueId) {
      const { data: links, error: e2 } = await getClient()
        .from('user_venues')
        .select('user_chat_id')
        .eq('venue_id', excludeVenueId)
        .limit(5000);
      if (e2) throw e2;
      venueLinked = new Set((links || []).map((r) => String(r.user_chat_id || '').trim()).filter(Boolean));
    }
    const out = [];
    const seen = new Set();
    for (const r of data || []) {
      const id = String(r.chat_id || '').trim();
      if (!id || seen.has(id) || !isNumericTelegramChatId(id)) continue;
      if (id === excludeChatId) continue;
      if (adsOnly && r.ads_opt_in === false) continue;
      const coachId = r.coach_id != null ? String(r.coach_id).trim() : '';
      if (excludeCoachId && coachId === excludeCoachId) continue;
      if (requireNoActiveCoach) {
        const archived = r.is_archived === true;
        if (coachId && !archived) continue;
      }
      if (excludeVenueId && venueLinked.has(id)) continue;
      seen.add(id);
      out.push(id);
    }
    return out;
  } catch (e) {
    console.error('Supabase listBroadcastPotentialChatIds', e.message);
    return [];
  }
}

async function listBroadcastLogSince(senderChatId, sinceIso, venueId = null) {
  const sid = String(senderChatId || '').trim();
  if (!sid || !sinceIso) return [];
  try {
    let q = getClient()
      .from('broadcast_log')
      .select('id, sender_chat_id, sender_role, venue_id, audience_type, city, sent_count, skipped_count, failed_count, created_at')
      .eq('sender_chat_id', sid)
      .gte('created_at', sinceIso)
      .order('created_at', { ascending: false })
      .limit(50);
    if (venueId) q = q.eq('venue_id', String(venueId));
    const { data, error } = await q;
    if (error) throw error;
    return (data || []).map((r) => ({
      id: r.id,
      senderChatId: String(r.sender_chat_id || ''),
      senderRole: String(r.sender_role || ''),
      venueId: r.venue_id ? String(r.venue_id) : '',
    ...
    console.error('Supabase listBroadcastLogSince', e.message);
    return [];
  }
}

async function insertBroadcastLog(row) {
  try {
    const payload = {
      sender_chat_id: String(row.senderChatId || ''),
      sender_role: String(row.senderRole || ''),
      venue_id: row.venueId ? String(row.venueId) : null,
      audience_type: String(row.audienceType || ''),
      city: row.city ? String(row.city) : null,
      body: String(row.body || '').slice(0, 2000),
      sent_count: Number(row.sentCount) || 0,
      skipped_count: Number(row.skippedCount) || 0,
      failed_count: Number(row.failedCount) || 0
    };
    if (!payload.sender_chat_id || !payload.audience_type) return null;
    const client = getAdminClient();
    const { data, error } = await client.from('broadcast_log').insert(payload).select('id').maybeSingle();
    if (error) throw error;
    return data && data.id ? data.id : true;
  } catch (e) {
    console.error('Supabase insertBroadcastLog', e.message);
    return null;
  }
}
...
module.exports = {
  ...
  userToRow,
  insertUser,
  getWebCabinetOnboarding,
  confirmWebCabinetOnboarding,
  completeWebCabinetOnboarding,
  updateUser,
  getAllCities,
  searchOblasts,
  ...
  insertVenue,
  updateVenue,
  updateVenueByManager,
  replaceVenueFacets,
  replaceVenueFacetsByManager,
  getVenueById,
  listVenuesForAdmin,
  deleteVenueCascade,
  getVenueHours,
  ...
  linkCoachVenue,
  unlinkCoachVenue,
  getCoachVenueLinkRow,
  getCoachVenues,
  getCoachVenuesWhereTeach,
  getCoachVenuesWhereTrain,
  ...
  getVenueIdForManager,
  listVenuesForManager,
  isVenueManager,
  assignVenueManager,
  listVenueManagers,
  countVenueManagerLinks,
  removeVenueManager,
  adminUnassignVenueOwner,
  setCoachVenueListingVisible,
  listVenueCoaches,
  listVenueCoachGroupClassLinks,
  listVenueCoachesForManager,
  ...
  listStudentIdsWithUpcomingBookings,
  listStudentIdsWithRecentWorkouts,
  listBroadcastPotentialChatIds,
  listBroadcastLogSince,
  insertBroadcastLog,
