...
async function getExerciseDetailById(exerciseId) {
  try {
    let q = getClient()
      .from('exercise_library')
      .select('id, group_level1, group_level2, group_level3, name_ua, name_ru, equipment, focus_point, common_mistakes, proper_feeling, static_holds, youtube_link, my_channel_link, vid, difficulty')
      .eq('id', parseInt(String(exerciseId), 10))
      .limit(1);
    q = activeFilter(q);
    const { data: rows, error } = await q;
