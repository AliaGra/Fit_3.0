...
async function adminGetUsersPage(offset = 0, limit = 20) {
  try {
    ...
    const { data: rows, error } = await getClient()
      .from('users')
      .select('chat_id, first_name, last_name, role, coach_id, created_at, is_archived, is_blocked')
      .order('created_at', { ascending: false })
      .range(off, off + lim - 1);
    if (error) throw error;
...
async function supportGenerateUniversalInvite({ operatorChatId } = {}) {
    ...
    while (attempts < MAX_ATTEMPTS) {
      ...
      if (!exists) {
        const ok = await insertUser({
          ...
          userId: code,
          role: 'student',
          // users.first_name is required in insertUser validation
          firstName: 'Інвайт',
          lastName: '',
          coachId: null,
          createdAt: new Date()
...
async function insertVenueRow(row, facetRows) {
  ...
  if (facetRows && facetRows.length) {
    const { error: e2 } = await getClient().from('venue_facets').insert(facetRows.map((f) => ({ venue_id: vid, facet_kind: f.facetKind, code: f.code })));
    if (e2) throw e2;
  }
  return vid;
}
 ...
 * @param {{ facetKind: string, code: string }[]} [p.facets]
 */
async function insertVenue(p = {}) {
  try {
    ...
    const facetRows = (p.facets || [])
      ...
      .map((f) => ({ facetKind: f.facetKind, code: String(f.code) }));
...
async function replaceVenueFacets(venueId, facets = []) {
  try {
    ...
    const rows = facets
      .filter((f) => f && f.code && f.facetKind)
      .map((f) => ({ venue_id: vid, facet_kind: String(f.facetKind), code: String(f.code) }));
...
async function getVenueById(venueId) {
  try {
    const { data: r, error } = await getClient()
      .from('venues')
      .select('*, venue_facets ( facet_kind, code )')
      .eq('id', String(venueId))
      .maybeSingle();
    if (error) throw error;
    if (!r) return null;
    const facets = (r.venue_facets || []).map((f) => ({ facetKind: f.facet_kind, code: f.code }));
...
async function searchVenues({
  ...
  try {
    let q = getClient().from('venues').select('*, venue_facets ( facet_kind, code )').eq('is_active', true);
