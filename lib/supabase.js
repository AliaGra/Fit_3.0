...
/**
 * Учень тренера за student_id зі слота.
 * Важливо: не використовуємо getStudentsByCoachId — там відфільтровані рядки INVITE+USED,
 * через що учень «зникає» з пошуку. Тягнемо всіх користувачів з coach_id і порівнюємо в JS.
 */
async function getUserStudentForCoach(coachChatId, rawStudentId) {
  const coach = String(coachChatId || '').trim();
  if (!coach || rawStudentId == null || String(rawStudentId).trim() === '') return null;
  const slotNorm = normalizeStudentIdForMatch(rawStudentId);
  if (!slotNorm) return null;

  const slotKeys = new Set([slotNorm]);
  if (!slotNorm.startsWith('INVITE_') && /^[A-F0-9]{4,}$/i.test(slotNorm)) {
    slotKeys.add('INVITE_' + slotNorm);
  }

  try {
    const { data: rows, error } = await getClient().from('users').select('*').eq('coach_id', coach);
    if (error) throw error;
    for (const r of rows || []) {
      const chatN = normalizeStudentIdForMatch(r.chat_id);
      const userN = normalizeStudentIdForMatch(r.user_id);
      for (const k of slotKeys) {
        if (k && (k === chatN || k === userN)) {
          return userFromRow(r);
        }
      }
    }
  } catch (e) {
    console.error('Supabase getUserStudentForCoach', e.message);
  }
  return null;
}

/**
 * Знайти користувача за student_id зі слота (без фільтра по coach): усі варіанти chat_id / user_id.
 * Додає варіанти USED_+INVITE (коли в users chat_id = USED_INVITE_…, а в слоті ще INVITE_…).
 */
async function getUserForScheduleStudentId(rawStudentId) {
  const id = String(rawStudentId || '').trim();
  if (!id) return null;
  const cands = new Set([id, id.toUpperCase(), id.toLowerCase(), normalizeStudentIdForMatch(id)]);
  const up = id.toUpperCase();
  if (!up.startsWith('INVITE_') && /^[A-F0-9]{4,}$/i.test(id)) {
    cands.add('INVITE_' + up);
  }
  if (up.startsWith('INVITE_')) {
    cands.add('USED_' + id);
    cands.add('USED_' + up);
  }
  try {
    for (const c of cands) {
      if (!c) continue;
      const { data: rows, error } = await getClient().from('users').select('*').eq('chat_id', c).limit(1);
      if (error) continue;
      if (rows && rows.length) return userFromRow(rows[0]);
    }
    for (const c of cands) {
      if (!c) continue;
      const { data: rows, error } = await getClient().from('users').select('*').eq('user_id', c).limit(1);
      if (error) continue;
      if (rows && rows.length) return userFromRow(rows[0]);
    }
  } catch (e) {
    console.error('Supabase getUserForScheduleStudentId', e.message);
  }
  return null;
}
...
async function getLatestMeasurementsForGoals(chatId) {
  try {
    const { data, error } = await getClient()
      ...
      .select('weight, waist, glutes, shoulders, chest, neck, wrist')
      .eq('chat_id', String(chatId))
...
module.exports = {
  ...
  findUserByAnyId,
  getUserStudentForCoach,
  getUserForScheduleStudentId,
  markInviteAsUsed,
