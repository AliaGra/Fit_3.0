...
/**
 * Учень тренера за student_id зі слота: фільтр по coach_id + збіг chat_id або user_id.
 * Покриває INVITE_*, USED_INVITE_*, суфікс без префікса тощо.
 */
function buildStudentIdLookupCandidates(rawStudentId) {
  const sid = String(rawStudentId || '').trim();
  if (!sid) return [];
  const upper = sid.toUpperCase();
  const out = new Set([sid, upper]);
  if (upper.startsWith('USED_')) {
    const rest = sid.slice(5);
    if (rest) {
      out.add(rest);
      out.add(rest.toUpperCase());
    }
  }
  if (!upper.startsWith('INVITE_') && /^[A-F0-9]{4,}$/i.test(sid)) {
    out.add('INVITE_' + upper);
  }
...
async function getUserStudentForCoach(coachChatId, rawStudentId) {
  ...
  if (!coach || !rawStudentId) return null;
  const candidates = buildStudentIdLookupCandidates(rawStudentId);
  try {
