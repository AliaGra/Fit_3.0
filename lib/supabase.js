...
function adminRealUsersQuery() {
  return getClient().from('users').not('chat_id', 'ilike', 'INVITE_%').not('chat_id', 'ilike', 'USED_INVITE_%');
}
...
async function adminLookupUsers({ inviteCode, nameQuery, limit = 25 } = {}) {
  ...
  try {
    if (/^\d{5,20}$/.test(rawQuery)) {
      const byChat = await getUserByChatId(rawQuery);
      if (byChat) {
        addRow(
          {
            chat_id: byChat.chatId,
            user_id: byChat.userId,
            first_name: byChat.firstName,
            last_name: byChat.lastName,
            role: byChat.role,
            coach_id: byChat.coachId,
            created_at: byChat.createdAt
          },
          'real_user',
          'За chat_id'
        );
    ...
    if (code) {
      const inv = await getUserByUserId(code);
      if (inv) {
        ...
        if (invCid === code) {
          ...
          if (fn || ln) {
            let q = getClient().from('users').select('*').limit(Math.min(limit, 20));
            if (fn) q = q.ilike('first_name', `%${fn}%`);
            if (ln) q = q.ilike('last_name', `%${ln}%`);
            const { data: nameHits, error: nhErr } = await q;
            if (!nhErr) {
    ...
    if (name.length >= 2) {
      ...
      const q1 = parts[0] || name;
      const q2 = parts.length > 1 ? parts.slice(1).join(' ') : '';
      let query = getClient().from('users').select('*').limit(Math.min(limit, 30));
      if (q2) {
        query = query.ilike('first_name', `%${q1}%`).ilike('last_name', `%${q2}%`);
      } else {
        query = query.or(`first_name.ilike.%${q1}%,last_name.ilike.%${q1}%`);
      }
...
async function adminEnsureUserVisibleInList(chatId) {
  ...
  const u = await getUserByChatId(id);
  if (!u) return { ok: false, reason: 'user_not_found' };
  await syncUserIdToChatId(id);
