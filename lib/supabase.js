...
/** Встановити активний план учня (інші для цього учня — is_active = false). */
async function setPlanActiveForStudent(planId, studentChatId) {
  try {
    ...
    const { error } = await getClient().from('training_plans').update({ is_active: true }).eq('plan_id', planId).eq('student_id', String(studentChatId));
    if (error) throw error;
    return true;
  } catch (e) {
