...
async function deleteVenueGroupClassPrice(venueId, rowId) {
  try {
    const vid = String(venueId || '').trim();
    const id = String(rowId || '').trim();
    if (!vid || !id) return false;
    const { error } = await getClient().from('venue_group_class_prices').delete().eq('id', id).eq('venue_id', vid);
    if (error) throw error;
    return true;
  } catch (e) {
...
async function deleteVenueGymMembershipOffer(venueId, rowId) {
  try {
    const vid = String(venueId || '').trim();
    const id = String(rowId || '').trim();
    if (!vid || !id) return false;
    const { error } = await getClient().from('venue_gym_membership_offers').delete().eq('id', id).eq('venue_id', vid);
    if (error) throw error;
    return true;
  } catch (e) {
...
async function deleteVenueAncillaryService(venueId, rowId) {
  try {
    const vid = String(venueId || '').trim();
    const id = String(rowId || '').trim();
    if (!vid || !id) return false;
    const { error } = await getClient().from('venue_ancillary_services').delete().eq('id', id).eq('venue_id', vid);
    if (error) throw error;
    return true;
  } catch (e) {
