...
async function deleteUserCascade(chatId, targetChatId) {
  ...
  // If coach: detach students (coach_id = null)
  if ((u.role || '').toLowerCase() === 'coach') {
    const students = await supabase.getStudentsByCoachId(String(u.chatId));
    for (const s of students || []) {
      await supabase.updateUser(String(s.chatId), { coachId: null });
    }
  }
  // If student: delete schedule rows and make slots free is not always possible; simplest: delete workout_schedule rows for student_id
  const ok = await supabase.deleteInviteUserAndAllRelatedData(String(u.chatId));
  await supabase.adminInsertLog({
    adminChatId: chatId,
    action: 'user_delete',
    targetUserChatId: String(u.chatId),
    payloadJson: { role: u.role || null }
  });
...
async function route(update) {
  ...
  if (!AdminHelpers.isAdminChat(data.chatId)) return;
