...
async function pickExercisesForDay(dayConfig, userMedConditions, goal, level, gender, options = {}) {
  ...
  function buildAllowed(prevIsolationIds) {
    ...
    for (const ex of exercises) {
      ...
      if (options.menstrualExcludeInversion && ex.isInversion === true) continue;
      if (options.menstrualExcludeHighImpact && ex.isHighImpact === true) continue;
      // Fallback for legacy rows that do not yet have deterministic flags.
      if (options.menstrualExcludeHighImpact && ex.isHighImpact !== true && /стриб|прыжок|стрибок/.test(name)) continue;
      if (mc018Class != null) {
...
async function generateTrainingPlan(studentChatId, options = {}) {
  ...
  if (activityLevel === 'active' || activityLevel === 'very_active') {
    const reduction = activityLevel === 'very_active' ? 2 : 1;
    dayConfigs.forEach((d) => {
      const current = d.exerciseCount != null ? d.exerciseCount : 6;
      d.exerciseCount = Math.max(4, current - reduction);
    });
  }
  ...
  let cycleCtx = null;
  let cycleSymptomStaleDays = null;
  if ((gender || '').toLowerCase() === 'female') {
    const cs = await supabase.getUserCycleSettings(studentChatId);
    const logs = await supabase.listCycleEventLogs(studentChatId, 24);
    cycleCtx = menstrualCycle.resolveTrainingContext(cs, logs, new Date());
    const latestSymptom = await supabase.getLatestCycleSymptomLog(studentChatId);
    if (latestSymptom && latestSymptom.logDate) {
      const d = new Date(String(latestSymptom.logDate).slice(0, 10) + 'T00:00:00');
      if (!isNaN(d.getTime())) {
        cycleSymptomStaleDays = Math.floor((Date.now() - d.getTime()) / 86400000);
      }
    }
    const symptomLoad = menstrualCycle.calcSymptomLoad(latestSymptom);
    if (cycleCtx && cycleCtx.modifiers) {
      cycleCtx = {
        ...cycleCtx,
        symptomLoad,
        modifiers: menstrualCycle.applySymptomOverlay(cycleCtx.modifiers, symptomLoad)
      };
    }
  }
  ...
  let planDescription = 'Авто-згенеровано за профілем';
  if (cycleCtx && cycleCtx.modifiers && cycleCtx.modifiers.hint) {
    const lbl = cycleCtx.phaseLabelUa ? `${cycleCtx.phaseLabelUa}` : (cycleCtx.phase || '');
    const dayNote = cycleCtx.dayInCycle != null ? ` · день циклу ~${cycleCtx.dayInCycle}` : '';
    planDescription += `\n\n🌸 Цикл: ${lbl}${dayNote}\n${cycleCtx.modifiers.hint}`;
  }
  if (cycleSymptomStaleDays != null && cycleSymptomStaleDays > 7) {
    planDescription += '\n\n🌡️ Нагадування: symptom-check старше 7 днів. Оновіть самопочуття в профілі перед наступною генерацією для точнішої адаптації навантаження.';
  }
  const cycleReasonBlock = cycleCtx
    ? (
      '🌸 Чому змінено навантаження:\n' +
      `• Фаза: ${cycleCtx.phaseLabelUa || cycleCtx.phase || '—'}${cycleCtx.dayInCycle != null ? ` (день ~${cycleCtx.dayInCycle})` : ''}\n` +
      `• Symptom-score: ${cycleCtx.symptomLoad ? `${cycleCtx.symptomLoad.score} (${cycleCtx.symptomLoad.level})` : 'немає'}\n` +
      `• Множник підходів: x${Number((cycleCtx.modifiers && cycleCtx.modifiers.setsMultiplier != null) ? cycleCtx.modifiers.setsMultiplier : 1).toFixed(2)}`
    )
    : null;
  ...
  for (const dayConfig of dayConfigs) {
    ...
    const dayExercises = await pickExercisesForDay(dayConfig, userMedConditions, goal, level, gender, {
      ...
      mc018Class,
      menstrualExcludeInversion: !!(cycleCtx && cycleCtx.modifiers && cycleCtx.modifiers.excludeInversion),
      menstrualExcludeHighImpact: !!(cycleCtx && cycleCtx.modifiers && cycleCtx.modifiers.excludeHighImpact)
    });
    ...
    if (dayExercises.length > 0) {
      ...
      notesByExercise = await planCommentsAI.generatePlanComments(
        profileForAI,
        dayExercises.map((ex) => {
          let sr = getSetsRepsRest(goal, level, null, ex.medicalStatus);
          if (cycleCtx && cycleCtx.modifiers) sr = menstrualCycle.applyToSetsReps(sr, cycleCtx.modifiers);
    ...
    for (let i = 0; i < dayExercises.length; i++) {
      ...
      if (i === 0 && cycleReasonBlock) {
        if (daySummary && notes) notes = cycleReasonBlock + '\n\n' + daySummary + '\n\n' + notes;
        else if (daySummary) notes = cycleReasonBlock + '\n\n' + daySummary;
        else if (notes) notes = cycleReasonBlock + '\n\n' + notes;
        else notes = cycleReasonBlock;
      } else if (daySummary && i === 0 && notes) notes = daySummary + '\n\n' + notes;
      ...
      let sr = getSetsRepsRest(goal, level, null, ex.medicalStatus);
      if (cycleCtx && cycleCtx.modifiers) {
        sr = menstrualCycle.applyToSetsReps(sr, cycleCtx.modifiers);
      }
