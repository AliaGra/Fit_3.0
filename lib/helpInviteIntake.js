...
async function askOblast(chatId, draft, requestId) {
  ...
  await HelpTelegram.sendMessage(chatId, CONSTANTS.HELP_INVITE_LOCATION_PROMPTS.OBLAST);
}
...
async function handleInviteIntakeText(chatId, text, state) {
  ...
  if (step === STEPS.OBLAST) {
    if (t.length < 2) {
      await HelpTelegram.sendMessage(chatId, '⚠️ Введи щонайменше 2 літери назви області:');
      return true;
    }
    const oblasts = await supabase.searchOblasts(t, 12);
    if (!oblasts.length) {
      await HelpTelegram.sendMessage(chatId, '❌ Область не знайдено. Спробуй інше написання (мінімум 2 літери):');
      return true;
    }
    const keyboard = oblasts.map((o) => [{ text: o, callback_data: `${CALLBACKS.LOC_OBLAST}:${o}` }]);
    await HelpTelegram.sendKeyboard(chatId, CONSTANTS.HELP_INVITE_LOCATION_PROMPTS.CHOOSE_OBLAST, keyboard);
    return true;
  }
  ...
  if (step === STEPS.CITY) {
    ...
    if (!cities.length) {
      await HelpTelegram.sendMessage(chatId, '❌ Населений пункт не знайдено. Спробуй інші 3 або більше літер:');
      return true;
    }
    ...
    await HelpTelegram.sendKeyboard(chatId, CONSTANTS.HELP_INVITE_LOCATION_PROMPTS.CHOOSE_CITY(oblast), keyboard);
    return true;
  }
...
async function handleInviteIntakeCallback(chatId, action, param, state) {
  ...
  if (action === CALLBACKS.LOC_OBLAST) {
    ...
    await HelpTelegram.sendMessage(chatId, CONSTANTS.HELP_INVITE_LOCATION_PROMPTS.CITY);
    return true;
  }
