...
/** Повертає id категорії, в якій є код mcCode. */
function getCategoryIdByCode(mcCode) {
  const cat = MC_CATEGORIES.find((c) => c.codes && c.codes.includes(mcCode));
  return cat ? cat.id : null;
}

/** Відкрити екран медичного профілю для targetChatId (клієнт). Тренер (chatId) переглядає/редагує. */
async function showMedicalProfile(chatId, targetChatId) {
  ...
  if (!student || String(student.coachId) !== String(chatId)) {
    await Helpers.safeSend(chatId, '❌ Клієнта не знайдено або доступ заборонено.');
    return;
  }
  ...
  keyboard.push([{ text: '🔙 До меню Обміри/Медицина/Досвід', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_HEALTH_MENU + ':' + targetChatId }]);
...
async function showCategoryList(chatId, targetChatId) {
  ...
  text += '• **Ні** — у клієнта немає станів з цієї категорії (нічого не додаємо).\n';
  ...
  keyboard.push([{ text: '✅ Готово', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_HEALTH_MENU + ':' + targetChatId }]);

  await Helpers.sendKeyboard(chatId, text, keyboard, { parse_mode: 'Markdown' });
}

/** Список захворювань у категорії (за вирахуванням уже доданих). Показує групу до натискання «До груп». */
async function showConditionListForCategory(chatId, targetChatId, categoryId) {
  ...
  await State.set(chatId, { step: CONSTANTS.FSM_STATES.MC_PROFILE_VIEW, medicalProfileTarget: targetChatId, mcCurrentCategoryId: categoryId });

  const existingList = await supabase.getMedicalConditionsList(targetChatId);
  const alreadySelected = new Set((existingList || []).filter((r) => r.is_active).map((r) => (r.mc_code || '').toString().trim()));
  ...
  keyboard.push([{ text: '🔙 До груп', callback_data: CONSTANTS.CALLBACK_PREFIXES.MC_ADD + ':' + targetChatId }]);
...
async function askSeverity(chatId, targetChatId, mcCode) {
  ...
  await State.set(chatId, {
    ...
    mcAddCode: mcCode,
    mcCurrentCategoryId: prev && prev.mcCurrentCategoryId ? prev.mcCurrentCategoryId : getCategoryIdByCode(mcCode)
  });
  const keyboard = [
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.MC_SEVERITY && params.length >= 2) {
    ...
    if (!state || state.step !== CONSTANTS.FSM_STATES.MC_ADD_SEVERITY || state.medicalProfileTarget !== targetChatId) {
      await Helpers.safeSend(chatId, '⚠️ Сесія змінилась. Поверніться до картки клієнта і відкрийте медичний профіль знову.');
      return true;
    }
    ...
    if (ok) {
      ...
      await Helpers.safeSend(chatId, '✅ Додано: ' + (codeToName(mcCode) || mcCode) + ' — ' + (severityToLabel(severityKey) || severityKey) + '\n\nМожна обрати ще стани з цієї групи або натиснути «До груп».');
    } else {
      await Helpers.safeSend(chatId, '❌ Не вдалося додати (можливо, такий стан вже є у профілі).');
    }
    const categoryId = state.mcCurrentCategoryId || getCategoryIdByCode(mcCode);
    if (categoryId) await showConditionListForCategory(chatId, targetChatId, categoryId);
    else await showCategoryList(chatId, targetChatId);
    return true;
  }
...
/** Після зміни медичного стану: перевірити активний план клієнта; при BLOCKED вправах — деактивувати план, лог у plan_adjustments, сповістити тренера (Логіка 9.3, ТЗ 8.4). */
  ...
  try {
    ...
    if (coachChatId) {
      ...
      await Helpers.safeSend(
        coachChatId,
        '⚠️ Медичний профіль клієнта змінено. У його активному плані є протипоказані вправи: ' + names + more + '.\nПлан деактивовано. Призначте клієнту новий план або відредагуйте поточний.'
      );
    }
  } catch (e) {
