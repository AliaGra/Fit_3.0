/**
 * Програма тренувань клієнта: список планів, авто-генерація, активація (Логіка складання плану тренувань.md, розд. 8).
 */
...
/** Показати списки планів клієнта та кнопку «Новий план». */
async function showPlanList(chatId, studentChatId) {
  ...
  if (plans.length === 0) {
    text += 'Планів ще немає. Натисни «➕ Новий план» і обери «Авто-підбір», щоб згенерувати план за профілем клієнта.';
  } else {
  ...
  else keyboard.push([{ text: '🔙 До картки клієнта', callback_data: CONSTANTS.CALLBACK_PREFIXES.VIEW_STUDENT + ':' + studentChatId }]);
...
/** Перевірка повноти профілю клієнта для авто-плану (мета, дата народження, зони акценту). */
...
/** Текст і клавіатура, коли профіль неповний для авто-плану (тренер vs solo-клієнт). */
function getAutoPlanIncompleteProfilePrompt(studentChatId, missing, isSelfMode) {
  ...
  if (isSelfMode) {
    ...
    msg =
      '⚠️ Для автоплану потрібно заповнити в профілі клієнта: ' +
      missingStr +
      '.\n\nЗаповніть у картці клієнта або створіть план вручну.';
    profileRow = [{ text: '👤 До картки клієнта', callback_data: CONSTANTS.CALLBACK_PREFIXES.VIEW_STUDENT + ':' + studentChatId }];
  }
  const keyboard = [
    profileRow,
...
async function showAutoSummary(chatId, studentChatId) {
  if (!studentChatId) {
    await Helpers.safeSend(chatId, '❌ Не вибрано клієнта.');
    return;
  }
  const access = await resolvePlanAccess(chatId, studentChatId);
  if (!access.ok) {
  ...
  const keyboard = [
    ...
    [{ text: '⚙️ Генерувати за профілем клієнта', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_GENERATE + ':' + studentChatId }],
    [{ text: '🔙 Назад', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_NEW + ':' + studentChatId }]
  ];
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.PLAN_DAYS && param) {
    ...
    const studentChatId = state.planStudentChatId;
    if (!studentChatId) {
      await Helpers.safeSend(chatId, '❌ Обери клієнта знову.');
      await showPlanList(chatId, studentChatId);
      return true;
    }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.PLAN_GENERATE && param) {
    ...
    try {
      ...
      if (!result) {
        await Helpers.safeSend(chatId, '❌ Не вдалося згенерувати план. Перевір профіль клієнта (ціль, рівень досвіду, тренувальних днів).');
        await showPlanList(chatId, studentChatId);
        return true;
      }
      await Helpers.safeSend(chatId, '✅ План створено: ' + result.planName + '\n' + result.daysPerWeek + ' дн./тиж, рівень ' + result.level + '.');
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.PLAN_ACTIVATE && param) {
    ...
    if (!studentChatId) {
      await Helpers.safeSend(chatId, '❌ Обери клієнта знову зі списку.');
      return true;
    }
    const ok = await supabase.setPlanActiveForStudent(planId, studentChatId);
    if (ok) {
      await Helpers.safeSend(chatId, '✅ План активовано для клієнта.');
    } else {
