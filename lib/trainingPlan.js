...
async function askAvoidZones(chatId) {
  ...
  for (const zone of AVOID_ZONES_ORDER) {
    const isInAccent = accentZones.includes(zone);
    const label = (isInAccent ? '— ' : '') + (ACCENT_LABELS[zone] || zone) + (avoidZones.includes(zone) ? ' ✓' : '');
    const cb = isInAccent ? CONSTANTS.CALLBACK_PREFIXES.PLAN_AVOID_DISABLED : CONSTANTS.CALLBACK_PREFIXES.PLAN_AVOID_TOGGLE + ':' + zone;
    keyboard.push([{ text: label, callback_data: cb }]);
  }
