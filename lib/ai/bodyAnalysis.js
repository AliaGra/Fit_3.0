...
function calcIdealWeight(heightCm, gender, bodyBuild, age, experienceStartDate) {
  ...
  // Множник тренованості — окремо для ж/ч, 4 рівні (ТЗ, фінальний варіант)
  let expFactor;
  if (gender === 'female') {
...
function buildBodyAnalysisBlock(bodyStatus, options) {
  ...
  // Ідеальна вага — 4 рівні (мін/комфорт/атлетична/макс)
  // П.4: підлітки (age <= 17) не бачать числових діапазонів — тільки тренер
  const isTeen = bodyStatus.age != null && bodyStatus.age <= 17;
  const showIdealNumbers = !isTeen || (options && options.forCoach);
  if (bodyStatus.idealWeight) {
    const iw = bodyStatus.idealWeight;
    if (!showIdealNumbers) {
      // Підліток-учень: без конкретних цифр
      lines.push('Ідеальна вага: розраховано з урахуванням зросту, статі та тілобудови. Конкретні цифри доступні тренеру.');
    } else if (iw.comfort != null) {
      lines.push('Ідеальна вага (мін / комфорт / атлетична / макс): ' + iw.min + ' / ' + iw.comfort + ' / ' + iw.athletic + ' / ' + iw.max + ' кг');
    } else {
      lines.push('Ідеальна вага для зросту: ' + iw.min + '–' + iw.max + ' кг');
    }
    if (showIdealNumbers && bodyStatus.concreteGoals && bodyStatus.concreteGoals.weightExcessKg > 0) {
      lines.push('Поточна вага вище максимуму норми на: ' + bodyStatus.concreteGoals.weightExcessKg + ' кг');
    }
  }
  ...
  if (bodyStatus.bodyType && bodyStatus.bodyType.type && bodyStatus.bodyType.type !== 'insufficient_data') {
    ...
    // Завжди показуємо тілобудову (навіть normosthenic — щоб AI знав, що вона врахована)
    const buildMap = { asthenic: 'астенік', normosthenic: 'нормостенік', hypersthenic: 'гіперстенік' };
    lines.push('Тілобудова (за запʼястям): ' + (buildMap[bt.bodyBuild] || bt.bodyBuild || 'не визначено'));
    // Завжди показуємо % жиру якщо є шия (navy) або ручні дані
    if (bt.fatPct != null) {
      const status = bt.fatStatus === 'above_normal' ? 'вище норми' : (bt.fatStatus === 'normal' ? 'в межах норми' : '—');
      lines.push('% жиру (оцінка): ' + bt.fatPct + '% — ' + status);
    } else if (bodyStatus.neckCm == null) {
      lines.push('% жиру: не розраховано (немає обміру шиї)');
    }
  ...
  try {
    ...
    if (bodyStatus.missingCore && bodyStatus.missingCore.length > 0) {
      ...
      await Helpers.safeSend(
        chatId,
        'Для детальної аналітики твого тіла потрібно вказати: ' +
          list +
