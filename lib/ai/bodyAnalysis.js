...
// Ідеальна вага за формулою Devine
function calcIdealWeight(heightCm, gender) {
  ...
  return {
    min: Math.round(ideal * 0.90),
    ideal: Math.round(ideal),
    max: Math.round(ideal * 1.10)
  };
}
...
function buildBodyStatus(user, measurements, scenario) {
  ...
  if (heightCm && user.gender) {
    bodyStatus.idealWeight = calcIdealWeight(heightCm, user.gender);
  }
...
function buildBodyAnalysisBlock(bodyStatus) {
  const lines = [];
  ...
  if (bodyStatus.bodyType && bodyStatus.bodyType.priority && bodyStatus.bodyType.priority.phase) {
    ...
    lines.push('Рекомендована фаза: ' + (phaseMap[bodyStatus.bodyType.priority.phase] || bodyStatus.bodyType.priority.phase));
  }
...
async function generateText(bodyStatus) {
  ...
  const analysisBlock = buildBodyAnalysisBlock(bodyStatus);
  const userContent = tpl.replace('{{bodyAnalysisBlock}}', analysisBlock);
 ...
 * @param {string} [labelPrefix] - префікс заголовку
 */
async function regenerateAndSendAnalysis(recipientChatId, studentChatId, labelPrefix) {
  await sendFullAnalysis(recipientChatId, studentChatId, labelPrefix, true);
}
 ...
 */
async function sendFullAnalysis(recipientChatId, studentChatId, labelPrefix, forceRegenerate = false) {
  try {
    const today = new Date().toLocaleDateString('uk-UA', { day: 'numeric', month: 'long', year: 'numeric' });
    const header = (labelPrefix || '📊 AI-аналітика тіла') + ' · v2026-03-18';

    // ── БЛОК 1: поточний стан тіла ──
    let bodyText = null;
    if (forceRegenerate && aiClient.isEnabled()) {
      bodyText = await generateAndSave(studentChatId, 'coach_request', {});
    } else {
      const stored = await getStoredAnalysis(studentChatId);
      if (stored && stored.text) {
        bodyText = stored.text;
      } else if (aiClient.isEnabled()) {
        bodyText = await generateAndSave(studentChatId, 'on_demand', {});
      }
    }
    ...
    // ── БЛОК 2: порівняння з цілями (якщо є) ──
    try {
      // Lazy require — уникаємо циклічних залежностей
      const supabaseModule = require('../supabase');
      const goals = await supabaseModule.getBodyGoals(String(studentChatId));
      if (!goals) return;

      const analysis = goals.goals_analysis || null;
      if (!analysis) return;

      // Дельти — детерміновано кодом, AI не потрібен
      const deltaText = buildDeltaLines(analysis.deltaItems);
      if (!deltaText) return;

      let goalsMessage = '🎯 Порівняння з цілями:\n\n' + deltaText;

      // AI коментар — тільки якщо є warnings/конфлікти/довгий термін
      // AI отримує готовий текстовий блок і лише переказує його (не вигадує)
      const bodyGoalsModule = require('../bodyGoals');
      if (bodyGoalsModule.shouldShowAIComment(analysis) && aiClient.isEnabled()) {
        const user = await User.getByChatId(String(studentChatId));
        if (user) {
          const aiInputBlock = bodyGoalsModule.buildAIInputBlock(analysis, user);
          const goalsAI = require('./goalsVsCurrent');
          const aiComment = await goalsAI.generateText(aiInputBlock);
          if (aiComment) {
            goalsMessage += '\n\n' + aiComment;
          }
        }
      ...
      await Helpers.safeSend(recipientChatId, goalsMessage);
    } catch (e) {
      // Блок цілей опційний — не зупиняємо основну аналітику
      console.error('BodyAnalysis.sendFullAnalysis goals block', e.message || e);
    }
  } catch (e) {
