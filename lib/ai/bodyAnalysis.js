...
function calcIdealWeight(heightCm, gender, bodyBuild, age, experienceStartDate) {
  ...
  // З 45 років "правильна" вага вище базової Devine: м'язи важливіші, трохи більша вага — норма
  const ageFactor = (ageNum != null && isFinite(ageNum))
    ? (ageNum >= 55 ? 1.05 : (ageNum >= 45 ? 1.03 : 1.0))
    : 1.0;
...
function buildBodyAnalysisBlock(bodyStatus, options) {
  ...
  if (bodyStatus.idealWeight) {
    ...
    if (iw.optMin != null && iw.optMax != null) {
      lines.push('Ідеальна вага: оптимальна ' + iw.optMin + '–' + iw.optMax + ' кг, клінічно прийнятна ' + iw.min + '–' + iw.max + ' кг');
    } else {
