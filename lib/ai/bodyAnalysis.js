...
function calcIdealWeight(heightCm, gender, bodyBuild, age, experienceStartDate) {
  ...
  // Клінічний діапазон: ±10% (45+) або ±8%. Оптимальна зона: ±5%
  const rangePct = (ageNum != null && ageNum >= 45) ? 0.10 : 0.08;
  const optPct = 0.05;
  ...
  return {
    ...
    optMin: Math.round(adjIdeal * (1 - optPct)),
    optMax: Math.round(adjIdeal * (1 + optPct)),
...
function buildBodyAnalysisBlock(bodyStatus, options) {
  ...
  // Ідеальна вага і надлишок — основне значення = оптимальна зона
  if (bodyStatus.idealWeight) {
    const iw = bodyStatus.idealWeight;
    if (iw.optMin != null && iw.optMax != null) {
      lines.push('Ідеальна вага для зросту і тілобудови: ' + iw.optMin + '–' + iw.optMax + ' кг');
      lines.push('Клінічно допустима вага: ' + iw.min + '–' + iw.max + ' кг');
    } else {
    ...
    if (bodyStatus.concreteGoals && bodyStatus.concreteGoals.weightExcessKg > 0) {
      lines.push('Поточна вага вище клінічного діапазону на: ' + bodyStatus.concreteGoals.weightExcessKg + ' кг');
    }
  }
