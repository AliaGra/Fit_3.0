...
async function showTrainingSubmenu(chatId) {
  ...
  if (user.role === CONSTANTS.ROLES.COACH) {
    ...
    if (!user.coachId) {
      keyboard.push([{ text: '⚙️ Авто-план', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_AUTO + ':' + chatId }]);
      keyboard.push([{ text: '✏️ Створити план вручну', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_MANUAL + ':' + chatId }]);
    }
  }
