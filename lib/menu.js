...
async function show(chatId, options = {}) {
  const force = options && options.force === true;
  const user = await User.getByChatId(chatId);
  if (user && User.isStudent(user) && !force) {
...
async function showCoachMenu(chatId, user) {
  ...
  try {
    ...
    if (withBirthdayIn7.length > 0) {
      ...
      const msg = '🎂 Через 7 днів (' + dateStr + ') день народження клієнта(ів): ' + names.join(', ');
  ...
  const Schedule = require('./schedule');
  const c = await Schedule.getCoachSlotsCountsMyScheduleWindow(chatId);
  const keyboard = [
    [{ text: '📋 Графік роботи (' + c.booked + ')', callback_data: CONSTANTS.CALLBACKS.SCH_7_BOOKED }],
    [{ text: '💪 Тренування', callback_data: CONSTANTS.CALLBACKS.MENU_TRAINING }],
    [{ text: '👥 Мої клієнти', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS }],
    ...
    [{ text: '📣 Розсилка', callback_data: CONSTANTS.CALLBACKS.MENU_BROADCAST }],
...
async function showTrainingSubmenu(chatId) {
  ...
  if (user.role === CONSTANTS.ROLES.COACH) {
    keyboard.push([{ text: '💪 Тренування клієнтів', callback_data: CONSTANTS.CALLBACKS.TRAINING_COACH_START }]);
...
/** Підменю для solo-клієнта: авто/вручну + дні на тиждень (як у картці клієнта в тренера). */
