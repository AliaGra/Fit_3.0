...
function severityToLabel(severity) {
  ...
  const key = severity.trim().toLowerCase().replace(/\s*[—–\-]\s*/g, '-').replace(/\s+/g, ' ');
  return SEVERITY_LABELS[key] || SEVERITY_LABELS[severity.trim()] || severity.trim();
}

/**
 * Декодує один елемент: "MC003 (mild)" або "MC015".
 */
function decodeOneItem(item) {
  const s = latinizeCodeInString(item.trim());
  if (!s) return '';
  const slashIdx = s.indexOf('/');
  if (slashIdx >= 0) {
    const codePart = s.slice(0, slashIdx).trim();
    const severityKey = s.slice(slashIdx + 1).trim();
    const name = codeToName(codePart);
    const severityLabel = severityToLabel(severityKey);
    if (severityLabel) return name ? name + ' — ' + severityLabel : item.trim();
    return name || item.trim();
  }
  const parenMatch = s.match(/^\s*(.+?)\s*\(([^)]+)\)\s*$/);
  if (parenMatch) {
    const codePart = parenMatch[1].trim();
    const severityKey = parenMatch[2].trim();
    const name = codeToName(codePart);
    const severityLabel = severityToLabel(severityKey);
    if (severityLabel) return name ? name + ' — ' + severityLabel : item.trim();
    return name || item.trim();
  }
  return codeToName(s) || item.trim();
}

/**
 * Декодує один фрагмент: "MC003 (mild)" або "MC001, MC002" або "MC002 (mild-moderate), MC004, MC009 (trimester1 — 2)".
...
function decodeSegment(segment) {
  ...
  if (slashIdx >= 0) {
    return decodeOneItem(latin);
  }
  const parts = latin.split(',').map((p) => p.trim()).filter(Boolean);
  if (parts.length === 0) return '';
  const decoded = parts.map((p) => decodeOneItem(p));
  return decoded.join('; ');
}
