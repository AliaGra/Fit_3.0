...
async function showExerciseDetail(chatId, exerciseId) {
  ...
  if (detail.medicalContraindications) lines.push('🚫 Абсолютні заборони: ' + detail.medicalContraindications);
  if (detail.medicalLimitations) lines.push('📋 Обмеження з примітками: ' + detail.medicalLimitations);
  if (detail.safeFor) lines.push('✅ Безпечно при цих станах: ' + detail.safeFor);
  if (detail.modifications) lines.push('🔄 Як модифікувати: ' + detail.modifications);
  if (detail.alternatives) lines.push('↔️ Альтернативні вправи: ' + detail.alternatives);
