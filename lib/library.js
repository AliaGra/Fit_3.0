...
async function showExerciseDetail(chatId, exerciseId) {
  ...
  let text = '📖 **' + (detail.name || 'Вправа') + '**\n\n';
  if (detail.groupPath) text += '📂 ' + detail.groupPath + '\n\n';
  if (detail.equipment) text += '🏋️ Обладнання: ' + detail.equipment + '\n\n';
  if (detail.focusPoint) text += '🎯 Фокус: ' + detail.focusPoint + '\n\n';
  if (detail.commonMistakes) text += '⚠️ Типові помилки: ' + detail.commonMistakes + '\n\n';
  if (detail.properFeeling) text += '✅ Відчуття: ' + detail.properFeeling + '\n\n';
  if (detail.staticHolds) text += '⏱ ' + detail.staticHolds + '\n\n';
  if (detail.vid) text += 'Тип: ' + detail.vid + '\n';
  if (detail.difficulty) text += 'Складність: ' + detail.difficulty + '\n';
  if (detail.youtubeLink) text += '\n🔗 [Відео](' + detail.youtubeLink + ')';
  if (detail.myChannelLink) text += '\n🔗 [Канал](' + detail.myChannelLink + ')';
