...
async function showExerciseDetail(chatId, exerciseId, options) {
  const opts = options || {};
  const returnToPlan = opts.returnToPlan;
  const detail = await supabase.getExerciseDetailById(exerciseId);
  if (!detail) {
    if (returnToPlan && returnToPlan.planId) {
      const keyboard = [[{ text: CONSTANTS.EMOJI.BACK + ' Назад до плану', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_BACK_TO_PLAN }]];
      await Helpers.sendKeyboard(chatId, '❌ Вправу не знайдено.', keyboard);
    } else {
  ...
  const lines = [];
  const baseName = detail.name || 'Вправа';
  let myAliasesText = '';
  try {
    const myAliases = await supabase.getAliasesByUserAndExercise(chatId, exerciseId);
    if (myAliases && myAliases.length > 0) {
      const names = myAliases.map((a) => a.alias).filter(Boolean);
      if (names.length) {
        myAliasesText = names.join(', ');
      }
    }
  } catch (e) {
    console.error('showExerciseDetail aliases', e.message);
  }
  if (myAliasesText) {
    lines.push('📖 **' + baseName + '** (моя назва: ' + myAliasesText + ')');
  } else {
    lines.push('📖 **' + baseName + '**');
  }

  const userMedConditions = await supabase.getActiveMedicalConditions(chatId);
  if (userMedConditions && userMedConditions.length > 0) {
    const result = filterExerciseForUser(detail, userMedConditions);
    if (result.status === 'BLOCKED') {
      lines.push('⛔ _Для вашого профілю: протипоказано_' + (result.reason ? ' (' + result.reason + ')' : ''));
    } else if (result.status === 'ALLOWED_WITH_MOD') {
      lines.push('⚠️ _Для вашого профілю: з обмеженнями_' + (result.modification ? '\n' + result.modification : ''));
    } else if (result.status === 'SAFE') {
      lines.push('✅ _Для вашого профілю: рекомендовано_' + (result.note ? ' — ' + result.note : ''));
    } else {
      lines.push('ℹ️ _Для вашого профілю: без особливих рекомендацій_');
    }
  }
  if (detail.groupPath) lines.push('📂 ' + detail.groupPath);
  const equipmentStr = detail.equipmentDisplay != null ? detail.equipmentDisplay : detail.equipment;
  if (equipmentStr) lines.push('🏋️ Обладнання: ' + equipmentStr);
  ...
  if (detail.safetyNotes) lines.push('🛡 Загальні примітки безпеки: ' + decodeMedicalText(detail.safetyNotes));
  ...
  const backRow = returnToPlan && returnToPlan.planId
    ? [{ text: CONSTANTS.EMOJI.BACK + ' Назад до плану', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_BACK_TO_PLAN }]
    : [{ text: CONSTANTS.EMOJI.BACK + ' Назад до списку', callback_data: CONSTANTS.CALLBACKS.LIBRARY_BACK }];
  const keyboard = [
    [{ text: '✏️ Редагувати назви', callback_data: CONSTANTS.CALLBACK_PREFIXES.ALIAS_LIST + ':' + exerciseId }, { text: '➕ Ввести своє найменування', callback_data: CONSTANTS.CALLBACK_PREFIXES.ALIAS_ADD + ':' + exerciseId }],
    backRow,
...
async function handleTextMessage(chatId, text) {
  ...
  if (query.length < 3) {
    await Helpers.safeSend(chatId, '⚠️ Введи мінімум 3 літери для пошуку.');
    return true;
  }
  const user = await User.getByChatId(chatId);
  const coachId = user && user.coach_id ? user.coach_id : null;
  const exercises = await Alias.searchExercisesWithAliases(query, chatId, coachId);
  if (!exercises || exercises.length === 0) {
  ...
  const keyboard = exercises.slice(0, 15).map((ex) => {
    ...
    return [{ text: label.slice(0, MAX_EXERCISE_BUTTON_LENGTH), callback_data: CONSTANTS.CALLBACKS.LIBRARY_EXERCISE + ':' + ex.id }];
  });
