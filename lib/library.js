...
async function showExerciseDetail(chatId, exerciseId, options) {
  ...
  if (!detail) {
    if (returnToPlan && returnToPlan.planId) {
      const keyboard = [[{ text: CONSTANTS.EMOJI.BACK + ' Назад до плану', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_BACK_TO_PLAN + ':' + returnToPlan.planId + ':' + (returnToPlan.studentChatId || '') }]];
  ...
  const backRow = returnToPlan && returnToPlan.planId
    ? [{ text: CONSTANTS.EMOJI.BACK + ' Назад до плану', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_BACK_TO_PLAN + ':' + returnToPlan.planId + ':' + (returnToPlan.studentChatId || '') }]
