...
const TEXT_SUB = Object.freeze({ OBLAST: 'oblast', CITY: 'city' });
...
async function startTextFlow(chatId) {
  ...
  if (presetOblast && presetCity) {
    await State.update(chatId, { ...base, step: CONSTANTS.FSM_STATES.VENUE_TEXT_SEARCH, venueTextSubstep: null });
    const place = [presetCity, presetOblast].filter(Boolean).join(', ');
    await Helpers.safeSend(
      chatId,
      `Шукаю за адресою з профілю: <b>${Helpers.escapeHtml(place)}</b>…`,
      { parse_mode: 'HTML' }
    );
  ...
  if (presetOblast && !presetCity) {
    await State.update(chatId, {
      ...
      venueTextSubstep: TEXT_SUB.CITY
    });
    await Helpers.safeSend(
      chatId,
      `Область: <b>${Helpers.escapeHtml(presetOblast)}</b>.\n\nВведи місто / населений пункт:`,
      { parse_mode: 'HTML' }
    );
    return;
  }
  if (presetCity && !presetOblast) {
    await State.update(chatId, {
      ...base,
      step: CONSTANTS.FSM_STATES.VENUE_TEXT_SEARCH,
      venueTextSubstep: TEXT_SUB.OBLAST
    });
    await Helpers.safeSend(
      chatId,
      `Місто з профілю: <b>${Helpers.escapeHtml(presetCity)}</b>.\n\n` +
  ...
  await Helpers.safeSend(
    chatId,
    'Введи <b>область</b> (як у довіднику, наприклад <b>Київська</b>):',
    { parse_mode: 'HTML' }
  );
}
...
async function handleTextFlow(chatId, text) {
  ...
  if (!t) {
    await Helpers.safeSend(chatId, '⚠️ Введи непорожнє значення.');
    return true;
  }
  if (sub === TEXT_SUB.OBLAST) {
    await State.update(chatId, { venueTmpOblast: t, venueTextSubstep: TEXT_SUB.CITY });
    await Helpers.safeSend(chatId, 'Введи <b>місто</b> / населений пункт:', { parse_mode: 'HTML' });
    return true;
  }
  if (sub === TEXT_SUB.CITY) {
    await State.update(chatId, { venueTmpCity: t, venueTextSubstep: null });
    const st2 = await State.getSafe(chatId);
    await runOblastCityVenueSearch(chatId, st2);
    return true;
  }
