...
const VENUE_LOC_OBLAST_MIN_LEN = 2;
const VENUE_LOC_CITY_MIN_LEN = 3;
const VENUE_NAME_SEARCH_MIN_LEN = 3;
const WD = Object.freeze({ 1: 'Пн', 2: 'Вт', 3: 'Ср', 4: 'Чт', 5: 'Пт', 6: 'Сб', 7: 'Нд' });
const COACH_LINK = Object.freeze({ TEACH: 'teach', TRAIN: 'train' });

/** Режим привʼязки закладу тренером: teach = «де треную», train = «де тренуюсь». */
function coachVenueLinkMode(st) {
  const m = st && st.venueCoachLinkMode ? String(st.venueCoachLinkMode).trim() : '';
  if (m === COACH_LINK.TEACH || m === COACH_LINK.TRAIN) return m;
  if (st && st.venueLinkCoach) return COACH_LINK.TEACH;
  return null;
}

function isCoachVenueLinkFlow(st) {
  return coachVenueLinkMode(st) !== null;
}
...
function venuesSearchBackCallback(st) {
  if (st?.venueFromRegistration || isCoachVenueLinkFlow(st)) return CONSTANTS.CALLBACKS.VENUES_MENU;
...
async function sendVenueCardMessage(chatId, v, options = {}) {
  ...
  const phoneLine =
    v.phone && String(v.phone).trim() ? `\n📞 ${Helpers.escapeHtml(String(v.phone).trim())}` : '';
  const links = [];
  ...
  const gcLine = await coachGroupPublic.buildVenueGroupClassesCoachesHtml(v.id, gcFacets, dirGroupMap);
  const coaches = await supabase.listVenueCoaches(v.id);
  ...
  if (st?.venueFromRegistration || isCoachVenueLinkFlow(st)) {
    const pickLabel =
      coachVenueLinkMode(st) === COACH_LINK.TRAIN ? '✅ Додати до «Де тренуюсь»' : '✅ Додати до «Де треную»';
    actionRows.push([{ text: pickLabel, callback_data: `${CONSTANTS.CALLBACKS.VENUES_PICK}:${v.id}` }]);
  }
...
async function handleMyVenueCardClick(chatId, venueId) {
  ...
  if (
    !user ||
    (user.role !== CONSTANTS.ROLES.COACH &&
      user.role !== CONSTANTS.ROLES.STUDENT &&
      user.role !== CONSTANTS.ROLES.VENUE_OWNER)
  ) {
  ...
  if (user.role === CONSTANTS.ROLES.COACH) {
    const teach = await supabase.getCoachVenuesWhereTeach(chatId);
    const train = await supabase.getCoachVenuesWhereTrain(chatId);
    const byId = new Map();
    for (const v of [...teach, ...train]) byId.set(String(v.id), v);
    linked = Array.from(byId.values());
...
async function handleVenueCardClick(chatId, venueId) {
  ...
  const fromSearchFlow =
    ...
    isCoachVenueLinkFlow(st) ||
    st?.step === CONSTANTS.FSM_STATES.VENUE_SEARCH_HUB ||
    st?.step === CONSTANTS.FSM_STATES.VENUE_LOC_OBLAST_INPUT ||
    st?.step === CONSTANTS.FSM_STATES.VENUE_LOC_CITY_INPUT ||
    st?.step === CONSTANTS.FSM_STATES.VENUE_NAME_SEARCH ||
...
async function buildVenuePricesHtml(venueId) {
  const vid = String(venueId || '').trim();
  if (!vid) return null;
  const v = await supabase.getVenueById(vid);
  if (!v) return null;
...
async function showVenuePrices(chatId, venueId) {
  const text = await buildVenuePricesHtml(venueId);
  if (!text) {
    await Helpers.safeSend(chatId, '❌ Заклад не знайдено.');
    return;
  }
...
async function showVenueSearchMenu(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: '🔎 За областю та містом', callback_data: CONSTANTS.CALLBACKS.VENUES_TEXT }],
    [{ text: '🔎 Пошук за назвою', callback_data: CONSTANTS.CALLBACKS.VENUES_NAME_SEARCH }],
    ...
    [{ text: '⚙ Всі типи закладів', callback_data: CONSTANTS.CALLBACKS.VENUES_ORG + ':pick' }],
...
async function openCoachVenuesTeach(chatId, options = {}) {
  await State.update(chatId, {
    venueLinkCoach: true,
    venueCoachLinkMode: COACH_LINK.TEACH,
    returnToHintsPublic: !!options.fromHintsPublic
  });
  return showHub(chatId, { linkCoach: true, coachLinkMode: COACH_LINK.TEACH });
}

async function finishCoachVenueTeachFlow(chatId) {
  const st = await State.getSafe(chatId);
  const returnToHints = !!(st && st.returnToHintsPublic);
  await State.update(chatId, {
    venueLinkCoach: false,
    venueCoachLinkMode: null,
    venueCardViewedId: null,
    step: null,
    returnToHintsPublic: false
  });
  if (returnToHints) {
    const CoachHints = require('./coachHints');
    return CoachHints.showPublicStepsChecklist(chatId);
  }
...
async function showHub(chatId, options = {}) {
  ...
  let linkMode =
    'coachLinkMode' in options && options.coachLinkMode
      ? String(options.coachLinkMode)
      : coachVenueLinkMode(prev);
  if ('linkCoach' in options && options.linkCoach) linkMode = COACH_LINK.TEACH;
  const linkCoachFlow = linkMode === COACH_LINK.TEACH;
  await State.update(chatId, {
    step: CONSTANTS.FSM_STATES.VENUE_SEARCH_HUB,
    venueFromRegistration: fromRegistration,
    venueLinkCoach: linkCoachFlow,
    venueCoachLinkMode: linkMode || null,
    venueFilterOrg: '',
    venueGroupCodes: [],
    venueStudioCodes: [],
    venueSectionCodes: []
  ...
  const user = await User.getByChatId(chatId);
  const coachListMode = user && user.role === CONSTANTS.ROLES.COACH && !fromRegistration;
  const myVenuesHub = coachListMode || (user && !fromRegistration && !linkCoachFlow && user.role === CONSTANTS.ROLES.STUDENT);
  let myVenues = [];
  if (myVenuesHub) {
    try {
      if (user.role === CONSTANTS.ROLES.COACH) {
        if (linkMode === COACH_LINK.TEACH) {
          myVenues = (await supabase.getCoachVenuesWhereTeach(chatId)) || [];
        } else {
          myVenues = (await supabase.getCoachVenuesWhereTrain(chatId)) || [];
        }
  ...
  const searchLabel =
    linkMode === COACH_LINK.TEACH
      ? '🔎 Додати заклад (де треную)'
      : user && user.role === CONSTANTS.ROLES.COACH
        ? '🔎 Додати заклад (де тренуюсь)'
        : '🔎 Пошук нового закладу';
  keyboard.push([{ text: searchLabel, callback_data: CONSTANTS.CALLBACKS.VENUES_SEARCH_NEW }]);
  if (user && user.role === CONSTANTS.ROLES.COACH && linkMode !== COACH_LINK.TEACH) {
    keyboard.push([{ text: '🎫 Абонемент', callback_data: CONSTANTS.CALLBACKS.MENU_SUBSCRIPTION }]);
  }
  keyboard.push(
    fromRegistration
      ? [{ text: '⏭️ Пізніше — продовжити реєстрацію', callback_data: CONSTANTS.CALLBACKS.REG_VENUE_SKIP }]
      : [{ text: CONSTANTS.EMOJI.HOME + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]
  );
  if (linkMode === COACH_LINK.TEACH) {
    const returnToHintsPublic = !!prev.returnToHintsPublic;
    keyboard.splice(keyboard.length - 1, 0, [
      {
        text: returnToHintsPublic ? '🔙 До публічних даних' : '⬅️ Мій профіль',
        callback_data: returnToHintsPublic
          ? CONSTANTS.CALLBACKS.HINTS_PUBLIC_STEPS
          : CONSTANTS.CALLBACKS.PROFILE_VIEW
      }
    ]);
  }
  ...
  if (linkMode === COACH_LINK.TEACH) {
    msg =
      '🏢 <b>Де треную</b>\n\n' +
      'Заклади, де ти <b>проводиш тренування</b> для учнів. Їх бачать учні та користувачі, які шукають тренера.';
  } else if (user && user.role === CONSTANTS.ROLES.COACH) {
    msg =
      '🏢 <b>Клуби, студії</b>\n\n' +
      '<b>Де тренуюсь</b> — заклади, де ти сам(а) займаєшся (будь-який тип організації).';
  } else {
    msg = '🏢 <b>Клуби, студії</b>';
  }
  if (myVenuesHub && myVenues.length) {
    const listTitle = linkMode === COACH_LINK.TEACH ? 'Де треную' : 'Де тренуюсь';
    msg +=
      '\n\n<b>' +
      listTitle +
      '</b>\n' +
      myVenues
        ...
        .join('\n');
  } else if (coachListMode) {
    msg += '\n\n<i>Поки немає закладів у цьому списку — додай через пошук.</i>';
  }
...
async function showOrgFilterPicker(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, '⚙ <b>Всі типи закладів</b>\n\nОбери тип (або «Будь-який»):', keyboard, {
    parse_mode: 'HTML'
  });
}
...
async function returnToVenueSearchRoot(chatId, st) {
  if (st?.venueFromRegistration || isCoachVenueLinkFlow(st)) {
    const mode = coachVenueLinkMode(st);
    return showHub(chatId, {
      fromRegistration: !!st?.venueFromRegistration,
      coachLinkMode: mode || undefined,
      linkCoach: mode === COACH_LINK.TEACH
    });
  }
...
async function startGeoFlow(chatId) {
  ...
  await State.update(chatId, {
    ...
    venueCoachLinkMode: st?.venueCoachLinkMode || null,
...
async function askRadiusThenSearch(chatId, lat, lng) {
  ...
  await State.update(chatId, {
    ...
    venueCoachLinkMode: prev?.venueCoachLinkMode || null,
...
function venueSearchFiltersFromState(st) {
  return {
    organizationType: st?.venueFilterOrg || '',
    studioCodes: Array.isArray(st?.venueStudioCodes) ? st.venueStudioCodes : [],
    sectionCodes: Array.isArray(st?.venueSectionCodes) ? st.venueSectionCodes : [],
...
async function runOblastCityVenueSearch(chatId, st) {
  const list = await supabase.searchVenues({
    oblast: st.venueTmpOblast || '',
    city: st.venueTmpCity || '',
    ...venueSearchFiltersFromState(st)
  });
...
async function startTextFlow(chatId) {
  const st = await State.getSafe(chatId);
  const user = await User.getByChatId(chatId);
  const fromReg = !!st?.venueFromRegistration;
  const presetOblast = fromReg ? String(st?.regVenueOblast || '').trim() : '';
  const presetCity = fromReg ? String(st?.regVenueCity || user?.city || '').trim() : '';
  const base = {
    venueFromRegistration: st?.venueFromRegistration,
    venueLinkCoach: st?.venueLinkCoach,
    venueCoachLinkMode: st?.venueCoachLinkMode || null,
  ...
  /** Під час реєстрації — швидкий пошук у вже вказаному місті. */
  if (fromReg && presetOblast && presetCity) {
    await State.update(chatId, {
      ...base,
    ...
    await Helpers.safeSend(
      chatId,
      `Шукаю заклади: <b>${Helpers.escapeHtml(place)}</b>…`,
      { parse_mode: 'HTML' }
    );
    const st2 = await State.getSafe(chatId);
    await runOblastCityVenueSearch(chatId, st2);
    return;
  }
  ...
  await promptVenueOblastInput(chatId);
}

async function promptVenueOblastInput(chatId) {
  const user = await User.getByChatId(chatId);
  const homeParts = [];
  if (user?.city) homeParts.push(String(user.city).trim());
  if (user?.oblast) homeParts.push(String(user.oblast).trim());
  const homeHint = homeParts.length
    ? `\n\n<i>У профілі: ${Helpers.escapeHtml(homeParts.join(', '))} — можна обрати <b>інший</b> регіон з довідника.</i>`
    : '';
  await Helpers.safeSend(
    chatId,
    'Обери область і населений пункт з <b>довідника</b> — можна шукати заклади в будь-якому місті.' +
      homeHint +
      '\n\nВведи назву <b>області</b> (від ' +
      VENUE_LOC_OBLAST_MIN_LEN +
      ' літер):',
    { parse_mode: 'HTML' }
  );
}

async function handleVenueOblastInputFlow(chatId, text) {
  const q = String(text || '').trim();
  if (q.length < VENUE_LOC_OBLAST_MIN_LEN) {
    await Helpers.safeSend(
      chatId,
      '⚠️ Введи щонайменше ' + VENUE_LOC_OBLAST_MIN_LEN + ' літери назви області:'
    );
    return true;
  }
  ...
  const keyboard = oblasts.map((o) => [
    { text: o.length > 64 ? o.slice(0, 61) + '…' : o, callback_data: CONSTANTS.CALLBACK_PREFIXES.VEN_LOC_OBL + ':' + o }
  ]);
  keyboard.push([{ text: '⬅️ До пошуку', callback_data: CONSTANTS.CALLBACKS.VENUES_SEARCH_NEW }]);
  await Helpers.sendKeyboard(chatId, 'Обери <b>область</b> зі списку:', keyboard, { parse_mode: 'HTML' });
  return true;
}

async function handleVenueCityInputFlow(chatId, text) {
  const st = await State.getSafe(chatId);
  const oblast = String(st?.venueTmpOblast || '').trim();
  if (!oblast) {
    await State.update(chatId, { step: CONSTANTS.FSM_STATES.VENUE_LOC_OBLAST_INPUT });
    await promptVenueOblastInput(chatId);
    return true;
  }
  const q = String(text || '').trim();
  if (q.length < VENUE_LOC_CITY_MIN_LEN) {
    await Helpers.safeSend(
      chatId,
      '⚠️ Введи щонайменше ' + VENUE_LOC_CITY_MIN_LEN + ' літери назви населеного пункту:'
    );
    return true;
  }
  const cities = await supabase.searchCitiesInOblast(oblast, q, 12);
  if (!cities.length) {
    await Helpers.safeSend(chatId, '❌ Не знайдено. Спробуй інші ' + VENUE_LOC_CITY_MIN_LEN + '+ літери:');
    return true;
  }
  const keyboard = cities.map((c) => [
    {
      text: c.length > 64 ? c.slice(0, 61) + '…' : c,
      callback_data: CONSTANTS.CALLBACK_PREFIXES.VEN_LOC_CIT + ':' + c
    }
  ]);
  keyboard.push([{ text: '⬅️ Змінити область', callback_data: CONSTANTS.CALLBACK_PREFIXES.VEN_LOC_OBL + ':__BACK__' }]);
...
async function handleVenueLocationPick(chatId, action, params) {
  const param = (params && params.length ? params.join(':') : '').trim();
  if (action === CONSTANTS.CALLBACK_PREFIXES.VEN_LOC_OBL) {
    if (param === '__BACK__') {
      await State.update(chatId, { venueTmpOblast: '', venueTmpCity: '', step: CONSTANTS.FSM_STATES.VENUE_LOC_OBLAST_INPUT });
      await promptVenueOblastInput(chatId);
      return true;
    }
    await State.update(chatId, {
      venueTmpOblast: param,
      venueTmpCity: '',
      step: CONSTANTS.FSM_STATES.VENUE_LOC_CITY_INPUT
    });
    await Helpers.safeSend(
      chatId,
      `Область: <b>${Helpers.escapeHtml(param)}</b>\n\nВведи назву <b>населеного пункту</b> (від ${VENUE_LOC_CITY_MIN_LEN} літер):`,
      { parse_mode: 'HTML' }
    );
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.VEN_LOC_CIT) {
    if (param === '__BACK__') {
      const st = await State.getSafe(chatId);
      await State.update(chatId, { venueTmpCity: '', step: CONSTANTS.FSM_STATES.VENUE_LOC_OBLAST_INPUT });
      const hint = st?.venueTmpOblast ? ` (зараз: ${st.venueTmpOblast})` : '';
      await Helpers.safeSend(chatId, 'Введи область знову (від ' + VENUE_LOC_OBLAST_MIN_LEN + ' літер)' + hint + ':');
      return true;
    }
    const st = await State.getSafe(chatId);
    const oblast = String(st?.venueTmpOblast || '').trim();
    ...
    await State.update(chatId, { venueTmpCity: param });
...
async function startNameSearchFlow(chatId) {
  const st = await State.getSafe(chatId);
  await State.update(chatId, {
  ...
  await Helpers.safeSend(
    chatId,
    'Введи частину <b>назви</b> закладу (мінімум ' + VENUE_NAME_SEARCH_MIN_LEN + ' літери):',
    { parse_mode: 'HTML' }
  );
}

async function handleNameSearchFlow(chatId, text) {
  const st = await State.getSafe(chatId);
  if (!st || st.step !== CONSTANTS.FSM_STATES.VENUE_NAME_SEARCH) return false;
  const t = String(text || '').trim();
  if (t.length < VENUE_NAME_SEARCH_MIN_LEN) {
    await Helpers.safeSend(
      chatId,
      '⚠️ Для пошуку за назвою потрібно щонайменше ' + VENUE_NAME_SEARCH_MIN_LEN + ' літери. Спробуй ще раз:'
    );
    return true;
  }
  const list = await supabase.searchVenues({
  ...
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.VENUE_SEARCH_HUB });
  await sendVenueResults(chatId, list, { st });
  return true;
}
...
async function handlePickVenue(chatId, venueId) {
  ...
  if (st?.venueFromRegistration) {
    ...
    const role = user?.role || st?.role;
    if (!user) {
      await State.update(chatId, {
        regVenueSelectedIds: [vid],
        venueFromRegistration: false,
        venueCardViewedId: null,
        step: null
      });
      await Helpers.safeSend(chatId, '✅ Заклад збережено для профілю.');
    } else if (role === CONSTANTS.ROLES.COACH) {
      const teachList = await supabase.getCoachVenuesWhereTeach(chatId);
      const linked = await supabase.linkCoachVenue(chatId, vid, {
        isPrimary: !teachList || teachList.length === 0,
        teachesHere: true
      });
      if (linked) {
        await Helpers.safeSend(chatId, '✅ Заклад додано до «Де треную».');
      } else {
        await Helpers.safeSend(
          chatId,
          '❌ Не вдалося зберегти заклад у «Де треную».\nПеревір, що в Supabase застосована міграція coach_venues, і спробуй ще раз.'
        );
      }
    } else {
    ...
    const st2 = await State.getSafe(chatId);
    const nextRole = st2?.role || role;
    await State.update(chatId, { venueFromRegistration: false, venueCardViewedId: null, step: null });
    const Registration = require('./registration');
    if (nextRole === CONSTANTS.ROLES.COACH) return Registration.askInstagram(chatId);
    return Registration.askRegHeight(chatId);
  }

  if (!user) return;
  const linkMode = coachVenueLinkMode(st);
  if (linkMode && user.role === CONSTANTS.ROLES.COACH) {
    if (String(st?.venueCardViewedId || '') !== vid) {
      await handleVenueCardClick(chatId, vid);
      return;
    }
    const prevLink = await supabase.getCoachVenueLinkRow(chatId, vid);
    const teachesBefore = !!(prevLink && prevLink.teaches_here);
    const teachList = await supabase.getCoachVenuesWhereTeach(chatId);
    const primary = linkMode === COACH_LINK.TEACH && (!teachList || !teachList.length);
    const opts =
      linkMode === COACH_LINK.TEACH
        ? { isPrimary: primary, teachesHere: true }
        : { isPrimary: false, trainsHere: true };
    const linked = await supabase.linkCoachVenue(chatId, vid, opts);
    if (!linked) {
      await Helpers.safeSend(
        chatId,
        '❌ Не вдалося зберегти заклад.\nПеревір, що в Supabase застосована міграція coach_venues, і спробуй ще раз.'
      );
      await State.update(chatId, { venueCardViewedId: null, step: CONSTANTS.FSM_STATES.VENUE_SEARCH_HUB });
      return showHub(chatId, { coachLinkMode: linkMode });
    }
    await Helpers.safeSend(
      chatId,
      linkMode === COACH_LINK.TEACH
        ? '✅ Заклад додано до «Де треную».'
        : '✅ Заклад додано до «Де тренуюсь».'
    );
    if (linkMode === COACH_LINK.TEACH && !teachesBefore) {
    ...
    if (linkMode === COACH_LINK.TEACH) {
      return finishCoachVenueTeachFlow(chatId);
    }
    await State.update(chatId, { venueCardViewedId: null, step: CONSTANTS.FSM_STATES.VENUE_SEARCH_HUB });
    return showHub(chatId, { coachLinkMode: COACH_LINK.TRAIN });
  }

  await Helpers.safeSend(
    chatId,
    'ℹ️ Для привʼязки закладу відкрий його зі списку пошуку у відповідному сценарії (реєстрація, «Де треную» або «Клуби, студії»).'
  );
  return showVenueSearchMenu(chatId);
}
...
async function handleCallback(chatId, action, params) {
  if (
    ...
    const handled = await handleVenueLocationPick(chatId, action, params);
    if (handled) return true;
  }
  if (action === CONSTANTS.CALLBACKS.VENUES_MENU) {
    const user = await User.getByChatId(chatId);
    if (user && user.role === CONSTANTS.ROLES.COACH) {
      await State.update(chatId, { venueLinkCoach: false, venueCoachLinkMode: null });
    }
    return showHub(chatId);
  }
  if (action === CONSTANTS.CALLBACKS.VENUES_SEARCH_NEW) {
    const user = await User.getByChatId(chatId);
    const st = await State.getSafe(chatId);
    if (user && user.role === CONSTANTS.ROLES.COACH && !st?.venueFromRegistration && coachVenueLinkMode(st) !== COACH_LINK.TEACH) {
      await State.update(chatId, { venueCoachLinkMode: COACH_LINK.TRAIN, venueLinkCoach: false });
    }
    return showVenueSearchMenu(chatId);
  }
  ...
  if (action === CONSTANTS.CALLBACKS.VENUES_NAME_SEARCH) return startNameSearchFlow(chatId);
  ...
  if (action === CONSTANTS.CALLBACKS.PROFILE_COACH_VENUES) {
    return openCoachVenuesTeach(chatId, { fromHintsPublic: false });
  }
  return null;
}
...
async function handleTextMessage(chatId, text) {
  const st = await State.getSafe(chatId);
  if (st && st.step === CONSTANTS.FSM_STATES.VENUE_LOC_OBLAST_INPUT) {
    return handleVenueOblastInputFlow(chatId, text);
  }
  if (st && st.step === CONSTANTS.FSM_STATES.VENUE_LOC_CITY_INPUT) {
    return handleVenueCityInputFlow(chatId, text);
  }
  if (st && st.step === CONSTANTS.FSM_STATES.VENUE_NAME_SEARCH) {
    return handleNameSearchFlow(chatId, text);
  }
  return false;
}

module.exports = {
  openCoachVenuesTeach,
  finishCoachVenueTeachFlow,
  showHub,
  showVenueSearchMenu,
  showVenuePrices,
  buildVenuePricesHtml,
  sendVenueCardMessage,
  handleCallback,
  handleTextMessage,
