...
async function handleCoachVenueCardClick(chatId, venueId) {
  const vid = String(venueId || '').trim();
  if (!vid) return;
  const user = await User.getByChatId(chatId);
  if (!user || user.role !== CONSTANTS.ROLES.COACH) {
    await Helpers.safeSend(chatId, '❌ Доступ лише для тренера.');
    return;
  }
  const linked = await supabase.getCoachVenues(chatId);
  if (!linked.some((x) => String(x.id) === vid)) {
    await Helpers.safeSend(chatId, '❌ Заклад не привʼязаний до твого профілю.');
    return;
  }
...
/** Deep link /start venue_<id> — лише тренер і лише привʼязаний заклад. */
async function openCoachVenueCardFromDeepLink(chatId, venueId) {
  const user = await User.getByChatId(chatId);
  if (!user || user.role !== CONSTANTS.ROLES.COACH) return false;
  await handleCoachVenueCardClick(chatId, venueId);
  return true;
}
...
async function showHub(chatId, options = {}) {
  ...
  if (coachHub) {
    try {
      coachVenues = (await supabase.getCoachVenues(chatId)) || [];
    } catch (_) {
      coachVenues = [];
    }
  }
  ...
  const keyboard = [];
  if (coachHub && coachVenues.length) {
    for (const v of coachVenues.slice(0, 10)) {
      const name = String(v.nameUa || 'Заклад').trim() || 'Заклад';
      const mark = v.isPrimary ? ' ⭐' : '';
      const btn = `📍 ${name}${mark}`;
      const short = btn.length > 64 ? btn.slice(0, 61) + '…' : btn;
      const vid = String(v.id);
      const url = Helpers.publicVenuePageLink(vid);
      const row = [{ text: short, callback_data: `${CONSTANTS.CALLBACKS.VENUES_CARD}:${vid}` }];
      if (url) row.push({ text: '🔗 Картка', url });
      keyboard.push(row);
    }
  }
  keyboard.push(
  ...
  if (coachHub && coachVenues.length) {
    msg +=
      ...
      coachVenues
        .map((v) => {
          const mark = v.isPrimary ? ' ⭐' : '';
          return `• ${Helpers.escapeHtml(v.nameUa || 'Заклад')}${mark}`;
        })
        .join('\n') +
      '\n\n<i>Картка в цьому чаті — кнопка 📍; зовнішнє посилання в бот — 🔗 Картка (потрібен TELEGRAM_BOT_USERNAME у змінних).</i>';
  }
...
async function handleCallback(chatId, action, params) {
  ...
  if (action === CONSTANTS.CALLBACKS.VENUES_CARD) {
    ...
    if (vid) await handleCoachVenueCardClick(chatId, vid);
    return;
  }
...
module.exports = {
  ...
  openCoachVenueCardFromDeepLink
};
