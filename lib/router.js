...
async function handleTextMessage(chatId, text) {
  ...
  if (step && (step === CONSTANTS.FSM_STATES.MC_ADD_CODE || step === CONSTANTS.FSM_STATES.MC_ADD_SEVERITY || step === CONSTANTS.FSM_STATES.MC_ADD_SEVERITY_CUSTOM)) {
...
async function handleCallback(chatId, callbackData, callbackQueryId) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.MC_PROFILE || action === CONSTANTS.CALLBACK_PREFIXES.MC_ADD || action === CONSTANTS.CALLBACK_PREFIXES.MC_REMOVE || action === CONSTANTS.CALLBACK_PREFIXES.MC_SEVERITY) {
  ...
  if (action === CONSTANTS.CALLBACKS.TRAINING_START) {
    ...
    if (userForTraining?.role === CONSTANTS.ROLES.COACH) {
      ...
      await Helpers.safeSend(chatId, '💪 Модуль «Почати тренування» для учня ще в розробці. Скоро буде доступний.');
      await Menu.show(chatId);
    }
    return;
  }
