...
async function route(update) {
  ...
  try {
    ...
    if (data.telegramUsername) {
      User.syncTelegramUsername(data.chatId, data.telegramUsername).catch((e) => {
        console.error('Router: syncTelegramUsername', e.message);
      });
    }
    ...
    if (data.type === 'location') {
      ...
      const handled = await Registration.handleWebContactMessage(data.chatId, data.contact);
      if (!handled) {
        await Helpers.safeSend(data.chatId, '⚠️ Контакт зараз не очікується. Напиши /start, щоб відкрити меню.');
      }
...
async function handleTextMessage(chatId, text) {
  if (Helpers.isCommand({ message: { text } }, 'start')) {
    ...
    if (startPayload && startPayload.indexOf('web_') === 0) {
      await Registration.startWebOnboarding(chatId, startPayload.slice(4));
      return;
    }
  ...
  if (step === CONSTANTS.FSM_STATES.BROADCAST_TEXT) {
    const bcHandled = await Broadcast.handleTextMessage(chatId, text);
    if (bcHandled) return;
  }
...
async function handleCallback(chatId, callbackData, callbackQueryId) {
  ...
  if (
    action === CONSTANTS.CALLBACKS.MENU_BROADCAST ||
    action === CONSTANTS.CALLBACKS.BC_OWN ||
    action === CONSTANTS.CALLBACKS.BC_POT ||
    action === CONSTANTS.CALLBACKS.BC_OWN_ALL ||
    action === CONSTANTS.CALLBACKS.BC_OWN_ACT ||
    action === CONSTANTS.CALLBACKS.BC_OWN_SLP ||
    action === CONSTANTS.CALLBACKS.BC_OK ||
    action === CONSTANTS.CALLBACKS.BC_CANCEL ||
    action === CONSTANTS.CALLBACK_PREFIXES.BC_VEN ||
    action === CONSTANTS.CALLBACK_PREFIXES.BC_CITY
  ) {
    const bcHandled = await Broadcast.handleCallback(chatId, callbackData);
    if (bcHandled) return;
  }
  if (action === CONSTANTS.CALLBACKS.BACK_TO_MAIN) {
    const currentState = await State.getSafe(chatId);
    if (currentState?.venueFromRegistration) {
      await Helpers.safeSend(chatId, '⚠️ Реєстрацію ще не завершено. Обери заклад або продовж реєстрацію пізніше.');
      await Venues.showHub(chatId, {
        fromRegistration: true,
        coachLinkMode: currentState.venueCoachLinkMode || undefined,
        linkCoach: currentState.venueCoachLinkMode === 'teach'
      });
      return;
    }
    await State.clear(chatId);
  ...
  if (
    action === CONSTANTS.CALLBACKS.SCH_CREATE_SLOTS ||
    action === CONSTANTS.CALLBACKS.SCH_CREATE_SLOTS_HINTS ||
    action === CONSTANTS.CALLBACKS.SCH_CREATE_TEMPLATE ||
    action === CONSTANTS.CALLBACKS.SCH_CREATE_SLOTS_RUN
  ) {
  ...
  if (
    ...
    action === CONSTANTS.CALLBACKS.HINTS_PUBLIC_INSTAGRAM ||
    action === CONSTANTS.CALLBACKS.HINTS_PUBLIC_COACH_VENUES ||
    action === CONSTANTS.CALLBACKS.HINTS_FIRST_PRICING ||
    action === CONSTANTS.CALLBACKS.HINTS_PLAN ||
    action === CONSTANTS.CALLBACKS.HINTS_PLAN_TEXT ||
    action === CONSTANTS.CALLBACKS.HINTS_PLAN_STUDENTS_LIST ||
