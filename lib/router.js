...
async function handleCallback(chatId, callbackData, callbackQueryId) {
  ...
  if (
    action === CONSTANTS.CALLBACKS.MENU_HINTS ||
    action === CONSTANTS.CALLBACKS.VOH_FIRST_STEPS ||
    action === CONSTANTS.CALLBACKS.VOH_FIRST_STEPS_TEXT ||
    action === CONSTANTS.CALLBACKS.VOH_PROFILE ||
    action === CONSTANTS.CALLBACKS.VOH_PROFILE_TEXT ||
    action === CONSTANTS.CALLBACKS.VOH_CONTENT ||
    action === CONSTANTS.CALLBACKS.VOH_CONTENT_TEXT ||
    action === CONSTANTS.CALLBACKS.VOH_COACHES ||
    action === CONSTANTS.CALLBACKS.VOH_COACHES_TEXT ||
    action === CONSTANTS.CALLBACKS.VOH_LIMITS ||
    action === CONSTANTS.CALLBACKS.VOH_LIMITS_TEXT ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_FS_TOGGLE ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_FS_VIEW ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_PRF_TOGGLE ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_PRF_VIEW ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_CNT_TOGGLE ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_CNT_VIEW ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_CCH_TOGGLE ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_CCH_VIEW ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_LIM_TOGGLE ||
    action === CONSTANTS.CALLBACK_PREFIXES.VOH_LIM_VIEW
  ) {
    const me = await User.getByChatId(chatId);
    if (me && me.role === CONSTANTS.ROLES.VENUE_OWNER) {
      const voHintsHandled = await VenueOwnerHints.handleCallback(chatId, callbackData);
      if (voHintsHandled) return;
    }
