...
async function handleTextMessage(chatId, text) {
  ...
  if (step.indexOf('reg_') === 0 || step === 'WAITING_FOR_START_CHOICE') {
    // Мінімум: лише /start і кнопки; повну реєстрацію можна додати пізніше
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.REG_ROLE_STUDENT || action === CONSTANTS.CALLBACKS.REG_ROLE_COACH) {
    await Helpers.safeSend(chatId, 'Далі кроки реєстрації (ім\'я, місто тощо) ще переносяться на новий бот. Напиши /start — повернешся в меню, коли будеш зареєстрований в системі.');
    await State.clear(chatId);
    return;
  }
