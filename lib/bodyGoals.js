...
function calcUnifiedIdealModel(user = {}) {
  ...
  let realisticMinK = shouldersOptimal.minK - 0.01;
  let realisticMaxK = shouldersOptimal.maxK + 0.01;
  if (is40plus) {
    realisticMinK -= 0.01;
    realisticMaxK += 0.01;
  }
  ...
  realisticMinK = Math.max(0.53, realisticMinK);
  realisticMaxK = Math.max(realisticMinK + 0.01, realisticMaxK);
  const shouldersRealisticBaseK = (realisticMinK + realisticMaxK) / 2;
