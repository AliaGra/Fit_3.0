/**
 * Бажані параметри тіла — валідація, дельта, збереження, тексти для клієнта/тренера.
 */
...
function calcUnifiedIdealModel(user = {}) {
  ...
  // Waist guidance:
  // - optimal range: long-term target
  // - realistic range: стартовий коридор з урахуванням статі/віку/досвіду
  const experienceDays = (() => {
    if (!user.experienceStartDate) return 0;
    const d = user.experienceStartDate instanceof Date ? user.experienceStartDate : new Date(user.experienceStartDate);
    if (isNaN(d.getTime())) return 0;
    return Math.max(0, Math.floor((Date.now() - d.getTime()) / (24 * 60 * 60 * 1000)));
  })();
  const isExperienced = experienceDays >= 90;
  const is40plus = age != null && age >= 40;
  const realisticBase = gender === 'female'
    ? { min: 0.44, max: 0.49 }
    : { min: 0.45, max: 0.50 };
  let realisticMinK = realisticBase.min;
  let realisticMaxK = realisticBase.max;
  if (is40plus) {
    realisticMinK += 0.01;
    realisticMaxK += 0.01;
  }
  ...
  const waistRealistic = {
    min: Math.round(heightCm * realisticMinK),
    max: Math.round(heightCm * realisticMaxK)
  };
  const waistOptimal = {
    min: Math.round(heightCm * 0.42),
    max: Math.round(heightCm * 0.45)
  };
  // Backward compatibility: existing flows reading `model.waist` get optimal band.
  const waist = waistOptimal;

  const hipsRatio = gender === 'female' ? 0.7 : 0.85;
  const hipsRealistic = {
    min: Math.round(waistRealistic.min / hipsRatio),
    max: Math.round(waistRealistic.max / hipsRatio)
  };
  const hipsOptimal = {
    min: Math.round(waistOptimal.min / hipsRatio),
    max: Math.round(waistOptimal.max / hipsRatio)
  };
  // Backward compatibility: existing flows reading `model.hips` get optimal band.
  const hips = hipsOptimal;

  // Shoulders are estimated from height and body build to avoid inflated values
  // when wrist-based chest coefficients produce oversized shoulder ranges.
  const buildShouldersFromHeight = () => {
    const baseByGender = gender === 'female'
      ? { min: 0.62, max: 0.68 }
      : { min: 0.69, max: 0.74 };
    const buildShift = bodyBuild === 'asthenic'
      ? -0.02
      : (bodyBuild === 'hypersthenic' ? 0.02 : 0);
    const minK = Math.max(0.55, baseByGender.min + buildShift);
    const maxK = Math.max(minK + 0.01, baseByGender.max + buildShift);
    const baseK = (minK + maxK) / 2;
    return {
      minK,
      maxK,
      min: Math.round(heightCm * minK),
      max: Math.round(heightCm * maxK),
      base: Math.round(heightCm * baseK),
      from: 'height_body_build'
    };
  };

  const shouldersOptimal = buildShouldersFromHeight();
  let shouldersRealisticMinK = shouldersOptimal.minK - 0.01;
  let shouldersRealisticMaxK = shouldersOptimal.maxK + 0.01;
  if (is40plus) {
    shouldersRealisticMinK -= 0.01;
    shouldersRealisticMaxK += 0.01;
  }
  if (isExperienced) {
    shouldersRealisticMinK += 0.01;
    shouldersRealisticMaxK -= 0.01;
  }
  shouldersRealisticMinK = Math.max(0.53, shouldersRealisticMinK);
  shouldersRealisticMaxK = Math.max(shouldersRealisticMinK + 0.01, shouldersRealisticMaxK);
  const shouldersRealisticBaseK = (shouldersRealisticMinK + shouldersRealisticMaxK) / 2;
  const shouldersRealistic = {
    min: Math.round(heightCm * shouldersRealisticMinK),
    max: Math.round(heightCm * shouldersRealisticMaxK),
    base: Math.round(heightCm * shouldersRealisticBaseK),
    from: 'height_body_build_realistic'
  };
  // Backward compatibility: existing flows reading `model.shoulders` get optimal band.
  let shoulders = shouldersOptimal;
  let chest = null;
  let biceps = null;
  if (wrist && isFinite(wrist) && wrist > 0) {
    const chestBaseK = gender === 'female' ? 6.1 : 6.9;
    const chestBase = wrist * chestBaseK;
    const bicepsK = 0.36;
...
function validateGoalField(field, value, heightCm, userCtx = null) {
  ...
  if (field === 'goal_weight' && userCtx) {
    const ctx = {
      ...(userCtx || {}),
      height: heightCm != null ? Number(heightCm) : (userCtx && userCtx.height != null ? Number(userCtx.height) : null)
    };
    const model = calcUnifiedIdealModel(ctx);
    const med = model && model.weight ? model.weight.medical : null;
    const aest = model && model.weight ? model.weight.aesthetic : null;
    const candidateMins = [];
    const candidateMaxs = [];
    if (med && Number.isFinite(med.min)) candidateMins.push(Number(med.min));
    if (aest && Number.isFinite(aest.min)) candidateMins.push(Number(aest.min));
    if (med && Number.isFinite(med.max)) candidateMaxs.push(Number(med.max));
    if (aest && Number.isFinite(aest.max)) candidateMaxs.push(Number(aest.max));
    // Goal-weight validation uses the union of personal medical/aesthetic ranges:
    // lower bound = smaller min, upper bound = larger max.
    if (candidateMins.length) limit.min = Math.min(...candidateMins);
    if (candidateMaxs.length) limit.max = Math.max(...candidateMaxs);
    ...
    if (goals.goal_weight < currentMeasurements.weight * 0.95) {
      ...
      result.errors.push({
        ...
        message: age != null && age < 16
          ? 'Для клієнтів до 16 років ціль на зниження ваги недоступна незалежно від налаштувань.'
          : 'Для клієнтів 16–17 років ціль на зниження ваги потребує додаткового підтвердження. Уточніть з тренером.'
      });
    }
  }
...
function formatGoalsSummaryForCoach(deltaItems, studentName) {
  if (!deltaItems || deltaItems.length === 0) {
    return 'Бажані параметри для ' + (studentName || 'клієнта') + ' не встановлені.';
  }
  const lines = ['Цілі клієнта ' + (studentName || '') + ':'];
  for (const item of deltaItems) {
    if (item.reached) {
...
async function saveBodyGoals(coachId, studentChatId, goals) {
  ...
  if (!student) {
    return { saved: false, error: 'Клієнта не знайдено.' };
  }
  if (!student.height) {
    return { saved: false, error: 'Спочатку вкажіть зріст клієнта.' };
  }
...
async function showGoalsToStudent(chatId) {
  ...
  if (shouldShowAIComment(analysis)) {
    ...
    if (user) {
      const block = buildDeterministicGoalsBlock(analysis);
