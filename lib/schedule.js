...
function isScheduleRestDay(settings, dayIdx) {
  const restDays = settings && Array.isArray(settings.restDays) ? settings.restDays : [];
  return restDays.indexOf(dayIdx) >= 0;
}
...
/** Прибирає робочий час для днів, позначених як вихідні. */
function normalizeScheduleSettings(settings) {
  if (!settings) return settings;
  const restDays = Array.isArray(settings.restDays) ? settings.restDays : [];
  if (!restDays.length) return settings;
  if (!settings.workHoursByWeekday || typeof settings.workHoursByWeekday !== 'object') {
    return settings;
  }
  const map = copyExplicitWorkHoursByWeekday(settings);
  let changed = false;
  ...
  return {
    ...settings,
    workHoursByWeekday: keys.length > 0 ? map : null
  };
}
...
function formatSettingsSummary(settings) {
  ...
  const restStr = !restDays.length
    ? 'без вихідних'
    : restDays.sort((a, b) => a - b).map((d) => WEEKDAYS_UA[d]).join(', ');
  let workStr = '';
  if (isPerDayWorkHoursMode(normalized)) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      workStr += '  ' + WEEKDAYS_UA[i] + ': ' + (h ? h.start + '–' + h.end : 'не вказано') + '\n';
    }
