...
/** Кількості майбутніх слотів у вікні «сьогодні + 6 днів» (як у showCoach7DaysView). */
async function getCoachSlotsCountsNext7Days(chatId) {
...
async function showCoachMyScheduleMenu(chatId) {
  ...
  await clearCoachMarkTrainingReturnMode(chatId);
  const c = await getCoachSlotsCountsNext7Days(chatId);
  const keyboard = [
  ...
  await Helpers.sendKeyboard(
    chatId,
    '📅 Мій розклад\n\nЧисла в дужках — кількість майбутніх слотів у найближчому тижні (сьогодні + 6 днів).\n\nОбери перегляд:',
    keyboard,
    { parse_mode: 'Markdown' }
  );
}
...
async function showCoach7DaysView(chatId, filter, page) {
  try {
    ...
    if (filtered.length === 0) {
      text += 'Немає слотів у цьому перегляді на найближчий тиждень. Створи слоти в меню «Розклад».';
    }
