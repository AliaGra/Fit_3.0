...
async function showCoachScheduleMenu(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: '⚙️ Створити', callback_data: CONSTANTS.CALLBACKS.SCH_CREATE_TEMPLATE }],
...
async function startSettingsFlow(chatId) {
  ...
  await showSettingsRestDays(chatId, []);
}

async function showSettingsRestDays(chatId, selectedDays) {
  ...
  const keyboard = [
    ...
    [{ text: '✓ Готово', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_DAY_DONE }],
    [{ text: '🔙 Скасувати', callback_data: CONSTANTS.CALLBACKS.CANCEL_ACTION }]
  ];
  const selectedStr = (selectedDays || []).length === 0 ? 'немає' : (selectedDays || []).sort((a, b) => a - b).map((d) => WEEKDAYS_UA[d]).join(', ');
  await Helpers.sendKeyboard(
    chatId,
    '📅 **Встановіть день відпочинку:**\n\nОбрані: ' + selectedStr + '\n\nНатисни день для вибору/зняття або «Без вихідних».',
    keyboard,
    { parse_mode: 'Markdown' }
  );
}
...
async function handleSettingsDayToggle(chatId, dayIndex) {
  ...
  await State.set(chatId, { ...state, step: CONSTANTS.FSM_STATES.SCH_SETTINGS_REST_DAYS, settingsRestDays: next });
  await showSettingsRestDays(chatId, next);
}
...
async function handleSettingsWorkCustom(chatId) {
  ...
  await Helpers.safeSend(
    chatId,
    'Введи робочий день у форматі **початок–кінець** (наприклад: 09:00–21:00)',
    { parse_mode: 'Markdown' }
  );
}
...
async function handleSettingsEditRest(chatId) {
  ...
  await showSettingsRestDays(chatId, restDays);
}
...
async function generateSlotsForCoach(chatId, numDays) {
  ...
  if (!info || !info.settings) return { ok: false, created: 0, message: 'Спочатку налаштуй шаблон («Створити»).' };
...
async function startCreateSlotsFlow(chatId) {
  ...
  const settings = await supabase.getCoachScheduleSettings(chatId);
  if (!settings) {
    await Helpers.safeSend(chatId, '❌ Спочатку налаштуй шаблон: натисни «Створити» і заповни дні відпочинку, тривалість тренування та робочий день.');
    await showCoachScheduleMenu(chatId);
    return;
  }
