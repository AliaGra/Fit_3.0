...
/** Скидає прапорець повернення після SCH_COMPLETE (меню «Відмітити тренування»). */
async function clearCoachMarkTrainingReturnMode(chatId) {
  try {
    await State.update(chatId, { afterCompleteSlot: null });
  } catch (e) {
    /* ignore */
  }
}
...
/** Підтверджені записи на 7 днів — лише кнопки ✔️ (відмітити як виконане). */
async function showCoachMarkTrainingView(chatId, page) {
  try {
    ...
    let text =
      ...
      'Підтверджені записи на 7 днів (майбутні). Натисни ✔️ біля часу, щоб відмітити тренування як **виконане**.\n\n' +
      '📊 Максимум зайнятих слотів на один день: ' +
      maxBookedPerDay +
      '\n\n';

    if (booked.length === 0) {
      text += 'Немає підтверджених записів на 7 днів.';
    } else {
      text += '✅ Підтверджені:\n';
      for (const s of booked) {
        const name = await getStudentDisplayName(s.studentId);
