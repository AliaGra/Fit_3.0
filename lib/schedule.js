...
async function showCoachMySchedule(chatId) {
  try {
    ...
    for (const s of available) {
      ...
      keyboard.push([{ text: btnText.length > 64 ? '👤 Слот ' + formatSlotDateTime(s) : btnText, callback_data: CONSTANTS.CALLBACKS.SCH_C_REQ + ':' + s.id }]);
    }
    ...
    console.error('Schedule.showCoachMySchedule', err.message);
    await Helpers.safeSend(chatId, '❌ Не вдалося завантажити розклад. Спробуй пізніше або натисни «До розкладу».');
