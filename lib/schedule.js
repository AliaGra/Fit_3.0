/**
 * Schedule — розклад тренувань (слоти), запис тренера/клієнта (без Google Calendar)
 */
...
/** Ім'я клієнта для відображення (в т.ч. для INVITE_ — з профілю запрошення). */
...
/** Вільні для запису клієнта слоти (AVAILABLE, без днів відпустки) */
...
/** Клієнт має майбутній слот REQUESTED — очікує підтвердження/відхилення (запис, перенос або скасування від тренера). */
...
async function showCoachScheduleMenu(chatId) {
  ...
  if (!User.isCoach(user)) {
    if (!user) {
      await Helpers.safeSend(
        chatId,
        '❌ Профіль не знайдено. Натисни /start — якщо реєстрацію вже пройдено, звернись до підтримки.'
      );
      return;
    }
  ...
  const workIntro =
    ...
    '2) Створи вільні слоти для запису клієнтів на тренування через «Слоти» ➡️ «📆 Створити слоти».\n\n';
  ...
  const bookingHint =
    ...
    'Або клієнт сам у своєму меню натискає «Записатись на тренування» — тобі надійде запит на підтвердження тренування.\n\n' +
    'Або ти записуєш клієнта сам:\n' +
    '1) «📆 Календар (21 день)» ➡️ «Вибрати потрібну дату й час»;\n' +
    '2) «👥 Мої клієнти» ➡️ «Вибрати клієнта» ➡️ «🖋️ Записати».';
...
// ——— Відпустка тренера: цілий день недоступний для клієнтів ———
async function showVacationMenu(chatId) {
  ...
  let text = '🏖 Відпустка\n\nДні, коли клієнти не бачать твої слоти (цей день недоступний для запису).\n\n';
  if (sorted.length === 0) {
...
async function showCoach7DaysView(chatId, filter, page, options) {
  try {
    ...
    const viewHint =
      filter === 'requested'
        ? '✅ — підтвердити запис, клієнт побачить час тренування у своєму розкладі | ❌ — скасувати'
        : '✅ — підтвердити запис | ❌ — скасувати | 🍔 — перерва (клієнти не бачать) | 🔄 — перенести. Вільний слот: можна записати клієнта або зробити перерву.';
    ...
    if (filter === 'booked') {
      keyboard.push([{ text: '👥 Мої клієнти', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS }]);
      keyboard.push([{ text: '📆 Календар (21 день)', callback_data: CONSTANTS.CALLBACKS.SCH_CALENDAR }]);
      keyboard.push([{ text: '📅 Розклад', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SCHEDULE }]);
...
async function showCoachMarkTrainingView(chatId, page) {
  try {
    ...
    let text =
      ...
      '💰💲 Відмітка про тренування потрібна для формування твого звіту про доходи, кількість тренувань, і ведення статистики “прогулів” клієнта.\n' +
...
async function showCoachCalendar(chatId) {
  try {
    ...
    const restDays = normalizeRestDayIndexes(settings && settings.restDays);
    const vacationSet = new Set(vacationKeys || []);
...
/** Після дій з розкладу з картки клієнта — повернути список записів або стандартний календар. */
...
/**
 * Розклад майбутніх записів клієнта з картки тренера (перенос / скасування як у «Мій розклад»).
...
async function showCoachStudentSchedule(chatId, studentChatId, options) {
  ...
  const name = ((student.firstName || '') + ' ' + (student.lastName || '')).trim() || 'Клієнт';
  let text = '📅 Записи клієнта — ' + name + '\n\n';
  if (sorted.length === 0) {
    text +=
      'Немає майбутніх записів (підтверджених або на підтвердження).\n\n' +
      'Щоб додати запис, натисни «🖋️ Записати» у картці клієнта.';
  } else {
    text +=
      'Нумерація відповідає кнопкам нижче.\n' +
      '• Підтверджений запис: скасування надсилається клієнту на підтвердження (як у «Мій розклад»).\n' +
  ...
  keyboard.push([{ text: '🔙 До картки клієнта', callback_data: CONSTANTS.CALLBACK_PREFIXES.VIEW_STUDENT + ':' + String(studentChatId) }]);
  await Helpers.sendKeyboard(chatId, text, keyboard);
}
...
async function showCoachDaySlots(chatId, dateKey, _page) {
  try {
    ...
    /** У тексті — рядки лише для зайнятих (клієнт / перерва) та на підтвердженні; вільні — лише кнопками. */
    ...
    if (daySlots.length === 0) {
      ...
      text +=
        '❌ — скасувати тренування (клієнт побачить повідомлення; після підтвердження скасування вашим клієнтом — слот стане вільним)\n' +
        '🔄 — перенести час тренування (клієнт побачить повідомлення; після підтвердження переносу вашим клієнтом — поточний слот стане вільним, а новий слот буде зайнятий)\n\n';
      text +=
        '👤 Вільний — ви можете записати клієнта на тренування в цей час. Клієнт побачить повідомлення, і після підтвердження — слот буде зайнятий.\n\n';
...
async function setSlotReserve(chatId, slotId) {
  ...
  if (ok) {
    await Helpers.safeSend(chatId, '🍔 ' + formatSlotDateTime(slot) + ' — позначено як перерва. Клієнти цей слот не бачать.');
...
async function setSlotUnreserve(chatId, slotId) {
  ...
  if (ok) {
    await Helpers.safeSend(chatId, '✅ Слот знову вільний для запису клієнтів.');
  }
  const dateKey = getDateKey(slot.date);
  await showCoachDaySlots(chatId, dateKey, 0);
}
...
async function askSelectStudentForSlot(chatId, slotId) {
  ...
  if (!students || students.length === 0) {
    await Helpers.safeSend(chatId, '❌ У тебе немає клієнтів. Додай клієнта в меню «Мої клієнти».');
    await showCoachScheduleMenu(chatId);
    return;
  }
  ...
  await Helpers.sendKeyboard(chatId, '👥 Обери клієнта для запису на слот ' + formatSlotDateTime(slot) + ':', keyboard);
}

async function sendCoachBookingPromptToStudent(studentChatId, slot, coachName) {
  ...
  const name = (coachName || 'Тренер').trim() || 'Тренер';
  const studentMsg = '👋 Твій тренер ' + name + ' хоче записати тебе на тренування:\n🗓 ' + dateTimeStr;
  return Helpers.sendKeyboard(studentChatId, studentMsg, kbd);
}
...
async function coachBookStudentToSlot(chatId, slotId, studentChatId) {
  try {
    ...
    if (isInviteStudent) {
      await Helpers.safeSend(chatId, '⏳ Запис збережено. Клієнт ще **не активував код** — повідомлення з кнопками «Підтвердити»/«Відхилити» прийде йому в бот після того, як він введе код у боті (У мене є код). Передай клієнту код і попроси активувати.');
      await showBookStudentCalendar(chatId, studentChatId);
      return;
    }
    const sentToStudent = await sendCoachBookingPromptToStudent(studentChatId, slot, coachName);
    if (sentToStudent) {
      await Helpers.safeSend(chatId, '⏳ Запит на запис надіслано клієнту. Очікуй підтвердження.');
    } else {
      console.error('Schedule.coachBookStudentToSlot: send to student failed (chatId=' + studentChatId + ')');
      await Helpers.safeSend(chatId, '⏳ Запис збережено, але не вдалося надіслати повідомлення клієнту (клієнт має спочатку написати боту /start). Попроси клієнта відкрити бота і підтвердити запис у «Мій розклад».');
    }
    await showBookStudentCalendar(chatId, studentChatId);
  } catch (e) {
...
async function studentConfirmsCoachBooking(chatId, slotId) {
  ...
  if (slot.status !== CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
    ...
    await showStudentMyScheduleAfterCoachBookingAction(chatId);
    return;
  }
  ...
  await Helpers.safeSend(slot.coachId, '✅ Клієнт ' + (student ? student.firstName : chatId) + ' підтвердив запис на ' + dateTimeStr);
  await showStudentMyScheduleAfterCoachBookingAction(chatId);
}
...
async function studentDeclinesCoachBooking(chatId, slotId) {
  ...
  const student = await User.getByChatId(chatId);
  await Helpers.safeSend(slot.coachId, '❌ Клієнт ' + (student ? student.firstName : chatId) + ' відхилив запис на ' + dateTimeStr);
  await showStudentMyScheduleAfterCoachBookingAction(chatId);
}
...
async function confirmSlot(chatId, slotId) {
  ...
  const dateTimeStr = formatSlotDateTime(slot);
  await Helpers.safeSend(chatId, '✅ Запис підтверджено. Клієнт: ' + (student ? student.firstName : slot.studentId) + ', ' + dateTimeStr);
  if (slot.studentId) {
...
async function cancelSlot(chatId, slotId) {
  ...
  /** Слот знову вільний (AVAILABLE + без клієнта), одним запитом до БД. */
...
// ——— Клієнт просить скасування → тренер підтверджує/відхиляє ———
...
// ——— Клієнт просить перенести: календар → дата → вільні слоти на дату ———
...
async function studentPicksRescheduleSlot(chatId, newSlotId) {
  ...
  // Беремо coachId з профілю клієнта (надійніше, ніж зі слота — слот може зберігати старий ID)
  const student = await User.getByChatId(chatId);
  ...
  // Зберігаємо запит у БД: резервуємо новий слот за клієнтом (REQUESTED) — запит не загубиться, навіть якщо сповіщення не дійде
...
async function coachDeclinesReschedule(chatId, oldSlotId, newSlotId) {
  ...
  // Повертаємо новий слот у AVAILABLE і знімаємо клієнта з нього (якщо був збережений запит)
  if (newSlotId) {
    const newSlot = await supabase.getSlotById(newSlotId);
...
// ——— Тренер «Скасувати» (SCH_CANCEL_REQ): запит клієнту — слот лишається BOOKED, доки клієнт не підтвердить або не відхилить (якщо запису клієнта немає — одразу cancelSlot).
async function coachRequestsCancel(chatId, slotId) {
  ...
  const dateTimeStr = formatSlotDateTime(slot);
  await Helpers.safeSend(chatId, '⏳ Запит на скасування надіслано клієнту. Очікуй підтвердження.');
  const kbd = [
...
async function studentConfirmsCoachCancel(chatId, slotId) {
  ...
  const dateTimeStr = formatSlotDateTime(slot);
  /** Клієнт підтверджує запит тренера: слот має бути BOOKED і його studentId. Якщо вже AVAILABLE (дубль кнопки) — ідемпотентно. */
  if (slot.status === CONSTANTS.SCHEDULE_STATUS.AVAILABLE) {
  ...
  await Helpers.safeSend(slot.coachId, '✅ Клієнт підтвердив скасування тренування ' + dateTimeStr + '.');
...
async function studentDeclinesCoachCancel(chatId, slotId) {
  ...
  const dateTimeStr = formatSlotDateTime(slot);
  await Helpers.safeSend(slot.coachId, '❌ Клієнт відхилив скасування тренування ' + dateTimeStr + '.');
  await Helpers.safeSend(chatId, '❌ Скасування відхилено.');
  const Menu = require('./menu');
  await Menu.showScheduleSubmenu(chatId);
}

// ——— Тренер просить перенести → обирає новий слот → клієнт підтверджує/відхиляє ———
...
async function coachPicksRescheduleSlot(chatId, newSlotId) {
  ...
  if (studentId) {
    ...
    await supabase.updateScheduleSlotStatus(newSlotId, CONSTANTS.SCHEDULE_STATUS.REQUESTED);
    await supabase.updateScheduleSlotStudentId(newSlotId, String(studentId));
    // Який саме старий слот звільняти при підтвердженні (у клієнта може бути кілька BOOKED у того ж тренера)
    ...
    await Helpers.safeSend(chatId, '⏳ Запит на перенос надіслано клієнту. Очікуй підтвердження.');
    const kbd = [
...
/** Підтвердження переносу від клієнта за newSlotId (короткий callback; ініціатор — тренер). */
async function studentConfirmsCoachRescheduleByNewSlot(chatId, newSlotId) {
  ...
  await supabase.updateScheduleSlotRescheduleFrom(newSlotId, null);
  const newStr = formatSlotDateTime(newSlot);
  await Helpers.safeSend(oldSlot.coachId, '✅ Клієнт підтвердив перенос тренування на ' + newStr + '.');
  await Helpers.safeSend(chatId, '✅ Перенос підтверджено. Тренування на ' + newStr + '.');
  const Menu = require('./menu');
  await Menu.showScheduleSubmenu(chatId);
}

/** Відхилення переносу від клієнта за newSlotId (короткий callback; ініціатор — тренер). */
async function studentDeclinesCoachRescheduleByNewSlot(chatId, newSlotId) {
  ...
  await Helpers.safeSend(
    newSlot.coachId,
    '❌ Клієнт відхилив перенос тренування' + (dateTimeStr ? ' ' + dateTimeStr : '') + '.'
  );
  await Helpers.safeSend(chatId, '❌ Перенос відхилено.');
  const Menu = require('./menu');
  await Menu.showScheduleSubmenu(chatId);
...
async function studentDeclinesCoachReschedule(chatId, oldSlotId) {
  ...
  const dateTimeStr = formatSlotDateTime(oldSlot);
  await Helpers.safeSend(oldSlot.coachId, '❌ Клієнт відхилив перенос тренування ' + dateTimeStr + '.');
  await Helpers.safeSend(chatId, '❌ Перенос відхилено.');
  const Menu = require('./menu');
  await Menu.showScheduleSubmenu(chatId);
}
...
/** Робочий інтервал для дня тижня 0=Пн … 6=Нд; якщо день вихідний — null; якщо немає workHoursByWeekday — work_start/work_end. */
function getWorkHoursForWeekday(settings, dow) {
  if (isScheduleRestDay(settings, dow)) return null;
...
function buildWorkHoursMapFromSettings(settings) {
  const out = {};
  for (let i = 0; i < 7; i++) {
    const wh = getWorkHoursForWeekday(settings || {}, i);
    if (wh) out[i] = { ...wh };
  }
  return out;
}

/** Час, явно вказаний для дня 0=Пн … 6=Нд у work_hours_by_weekday; інакше null. */
function getExplicitWorkHoursForDay(settings, dow) {
  const by = settings && settings.workHoursByWeekday;
...
function isScheduleRestDay(settings, dayIdx) {
  const restDays = normalizeRestDayIndexes(settings && settings.restDays);
  const d = parseInt(dayIdx, 10);
  return !isNaN(d) && restDays.indexOf(d) >= 0;
}

/** Узгоджує індекси днів відпочинку (0=Пн…6=Нд) як числа — інакше indexOf ламається після JSON. */
function normalizeRestDayIndexes(restDays) {
  if (!Array.isArray(restDays)) return [];
  const out = [];
  for (const raw of restDays) {
...
/** Прибирає дні з restDays (коли користувач робить їх робочими через інтервал). */
function removeRestDaysFromSettings(settings, dayIndexes) {
  if (!settings) return settings;
  const toRemove = Array.isArray(dayIndexes) ? dayIndexes : [];
  if (!toRemove.length) return settings;
  const restDays = Array.isArray(settings.restDays) ? settings.restDays : [];
  const nextRest = restDays.filter((d) => toRemove.indexOf(d) < 0);
  if (nextRest.length === restDays.length) return settings;
  return { ...settings, restDays: nextRest };
}

function restDaysAmongSelected(settings, selectedDays) {
  const selected = Array.isArray(selectedDays) ? selectedDays : [];
  return selected.filter((d) => isScheduleRestDay(settings, d));
}

/** Попередження, коли серед обраних днів є раніше позначені вихідні. */
function restDayToWorkIntervalWarningText(restDayIndexes) {
  const days = Array.isArray(restDayIndexes) ? restDayIndexes : [];
  ...
  if (days.length === 1) {
    return (
      '\n\n⚠️ **Цей день (' +
      names +
      ') ти вказав як вихідний.** Ти впевнений, що відтепер він буде робочим і для нього встановлюється обраний робочий інтервал?\n' +
      'Якщо так — натисни «Застосувати до обраних днів». Якщо ні — обери інший день тижня.'
    );
  }
  return (
    '\n\n⚠️ **Такі дні ти вказав як вихідні: ' +
    names +
    '.** Ти впевнений, що відтепер вони будуть робочими і для них встановлюється обраний робочий інтервал?\n' +
    'Якщо так — натисни «Застосувати до обраних днів». Якщо ні — обери інші дні тижня.'
  );
}

/** Прибирає робочий час для днів, позначених як вихідні; restDays завжди числа 0–6. */
function normalizeScheduleSettings(settings) {
  if (!settings) return settings;
  const restDays = normalizeRestDayIndexes(settings.restDays);
  let next = { ...settings, restDays };
  if (!next.workHoursByWeekday || typeof next.workHoursByWeekday !== 'object') {
    return next;
  }
  const map = copyExplicitWorkHoursByWeekday(next);
  for (const d of restDays) {
    delete map[String(d)];
  }
  const keys = Object.keys(map).filter((k) => map[k] && map[k].start && map[k].end);
  next.workHoursByWeekday = keys.length > 0 ? map : null;
  return next;
}

function copyExplicitWorkHoursByWeekday(settings) {
  const out = {};
  const by = settings && settings.workHoursByWeekday;
  ...
  for (const k of Object.keys(by)) {
    ...
    if (entry && entry.start && entry.end) {
      out[String(k)] = { start: String(entry.start), end: String(entry.end) };
    }
  }
  return out;
}

/** Об'єднати явні інтервали з кількох джерел (пізніші перекривають той самий день). */
function mergeExplicitWorkHoursByWeekday(...sources) {
  const out = {};
  for (const settings of sources) {
    const part = copyExplicitWorkHoursByWeekday(settings);
...
function resolveSettingsDraftForEdit(chatId, state) {
  const fromState = state && state.settingsDraft ? state.settingsDraft : null;
  const fromDbPromise = supabase.getCoachScheduleSettings(chatId);
  return fromDbPromise.then((fromDb) => {
    if (!fromState) return fromDb;
    if (!fromDb) return fromState;
    const workHoursByWeekday = mergeExplicitWorkHoursByWeekday(fromDb, fromState);
    return buildDraftWithWorkHours(
      { ...fromDb, ...fromState, coachId: String(fromState.coachId || fromDb.coachId || chatId) },
      workHoursByWeekday
    );
  });
}

function formatWorkPerDayButtonLabel(dayIdx, settings) {
  if (isScheduleRestDay(settings, dayIdx)) return WEEKDAYS_UA[dayIdx] + ': вихідний';
  const h = getExplicitWorkHoursForDay(settings, dayIdx);
  if (h) return WEEKDAYS_UA[dayIdx] + ': ' + h.start + '–' + h.end;
  return WEEKDAYS_UA[dayIdx] + ': —';
}

function perDayDraftFromExisting(existing, state) {
  const draftSource = state?.settingsDraft?.workHoursByWeekday != null ? state.settingsDraft : existing;
  let map = copyExplicitWorkHoursByWeekday(draftSource);
  const keys = Object.keys(map);
  if (keys.length === 7) {
    const first = map[keys[0]];
    const ws = padHHMM((existing && existing.workStart) || CONSTANTS.SCHEDULE_SETTINGS.WORK_START_DEFAULT);
    const we = padHHMM((existing && existing.workEnd) || CONSTANTS.SCHEDULE_SETTINGS.WORK_END_DEFAULT);
    const allIdentical = keys.every((k) => map[k].start === first.start && map[k].end === first.end);
    if (allIdentical && first.start === ws && first.end === we) {
      map = {};
    }
  }
  return map;
...
function isPerDayWorkHoursMode(settings) {
  return settings && settings.workHoursByWeekday != null && typeof settings.workHoursByWeekday === 'object';
}

function buildSettingsWorkNavKeyboard(backCallback) {
  return [
...
async function loadSettingsDraftBase(chatId, state) {
  const fromState = state?.settingsDraft;
  if (fromState) return fromState;
  const fromDb = await supabase.getCoachScheduleSettings(chatId);
...
function buildDraftWithWorkHours(base, workHoursByWeekday) {
  const map = workHoursByWeekday && typeof workHoursByWeekday === 'object' ? workHoursByWeekday : {};
  let workStart = base.workStart || CONSTANTS.SCHEDULE_SETTINGS.WORK_START_DEFAULT;
  let workEnd = base.workEnd || CONSTANTS.SCHEDULE_SETTINGS.WORK_END_DEFAULT;
  const keys = Object.keys(map).filter((k) => map[k] && map[k].start && map[k].end);
  if (keys.length > 0) {
    const first = map[keys[0]];
  ...
  return {
    ...base,
    coachId: String(base.coachId || base.coach_id || ''),
...
function formatWorkOverviewText(draft) {
  const settings = normalizeScheduleSettings(draft);
  let lines = '🕐 **Робочий час по днях:**\n\n';
  for (let i = 0; i < 7; i++) {
    if (isScheduleRestDay(settings, i)) {
      lines += WEEKDAYS_UA[i] + ': вихідний\n';
      continue;
    }
    const h = getExplicitWorkHoursForDay(settings, i);
    lines += WEEKDAYS_UA[i] + ': ' + (h ? h.start + '–' + h.end : 'не вказано') + '\n';
  }
  lines += '\nНатисни день, щоб змінити час для одного дня, або «Додати інтервал» для інших днів.';
  return lines;
}

/** Блок уже заданих інтервалів (для екрану «Додати інтервал»). */
function formatSavedWorkIntervalsText(draft) {
  if (!draft) return '';
  const settings = normalizeScheduleSettings(draft);
  let hasAny = false;
  let lines = '📋 **Вже задано:**\n';
  for (let i = 0; i < 7; i++) {
    if (isScheduleRestDay(settings, i)) {
      lines += WEEKDAYS_UA[i] + ': вихідний\n';
      continue;
    ...
    const h = getExplicitWorkHoursForDay(settings, i);
    if (h) hasAny = true;
    lines += WEEKDAYS_UA[i] + ': ' + (h ? h.start + '–' + h.end : '—') + '\n';
  }
  if (!hasAny) return '';
  return lines + '\n';
}
...
function formatSettingsSummary(settings) {
  ...
  const restDays = normalized.restDays || [];
  const restStr = !restDays.length
    ? 'без вихідних'
    : restDays.slice().sort((a, b) => a - b).map((d) => WEEKDAYS_UA[d]).join(', ');
  let workStr = '';
  if (isPerDayWorkHoursMode(normalized) || restDays.length > 0) {
    workStr = '🕐 Робочий час по днях:\n';
    for (let i = 0; i < 7; i++) {
      if (isScheduleRestDay(normalized, i)) {
        workStr += '  ' + WEEKDAYS_UA[i] + ': вихідний\n';
        continue;
      }
      const h = getExplicitWorkHoursForDay(normalized, i);
      if (h) {
        workStr += '  ' + WEEKDAYS_UA[i] + ': ' + h.start + '–' + h.end + '\n';
      } else if (!isPerDayWorkHoursMode(normalized)) {
        workStr +=
          '  ' +
          WEEKDAYS_UA[i] +
          ': ' +
          (normalized.workStart || '09:00') +
          '–' +
          (normalized.workEnd || '21:00') +
          '\n';
      } else {
        workStr += '  ' + WEEKDAYS_UA[i] + ': не вказано\n';
      }
    }
    workStr = workStr.trimEnd();
  } else {
    workStr = '🕐 Робочий день: ' + (normalized.workStart || '09:00') + ' — ' + (normalized.workEnd || '21:00');
  }
  return (
    'Ваші дані:\n\n' +
    '📅 Дні відпочинку: ' + restStr + '\n' +
    '⏱️ Тривалість тренування: ' + (normalized.workoutDurationMin || 60) + ' хв\n' +
    workStr
  );
}
...
async function showSettingsWorkDay(chatId) {
  const state = await State.get(chatId);
  const backCallback = state?.editingFromSummary
    ? CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT
    : CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK + ':duration';
  const keyboard = [
    [{ text: 'За замовчуванням (09:00–21:00)', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK + ':default' }],
    [{ text: '✏️ Ввести вручну', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK + ':custom' }],
    ...buildSettingsWorkNavKeyboard(backCallback)
  ];
  await Helpers.sendKeyboard(
    chatId,
    '🕐 Встанови робочий час:\n\n' +
      '• **За замовчуванням** — 09:00–21:00 для всіх днів.\n' +
      '• **Ввести вручну** — вкажеш інтервал і обереш дні тижня (можна додати кілька інтервалів для різних днів).',
    keyboard,
    { parse_mode: 'Markdown' }
  );
}

async function showSettingsWorkOverview(chatId) {
  const state = await State.get(chatId);
  const draft = state?.settingsDraft || {};
  const keyboard = [];
  for (let i = 0; i < 7; i++) {
    const label = formatWorkPerDayButtonLabel(i, draft);
    keyboard.push([{ text: label, callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_DAY_HOURS + ':' + i }]);
  }
  keyboard.push([{ text: '➕ Додати інтервал', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_WORK_CUSTOM_ADD }]);
  keyboard.push([
  ...
  await Helpers.sendKeyboard(chatId, formatWorkOverviewText(draft), keyboard, { parse_mode: 'Markdown' });
}

/** @deprecated alias */
async function showSettingsWorkPerDay(chatId) {
  await showSettingsWorkOverview(chatId);
}

async function showSettingsWorkCustomDayPick(chatId) {
  const state = await State.get(chatId);
  const pending = state?.pendingWorkInterval;
  if (!pending || !pending.start || !pending.end) {
    await handleSettingsWorkCustom(chatId);
    return;
  }
  const draft = state?.settingsDraft || {};
  const selected = Array.isArray(state?.settingsWorkCustomDays) ? state.settingsWorkCustomDays : [];
  const dayButtons = WEEKDAYS_UA.map((label, idx) => {
    const isSelected = selected.indexOf(idx) >= 0;
    const dayLabel = formatWorkPerDayButtonLabel(idx, draft);
    return {
      text: (isSelected ? '✓ ' : '') + dayLabel,
      callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_WORK_CDAY + ':' + idx
    };
  });
  const keyboard = dayButtons.map((btn) => [btn]);
  keyboard.push([{ text: '✓ Застосувати до обраних днів', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_WORK_CUSTOM_APPLY }]);
  keyboard.push(...buildSettingsWorkNavKeyboard(CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK + ':perday'));
  const restSelected = restDaysAmongSelected(draft, selected);
  await Helpers.sendKeyboard(
    chatId,
    '➕ **Новий інтервал:** **' +
      pending.start +
      ...
      pending.end +
      '**\n\n' +
      'Обери дні тижня для цього часу (кожен день — окремий рядок з уже заданим інтервалом).\n' +
      'Новий інтервал замінить попередній лише для обраних днів.\n\n' +
      'Потім натисни «Застосувати».' +
      restDayToWorkIntervalWarningText(restSelected),
    keyboard,
    { parse_mode: 'Markdown' }
  );
}

async function handleSettingsWorkCustomDayToggle(chatId, dayIdx) {
  const state = await State.get(chatId);
  const current = Array.isArray(state?.settingsWorkCustomDays) ? state.settingsWorkCustomDays : [];
  const idx = current.indexOf(dayIdx);
  const next = idx >= 0 ? current.filter((d) => d !== dayIdx) : [...current, dayIdx].sort((a, b) => a - b);
  await State.update(chatId, { settingsWorkCustomDays: next });
  await showSettingsWorkCustomDayPick(chatId);
}

async function handleSettingsWorkCustomApply(chatId) {
  const state = await State.get(chatId);
  const pending = state?.pendingWorkInterval;
  const selected = Array.isArray(state?.settingsWorkCustomDays) ? state.settingsWorkCustomDays : [];
  if (!pending || !pending.start || !pending.end) {
  ...
  if (!selected.length) {
    await Helpers.safeSend(chatId, '⚠️ Обери хоча б один день тижня.');
    await showSettingsWorkCustomDayPick(chatId);
    return;
  }
  const base = await loadSettingsDraftBase(chatId, state);
  const withRestUpdated = removeRestDaysFromSettings(
    { ...base, ...(state?.settingsDraft || {}), coachId: String(chatId) },
    selected
  );
  const workHoursByWeekday = mergeExplicitWorkHoursByWeekday(base, state?.settingsDraft);
  const start = padHHMM(pending.start);
  const end = padHHMM(pending.end);
  for (const d of selected) {
    workHoursByWeekday[String(d)] = { start, end };
  }
  const draft = normalizeScheduleSettings(buildDraftWithWorkHours(withRestUpdated, workHoursByWeekday));
  await State.set(chatId, {
    ...state,
    step: CONSTANTS.FSM_STATES.SCH_SETTINGS_WORK_PER_DAY,
    settingsDraft: draft,
    settingsWorkCustomFlow: true,
    pendingWorkInterval: null,
    settingsWorkCustomDays: [],
...
async function handleSettingsWorkCustomAdd(chatId) {
  const state = (await State.get(chatId)) || {};
  const base = await loadSettingsDraftBase(chatId, state);
  const workHoursByWeekday = mergeExplicitWorkHoursByWeekday(base, state.settingsDraft);
  const draft = buildDraftWithWorkHours(
    { ...base, ...(state.settingsDraft || {}), coachId: String(chatId) },
    workHoursByWeekday
  );
  await State.set(chatId, {
  ...
  await Helpers.sendKeyboard(
    chatId,
    formatSavedWorkIntervalsText(draft) +
      '➕ **Новий інтервал**\n\n' +
      'Введи робочий інтервал (початок–кінець), наприклад: 09:00–18:00\n\n' +
      'Потім обереш дні тижня. Вже задані інтервали збережуться.',
    buildSettingsWorkNavKeyboard(CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK + ':perday'),
    { parse_mode: 'Markdown' }
  );
}
...
async function handleSettingsWorkPerDayOpen(chatId) {
  const state = await State.get(chatId);
  const existing = await loadSettingsDraftBase(chatId, state);
  const workHoursByWeekday = perDayDraftFromExisting(existing, state);
  const draft = buildDraftWithWorkHours({ ...existing, coachId: String(chatId) }, workHoursByWeekday);
  await State.set(chatId, {
    ...state,
    step: CONSTANTS.FSM_STATES.SCH_SETTINGS_WORK_PER_DAY,
    settingsDraft: draft,
...
async function handleSettingsDayHoursPick(chatId, dayIdx) {
  const state = await State.get(chatId);
  const base = state?.settingsDraft || (await loadSettingsDraftBase(chatId, state));
  await State.set(chatId, {
    ...state,
    step: CONSTANTS.FSM_STATES.SCH_SETTINGS_WORK_PER_DAY,
  ...
  let restWarning = '';
  if (isScheduleRestDay(base, dayIdx)) {
    restWarning =
      '\n\n⚠️ **Цей день ти вказав як вихідний.** Ти впевнений, що відтепер він буде робочим і для нього встановлюється робочий інтервал?\n' +
      'Якщо так — введи час (наприклад 09:00–18:00). Якщо ні — повернись назад і обери інший день.';
  }
  await Helpers.safeSend(
    chatId,
    'Введи робочий час для **' + dayName + '** (формат 09:00–21:00):' + restWarning,
    { parse_mode: 'Markdown' }
  );
}
...
async function handleSettingsWorkPerDayTimeInput(chatId, text) {
  ...
  const base = state.settingsDraft || (await loadSettingsDraftBase(chatId, state));
  const withRestUpdated = removeRestDaysFromSettings({ ...base, coachId: String(chatId) }, [dayIdx]);
  const workHoursByWeekday = mergeExplicitWorkHoursByWeekday(withRestUpdated);
  workHoursByWeekday[String(dayIdx)] = { start, end };
  const draft = normalizeScheduleSettings(buildDraftWithWorkHours(withRestUpdated, workHoursByWeekday));
  await State.set(chatId, {
    ...state,
    step: CONSTANTS.FSM_STATES.SCH_SETTINGS_WORK_PER_DAY,
...
async function handleSettingsWorkPerDayDone(chatId) {
  const state = await State.get(chatId);
  const draft = normalizeScheduleSettings(state?.settingsDraft);
  if (!draft) {
...
async function showSettingsSummary(chatId, settings) {
  ...
  const state = await State.get(chatId);
  let savedNow = false;
  if (settings && !state?.editingFromSummary) {
    const existing = await supabase.getCoachScheduleSettings(chatId);
    if (!existing) {
      const payload = { ...settings, coachId: String(settings.coachId || chatId) };
      savedNow = await supabase.upsertCoachScheduleSettings(payload);
      if (savedNow) {
        const { setStepDone } = require('./coachFirstSteps');
        await setStepDone(chatId, 1, true);
      }
    }
  }
  const text =
    ...
    (savedNow
      ? '\n\n✅ Налаштування збережено. Якщо розклад правильний — натисни «Зберегти і створити слоти на найближчі 21 день».'
      : '');
  const createDays = CONSTANTS.SCHEDULE_SETTINGS.SLOTS_INITIAL_CREATE_DAYS || 21;
  const keyboard = [
    [{ text: '✏️ Змінити', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT }],
    [
      {
        text: '📆 Зберегти і створити слоти на найближчі ' + createDays + ' днів',
        callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_SAVE_CREATE + ':' + createDays
      }
    ],
    [{ text: '💡 Підказки', callback_data: CONSTANTS.CALLBACKS.MENU_HINTS }]
  ];
...
async function handleSettingsDayToggle(chatId, dayIndex) {
  ...
  if (state?.editingFromSummary) {
    const current = normalizeRestDayIndexes(state?.settingsRestDays || []);
    const idx = current.indexOf(dayIndex);
    const next = idx >= 0 ? current.filter((d) => d !== dayIndex) : [...current, dayIndex].sort((a, b) => a - b);
...
async function handleSettingsDayDone(chatId) {
  const state = await State.get(chatId);
  const restDays = normalizeRestDayIndexes(state?.settingsRestDays || []);
  if (state?.editingFromSummary) {
    const existing = await resolveSettingsDraftForEdit(chatId, state || {});
    const draft = normalizeScheduleSettings({
      ...(existing || {
        coachId: String(chatId),
      ...
      coachId: String((existing && existing.coachId) || chatId),
      restDays
    });
    const saved = await persistScheduleSettingsAndCleanupSlots(chatId, draft);
    await State.set(chatId, { step: CONSTANTS.FSM_STATES.SCH_SETTINGS_SUMMARY, settingsDraft: draft });
    if (saved.ok && saved.removedSlots > 0) {
      await Helpers.safeSend(
        chatId,
        '✅ Вихідні збережено. Видалено вільних слотів на ці дні: ' + saved.removedSlots + '.'
      );
    }
    await showSettingsSummary(chatId, draft);
  } else {
...
async function handleSettingsWorkCustom(chatId) {
  const state = await State.get(chatId);
  const existing = await loadSettingsDraftBase(chatId, state);
  let workHoursByWeekday = mergeExplicitWorkHoursByWeekday(existing, state?.settingsDraft);
  const keys = Object.keys(workHoursByWeekday);
  if (keys.length === 7) {
    const first = workHoursByWeekday[keys[0]];
    const ws = padHHMM((existing && existing.workStart) || CONSTANTS.SCHEDULE_SETTINGS.WORK_START_DEFAULT);
    const we = padHHMM((existing && existing.workEnd) || CONSTANTS.SCHEDULE_SETTINGS.WORK_END_DEFAULT);
    const allIdentical = keys.every((k) => workHoursByWeekday[k].start === first.start && workHoursByWeekday[k].end === first.end);
    if (allIdentical && padHHMM(first.start) === ws && padHHMM(first.end) === we) {
      workHoursByWeekday = {};
    }
  }
  const draft = buildDraftWithWorkHours({ ...existing, coachId: String(chatId) }, workHoursByWeekday);
  ...
  await Helpers.sendKeyboard(
    chatId,
    'Введи робочий час (початок–кінець), наприклад: 09:00–21:00\n\n' +
      'Далі обереш дні тижня, для яких діє цей інтервал.',
    buildSettingsWorkNavKeyboard(CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK + ':menu')
  );
}
...
async function handleSettingsWorkTimeInput(chatId, text) {
  ...
  const state = await State.get(chatId);
  if (state?.settingsWorkCustomFlow) {
    const start = padHHMM(parsed.workStart);
    const end = padHHMM(parsed.workEnd);
    const base = await loadSettingsDraftBase(chatId, state);
    const workHoursByWeekday = mergeExplicitWorkHoursByWeekday(base, state.settingsDraft);
    ...
    await State.update(chatId, {
      settingsDraft: draft,
      pendingWorkInterval: { start, end },
      settingsWorkCustomDays: [],
      awaitingWorkTime: false,
      step: CONSTANTS.FSM_STATES.SCH_SETTINGS_WORK_CUSTOM_DAYS
    });
    await showSettingsWorkCustomDayPick(chatId);
    return;
  }
...
async function handleSettingsEdit(chatId) {
  const keyboard = [
    ...
    [{ text: '⏱️ Тривалість одного тренування (слота)', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_DURATION }],
    [{ text: '🕐 Початок і кінець робочого дня', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK + ':menu' }],
...
async function handleSettingsEditRest(chatId) {
  const state = await State.get(chatId);
  const existing = (await resolveSettingsDraftForEdit(chatId, state || {})) || state?.settingsDraft;
  const restDays = normalizeRestDayIndexes(existing ? existing.restDays : []);
  await State.set(chatId, {
    ...state,
    step: CONSTANTS.FSM_STATES.SCH_SETTINGS_REST_DAYS,
    settingsRestDays: restDays,
    settingsDraft: existing || state?.settingsDraft,
...
async function handleSettingsSave(chatId) {
  const state = await State.get(chatId);
  let settings = normalizeScheduleSettings(state?.settingsDraft);
  if (!settings) {
    settings = normalizeScheduleSettings(await supabase.getCoachScheduleSettings(chatId));
  }
  if (!settings) {
  ...
  const saved = await persistScheduleSettingsAndCleanupSlots(chatId, settings);
  if (saved.ok) {
    const { setStepDone } = require('./coachFirstSteps');
    await setStepDone(chatId, 1, true);
    await State.clear(chatId);
    let msg = '✅ Налаштування збережено!';
    if (saved.removedSlots > 0) {
      msg += '\nВидалено вільних слотів на вихідні дні: ' + saved.removedSlots + '.';
    }
...
/** Зберегти шаблон і створити слоти на найближчі N календарних днів. */
async function handleSettingsSaveAndCreateSlots(chatId, numDays) {
  const days =
    parseInt(numDays, 10) || CONSTANTS.SCHEDULE_SETTINGS.SLOTS_INITIAL_CREATE_DAYS || 21;
  const state = await State.get(chatId);
  let settings = normalizeScheduleSettings(state?.settingsDraft);
  if (!settings) {
    settings = normalizeScheduleSettings(await supabase.getCoachScheduleSettings(chatId));
  ...
  await Helpers.safeSend(chatId, msg);
  await Helpers.safeSend(chatId, '⏳ Створюю слоти на найближчі ' + days + ' днів...');

  const result = await generateSlotsForCoach(chatId, days);
  if (result.ok) {
    await Helpers.safeSend(chatId, '✅ Створено ' + result.created + ' слотів на найближчі ' + days + ' днів.');
  } else {
    await Helpers.safeSend(
      chatId,
      '⚠️ Налаштування збережено, але слоти не створено: ' +
        (result.message || 'немає доступних днів. Спробуй пізніше через «Створити слоти».')
    );
  }
  await showCoachScheduleMenu(chatId);
}

/** Зберегти шаблон і прибрати майбутні вільні слоти на днях, позначених як вихідні. */
async function persistScheduleSettingsAndCleanupSlots(chatId, settings) {
  const draft = normalizeScheduleSettings({
    ...settings,
    coachId: String((settings && settings.coachId) || chatId)
  });
  const ok = await supabase.upsertCoachScheduleSettings(draft);
  if (!ok) return { ok: false, removedSlots: 0, settings: draft };
  const removedSlots = await removeFutureAvailableSlotsOnRestDays(chatId, draft.restDays || []);
  return { ok: true, removedSlots, settings: draft };
}

async function removeFutureAvailableSlotsOnRestDays(chatId, restDays) {
  const days = normalizeRestDayIndexes(restDays);
  ...
  for (const slot of slots || []) {
    if (!slot || !slot.id || !slot.date) continue;
    ...
    const deleted = await supabase.deleteScheduleSlot(slot.id);
    if (deleted) removed++;
  }
  return removed;
}
...
/** Налаштування з БД або збереження чернетки FSM (після майстра без кнопки «Розклад»). */
async function resolveCoachScheduleSettings(chatId) {
  const fromDb = await supabase.getCoachScheduleSettings(chatId);
  if (fromDb) return fromDb;
  const state = await State.get(chatId);
  const draft = state?.settingsDraft;
  if (!draft) return null;
  const payload = { ...draft, coachId: String(draft.coachId || chatId) };
  const ok = await supabase.upsertCoachScheduleSettings(payload);
  return ok ? payload : null;
}

async function computeDaysAvailable(chatId) {
  const settings = await resolveCoachScheduleSettings(chatId);
...
async function generateSlotsForCoach(chatId, numDays) {
  ...
  const maxCreatable = Math.min(info.daysAvailable, CONSTANTS.SCHEDULE_SETTINGS.SLOTS_DAYS_MAX);
  const reqDaysCapped = Math.min(Math.max(1, parseInt(numDays, 10) || 0), maxCreatable);
  if (reqDaysCapped < 1) {
    return { ok: false, created: 0, message: 'Наразі немає доступних днів для створення слотів.' };
  }
...
async function createSlotsForCoachForDate(chatId, dateKey) {
  ...
  const wh = getWorkHoursForWeekday(settings, dow);
  if (!wh) return { ok: false, created: 0, message: 'Для цього дня не задано робочий інтервал.' };
  const workStartMin = parseTimeToMinutes(wh.start);
  const workEndMin = parseTimeToMinutes(wh.end);
  const durationMin = settings.workoutDurationMin || 60;
  let created = 0;
...
function formatDaysUk(n) {
  const num = parseInt(n, 10);
  if (isNaN(num) || num < 1) return String(n);
  const mod10 = num % 10;
  const mod100 = num % 100;
  if (mod10 === 1 && mod100 !== 11) return num + ' день';
  if (mod10 >= 2 && mod10 <= 4 && (mod100 < 12 || mod100 > 14)) return num + ' дні';
  return num + ' днів';
}

async function navigateAfterCreateSlots(chatId, priorState, result) {
  const fromHints = !!priorState?.createSlotsFromHints;
  await State.update(chatId, {
    step: null,
    createSlotsMaxDays: null,
    createSlotsMinDays: null,
    createSlotsFromHints: null
  });
  if (result.ok) {
  ...
  if (fromHints) {
    ...
    await CoachHints.showFirstStepsChecklist(chatId);
    return;
  }
  await showCoachMyScheduleMenu(chatId);
}

async function runCreateSlotsForDays(chatId, num) {
  const state = await State.get(chatId);
  const maxD = state?.createSlotsMaxDays ?? CONSTANTS.SCHEDULE_SETTINGS.SLOTS_DAYS_MAX;
  const minD = state?.createSlotsMinDays ?? CONSTANTS.SCHEDULE_SETTINGS.SLOTS_DAYS_MIN;
  ...
  await navigateAfterCreateSlots(chatId, state, result);
  return true;
}

async function startCreateSlotsFlow(chatId, options) {
  const opts = options && typeof options === 'object' ? options : {};
  const fromHints = !!opts.fromHints;
  const user = await User.getByChatId(chatId);
  if (!user || user.role !== CONSTANTS.ROLES.COACH) {
    const Menu = require('./menu');
    await Menu.show(chatId);
  ...
  const minD = Math.min(CONSTANTS.SCHEDULE_SETTINGS.SLOTS_DAYS_MIN, info.daysAvailable);
  const maxD = Math.min(info.daysAvailable, CONSTANTS.SCHEDULE_SETTINGS.SLOTS_DAYS_MAX);
  await State.update(chatId, {
    step: CONSTANTS.FSM_STATES.SCH_CREATE_SLOTS_DAYS,
    createSlotsMaxDays: maxD,
    createSlotsMinDays: minD,
    createSlotsFromHints: fromHints
  });
  const keyboard = [
    [{ text: '📆 Створити слоти на ' + formatDaysUk(maxD), callback_data: CONSTANTS.CALLBACKS.SCH_CREATE_SLOTS_RUN + ':' + maxD }]
  ];
  if (minD < maxD) {
    const quickD = Math.min(7, maxD);
    if (quickD >= minD && quickD < maxD) {
      keyboard.push([
        {
          text: '📆 На ' + formatDaysUk(quickD),
          callback_data: CONSTANTS.CALLBACKS.SCH_CREATE_SLOTS_RUN + ':' + quickD
        }
      ]);
    }
  }
  if (fromHints) {
    keyboard.push([{ text: '🔙 До підказок', callback_data: CONSTANTS.CALLBACKS.HINTS_FIRST_STEPS }]);
  } else {
    keyboard.push([{ text: '🔙 Назад', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SLOTS }]);
  }
  const slotsExplain =
    'Слот — це час у календарі (наприклад, 10:00–11:00), коли клієнт може записатися. Слоти формуються автоматично за твоїм робочим графіком.\n\n';
  await Helpers.sendKeyboard(
    chatId,
    '📅 Створити слоти\n\n' +
      slotsExplain +
      'Можна створити слоти на ' +
      formatDaysUk(maxD) +
      (minD < maxD ? ' (або менше — введи число від ' + minD + ' до ' + maxD + ').' : '.') +
      '\n\nНатисни кнопку нижче або введи кількість днів цифрами.',
    keyboard
  );
}
...
async function handleCreateSlotsDaysInput(chatId, text) {
  const num = parseInt(String(text).trim(), 10);
  await runCreateSlotsForDays(chatId, num);
}

// ——— Тренер записує клієнта: календар → дата → вільні + резерв слоти ———
...
async function showBookStudentCalendar(chatId, studentChatId) {
  ...
  const todayKey = getDateKey(today);
  const student = await User.getByChatId(studentChatId);
  const studentName = student ? (student.firstName || '') + (student.lastName ? ' ' + student.lastName : '').trim() : 'клієнта';
  const keyboard = [];
  /** Як «Мій розклад» → календар: 7 рядків × 3 стовпці = 21 день. */
  const CAL_COLS = 3;
  const CAL_ROWS = 7;
  ...
  keyboard.push([{ text: '👥 Мої клієнти', callback_data: CONSTANTS.CALLBACKS.SCH_BOOK_EXIT_STUDENTS }, { text: homeEmoji + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.SCH_BOOK_EXIT_MAIN }]);
...
async function showBookStudentDaySlots(chatId, dateKey) {
  ...
  const homeEmoji = (CONSTANTS.EMOJI && CONSTANTS.EMOJI.HOME) ? CONSTANTS.EMOJI.HOME : '🏠';
  keyboard.push([{ text: '👥 Мої клієнти', callback_data: CONSTANTS.CALLBACKS.SCH_BOOK_EXIT_STUDENTS }, { text: homeEmoji + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.SCH_BOOK_EXIT_MAIN }]);
  const slotHint = 'Обери час — клієнта буде записано на обраний слот. Клієнт отримає запит на підтвердження в «Мій розклад».';
...
/** Дати з записом клієнта в календарі «Записатись»: окремо очікує підтвердження (REQUESTED) і підтверджено (BOOKED). */
...
async function requestBookSlot(chatId, slotId) {
  ...
  const dateTimeStr = formatSlotDateTime(slot);
  const studentName = (user.firstName || '') + (user.lastName ? ' ' + user.lastName : '').trim() || 'Клієнт';
  ...
  // Після відправки запиту повертаємо клієнта в календар (не в список слотів дня).
  await rerenderStudentBookingView();
}
...
async function showStudentMySchedule(chatId) {
  try {
    ...
    await Helpers.sendKeyboard(chatId, text, keyboard);
    const hasCoachBookingPending = future.some(
      (s) => s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED && !s.rescheduleFromSlotId
    );
    if (hasCoachBookingPending) {
      await resendPendingCoachBookingPrompts(chatId);
    }
  } catch (err) {
...
/** Повторно надсилає клієнту запити тренера на підтвердження запису (без переносу). */
async function resendPendingCoachBookingPrompts(studentChatId) {
  try {
    const slots = await supabase.getSlotsByStudentAndStatus(String(studentChatId), CONSTANTS.SCHEDULE_STATUS.REQUESTED);
    const pending = (slots || []).filter((s) => s && isSlotInFuture(s) && !s.rescheduleFromSlotId);
    for (const slot of pending) {
      const coach = await User.getByChatId(slot.coachId);
      const coachName = coach
        ? ((coach.firstName || '') + (coach.lastName ? ' ' + coach.lastName : '')).trim() || 'Тренер'
        : 'Тренер';
      try {
        await sendCoachBookingPromptToStudent(studentChatId, slot, coachName);
      } catch (e) {
        console.error('Schedule.resendPendingCoachBookingPrompts slot', slot.id, e.message);
      }
    }
  } catch (e) {
    console.error('Schedule.resendPendingCoachBookingPrompts', e.message);
  }
}

async function showStudentMyScheduleAfterCoachBookingAction(chatId) {
  await showStudentMySchedule(chatId);
}
...
async function showStudentMyScheduleEdit(chatId) {
  try {
    ...
    const student = await User.getByChatId(chatId);
    const coachId = student && student.coachId ? String(student.coachId).trim() : '';
    let coach = null;
    if (coachId) coach = await User.getByChatId(coachId);
    const coachTgBtn = coach ? Helpers.telegramContactButton('💬 Написати тренеру в Telegram', coach) : null;

    let headerText =
      '🔄 Змінити запис\n\nОбери тренування та дію:\n\n' +
      '💡 Якщо тренування потрібно скасувати — звернись до тренера.';
    if (coachTgBtn) {
      headerText += '\nНатисни кнопку нижче, щоб написати тренеру в Telegram.';
    }

    if (coachTgBtn) {
      await Helpers.sendKeyboard(chatId, headerText, [[coachTgBtn]]);
    } else {
      await Helpers.safeSend(chatId, headerText);
    }
...
async function coachConfirmsRescheduleByNewSlot(chatId, newSlotId) {
  ...
  if (!studentId) {
    await Helpers.safeSend(chatId, '❌ Клієнта не визначено для цього слоту.');
    await showCoachMyScheduleMenu(chatId);
    return;
  }
  // Знаходимо старий слот: BOOKED слот цього клієнта у цього тренера
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.SCH_CREATE_RESERVE) {
    await Helpers.safeSend(chatId, '🍔 **Створити перерву**\n\nОберіть день в календарі нижче, потім на вільному слоті натисніть «🍔 Хочу перерву». Слот у перерві клієнти не бачать; записати на нього можна з «Записати» клієнта.', { parse_mode: 'Markdown' });
    await showCoachCalendar(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACKS.SCH_CREATE_SLOTS_HINTS) {
    await startCreateSlotsFlow(chatId, { fromHints: true });
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.SCH_CREATE_SLOTS_RUN && rest) {
    const num = parseInt(rest.trim(), 10);
    await runCreateSlotsForDays(chatId, num);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACKS.SCH_SETTINGS_DAY_NONE) {
    ...
    if (state?.editingFromSummary) {
      ...
      await persistScheduleSettingsAndCleanupSlots(chatId, draft);
      await State.set(chatId, { step: CONSTANTS.FSM_STATES.SCH_SETTINGS_SUMMARY, settingsDraft: draft });
      await showSettingsSummary(chatId, draft);
    } else {
      await State.set(chatId, { ...state, step: CONSTANTS.FSM_STATES.SCH_SETTINGS_DURATION, settingsRestDays: restDays });
  ...
  if (action === CONSTANTS.CALLBACKS.SCH_SETTINGS_WORK_CUSTOM_APPLY) {
    await handleSettingsWorkCustomApply(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.SCH_SETTINGS_WORK_CUSTOM_ADD) {
    await handleSettingsWorkCustomAdd(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.SCH_SETTINGS_WORK_CDAY && rest) {
    const dayIdx = parseInt(rest.trim(), 10);
    if (!isNaN(dayIdx) && dayIdx >= 0 && dayIdx <= 6) await handleSettingsWorkCustomDayToggle(chatId, dayIdx);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK && rest) {
    ...
    if (param === 'default') await handleSettingsWorkDefault(chatId);
      ...
      await State.set(chatId, {
        ...st,
        step: CONSTANTS.FSM_STATES.SCH_SETTINGS_DURATION,
        awaitingWorkTime: false,
        awaitingWorkTimeForDay: null,
        pendingWorkInterval: null,
        settingsWorkCustomDays: []
  ...
  if (action === CONSTANTS.CALLBACKS.SCH_SETTINGS_SAVE_CREATE) {
    const days = rest ? parseInt(rest.trim(), 10) : CONSTANTS.SCHEDULE_SETTINGS.SLOTS_INITIAL_CREATE_DAYS;
    await handleSettingsSaveAndCreateSlots(chatId, days);
    return true;
  }
...
async function handleTextMessage(chatId, text) {
  ...
  if (step === CONSTANTS.FSM_STATES.SCH_SETTINGS_WORK_DAY) {
    if (!state.awaitingWorkTime && parseWorkTime(text)) {
      await State.update(chatId, {
        settingsWorkCustomFlow: true,
        settingsWorkCustomDays: [],
        pendingWorkInterval: null,
    ...
    await Helpers.safeSend(
      chatId,
      'Обери варіант кнопкою вище або введи інтервал, наприклад: 09:00–21:00'
    );
    return true;
  }

  if (step === CONSTANTS.FSM_STATES.SCH_SETTINGS_WORK_CUSTOM_DAYS) {
    await Helpers.safeSend(chatId, 'Обери дні тижня кнопками вище, потім натисни «Застосувати до обраних днів».');
    return true;
  }
...
module.exports = {
  getCoachSlotsCountsMyScheduleWindow,
  formatSlotDateTime,
  startBookStudent,
