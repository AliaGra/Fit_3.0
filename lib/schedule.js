...
async function studentRequestsReschedule(chatId, slotId) {
  ...
  const keyboard = showSlots.map((s) => [{ text: formatSlotDateTime(s), callback_data: CONSTANTS.CALLBACKS.SCH_S_RESCHEDULE_PICK + ':' + s.id }]);
  keyboard.push([{ text: '🔙 Скасувати', callback_data: CONSTANTS.CALLBACKS.CANCEL_ACTION }]);
...
// ——— Тренер записує учня: календар → дата → тільки вільні слоти ———
...
async function showStudentMySchedule(chatId) {
  try {
    ...
    for (const s of showSlots) {
      ...
      const cbCancel = CONSTANTS.CALLBACKS.SCH_S_CANCEL_REQ + ':' + String(s.id);
      ...
      keyboard.push([{ text: '❌ Скасувати', callback_data: cbCancel }, { text: '🔄 Перенести', callback_data: cbRes }]);
      try {
        await Helpers.sendKeyboard(chatId, line, keyboard);
      } catch (e) {
