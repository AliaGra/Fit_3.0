...
async function showSettingsWorkCustomDayPick(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: '🔙 Назад', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK + ':menu' }]
  ];
  await Helpers.sendKeyboard(
    chatId,
    '🕐 Інтервал: **' +
...
async function handleSettingsWorkCustomApply(chatId) {
  ...
  const workHoursByWeekday = copyExplicitWorkHoursByWeekday(state?.settingsDraft || base);
...
async function handleSettingsWorkPerDayTimeInput(chatId, text) {
  ...
  const workHoursByWeekday = copyExplicitWorkHoursByWeekday(base);
