...
async function showCoach7DaysView(chatId, filter, page) {
  try {
    ...
    /** Підказку з діями не показуємо у «Вільні слоти», «Зайняті» та «Мої перерви». У «Чекають підтвердження» — коротка підказка без перерви/перенесення/вільного слоту. */
    const showHint = filter !== 'booked' && filter !== 'reserved' && filter !== 'available';
    const viewHint =
      filter === 'requested'
        ? '✅ — підтвердити запис, учень побачить час тренування у своєму розкладі | ❌ — відхилити/скасувати'
        : '✅ — підтвердити запис | ❌ — відхилити/скасувати | 🍔 — перерва (учні не бачать) | 🔄 — перенести. Вільний слот: можна записати учня або зробити перерву.';
