...
async function findUserByAnyStudentId(studentId) {
  ...
  const candidates = Array.from(new Set([
    raw,
    upper,
    upper.startsWith('USED_') ? upper.slice(5) : upper,
    upper.startsWith('USED_') ? raw.slice(5) : raw
  ].filter(Boolean)));

  for (const id of candidates) {
    const byChat = await supabase.getUserByChatId(id);
  ...
  try {
    ...
    if (requested.length && (filter === 'all' || filter === 'requested')) {
      ...
      for (const s of requested) {
        ...
        const rawName = await getStudentSurnameFirstName(s.studentId);
        const name = sanitizeStudentNameForUi(rawName, 'Учень');
        const inviteMark = isInviteStudent ? ' [INV]' : '';
        text += '• ' + formatSlotDateTime(s, { noYear: true }) + ' — ' + name + inviteMark + '\n';
      }
      if (filter === 'all') text += '\n';
    }
    ...
    for (const s of pageSlots) {
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        const requestedStudentNameRaw = await getStudentSurnameFirstName(s.studentId);
        const requestedStudentName = sanitizeStudentNameForUi(requestedStudentNameRaw, 'Учень');
