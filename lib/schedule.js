...
const MAX_STUDENT_MY_SCHEDULE_SLOTS = 3;
...
async function showCoachScheduleMenu(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: '📌 Створити резерв', callback_data: CONSTANTS.CALLBACKS.SCH_CREATE_RESERVE }],
    [{ text: '🏖 Відпустка', callback_data: CONSTANTS.CALLBACKS.SCH_VACATION }],
...
async function showCoachMyScheduleMenu(chatId) {
  ...
  const keyboard = [
    [{ text: 'Зайняті слоти (7 днів)', callback_data: CONSTANTS.CALLBACKS.SCH_7_BOOKED }],
    [{ text: 'Вільні слоти (7 днів)', callback_data: CONSTANTS.CALLBACKS.SCH_7_AVAILABLE }],
    [{ text: 'Всі слоти (7 днів)', callback_data: CONSTANTS.CALLBACKS.SCH_7_ALL }],
    [{ text: 'Мої резерви (7 днів)', callback_data: CONSTANTS.CALLBACKS.SCH_7_RESERVED }],
    [{ text: 'Чекають підтвердження (7 днів)', callback_data: CONSTANTS.CALLBACKS.SCH_7_REQUESTED }],
    [{ text: '📆 Календар (28 днів)', callback_data: CONSTANTS.CALLBACKS.SCH_CALENDAR }],
    [{ text: CONSTANTS.EMOJI.BACK + ' До розкладу', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SCHEDULE }],
    [{ text: CONSTANTS.EMOJI.HOME + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]
  ];
  const menuHint = '📌 **Як користуватися:**\n• Календар (28 днів) — кольори: 🟢 сьогодні, 🔴 вихідний/відпустка, ⬜ немає слотів. Число в дужках — зайняті слоти.\n• Обери вид (зайняті/вільні/всі/резерв/очікують) або календар — потім обери дату або слот. На слоті: ✅ підтвердити, ❌ відхилити, 📌 резерв, 🔄 перенести.';
  await Helpers.sendKeyboard(chatId, '📅 Мій розклад\n\n' + menuHint + '\n\nОбери перегляд:', keyboard, { parse_mode: 'Markdown' });
}
...
async function showCoach7DaysView(chatId, filter, page) {
  try {
    ...
    const startKey = getDateKey(today);
    ...
    const viewHint = '✅ — підтвердити запис | ❌ — відхилити/скасувати | 📌 — резерв (учні не бачать) | 🔄 — перенести. Вільний слот: можна записати учня або поставити в резерв.';
    let text = '📅 Мій розклад — 7 днів (' + filterLabel + ')\n\n' + viewHint + '\n\n';
    ...
    if (reserved.length && (filter === 'all' || filter === 'reserved')) {
      text += '📌 Резерв:\n';
      for (const s of reserved) text += '• ' + formatSlotDateTime(s) + ' — резерв\n';
    }
    if (filtered.length === 0) {
      text += 'Немає слотів за 7 днів. Створи слоти в меню «Розклад».';
    }
    ...
    for (const s of pageSlots) {
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        ...
        keyboard.push([
          { text: '📌 ' + formatSlotDateTime(s) + ' (резерв)', callback_data: CONSTANTS.CALLBACKS.SCH_C_REQ + ':' + s.id },
  ...
  try {
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      for (let col = 0; col < COLS; col++) {
        ...
        const isVacation = vacationSet.has(key);
        const isDayOff = isRestDay || isVacation;
        const isToday = key === todayKey;
        let emoji = '';
        if (isToday) emoji = '🟢 ';
        else if (isDayOff) emoji = '🔴 ';
        else if (total === 0) emoji = '⬜ ';
        const dateLabel = formatDateShortWithWeekday(d);
        const label = emoji + dateLabel + ' (' + occupied + ')';
        rowButtons.push({ text: label.length > 64 ? dateLabel + ' (' + occupied + ')' : label, callback_data: CONSTANTS.CALLBACKS.SCH_CALENDAR_DAY + ':' + key });
      }
      keyboard.push(rowButtons);
    }
    keyboard.push([{ text: CONSTANTS.EMOJI.BACK + ' До Мій розклад', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SLOTS }]);
    const hint = '**Кольори кнопок:**\n🟢 — сьогодні\n🔴 — вихідний або відпустка\n⬜ — немає слотів (створи в «Створити слоти»)\n\nЧисло в дужках — кількість зайнятих слотів на день. Натисни на дату — відкриються слоти та дії (підтвердити, резерв, перенести).';
    await Helpers.sendKeyboard(chatId, '📆 Календар — 28 днів від сьогодні\n\n' + hint + '\n\nОбері дату:', keyboard, { parse_mode: 'Markdown' });
  } catch (err) {
...
async function showCoachDaySlots(chatId, dateKey, page) {
  try {
    ...
    const dayHint = 'Дії: ✅ підтвердити запис | ❌ відхилити/скасувати | 📌 резерв або відкрити слот | 🔄 перенести.';
    let text = '📅 Слоти на ' + dateLabel + '\n\n' + dayHint + '\n\n';
    if (daySlots.length === 0) {
      ...
      text += '\n📌 **Резерв:**\n';
      if (reserved.length) {
        for (const s of reserved) text += '• ' + formatSlotDateTime(s) + '\n';
      } else text += '—\n';
      text += '\n⏳ **На підтвердженні:**\n';
      if (requested.length) {
        for (const s of requested) {
    ...
    for (const s of pageSlots) {
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        ...
        const btnText = '👤 ' + formatSlotDateTime(s) + ' (Вільний)';
        keyboard.push([
          { text: btnText.length > 64 ? '👤 ' + formatSlotDateTime(s) : btnText, callback_data: CONSTANTS.CALLBACKS.SCH_C_REQ + ':' + s.id },
          { text: '📌 Резерв', callback_data: CONSTANTS.CALLBACKS.SCH_SLOT_RESERVE + ':' + s.id }
        ]);
      } else if (s.status === CONSTANTS.SCHEDULE_STATUS.RESERVED) {
        keyboard.push([
          { text: '📌 ' + formatSlotDateTime(s) + ' (резерв)', callback_data: CONSTANTS.CALLBACKS.SCH_C_REQ + ':' + s.id },
...
async function setSlotReserve(chatId, slotId) {
  ...
  if (ok) {
    await Helpers.safeSend(chatId, '📌 Слот позначено як резерв. Учні його не бачать; ти можеш записати учня з календаря.');
  }
...
async function showStudentRescheduleCalendar(chatId) {
  try {
    ...
    const msg = '🔄 Перенести тренування (було: ' + formatSlotDateTime(slot) + ')\n\n📆 Обері дату:';
...
async function studentPicksRescheduleSlot(chatId, newSlotId) {
  ...
  const coachChatId = (oldSlot.coachId != null && String(oldSlot.coachId).trim() !== '') ? String(oldSlot.coachId) : null;
  if (!coachChatId) {
    console.error('Schedule.studentPicksRescheduleSlot: oldSlot.coachId missing', { oldSlotId, newSlotId });
    await State.clear(chatId);
    await Helpers.safeSend(chatId, '❌ Помилка: тренера не визначено для цього слоту. Звернись до підтримки.');
    const Menu = require('./menu');
  ...
  try {
    ...
    const sent = await telegram.sendKeyboard(coachChatId, coachMsg, kbd);
    if (!sent) {
      console.error('Schedule.studentPicksRescheduleSlot: failed to send to coach', { coachChatId });
      await Helpers.safeSend(chatId, '⚠️ Запит збережено, але не вдалося надіслати повідомлення тренеру (тренер має спочатку написати боту /start). Повідом тренеру про перенос особисто.');
    } else {
      await Helpers.safeSend(chatId, '⏳ Запит на перенос надіслано тренеру. Очікуй підтвердження.');
    }
  } catch (err) {
    console.error('Schedule.studentPicksRescheduleSlot: coach send error', err.message);
    await Helpers.safeSend(chatId, '⚠️ Не вдалося надіслати запит тренеру. Спробуй пізніше або повідом тренеру особисто.');
  }
  await State.clear(chatId);
  const Menu = require('./menu');
  await Menu.showScheduleSubmenu(chatId);
...
async function coachRequestsReschedule(chatId, slotId) {
  ...
  const future = bookable.filter((s) => isSlotInFuture(s) && String(s.id) !== String(slotId));
  if (future.length === 0) {
  ...
  const showSlots = future.slice(0, MAX_AVAILABLE_SLOT_BUTTONS);
  const keyboard = showSlots.map((s) => [{ text: formatSlotDateTime(s), callback_data: CONSTANTS.CALLBACKS.SCH_RESCHEDULE_PICK + ':' + s.id }]);
  keyboard.push([{ text: '🔙 Скасувати', callback_data: CONSTANTS.CALLBACKS.CANCEL_ACTION }]);
  let msg = '🔄 Обери новий слот для переносу (було: ' + formatSlotDateTime(slot) + '):';
  if (future.length > MAX_AVAILABLE_SLOT_BUTTONS) {
    msg += '\n\n(Показано перші ' + MAX_AVAILABLE_SLOT_BUTTONS + ' слотів.)';
  }
  await Helpers.sendKeyboard(chatId, msg, keyboard);
}
...
async function coachPicksRescheduleSlot(chatId, newSlotId) {
  ...
  const oldSlot = await supabase.getSlotById(oldSlotId);
  const newSlot = await supabase.getSlotById(newSlotId);
  const newSlotOk = newSlot.status === CONSTANTS.SCHEDULE_STATUS.AVAILABLE || newSlot.status === CONSTANTS.SCHEDULE_STATUS.RESERVED;
...
async function requestBookSlot(chatId, slotId) {
  ...
  const rerenderStudentBookingView = async () => {
    if (dateKey) return showStudentBookDaySlots(chatId, dateKey);
    return showStudentBookCalendar(chatId);
  };
  ...
  // Не виходимо з меню "Записатись на тренування", поки учень сам не натисне "Назад".
  // Оновлюємо список доступних слотів на тій самій даті.
  await rerenderStudentBookingView();
}
...
async function showStudentMySchedule(chatId) {
  try {
    ...
    console.log('Schedule.showStudentMySchedule chatId=' + studentId);
    ...
    if (future.length === 0) {
      const text = '📅 Мій розклад\n\nНемає майбутніх записів. Записатись можна кнопкою «Записатись на тренування».\n\nЯкщо тренер щойно записав тебе — натисни «Мій розклад» ще раз або /start.';
      await Helpers.safeSend(chatId, text);
      await Helpers.sendKeyboard(chatId, '.', [[{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]]);
      return;
    }
    ...
    text += '\nКнопки до кожного запису — нижче.';
    await Helpers.safeSend(chatId, text);
    ...
    for (const s of showSlots) {
      ...
      const cbSlotInfo = CONSTANTS.CALLBACKS.SCH_S_SLOT_INFO + ':' + String(s.id);
      const keyboard = [];
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        keyboard.push([{ text: '✅ Підтвердити', callback_data: cbConf }, { text: '❌ Відхилити', callback_data: cbDecl }]);
      }
      keyboard.push([{ text: line, callback_data: cbSlotInfo }, { text: '🔄 Перенести', callback_data: cbRes }]);
      try {
        await Helpers.sendKeyboard(chatId, '.', keyboard);
      } catch (e) {
        console.error('Schedule.showStudentMySchedule send slot', s.id, e.message);
      }
    }
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.SCH_CREATE_RESERVE) {
    await Helpers.safeSend(chatId, '📌 **Створити резерв**\n\nОберіть день в календарі нижче, потім на вільному слоті натисніть «📌 Резерв». Слот у резерві учні не бачать; записати на нього можна з «Записати» учня.', { parse_mode: 'Markdown' });
    await showCoachCalendar(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.SCH_7_ALL) {
    await showCoach7DaysView(chatId, 'all', 0);
    return true;
  }
