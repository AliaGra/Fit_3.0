...
async function showSettingsWorkCustomDayPick(chatId) {
  ...
  const draft = state?.settingsDraft || {};
  const savedBlock = formatSavedWorkIntervalsText(draft);
  ...
  const keyboard = [
    dayButtons.slice(0, 4),
    dayButtons.slice(4, 7),
    [{ text: '✓ Застосувати до обраних днів', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_WORK_CUSTOM_APPLY }],
    [{ text: '🔙 Назад', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT_WORK + ':perday' }]
  ];
  await Helpers.sendKeyboard(
    chatId,
    savedBlock +
      ...
      pending.end +
      '**\n\n' +
      'Обери дні тижня для цього часу. На кнопках видно вже заданий час; новий інтервал замінить його для обраних днів.\n\n' +
