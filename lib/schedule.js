...
async function showSettingsWorkCustomDayPick(chatId) {
  ...
  const dayButtons = WEEKDAYS_UA.map((label, idx) => {
    ...
    return { text: (isSelected ? '✓ ' : '') + label, callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_WORK_CDAY + ':' + idx };
  });
  const keyboard = [
    dayButtons.slice(0, 4),
    dayButtons.slice(4, 7),
  ...
  await Helpers.sendKeyboard(
    ...
    '🕐 Інтервал: **' +
      ...
      pending.end +
      '**\n\n' +
      'Обери дні тижня, для яких діє цей час (можна кілька). Потім натисни «Застосувати».',
    keyboard,
    { parse_mode: 'Markdown' }
  );
}
