...
async function showCoach7DaysView(chatId, filter, page) {
  try {
    ...
    /** Підказку з діями (підтвердити / скасувати / перерва / перенести) не показуємо у «Вільні слоти». */
    ...
    /** Для «зайняті» — один екран без пагінації; для інших фільтрів — сторінки по MAX_SLOT_BUTTONS_PER_PAGE. */
    const fromIdx = filter === 'booked' ? 0 : page * MAX_SLOT_BUTTONS_PER_PAGE;
    let sortedAvailableForPaging = null;
    if (filter === 'available') {
      sortedAvailableForPaging = [...filtered].sort((a, b) => {
    ...
    if (filter === 'booked') {
      ...
      pageSlots = sortedAvailableForPaging.slice(fromIdx, fromIdx + MAX_SLOT_BUTTONS_PER_PAGE);
    } else {
      pageSlots = filtered.slice(fromIdx, fromIdx + MAX_SLOT_BUTTONS_PER_PAGE);
    }
    const lengthForPaging = filter === 'available' ? sortedAvailableForPaging.length : filtered.length;
    ...
    text += (showHint ? '\n\n' + viewHint : '') + '\n\n';
    if (filter === 'available' && pageSlots.length) {
      let prevDateKey = '';
    ...
    if (available.length && (filter === 'all' || filter === 'available') && filter !== 'available') {
      text += '🕐 Вільні вікна:\n';
    ...
    for (const s of pageSlots) {
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        ...
        const timeOnly = formatSlotTimeOnly(s);
        const btnText =
          filter === 'available' ? '👤 ' + timeOnly + ' (Вільний)' : '👤 ' + formatSlotDateTime(s) + ' (Вільний)';
        keyboard.push([
          {
            text: btnText.length > 64 ? '👤 ' + timeOnly : btnText,
            callback_data: CONSTANTS.CALLBACKS.SCH_C_REQ + ':' + s.id
          }
        ]);
      }
    }
    const totalPages =
      filter === 'booked' ? 1 : Math.max(1, Math.ceil(lengthForPaging / MAX_SLOT_BUTTONS_PER_PAGE));
    if (totalPages > 1) {
      const nav = [];
