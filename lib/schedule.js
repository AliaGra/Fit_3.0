...
async function showCoachScheduleMenu(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: '🕒 Слоти - праця с ними', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SLOTS }],
  ...
  const bookingHint =
    '💡 Записати учня на тренування можна:\n' +
    '1) Через меню «Мої учні» ➡️ «Вибрати учня» ➡️ «Записати»\n' +
    '2) Або через «Календар» ➡️ «Вибрати потрібну дату і час»';
...
async function showCoach7DaysView(chatId, filter, page, options) {
  try {
    ...
    keyboard.push([{ text: '🕒 Слоти - праця з ними', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SLOTS }]);
    keyboard.push([{ text: CONSTANTS.EMOJI.BACK + ' До розкладу', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SCHEDULE }]);
