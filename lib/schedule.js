...
async function showStudentMySchedule(chatId) {
  try {
    ...
    if (future.length === 0) {
      ...
      await Helpers.sendKeyboard(chatId, text, [[{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]]);
      return;
    }
    ...
    for (const s of showSlots) {
      const statusLabel = s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED ? '⏳ очікує' : '✅';
      const line = statusLabel + ' ' + formatSlotDateTime(s);
      const keyboard = [];
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        keyboard.push([
          { text: '✅ Підтвердити', callback_data: CONSTANTS.CALLBACKS.SCH_SC_CONF + ':' + s.id },
          { text: '❌ Відхилити', callback_data: CONSTANTS.CALLBACKS.SCH_SC_DECL + ':' + s.id }
        ]);
      }
      keyboard.push([
        { text: '❌ Скасувати', callback_data: CONSTANTS.CALLBACKS.SCH_S_CANCEL_REQ + ':' + s.id },
        { text: '🔄 Перенести', callback_data: CONSTANTS.CALLBACKS.SCH_S_RES + ':' + s.id }
      ]);
