...
async function showCoach7DaysView(chatId, filter, page) {
  try {
    ...
    const viewHint =
      filter === 'requested'
        ? '✅ — підтвердити запис, учень побачить час тренування у своєму розкладі | ❌ — відмінити'
        : '✅ — підтвердити запис | ❌ — відмінити | 🍔 — перерва (учні не бачать) | 🔄 — перенести. Вільний слот: можна записати учня або зробити перерву.';
