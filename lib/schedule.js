...
async function showCoach7DaysView(chatId, filter, page, options) {
  try {
    ...
    if (filter === 'booked') {
      keyboard.push([{ text: '👥 Мої учні', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS }]);
...
async function startCreateSlotsFlow(chatId, options) {
  ...
  const slotsExplain =
    'Слот — це час у календарі (наприклад, 10:00–11:00), коли учень може записатися. Слоти формуються автоматично за твоїм робочим графіком.\n\n';
  await Helpers.sendKeyboard(
    chatId,
    '📅 Створити слоти\n\n' +
      slotsExplain +
...
/** Повторно надсилає учню запити тренера на підтвердження запису (без переносу). */
