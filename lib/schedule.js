  ...
  try {
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      for (let col = 0; col < DAYS_IN_WEEK; col++) {
        ...
        const label = emoji + formatDateShort(d) + (occupied ? ' (' + occupied + ')' : '');
        rowButtons.push({ text: label.length > 64 ? formatDateShort(d) + (occupied ? '(' + occupied + ')' : '') : label, callback_data: CONSTANTS.CALLBACKS.SCH_CALENDAR_DAY + ':' + key });
      }
      keyboard.push(rowButtons);
    }
    keyboard.push([{ text: CONSTANTS.EMOJI.BACK + ' До Мій розклад', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SLOTS }]);
