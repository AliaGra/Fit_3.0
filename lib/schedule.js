...
async function studentPicksRescheduleSlot(chatId, newSlotId) {
  ...
  // Сповіщення тренеру (await — щоб логувати результат і не загубити при завершенні запиту)
  const student = await User.getByChatId(chatId);
  ...
  try {
    ...
    if (!sent) {
      ...
      console.log('Schedule.studentPicksRescheduleSlot: coach notified ok, coachChatId=' + coachChatId);
    }
  } catch (err) {
