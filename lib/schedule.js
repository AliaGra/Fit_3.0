...
/** Прізвище + ім'я для «Чекають підтвердження» (обидва поля з users, якщо є). */
async function resolveRequestedStudentLabel(coachChatId, studentId) {
  if (studentId == null || String(studentId).trim() === '') return '';
  let u = await supabase.getUserForScheduleStudentId(studentId);
  if (!u) u = await supabase.getUserStudentForCoach(coachChatId, studentId);
  if (!u) u = await findUserByAnyStudentId(studentId);
  if (!u) return '';
  const fn = (u.firstName || u.first_name || '').toString().trim();
  const ln = (u.lastName || u.last_name || '').toString().trim();
  let label = '';
  if (ln && fn) label = ln + ' ' + fn;
  else label = fn || ln || '';
