  ...
  try {
    ...
    if (filter === 'available') {
      if (availableSlice === 'first3') {
        text +=
          '💡 «На екрані показано вільні слоти на 3 дні (від поточної дати). Щоб переглянути решту вільних слотів — натисніть «Інші».»\n\n';
      } else {
    ...
    if (filter === 'available') {
      if (availableSlice === 'first3' && availableRestSlots.length > 0) {
        keyboard.push([{ text: 'Інші', callback_data: CONSTANTS.CALLBACKS.SCH_7_AVAILABLE_REST }]);
      }
      if (availableSlice === 'rest') {
