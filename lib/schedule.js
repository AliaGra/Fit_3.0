...
/** Короткий текст кнопки (до 64 символів), щоб не втрачати callback-кнопку через ліміт Telegram. */
function trimInlineButtonText(label) {
  const max = 64;
  if (!label) return '';
  if (label.length <= max) return label;
...
function isInviteLikeStudentId(studentId) {
  const id = String(studentId || '').trim().toUpperCase();
  return id.startsWith('INVITE_') || id.startsWith('USED_INVITE_');
}

function sanitizeStudentNameForUi(name, fallback) {
  const raw = String(name || '').trim();
  if (!raw) return fallback;
  const upper = raw.toUpperCase();
  if (upper.includes('INVITE')) return fallback;
  if (upper.startsWith('USED_')) return fallback;
  return raw;
}
...
async function findUserByAnyStudentId(studentId) {
  ...
  if (!raw) return null;
  const upper = raw.toUpperCase();
  const baseRaw = upper.startsWith('USED_') ? raw.slice(5) : raw;
  const baseUpper = upper.startsWith('USED_') ? upper.slice(5) : upper;
  const candidates = Array.from(new Set([raw, upper, baseRaw, baseUpper].filter(Boolean)));
  ...
  for (const id of candidates) {
    const byChatId = await supabase.getUserByChatId(id);
    if (byChatId) return byChatId;
  }
  for (const id of candidates) {
    const byInvite = await supabase.findUserByInviteCode(id);
...
/** Тільки ім'я учня для компактних кнопок у «Чекають підтвердження». */
async function getStudentFirstName(studentId) {
  if (!studentId) return '';
  const id = String(studentId).trim();
  const u = await findUserByAnyStudentId(id);
  if (!u) return '';
...
async function showCoachScheduleMenu(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: '🔧 Перевірити й відновити слоти', callback_data: CONSTANTS.CALLBACKS.SCH_RECONCILE }],
  ...
  const createHint = '💡 **Як створити слоти:** натисни «Створити слоти» → введи кількість днів (наприклад 7) → слоти створюються автоматично за робочим часом з «Налаштування розкладу». Можна також «Додати слоти на день» для одного обраного дня.';
...
async function showCoach7DaysView(chatId, filter, page) {
  try {
    ...
    // Для "чекають підтвердження" будуємо мапу імен учнів тренера,
    // щоб стабільно знаходити ім'я за student_id слота (chat_id / user_id / INVITE / USED_INVITE).
    const coachStudents = await User.getStudentsByCoach(chatId);
    const studentFirstNameByAnyId = new Map();
    for (const st of coachStudents || []) {
      const fn = (st?.firstName || '').toString().trim();
      if (!fn) continue;
      const chatKey = String(st?.chatId || '').trim();
      const userKey = String(st?.userId || '').trim();
      if (chatKey) {
        studentFirstNameByAnyId.set(chatKey, fn);
        studentFirstNameByAnyId.set(chatKey.toUpperCase(), fn);
      }
      if (userKey) {
        studentFirstNameByAnyId.set(userKey, fn);
        studentFirstNameByAnyId.set(userKey.toUpperCase(), fn);
      }
      if (chatKey.toUpperCase().startsWith('USED_')) {
        const base = chatKey.slice(5);
        if (base) {
          studentFirstNameByAnyId.set(base, fn);
          studentFirstNameByAnyId.set(base.toUpperCase(), fn);
        }
      }
      if (userKey.toUpperCase().startsWith('USED_')) {
        const base = userKey.slice(5);
        if (base) {
          studentFirstNameByAnyId.set(base, fn);
          studentFirstNameByAnyId.set(base.toUpperCase(), fn);
        }
    ...
    /** Підказку з діями не показуємо у «Вільні слоти», «Зайняті», «Мої перерви» та «Чекають підтвердження» (без довгої легенди кнопок). */
    const showHint = filter !== 'booked' && filter !== 'reserved' && filter !== 'available' && filter !== 'requested';
    ...
    if (filter === 'requested') {
      text +=
        '✅ — підтвердити час тренування, учень побачить час у своєму розкладі\n' +
        '❌ — скасувати тренування\n\n';
    }
    if (requested.length && (filter === 'all' || filter === 'requested')) {
      text += '⏳ Запити на підтвердження:\n';
      for (const s of requested) {
        const isInviteStudent = isInviteLikeStudentId(s.studentId);
        const sid = String(s.studentId || '').trim();
        const rawName = studentFirstNameByAnyId.get(sid) || studentFirstNameByAnyId.get(sid.toUpperCase()) || await getStudentFirstName(s.studentId);
        const name = sanitizeStudentNameForUi(rawName, '');
        const inviteMark = isInviteStudent ? ' [INV]' : '';
        text += '• ' + formatSlotDateTime(s, { noYear: true }) + (name ? ' — ' + name : '') + inviteMark + '\n';
      }
      if (filter === 'all') text += '\n';
    }
    ...
    if (available.length && filter === 'available') {
    ...
    for (const s of pageSlots) {
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        const sid = String(s.studentId || '').trim();
        const requestedStudentNameRaw = studentFirstNameByAnyId.get(sid) || studentFirstNameByAnyId.get(sid.toUpperCase()) || await getStudentFirstName(s.studentId);
        const requestedStudentName = sanitizeStudentNameForUi(requestedStudentNameRaw, '');
        const requestedBtnText = requestedStudentName
          ? '✅ ' + requestedStudentName + ' — ' + formatSlotDateTime(s, { noYear: true })
          : '✅ ' + formatSlotDateTime(s, { noYear: true });
        keyboard.push([
          { text: trimInlineButtonText(requestedBtnText), callback_data: CONSTANTS.CALLBACKS.SCH_CONF + ':' + s.id },
          { text: '❌', callback_data: CONSTANTS.CALLBACKS.SCH_DECLINE + ':' + s.id }
        ]);
...
/**
 * Разово: перевірити розклад тренера за шаблоном на найближчі COACH_MY_SCHEDULE_WINDOW_DAYS днів.
 * 1) Виправити «підвішені» рядки (CANCELED, AVAILABLE з учнем).
 * 2) Додати відсутні вільні слоти там, де за робочим часом має бути вікно (без вихідних і днів відпустки).
 */
async function reconcileCoachScheduleSlots(chatId) {
  const user = await User.getByChatId(chatId);
  if (!user || user.role !== CONSTANTS.ROLES.COACH) {
    return { ok: false, repairedRows: 0, createdSlots: 0, message: 'Доступ лише для тренера.' };
  }
  const settings = await supabase.getCoachScheduleSettings(chatId);
  if (!settings) {
    return { ok: false, repairedRows: 0, createdSlots: 0, message: 'Спочатку налаштуй шаблон: «Налаштування розкладу».' };
  }

  let repairedRows = 0;
  try {
    const { startKey, endKey } = getCoachMyScheduleWindowStartEndKeys();
    const all = await supabase.getSlotsByCoachAndStatus(chatId, null);
    /** Не фільтруємо isSlotInFuture: інакше «сьогодні 9:00» після полудня не ремонтується (CANCELED залишається, slotExists блокує новий рядок). */
    for (const s of all || []) {
      if (!isSlotInDateRange(s, startKey, endKey)) continue;
      const st = s.status;
      if (
        st === CONSTANTS.SCHEDULE_STATUS.CANCELED ||
        (st === CONSTANTS.SCHEDULE_STATUS.AVAILABLE && s.studentId)
      ) {
        const ok = await supabase.releaseScheduleSlotToAvailable(s.id);
        if (ok) repairedRows++;
      }
    }
  } catch (e) {
    console.error('reconcileCoachScheduleSlots repair', e.message);
    return { ok: false, repairedRows: 0, createdSlots: 0, message: 'Помилка при оновленні записів у БД.' };
  }

  const vacationSet = new Set((await supabase.getCoachVacationDateKeys(chatId)) || []);
  const restDays = settings.restDays || [];
  const durationMin = settings.workoutDurationMin || 60;
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const end = new Date(today);
  end.setDate(end.getDate() + COACH_MY_SCHEDULE_WINDOW_DAYS - 1);
  ...
  return { ok: true, repairedRows, createdSlots };
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.SCH_RECONCILE) {
    const r = await reconcileCoachScheduleSlots(chatId);
    if (r.ok) {
      await Helpers.safeSend(
        chatId,
        '✅ **Перевірка розкладу завершена** (найближчі ' +
          COACH_MY_SCHEDULE_WINDOW_DAYS +
          ' дні за шаблоном).\n\n' +
          '• Виправлено записів у БД: **' +
          r.repairedRows +
          '** (підвішені CANCELED / AVAILABLE з учнем).\n' +
          '• Додано вільних слотів за шаблоном: **' +
          r.createdSlots +
          '** (без вихідних і днів відпустки).',
        { parse_mode: 'Markdown' }
      );
    } else {
      await Helpers.safeSend(chatId, '❌ ' + (r.message || 'Не вдалося виконати перевірку.'));
    }
    await showCoachScheduleMenu(chatId);
    return true;
  }
