...
async function showStudentRescheduleCalendar(chatId) {
  try {
    ...
    const myBookedDates = await buildStudentBookedDateKeys(chatId);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const todayKey = getDateKey(today);
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      for (let col = 0; col < DAYS_IN_WEEK; col++) {
        ...
        const isSunday = dayOfWeek === 0;
        const isToday = key === todayKey;
        const hasMyTraining = myBookedDates.has(key);
        let emoji = '';
        if (hasMyTraining) emoji = '🔵 ';
...
async function buildStudentBookedDateKeys(studentChatId) {
  try {
    const slots = await supabase.getSlotsByStudentAndStatus(String(studentChatId), null);
    const list = Array.isArray(slots) ? slots : [];
    const booked = list.filter((s) => s && isSlotInFuture(s) && (s.status === CONSTANTS.SCHEDULE_STATUS.BOOKED || s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED));
    const keys = new Set();
    for (const s of booked) {
    ...
    console.error('buildStudentBookedDateKeys', e.message);
    return new Set();
  }
}
...
async function showStudentMySchedule(chatId) {
  try {
    ...
    const keyboard = [
      [{ text: '🔄 Змінити', callback_data: CONSTANTS.CALLBACKS.SCH_S_MY_EDIT }],
      [{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]
    ];
    await Helpers.sendKeyboard(chatId, text, keyboard);
  } catch (err) {
