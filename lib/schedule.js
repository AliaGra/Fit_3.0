...
function workPerDayHintText() {
  return (
    CONSTANTS.SCHEDULE_SETTINGS.WORK_PER_DAY_LAST_TRAINING_HINT ||
    '💡 Урахуй тривалість заняття: час початку останнього тренування має бути раніше за час завершення роботи залу, щоб заняття встигло завершитися.'
  );
}
...
async function showSettingsSummary(chatId, settings) {
  ...
  const text =
    formatSettingsSummary(settings) + (savedNow ? '\n\n✅ Налаштування збережено. Якщо розклад правильний, повернись у меню «Підказки» і продовж налаштування свого робочого кабінету.' : '');
  const keyboard = [
    [{ text: '✏️ Змінити', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_EDIT }],
    [{ text: '📅 Розклад', callback_data: CONSTANTS.CALLBACKS.SCH_SETTINGS_SAVE }],
    [{ text: '💡 Підказки', callback_data: CONSTANTS.CALLBACKS.MENU_HINTS }]
...
async function navigateAfterCreateSlots(chatId, priorState, result) {
  ...
  if (fromHints) {
    if (result.ok) {
      const { setStepDone } = require('./coachFirstSteps');
      await setStepDone(chatId, 2, true);
    }
