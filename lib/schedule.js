...
async function showVacationCalendar(chatId) {
  ...
  for (let row = 0; row < ROWS; row++) {
    ...
    for (let col = 0; col < DAYS_IN_WEEK; col++) {
      ...
      const label = emoji + formatDateShort(d);
      rowButtons.push({
        text: label.length > 64 ? formatDateShort(d) : label,
  ...
  try {
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      for (let col = 0; col < COLS; col++) {
        ...
        const label = emoji + formatDateShort(d) + ' (' + occupied + ')';
        rowButtons.push({ text: label.length > 64 ? formatDateShort(d) + ' (' + occupied + ')' : label, callback_data: CONSTANTS.CALLBACKS.SCH_CALENDAR_DAY + ':' + key });
      }
      keyboard.push(rowButtons);
    }
    keyboard.push([{ text: CONSTANTS.EMOJI.BACK + ' До Мій розклад', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SLOTS }]);
  ...
  try {
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      for (let col = 0; col < DAYS_IN_WEEK; col++) {
        ...
        const label = emoji + formatDateShort(d) + (count ? ' (' + count + ')' : '');
        rowButtons.push({
          text: label.length > 64 ? formatDateShort(d) + (count ? '(' + count + ')' : '') : label,
...
async function studentPicksRescheduleSlot(chatId, newSlotId) {
  ...
  await Helpers.sendKeyboard(oldSlot.coachId, coachMsg, kbd);
  await State.clear(chatId);
  const Menu = require('./menu');
...
async function showAddSlotsForDayCalendar(chatId) {
  ...
  for (let row = 0; row < ROWS; row++) {
    ...
    for (let col = 0; col < DAYS_IN_WEEK; col++) {
      ...
      const label = emoji + formatDateShort(d);
      rowButtons.push({ text: label.length > 64 ? formatDateShort(d) : label, callback_data: CONSTANTS.CALLBACKS.SCH_ADD_SLOTS_DAY_PICK + ':' + key });
    }
    keyboard.push(rowButtons);
  }
  keyboard.push([{ text: CONSTANTS.EMOJI.BACK + ' До розкладу', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SCHEDULE }]);
