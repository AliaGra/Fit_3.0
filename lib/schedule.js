...
/** Ім'я для «Чекають підтвердження»: спочатку рядок users цього тренера за student_id слота. */
async function resolveRequestedStudentLabel(coachChatId, studentId) {
  let u = await supabase.getUserStudentForCoach(coachChatId, studentId);
  if (!u) u = await findUserByAnyStudentId(studentId);
  if (!u) return '';
  const fn = (u.firstName || u.first_name || '').toString().trim();
  const ln = (u.lastName || u.last_name || '').toString().trim();
  const label = fn || ln || '';
  return sanitizeStudentNameForUi(label, '');
}
  ...
  try {
    ...
    if (requested.length && (filter === 'all' || filter === 'requested')) {
      ...
      for (const s of requested) {
        ...
        const name = await resolveRequestedStudentLabel(chatId, s.studentId);
    ...
    for (const s of pageSlots) {
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        const requestedStudentName = await resolveRequestedStudentLabel(chatId, s.studentId);
