...
async function startBookStudent(chatId, studentChatId) {
  ...
  if (future.length === 0) {
    await Helpers.safeSend(chatId, '📅 Немає вільних слотів. Створи слоти в «Розклад» → «Створити слоти».');
...
async function showBookStudentCalendar(chatId, studentChatId) {
  ...
  keyboard.push([{ text: '🔙 Скасувати', callback_data: CONSTANTS.CALLBACKS.CANCEL_ACTION }]);
  await Helpers.sendKeyboard(chatId, '📅 Обери дату для запису ' + studentName + ':', keyboard);
}
...
async function showBookStudentDaySlots(chatId, dateKey) {
  ...
  await Helpers.sendKeyboard(chatId, '📅 Вільні слоти на ' + dateLabel + ' для ' + studentName + ':', keyboard);
}
