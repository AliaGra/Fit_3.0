...
async function studentPicksRescheduleSlot(chatId, newSlotId) {
  ...
  // Сповіщення тренеру — тихо у фоні, без страшних повідомлень учню
  const student = await User.getByChatId(chatId);
  const studentName = student ? (student.firstName || '') + (student.lastName ? ' ' + student.lastName : '') : String(chatId);
  const kbd = [
    [{ text: '✅ Підтвердити перенос', callback_data: CONSTANTS.CALLBACKS.SCH_COACH_CONF_RESCHEDULE + ':' + oldSlotId + ':' + newSlotId }],
    [{ text: '❌ Відхилити', callback_data: CONSTANTS.CALLBACKS.SCH_COACH_DECLINE_RESCHEDULE + ':' + oldSlotId + ':' + newSlotId }]
  ];
  ...
  Helpers.sendKeyboard(coachChatId, coachMsg, kbd).catch((err) => {
    console.error('Schedule.studentPicksRescheduleSlot: coach notify error', err.message);
  });
