...
async function runCreateSlotsForDays(chatId, num) {
  ...
  await State.update(chatId, { step: null, createSlotsMaxDays: null, createSlotsMinDays: null });
  if (result.ok) {
...
async function startCreateSlotsFlow(chatId) {
  ...
  keyboard.push(
    [{ text: '🔙 До підказок', callback_data: CONSTANTS.CALLBACKS.HINTS_FIRST_STEPS }],
    [{ text: '🔙 До слотів', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SLOTS }]
  );
  const slotsExplain =
