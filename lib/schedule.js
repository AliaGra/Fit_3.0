  ...
  try {
    ...
    if (requested.length && (filter === 'all' || filter === 'requested')) {
      ...
      for (const s of requested) {
        const isInviteStudent = String(s.studentId || '').startsWith('INVITE_');
        const name = isInviteStudent ? await getStudentSurnameFirstName(s.studentId) : await getStudentDisplayName(s.studentId);
    ...
    for (const s of pageSlots) {
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        const isInviteStudent = String(s.studentId || '').startsWith('INVITE_');
        const requestedStudentName = isInviteStudent ? await getStudentSurnameFirstName(s.studentId) : '';
        const inviteMark = isInviteStudent ? ' [INV]' : '';
        const requestedBtnText = requestedStudentName
          ? '✅ ' + formatSlotDateTime(s, { noYear: true }) + ' — ' + requestedStudentName + inviteMark
          : '✅ ' + formatSlotDateTime(s, { noYear: true }) + inviteMark;
        keyboard.push([
