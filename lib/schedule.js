...
async function showCoachCalendar(chatId) {
  try {
    ...
    for (const s of inRange) {
      ...
      // Для календаря у дужках показуємо тільки підтверджені тренування (BOOKED).
      if (s.status === CONSTANTS.SCHEDULE_STATUS.BOOKED) {
    ...
    await Helpers.sendKeyboard(
      ...
      '📆 Календар — 21 день від сьогодні\n\n' +
        '🟢 — поточна дата\n' +
        '🟡 — неділя/відпустка\n' +
        '🟫 — немає слотів (створи через меню «Створити слоти»)\n' +
        'У дужках: кількість підтверджених тренувань у цей день\n\n' +
        pendingLine +
        '\n\nОбері день:',
      keyboard
    );
