...
async function cancelSlot(chatId, slotId) {
  ...
  /** Як у coachConfirmsStudentCancel / declineSlot: слот знову стає вільним (не CANCELED — інакше не потрапляє в AVAILABLE). */
  await supabase.updateScheduleSlotStatus(slotId, CONSTANTS.SCHEDULE_STATUS.AVAILABLE);
  await supabase.updateScheduleSlotStudentId(slotId, null);
...
// ——— Тренер натискає «Скасувати» (SCH_CANCEL_REQ): одразу звільняємо слот (як cancelSlot). Раніше йшов запит учню — слот лишався BOOKED, поки учень не підтвердить.
