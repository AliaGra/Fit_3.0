...
async function showSettingsWorkPerDay(chatId) {
  ...
  await Helpers.sendKeyboard(
    chatId,
    '🕐 Встанови час для кожного дня (натисни день і введи інтервал у форматі 09:00–21:00):\n\n' + hint,
    keyboard
  );
}
...
async function handleSettingsWorkPerDayOpen(chatId) {
  ...
  const draft = {
    ...
    coachId: String(chatId),
    workHoursByWeekday,
    workStart: existing.workStart || CONSTANTS.SCHEDULE_SETTINGS.WORK_START_DEFAULT,
    workEnd: existing.workEnd || CONSTANTS.SCHEDULE_SETTINGS.WORK_END_DEFAULT
  };
  await State.set(chatId, {
    ...state,
...
async function handleSettingsWorkPerDayTimeInput(chatId, text) {
  ...
  const draft = {
    ...
    coachId: String(chatId),
    workHoursByWeekday,
    workStart: base.workStart || CONSTANTS.SCHEDULE_SETTINGS.WORK_START_DEFAULT,
    workEnd: base.workEnd || CONSTANTS.SCHEDULE_SETTINGS.WORK_END_DEFAULT
  };
  await State.set(chatId, {
    ...state,
    step: CONSTANTS.FSM_STATES.SCH_SETTINGS_WORK_PER_DAY,
