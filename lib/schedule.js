...
async function coachBookStudentToSlot(chatId, slotId, studentChatId) {
  ...
  const kbd = [
    [{ text: '✅ Підтвердити', callback_data: CONSTANTS.CALLBACKS.SCH_STUDENT_CONF_COACH_BOOK + ':' + slotId }],
    [{ text: '❌ Відхилити', callback_data: CONSTANTS.CALLBACKS.SCH_STUDENT_DECLINE_COACH_BOOK + ':' + slotId }]
  ];
  const sentToStudent = await Helpers.sendKeyboard(
    studentChatId,
    '👋 Твій тренер **' + coachName + '** хоче записати тебе на тренування:\n🗓 **' + dateTimeStr + '**',
    kbd,
    { parse_mode: 'Markdown' }
  );
  if (sentToStudent) {
...
async function showStudentMySchedule(chatId) {
  try {
    ...
    if (future.length === 0) {
      text += 'Немає майбутніх записів. Записатись можна кнопкою «Записатись на тренування».\n\nЯкщо тренер щойно записав тебе на тренування — натисни «Мій розклад» ще раз або перезапусти бота (/start).';
    } else {
    ...
    for (const s of future) {
      if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
        keyboard.push([
          { text: '✅ Підтвердити', callback_data: CONSTANTS.CALLBACKS.SCH_STUDENT_CONF_COACH_BOOK + ':' + s.id },
          { text: '❌ Відхилити', callback_data: CONSTANTS.CALLBACKS.SCH_STUDENT_DECLINE_COACH_BOOK + ':' + s.id }
        ]);
      }
      const btnText = '❌ Скасувати ' + formatSlotDateTime(s);
      keyboard.push([
        { text: btnText.length > 64 ? '❌ ' + formatSlotDateTime(s) : btnText, callback_data: CONSTANTS.CALLBACKS.SCH_S_CANCEL_REQ + ':' + s.id },
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.SCH_STUDENT_CONF_COACH_BOOK && rest) {
    await studentConfirmsCoachBooking(chatId, rest.trim());
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.SCH_STUDENT_DECLINE_COACH_BOOK && rest) {
