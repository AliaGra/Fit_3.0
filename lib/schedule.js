...
async function showCoach7DaysView(chatId, filter, page) {
  try {
    ...
    /** Підказку з діями не показуємо у «Вільні слоти» (лише список, як у «Зайняті»). */
  ...
  try {
    ...
    for (let row = 0; row < CAL_ROWS; row++) {
      ...
      for (let col = 0; col < CAL_COLS; col++) {
        ...
        /** Без дати на кнопці — лише день тижня (скорочено) + зайняті в дужках. */
        const weekdayShort = WEEKDAY_UA[d.getDay()] || '';
        const label = emoji + weekdayShort + ' (' + occupied + ')';
        rowButtons.push({
          text: label.length > 64 ? weekdayShort + ' (' + occupied + ')' : label,
    ...
    keyboard.push([{ text: CONSTANTS.EMOJI.BACK + ' До Мій розклад', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SLOTS }]);
    /** Лише сітка кнопок днів (без текстових списків дат); на кнопці — день тижня + зайняті в дужках. */
...
async function showCoachDaySlots(chatId, dateKey, _page) {
  try {
    ...
    /** Заголовок + повний текстовий перелік усіх слотів дня; кнопки — час без дати (formatSlotTimeOnly). */
