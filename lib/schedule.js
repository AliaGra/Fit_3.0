...
async function startCreateSlotsFlow(chatId) {
  ...
  const keyboard = [[{ text: '🔙 Назад', callback_data: CONSTANTS.CALLBACKS.SCH_MY_SLOTS }]];
  const slotsExplain =
    'Що таке слоти?\n' +
    'Слот — це конкретний час у календарі (наприклад, 10:00–11:00), коли учень може записатися на тренування. Слоти формуються автоматично за робочим графіком із «Налаштування розкладу».\n\n' +
    'Навіщо вони потрібні?\n' +
    'Без вільних слотів учні не бачать доступний час для запису. Після створення слотів учень обирає зручний час — тобі надійде запит на підтвердження (або ти записуєш учня сам через календар).\n\n';
  await Helpers.sendKeyboard(
    chatId,
    '📅 Створити слоти\n\n' +
      slotsExplain +
