...
function isInviteStudentId(studentId) {
  return studentId != null && String(studentId).startsWith('INVITE_');
}

/**
 * Запит REQUESTED від учня, який уже в боті (не інвайт без активації).
 * Записи на INVITE_* не показуємо в «Чекають підтвердження» / лічильнику «На підтвердження» в календарі.
 */
function isCoachPendingStudentConfirmationSlot(slot) {
  return (
    slot &&
    slot.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED &&
    !isInviteStudentId(slot.studentId)
  );
}
...
async function getCoachSlotsCountsMyScheduleWindow(chatId) {
  ...
  return {
    ...
    requested: inRange.filter((s) => isCoachPendingStudentConfirmationSlot(s)).length
  };
}
...
async function showCoach7DaysView(chatId, filter, page) {
  try {
    ...
    const slots = await supabase.getSlotsByCoachAndStatus(chatId, null);
    const inRange = (slots || []).filter((s) => isSlotInDateRange(s, startKey, endKey) && isSlotInFuture(s));
    const requested = inRange.filter((s) => isCoachPendingStudentConfirmationSlot(s));
    const inviteRequested = inRange.filter(
      (s) => s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED && isInviteStudentId(s.studentId)
    );
    ...
    if (inviteRequested.length && filter === 'all') {
      text += '🎟️ Інвайти (очікують активації коду):\n';
      for (const s of inviteRequested) {
        const name = await getStudentDisplayName(s.studentId);
        text += '• ' + formatSlotDateTime(s) + ' — ' + name + '\n';
      }
      text += '\n';
...
async function showCoachCalendar(chatId) {
  try {
    ...
    for (const s of inRange) {
      ...
      if (isCoachPendingStudentConfirmationSlot(s)) pendingInWindow++;
      if (s.status === CONSTANTS.SCHEDULE_STATUS.BOOKED || isCoachPendingStudentConfirmationSlot(s)) {
    ...
    for (const s of sorted) {
      const stLabel =
        s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED
          ? isInviteStudentId(s.studentId)
            ? '🎟️ інвайт (очікує активації коду)'
            : '⏳ на підтвердження'
          : '✅ підтверджено';
    ...
    if (daySlots.length === 0) {
      ...
      if (forTextList.length > 0) {
        for (const s of forTextList) {
          ...
          if (s.status === CONSTANTS.SCHEDULE_STATUS.REQUESTED) {
            const name = await getStudentDisplayName(s.studentId);
            if (isInviteStudentId(s.studentId)) {
              text += '• ' + t + ' — 🎟️ ' + name + ' (інвайт, очікує активації коду)\n';
            } else {
