...
async function showVenuePreview(chatId) {
  ...
  const keyboard = sectionFooterKeyboard(null);
...
async function showCoachesView(chatId) {
  ...
  let text = '<b>🧑‍🏫 Тренери закладу</b> (на платформі зараз)\n\n';
  if (!coaches.length) {
    text += '<i>Поки немає тренерів з відміткою «де треную».</i>\n\nТренери додають заклад у своєму профілі.';
  } else {
    for (const c of coaches) {
      const name = Helpers.escapeHtml([c.firstName, c.lastName].filter(Boolean).join(' ').trim() || String(c.chatId));
      const vis = c.listingVisible ? '✅ на картці' : '⏸ приховано';
      text += '• ' + name + ' — ' + vis + '\n';
    }
    text += '\n\n<i>Обери тренера нижче, щоб показати, приховати або відв’язати.</i>';
  }
  const keyboard = [];
