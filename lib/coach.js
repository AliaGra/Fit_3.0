...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.COACH_ADD_STUDENT_CONTINUE_PROFILE) {
    const state = await State.get(chatId);
    if (!state || state.step !== CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_AGE) return false;
...
async function finishCreateStudentByInvite(chatId, state, days, conditions) {
  ...
  try {
    ...
    let doneMsg =
      '✅ Учня створено!\n\n' +
      'Нижче — **окреме повідомлення** лише з інвайт-кодом. Перешли його учню або скопіюй.\n\n' +
