/**
 * Coach — Мої клієнти (список, профіль клієнта), Тренування клієнтів (заглушка).
 * Callback: COACH_STUDENTS (хаб), COACH_STUDENTS_LIST (список), VIEW_STUDENT:id, COACH_ADD_STUDENT, …
 */
...
const coachGroupPublic = require('./coachGroupPublic');

async function isStudentsListFromHintsPlan(chatId) {
  const st = await State.getSafe(chatId);
  return !!(st && st.coachStudentsListFromHintsPlan);
}

async function clearStateKeepingHintsPlanFlag(chatId) {
  const fromHintsPlan = await isStudentsListFromHintsPlan(chatId);
  await State.clear(chatId);
  if (fromHintsPlan) await State.update(chatId, { coachStudentsListFromHintsPlan: true });
}

async function returnAfterStudentHealthCompletion(chatId, studentChatId) {
  if (await isStudentsListFromHintsPlan(chatId)) {
    await State.update(chatId, { coachStudentsListFromHintsPlan: true });
    return showStudentsList(chatId);
  }
  return showStudentProfile(chatId, studentChatId);
}

async function openStudentsListFromHintsPlan(chatId) {
  await State.update(chatId, { coachStudentsListFromHintsPlan: true });
  return showStudentsList(chatId);
}
...
const WEEKDAYS_UA_STUDENT = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Нд'];

function coachWorkHoursForWeekday(settings, dow) {
...
function formatCoachBirthDateLine(coach) {
  const bd = coach && coach.birthDate;
  if (!bd) return '🎂 День народження: не вказано\n';
  const d = bd instanceof Date ? bd : new Date(bd);
  if (isNaN(d.getTime())) return '🎂 День народження: не вказано\n';
  return (
    '🎂 День народження: ' +
...
function formatCoachInstagramLineHtml(coach) {
  const raw = coach && coach.instagram ? String(coach.instagram).trim() : '';
  if (!raw) return '📸 Instagram: не вказано\n';
  let url = raw;
  if (!/^https?:\/\//i.test(url)) {
    url = 'https://instagram.com/' + raw.replace(/^@/, '');
  }
  return '📸 Instagram: <a href="' + Helpers.escapeHtml(url) + '">' + Helpers.escapeHtml(url) + '</a>\n';
}

/** Режим роботи тренера для картки клієнта (з «Налаштування розкладу»). */
function formatCoachWorkScheduleBlock(settings) {
  if (!settings) {
    return '🕐 Режим роботи: тренер ще не налаштував розклад.\n';
  }
  const restDays = Array.isArray(settings.restDays) ? settings.restDays : [];
  const lines = ['🕐 Режим роботи:\n'];
  if (restDays.length > 0) {
    lines.push(
      'Вихідний день: ' + restDays.sort((a, b) => a - b).map((d) => WEEKDAYS_UA_STUDENT[d]).join(', ') + '\n'
    );
  } else {
    lines.push('Вихідний день: немає\n');
  }
  const hoursByDay = [];
  for (let i = 0; i < 7; i++) {
    const h = coachWorkHoursForWeekday(settings, i);
    hoursByDay.push({ dow: i, start: h.start, end: h.end, rest: restDays.indexOf(i) >= 0 });
  }
  const working = hoursByDay.filter((x) => !x.rest);
  const signatures = new Set(working.map((x) => x.start + '|' + x.end));
  if (signatures.size === 1 && working.length > 0) {
    lines.push('Робочий час: ' + working[0].start + ' – ' + working[0].end + '\n');
  } else {
    for (const row of hoursByDay) {
      if (row.rest) {
        lines.push(WEEKDAYS_UA_STUDENT[row.dow] + ': вихідний\n');
      } else {
        lines.push(WEEKDAYS_UA_STUDENT[row.dow] + ': ' + row.start + ' – ' + row.end + '\n');
      }
    }
  }
  return lines.join('');
...
/** Окреме повідомлення лише з кодом — зручно копіювати та переслати клієнту в Telegram. */
async function sendInviteCodeForForwarding(chatId, inviteCode) {
  const code = inviteCode != null ? String(inviteCode).trim() : '';
  if (!code) return;
  await Helpers.safeSend(chatId, code);
}

/** Після створення інвайту — без списку «Твої клієнти». */
async function showAfterInviteCreatedMenu(chatId) {
  const keyboard = [
    [{ text: '➕ Додати клієнта', callback_data: CONSTANTS.CALLBACKS.COACH_ADD_STUDENT }],
    [{ text: '🔙 Розділ «Мої клієнти»', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS }],
...
/** Підменю «Мої клієнти»: список чи архів. */
async function showCoachStudentsHub(chatId) {
  const me = await User.getByChatId(chatId);
  if (!User.isCoach(me)) {
    await Helpers.safeSend(chatId, '👥 Розділ «Мої клієнти» доступний тільки тренерам.');
    return;
  }
  const keyboard = [
    [{ text: '📋 Список клієнтів', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS_LIST }],
    [{ text: '➕ Додати клієнта', callback_data: CONSTANTS.CALLBACKS.COACH_ADD_STUDENT }],
    [{ text: '📦 Архів клієнтів', callback_data: CONSTANTS.CALLBACKS.COACH_ARCHIVE_MENU }],
    [{ text: CONSTANTS.EMOJI.HOME + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]
  ];
  await Helpers.sendKeyboard(chatId, '👥 **Мої клієнти**\n\nОбери розділ:', keyboard, { parse_mode: 'Markdown' });
}
...
async function showStudentsList(chatId) {
  try {
    const me = await User.getByChatId(chatId);
    if (!User.isCoach(me)) {
      await Helpers.safeSend(chatId, '👥 Список клієнтів доступний тільки тренерам.');
      return;
    }
    ...
    if (!students || students.length === 0) {
      ...
      await Helpers.sendKeyboard(
        chatId,
        '📋 У тебе поки немає клієнтів.\n\nДодай клієнта: **Мої клієнти** → **➕ Додати клієнта**.',
        keyboard,
        { parse_mode: 'Markdown' }
      );
      return;
    ...
    for (const student of students) {
      ...
      const pickCb = fromHintsPlan
        ? CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_HEALTH_MENU + ':' + student.chatId
        : CONSTANTS.CALLBACK_PREFIXES.VIEW_STUDENT + ':' + student.chatId;
      keyboard.push([{ text: name + ' (' + status + ')', callback_data: pickCb }]);
    }
    keyboard.push([{ text: '🔙 Розділ «Мої клієнти»', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS }]);
    ...
    await Helpers.sendKeyboard(chatId, '👥 Твої клієнти (' + students.length + '):\n\nОбери клієнта:', keyboard);
  } catch (err) {
    console.error('Coach.showStudentsList', err.message);
    await Helpers.safeSend(chatId, '❌ Помилка завантаження списку клієнтів.');
  }
}
...
async function showStudentProfile(chatId, studentChatId) {
  try {
    ...
    if (!student) {
      await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
      return;
    }
    ...
    await State.update(chatId, { scheduleStudentCardReturn: undefined });
    let message = '👤 **Профіль клієнта**\n\n';
    ...
    const kbd = [];
    if (!isInvite) {
      const studentTgBtn = Helpers.telegramContactButton('💬 Написати клієнту в Telegram', student);
      if (studentTgBtn) kbd.push([studentTgBtn]);
    }
    kbd.push(
      [{ text: '🖋️ Записати', callback_data: CONSTANTS.CALLBACKS.COACH_BOOK + ':' + studentChatId }, { text: '📋 План тренувань', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_LIST + ':' + studentChatId }],
      [{ text: '📆 Поточні записи та перенос', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_SCHEDULE + ':' + studentChatId }],
      ...
      [{ text: '📏 Обміри/Медицина/Досвід', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_HEALTH_MENU + ':' + studentChatId }],
      [{ text: '🤖 AI-аналітика', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_AI_ANALYTICS + ':' + studentChatId }],
      [{ text: '📦 Архівувати профіль', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_ARCHIVE_STUDENT + ':' + studentChatId }],
      [{ text: '🔙 До списку', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS_LIST }],
      [{ text: CONSTANTS.EMOJI.HOME + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]
    );
    await Helpers.sendKeyboard(chatId, message, kbd);
    if (isInvite && student.userId) {
      await sendInviteCodeForForwarding(chatId, student.userId);
    }
  } catch (err) {
...
function formatMeasurementDetails(m) {
  if (!m) return '';
  const lines = [];
  if (m.height != null) lines.push('📏 Зріст: ' + m.height + ' см');
  if (m.weight != null) lines.push('⚖️ Вага: ' + m.weight + ' кг');
  if (m.waist != null) lines.push('⭕ Талія: ' + m.waist + ' см');
  if (m.hip != null) lines.push('⭕ Стегно: ' + m.hip + ' см');
  if (m.glutes != null) lines.push('⭕ Ягодиці: ' + m.glutes + ' см');
  if (m.arm != null) lines.push('💪 Рука: ' + m.arm + ' см');
  if (m.shoulders != null) lines.push('📐 Плечі: ' + m.shoulders + ' см');
  if (m.chest != null) lines.push('📐 Груди: ' + m.chest + ' см');
  if (m.neck != null) lines.push('🧣 Шия: ' + m.neck + ' см');
  if (m.wrist != null) lines.push('⌚ Запʼястя: ' + m.wrist + ' см');
  if (m.bodyFatPct != null) lines.push('📊 Жир: ' + m.bodyFatPct + '%');
  return lines.join('\n');
}

async function showStudentHealthMenu(chatId, studentChatId, notice = null) {
  const student = await User.getByChatId(studentChatId);
  ...
  const name = ((student.firstName || '') + ' ' + (student.lastName || '')).trim();
  const noticeLine = notice ? String(notice).trim() + '\n\n' : '';
  const fromHintsPlan = await isStudentsListFromHintsPlan(chatId);
  const keyboard = [
    [{ text: '📏 Обміри/Активність', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_BODY_MENU + ':' + studentChatId }],
    [{ text: '🩺 Медичний профіль', callback_data: CONSTANTS.CALLBACK_PREFIXES.MC_PROFILE + ':' + studentChatId }],
    [{ text: '📅 Досвід', callback_data: CONSTANTS.CALLBACK_PREFIXES.STUDENT_EXPERIENCE + ':' + studentChatId }]
  ];
  if (fromHintsPlan) {
    keyboard.push([{ text: '🔙 До списку клієнтів', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS_LIST }]);
  } else {
    keyboard.push([{ text: '🔙 До картки клієнта', callback_data: CONSTANTS.CALLBACK_PREFIXES.VIEW_STUDENT + ':' + studentChatId }]);
  }
  keyboard.push([{ text: CONSTANTS.EMOJI.HOME + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]);
  await Helpers.sendKeyboard(
    chatId,
    noticeLine + '📏 **Обміри/Медицина/Досвід**\n\nКлієнт: ' + name,
    keyboard,
    { parse_mode: 'Markdown' }
  );
}

async function showStudentBodyMenu(chatId, studentChatId, notice = null) {
  const student = await User.getByChatId(studentChatId);
  if (!student || String(student.coachId) !== String(chatId)) {
    await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
    return;
  ...
  const noticeLine = notice ? (String(notice).trim() + '\n\n') : '';
  const keyboard = [
    [{ text: '🏃 Активність', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_EDIT_STUDENT_ACTIVITY + ':' + studentChatId }],
    [{ text: '🎯 Бажані параметри', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_BODY_GOALS + ':' + studentChatId }],
    [{ text: '📏 Ввести обміри тіла', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_EDIT_STUDENT_MEASUREMENTS + ':' + studentChatId }],
    [{ text: '🗓 Архів обмірів', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_MEAS_HISTORY + ':' + studentChatId }],
    [{ text: '🔙 До меню Обміри/Медицина/Досвід', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_HEALTH_MENU + ':' + studentChatId }],
    [{ text: CONSTANTS.EMOJI.HOME + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]
  ];
  await Helpers.sendKeyboard(chatId, noticeLine + '📏 **Обміри/Активність**\n\nКлієнт: ' + name.trim(), keyboard, { parse_mode: 'Markdown' });
}

function formatShortMeasurementValueFromValue(v, field) {
  if (v == null || v === '') return null;
  // bodyFatPct виводимо як відсоток
  if (field === 'bodyFatPct') return String(v) + '%';
  if (field === 'weight') return String(v) + ' кг';
  // решта довжин/обхватів у см
  return String(v) + ' см';
}

function buildCoachMeasurementsPickerKeyboard(studentChatId, latest = null, fallback = null) {
  const p = CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_MEAS_PICK;
  const withVal = (baseText, field) => {
    // пріоритет: останні обміри, але якщо в останньому записі поле null — показуємо fallback
    const v = (latest && latest[field] != null) ? latest[field] : (fallback ? fallback[field] : null);
    const sv = formatShortMeasurementValueFromValue(v, field);
    return sv ? (baseText + ': ' + sv) : baseText;
  };
  return [
    [{ text: withVal('📏 Зріст', 'height'), callback_data: p + ':height:' + studentChatId }, { text: withVal('⚖️ Вага', 'weight'), callback_data: p + ':weight:' + studentChatId }],
    [{ text: withVal('⭕ Талія', 'waist'), callback_data: p + ':waist:' + studentChatId }, { text: withVal('⭕ Стегно', 'hip'), callback_data: p + ':hip:' + studentChatId }],
    [{ text: withVal('⭕ Ягодиці', 'glutes'), callback_data: p + ':glutes:' + studentChatId }, { text: withVal('💪 Рука', 'arm'), callback_data: p + ':arm:' + studentChatId }],
    [{ text: withVal('🧣 Шия', 'neck'), callback_data: p + ':neck:' + studentChatId }, { text: withVal('⌚ Запʼястя', 'wrist'), callback_data: p + ':wrist:' + studentChatId }],
    [{ text: withVal('📐 Плечі', 'shoulders'), callback_data: p + ':shoulders:' + studentChatId }, { text: withVal('📐 Груди', 'chest'), callback_data: p + ':chest:' + studentChatId }],
    [{ text: withVal('📊 Жир (%)', 'bodyFatPct'), callback_data: p + ':bodyFatPct:' + studentChatId }],
    [{ text: '🔙 До меню обмірів', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_BODY_MENU + ':' + studentChatId }]
  ];
}

function getCoachMeasAskText(field) {
  // той самий текст, але “клієнта”
  switch (field) {
    case 'height': return '📏 Введіть зріст клієнта (см):\n\nПриклад: 175';
    case 'weight': return '⚖️ Введіть вагу клієнта (кг):\n\nПриклад: 72';
    case 'waist': return '⭕ Введіть обхват талії клієнта (см):\n\nПриклад: 80';
    case 'hip': return '⭕ Введіть обхват стегна клієнта (см):\n\nПриклад: 95';
    case 'glutes': return '⭕ Введіть обхват ягодиць клієнта (см):\n\nПриклад: 98';
    case 'arm': return '💪 Введіть обхват руки клієнта (см):\n\nПриклад: 32';
    case 'neck': return '🧣 Введіть обхват шиї клієнта (см):\n\nПриклад: 36';
    case 'wrist': return '⌚ Введіть обхват запʼястя клієнта (см):\n\nПриклад: 16';
    case 'shoulders': return "📐 Введіть обхват плечей клієнта (см)\nПриклад: 98";
    case 'chest': return '📐 Введіть обхват грудей клієнта (см)\nПриклад: 86';
    case 'bodyFatPct': return '📊 Введіть відсоток жиру клієнта (%):\n\nПриклад: 22.5';
    default: return 'Введіть значення:';
  }
}

function parseAndValidateCoachMeasurement(field, text, v) {
...
const COACH_GOAL_FIELDS = [
  { key: 'goal_weight', text: '⚖️ Вага', unit: 'кг', example: '65.0' },
  { key: 'goal_waist', text: '⭕ Талія', unit: 'см', example: '70.0' },
  { key: 'goal_hips', text: '⭕ Ягодиці', unit: 'см', example: '95.0' },
  { key: 'goal_shoulders', text: '📐 Плечі', unit: 'см', example: '105.0' },
  { key: 'goal_chest', text: '📐 Груди', unit: 'см', example: '90.0' },
  { key: 'goal_arm', text: '💪 Біцепс', unit: 'см', example: '36.0' }
];

function mapCoachGoalsDraft(goalsRow) {
  const g = goalsRow || {};
  return {
    goal_weight: g.goal_weight != null ? g.goal_weight : null,
    goal_waist: g.goal_waist != null ? g.goal_waist : null,
    goal_hips: g.goal_hips != null ? g.goal_hips : null,
    goal_shoulders: g.goal_shoulders != null ? g.goal_shoulders : null,
    goal_chest: g.goal_chest != null ? g.goal_chest : null,
    goal_arm: g.goal_arm != null ? g.goal_arm : null
  };
}

function formatCoachGoalValue(v, unit) {
  if (v == null) return null;
  return `${v} ${unit}`.trim();
}

function buildCoachGoalsPickerKeyboard(studentChatId, draft) {
  const d = draft || {};
  const p = CONSTANTS.CALLBACK_PREFIXES.COACH_BODY_GOALS_PICK;
  const keyboard = [];
  for (let i = 0; i < COACH_GOAL_FIELDS.length; i += 2) {
    const row = [];
    for (let j = i; j < i + 2 && j < COACH_GOAL_FIELDS.length; j++) {
      const f = COACH_GOAL_FIELDS[j];
      const sv = formatCoachGoalValue(d[f.key], f.unit);
      const txt = sv ? `${f.text}: ${sv}` : f.text;
      row.push({ text: txt.slice(0, 64), callback_data: `${p}:${f.key}:${studentChatId}` });
    }
    keyboard.push(row);
  }
  keyboard.push([{ text: '💾 Зберегти', callback_data: `${CONSTANTS.CALLBACK_PREFIXES.COACH_BODY_GOALS_SAVE}:${studentChatId}` }]);
  keyboard.push([{ text: '🔙 До меню обмірів', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_BODY_MENU + ':' + studentChatId }]);
  return keyboard;
}

async function showCoachGoalsPicker(chatId, studentChatId, notice = null) {
  const student = await User.getByChatId(studentChatId);
  if (!student || String(student.coachId) !== String(chatId)) {
    await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
    return;
  ...
  const draft = state && state.coachBodyGoalsDraft
    ? state.coachBodyGoalsDraft
    : mapCoachGoalsDraft(await supabase.getBodyGoals(studentChatId));
  await State.set(chatId, {
    step: CONSTANTS.FSM_STATES.COACH_BODY_GOALS_EDIT_VALUE,
    coachBodyGoalsStudentChatId: studentChatId,
    coachBodyGoalsDraft: draft,
    coachBodyGoalsEditField: null
  });
  const noticeLine = notice ? String(notice).trim() + '\n\n' : '';
  await Helpers.sendKeyboard(
    chatId,
    noticeLine + '🎯 Бажані параметри клієнта\n\nОбери параметр, який хочеш змінити:',
    buildCoachGoalsPickerKeyboard(studentChatId, draft)
  );
}

async function askCoachGoalField(chatId, student, goalField) {
  const meta = COACH_GOAL_FIELDS.find((x) => x.key === goalField);
  if (!meta) return;
  const hint = buildGoalPromptHint(goalField, buildCoachGoalsUserCtx(student));
  await Helpers.sendKeyboard(
    chatId,
    hint + `Введіть бажане значення для «${meta.text}» (${meta.unit})\nПриклад: ${meta.example}`,
    [[{ text: '🔙 До меню бажаних параметрів', callback_data: `${CONSTANTS.CALLBACK_PREFIXES.COACH_BODY_GOALS}:${student.chatId}` }]]
  );
}

async function showCoachMeasurementsPicker(chatId, studentChatId, notice = null) {
  const student = await User.getByChatId(studentChatId);
  if (!student || String(student.coachId) !== String(chatId)) {
    await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
    return;
  ...
  const noticeLine = notice ? (String(notice).trim() + '\n\n') : '';
  const [latest] = await supabase.getMeasurementHistory(studentChatId, 1);
  const fallbackFromUser = {
    height: student.height != null ? student.height : null,
    weight: student.weight != null ? student.weight : null,
    waist: student.waist != null ? student.waist : null,
    hip: student.hip != null ? student.hip : null,
    glutes: student.glutes != null ? student.glutes : null,
    arm: student.arm != null ? student.arm : null,
    shoulders: student.shoulders != null ? student.shoulders : null,
    chest: student.chest != null ? student.chest : null,
    neck: student.neck != null ? student.neck : null,
    wrist: student.wrist != null ? student.wrist : null,
    bodyFatPct: student.bodyFatPct != null ? student.bodyFatPct : null
  };
  const keyboard = buildCoachMeasurementsPickerKeyboard(studentChatId, latest || null, fallbackFromUser);
  await Helpers.sendKeyboard(chatId, noticeLine + '📏 Ввести обміри тіла\n\nОбери, який обмір хочеш змінити:', keyboard);
}

async function showStudentCurrentMeasurements(chatId, studentChatId) {
  const student = await User.getByChatId(studentChatId);
  if (!student || String(student.coachId) !== String(chatId)) {
    await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
    return;
  ...
  const [latest] = await supabase.getMeasurementHistory(studentChatId, 1);
  const dateStr = latest && latest.date ? latest.date.toLocaleDateString('uk-UA') : null;
  const title = '📌 Поточні обміри' + (dateStr ? ' (останнє оновлення: ' + dateStr + ')' : '');
  const fallbackFromUser = formatMeasurementDetails({
    height: student.height != null ? student.height : null,
    weight: student.weight != null ? student.weight : null,
    waist: student.waist != null ? student.waist : null,
    hip: student.hip != null ? student.hip : null,
  ...
  const text = (latest ? formatMeasurementDetails(latest) : '') || fallbackFromUser || 'Немає збережених обмірів.';
  const keyboard = [
    [{ text: '🗓 Архів обмірів', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_MEAS_HISTORY + ':' + studentChatId }],
    [{ text: '🔙 До меню обмірів', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_BODY_MENU + ':' + studentChatId }]
  ];
...
async function showStudentMeasurementsHistory(chatId, studentChatId) {
  const student = await User.getByChatId(studentChatId);
  if (!student || String(student.coachId) !== String(chatId)) {
    await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
    return;
  }
  const rows = await supabase.getMeasurementHistory(studentChatId, 20);
  await State.set(chatId, { coachMeasHistoryStudentChatId: studentChatId, coachMeasHistoryRows: rows });
  if (!rows || rows.length === 0) {
    const keyboard = [[{ text: '🔙 До меню обмірів', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_BODY_MENU + ':' + studentChatId }]];
    await Helpers.sendKeyboard(chatId, '🗓 Архів обмірів\n\nПоки немає жодного збереженого заміру.', keyboard);
    return;
  }
  const keyboard = rows.map((r, idx) => {
    const d = r.date instanceof Date ? r.date : (r.date ? new Date(r.date) : null);
    const label = d && !isNaN(d.getTime()) ? d.toLocaleDateString('uk-UA', { day: '2-digit', month: '2-digit', year: 'numeric' }) : ('Запис #' + (idx + 1));
    return [{ text: '📅 ' + label, callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_MEAS_VIEW + ':' + idx }];
  });
  keyboard.push([{ text: '🔙 До меню обмірів', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_BODY_MENU + ':' + studentChatId }]);
  await Helpers.sendKeyboard(chatId, '🗓 Архів обмірів\n\nОбери дату:', keyboard);
}

async function showStudentMeasurementByIndex(chatId, indexStr) {
  const state = await State.get(chatId);
  const idx = parseInt(String(indexStr), 10);
  const studentChatId = state && state.coachMeasHistoryStudentChatId ? String(state.coachMeasHistoryStudentChatId) : null;
  const rows = state && Array.isArray(state.coachMeasHistoryRows) ? state.coachMeasHistoryRows : [];
  if (!studentChatId || !rows || idx < 0 || idx >= rows.length) {
    await Helpers.safeSend(chatId, '❌ Запис не знайдено. Відкрий архів обмірів ще раз.');
    return;
  }
  const r = rows[idx];
  const d = r && r.date ? (r.date instanceof Date ? r.date : new Date(r.date)) : null;
  const label = d && !isNaN(d.getTime()) ? d.toLocaleDateString('uk-UA', { day: 'numeric', month: 'long', year: 'numeric' }) : '';
  const text = formatMeasurementDetails(r) || 'Немає даних.';
  const keyboard = [
    [{ text: '🔙 До архіву', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_MEAS_HISTORY + ':' + studentChatId }],
    [{ text: '🔙 До меню обмірів', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_BODY_MENU + ':' + studentChatId }]
  ];
  await Helpers.sendKeyboard(chatId, '📅 Обміри за ' + (label || 'датою') + '\n\n' + text, keyboard);
}
...
async function showArchivedList(chatId) {
  try {
    ...
    if (students.length === 0) {
      await Helpers.sendKeyboard(chatId, '📦 **Архів клієнтів**\n\nТут з\'являться клієнти, яких ти архівував. Поки порожньо.', keyboard, { parse_mode: 'Markdown' });
    } else {
      await Helpers.sendKeyboard(chatId, '📦 **Архів клієнтів** (' + students.length + '):\n\nОбери клієнта:', keyboard, { parse_mode: 'Markdown' });
    }
  } catch (err) {
...
async function archiveStudent(chatId, studentChatId) {
  try {
    ...
    const name = (student.firstName || '') + ' ' + (student.lastName || '').trim();
    await Helpers.safeSend(chatId, '✅ Профіль клієнта **' + name + '** архівовано. Він більше не відображатиметься в списку «Мої клієнти». Усі дані збережено. Переглянути можна в **Мої клієнти** → **Архів клієнтів**.', { parse_mode: 'Markdown' });
...
async function showArchivedStudentCard(chatId, studentChatId) {
  try {
    ...
    if (!student || String(student.coachId) !== String(chatId) || !student.isArchived) {
      await Helpers.safeSend(chatId, '❌ Клієнта не знайдено в архіві.');
...
async function activateArchivedStudent(chatId, studentChatId) {
  try {
    ...
    const name = (student.firstName || '') + ' ' + (student.lastName || '').trim();
    await Helpers.safeSend(chatId, '✅ Профіль клієнта **' + name + '** активовано. Він знову з\'явиться в списку «Мої клієнти».', { parse_mode: 'Markdown' });
    await showArchivedList(chatId);
  } catch (err) {
...
async function deleteArchivedStudent(chatId, studentChatId) {
  try {
    ...
    if (isInvite) {
      ...
      await Helpers.safeSend(chatId, '✅ Клієнт **' + name + '** видалено з твого списку. Його профіль більше не прив\'язаний до тебе (дані клієнта збережено).', { parse_mode: 'Markdown' });
    }
    await showArchivedList(chatId);
  } catch (err) {
...
/** Меню «Мій тренер»: з прив’язаним тренером — одразу картка; без тренера — пошук / інвайт. */
async function showMyCoachMenu(chatId) {
  try {
    ...
    const coachId = user.coachId ? String(user.coachId).trim() : '';
    if (user.role === CONSTANTS.ROLES.STUDENT && coachId) {
      await showCoachProfileToStudent(chatId, coachId);
      return;
    }
    const keyboard = [
    ...
    await Helpers.sendKeyboard(
      chatId,
      '👨‍🏫 <b>Мій тренер</b>\n\nУ тебе поки немає прив’язаного тренера. Обери дію:',
      keyboard,
      { parse_mode: 'HTML' }
    );
  } catch (err) {
...
async function showCoachDocumentsToStudent(chatId) {
  try {
    const student = await User.getByChatId(chatId);
    if (!student || student.role !== CONSTANTS.ROLES.STUDENT) {
      await Helpers.safeSend(chatId, '❌ Доступ тільки для клієнтів.');
      const Menu = require('./menu');
      await Menu.show(chatId);
      return;
    }
    const coachId = student.coachId ? String(student.coachId) : null;
    if (!coachId) {
      await Helpers.safeSend(chatId, '⚠️ У тебе поки немає тренера.');
      await showMyCoachMenu(chatId);
      return;
    }
    const docs = await supabase.getCoachDocuments(coachId, 20);
    if (!docs.length) {
      await Helpers.safeSend(chatId, '📄 Документів поки немає.');
      await showCoachProfileToStudent(chatId, coachId);
      return;
    }
    await Helpers.safeSend(chatId, '📄 Документи тренера (' + docs.length + '):');
    await sendCoachDocumentsMedia(chatId, docs);
    if (coachId) {
      await showCoachProfileToStudent(chatId, coachId);
    } else {
      await showMyCoachMenu(chatId);
    ...
    console.error('Coach.showCoachDocumentsToStudent', e.message);
    await Helpers.safeSend(chatId, '❌ Помилка при відкритті документів.');
    const st = await User.getByChatId(chatId);
    if (st && st.coachId) await showCoachProfileToStudent(chatId, String(st.coachId));
    else await showMyCoachMenu(chatId);
  }
}

function fmtPublicCoachTypes(types) {
  const arr = Array.isArray(types) ? types : [];
  const labels = [];
  if (arr.includes('individual')) labels.push('індивідуальні');
  if (arr.includes('group')) labels.push('групові');
  return labels.length ? labels.join(', ') : 'не вказано';
}

/**
 * Публічна «сторінка» тренера з каталогу закладів (за посиланням / кнопкою PVCH).
 */
async function showPublicVenueCoachCard(chatId, coachChatId) {
  try {
    const cid = String(coachChatId || '').trim();
    if (!cid) return;
    const allowed = await supabase.coachHasVenueListing(cid);
    if (!allowed) {
      await Helpers.safeSend(chatId, '❌ Тренера не знайдено в каталозі закладів.');
      return;
    }
    const coach = await User.getByChatId(cid);
    if (!coach || coach.role !== CONSTANTS.ROLES.COACH) {
      await Helpers.safeSend(chatId, '❌ Тренера не знайдено.');
      return;
    }
    const name = ((coach.firstName || '') + ' ' + (coach.lastName || '')).trim() || 'Тренер';
    let text =
      '👨‍🏫 <b>' +
      Helpers.escapeHtml(name) +
      '</b>\n\n' +
      '🏙 Місто: ' +
      Helpers.escapeHtml(coach.city || 'не вказано') +
      '\n' +
      '💪 Типи тренувань: ' +
      Helpers.escapeHtml(fmtPublicCoachTypes(coach.coachTrainingTypes)) +
      '\n';
    if (coach.instagram) {
      text += '\n📸 Instagram: ' + Helpers.escapeHtml(String(coach.instagram)) + '\n';
    }
    text += await coachGroupPublic.buildCoachGroupTrainingHtmlBlock(cid, coach.coachTrainingTypes, {
      groupByVenue: false
    });
    const viewer = await User.getByChatId(chatId);
    const isVenueOwner = viewer && viewer.role === CONSTANTS.ROLES.VENUE_OWNER;
    if (!isVenueOwner) {
      text +=
        '\n<i>Щоб тренуватися з цим тренером, потрібен інвайт-код або запрошення від тренера.</i>';
    }
    const keyboard = [];
    if (isVenueOwner) {
      keyboard.push([
    ...
    console.error('Coach.showPublicVenueCoachCard', err.message);
    await Helpers.safeSend(chatId, '❌ Не вдалося відкрити картку тренера.');
  }
}

/** Надіслати збережені фото/файли документів тренера (повертає кількість успішних відправок). */
async function sendCoachDocumentsMedia(chatId, docs) {
...
/** Картка тренера для клієнта або попереднього перегляду тренером. */
async function sendCoachPublicProfileCard(viewerChatId, coachId, options = {}) {
  const preview = options.preview === true;
  const coach = await User.getByChatId(coachId);
  if (!coach) {
    await Helpers.safeSend(viewerChatId, '❌ Тренера не знайдено.');
    return;
  }
  const name = ((coach.firstName || '') + ' ' + (coach.lastName || '').trim()).trim() || 'Тренер';
  const docs = await supabase.getCoachDocuments(coachId, 20);
  const docsCount = docs.length;
  const venues = (await supabase.getCoachVenuesWhereTeach(coachId)) || [];
  const activeVenues = venues.filter((v) => v && v.isActive !== false);
  const settings = await supabase.getCoachScheduleSettings(coachId);
  ...
  let text =
    ...
    Helpers.escapeHtml(coach.city || 'не вказано') +
    '\n' +
    formatCoachBirthDateLine(coach) +
    formatCoachInstagramLineHtml(coach) +
    '\n📄 Документи про освіту: ' +
    (docsCount > 0 ? docsCount + ' шт. (див. нижче)' : 'немає') +
    '\n';

  if (activeVenues.length > 0) {
    text += '\n🏢 Заклади, де тренує:\n';
    for (const v of activeVenues) {
      const vname = (v.nameUa || v.city || 'Заклад').trim();
      text += '• ' + Helpers.escapeHtml(vname) + '\n';
    }
    text += '\n<i>Натисни заклад нижче — відкриється картка.</i>\n';
  } else {
    text += '\n🏢 Заклади: не вказано\n';
  }

  text += '\n' + formatCoachWorkScheduleBlock(settings);

  text += await coachGroupPublic.buildCoachGroupTrainingHtmlBlock(coachId, coach.coachTrainingTypes);

  if (preview) {
    text =
      '👁 <b>Попередній перегляд картки тренера</b>\n\n' +
      '<i>Так бачать клієнти та користувачі, які шукають тренера:</i>\n\n' +
      text;
  }

  const keyboard = [];
  if (!preview) {
    const coachTgBtn = Helpers.telegramContactButton('💬 Написати тренеру в Telegram', coach);
    if (coachTgBtn) keyboard.push([coachTgBtn]);
  }
  for (const v of activeVenues) {
    let label = '🏢 ' + (v.nameUa || v.city || 'Заклад').trim();
    if (label.length > 64) label = label.slice(0, 61) + '…';
    keyboard.push([{ text: label, callback_data: CONSTANTS.CALLBACKS.VENUES_CARD + ':' + v.id }]);
  }
  if (preview) {
    keyboard.push([{ text: '🔙 До публічних даних', callback_data: CONSTANTS.CALLBACKS.HINTS_PUBLIC_STEPS }]);
    keyboard.push([{ text: '🔙 Підказки', callback_data: CONSTANTS.CALLBACKS.MENU_HINTS }]);
  } else {
    keyboard.push([{ text: '🔙 До меню «Тренування»', callback_data: CONSTANTS.CALLBACKS.MENU_TRAINING }]);
  }
  keyboard.push([{ text: CONSTANTS.EMOJI.HOME + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]);

  await Helpers.sendKeyboard(viewerChatId, text, keyboard, { parse_mode: 'HTML' });

  if (docsCount > 0) {
    const sent = await sendCoachDocumentsMedia(viewerChatId, docs);
    if (sent === 0) {
      await Helpers.safeSend(
        viewerChatId,
        '⚠️ Документи в профілі є, але Telegram не показав файли. Спробуй перевідправити їх у «Мої документи».'
      );
    }
  }
}

async function showCoachProfilePreviewForCoach(chatId) {
  const me = await User.getByChatId(chatId);
  if (!me || me.role !== CONSTANTS.ROLES.COACH) {
    await Helpers.safeSend(chatId, '👁 Перегляд доступний лише тренерам.');
    return;
  }
  try {
    await sendCoachPublicProfileCard(chatId, chatId, { preview: true });
  } catch (err) {
    console.error('Coach.showCoachProfilePreviewForCoach', err.message);
    const Menu = require('./menu');
    await Menu.show(chatId);
  }
}
...
async function showCoachProfileToStudent(chatId, coachId) {
  try {
    ...
    await sendCoachPublicProfileCard(chatId, coachId, { preview: false });
  } catch (err) {
...
async function showPricingMenu(chatId) {
  const keyboard = [
    ...
    [{ text: '👤 Індивідуально для клієнта', callback_data: CONSTANTS.CALLBACKS.PRICING_SET_INDIVIDUAL }],
    [{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS_LIST }]
  ];
  await Helpers.sendKeyboard(chatId, '💰 **Вартість тренувань**\n\nОбери, що налаштувати:', keyboard, { parse_mode: 'Markdown' });
}

function pricingAmountIsSet(value) {
  if (value == null || value === '') return false;
  const n = parseFloat(value);
  return !isNaN(n) && n >= 0;
}

function formatStoredPriceLabel(amount, currency) {
  const cur = String(currency || 'UAH').toUpperCase() === 'UAH' ? 'грн' : String(currency || 'UAH');
  const n = parseFloat(amount);
  if (isNaN(n)) return '';
  const num = Number.isInteger(n) ? String(n) : String(Math.round(n));
  return num + ' ' + cur;
}

function buildPricingTypeStatusLines(pricing) {
  const cur = (pricing && pricing.currency) || 'UAH';
  const personal = pricing && pricing.pricePersonal;
  const split = pricing && pricing.priceSplit;
  const trio = pricing && pricing.priceTrio;
  const lines = ['Поточні тарифи:'];
  if (pricingAmountIsSet(personal)) {
    lines.push('• Персональна: ' + formatStoredPriceLabel(personal, cur) + ' (за 1 клієнта)');
  } else {
    lines.push('• Персональна: ще не встановлено');
  }
  if (pricingAmountIsSet(split)) {
    lines.push(
      '• Спліт: ' +
        formatStoredPriceLabel(split, cur) +
        ' за двох (' +
        formatStoredPriceLabel(parseFloat(formatPerStudentShare(split, 2)), cur) +
        ' з клієнта)'
    );
  } else {
    lines.push('• Спліт: ще не встановлено');
  }
  if (pricingAmountIsSet(trio)) {
    lines.push(
      '• Тріо: ' +
        formatStoredPriceLabel(trio, cur) +
        ' за трьох (' +
        formatStoredPriceLabel(parseFloat(formatPerStudentShare(trio, 3)), cur) +
        ' з клієнта)'
    );
  } else {
    lines.push('• Тріо: ще не встановлено');
  }
  return lines.join('\n');
}

function pricingTypeButtonLabel(title, amount, currency) {
  if (!pricingAmountIsSet(amount)) return title;
  return '✅ ' + title + ' — ' + formatStoredPriceLabel(amount, currency);
}

async function loadPricingForTypeSelect(chatId, studentChatId) {
  if (studentChatId) {
    let row = await supabase.getStudentPricing(chatId, studentChatId);
    if (!row) row = await supabase.getCoachPricing(chatId);
    return row;
  }
  return await supabase.getCoachPricing(chatId);
}

async function showPricingTypeSelect(chatId, studentChatId) {
  const state = await State.get(chatId) || {};
  const returnToProfile = !!state.returnToProfile;
  const hintsFirstStepId = state.returnToHintsFirstStep != null ? parseInt(String(state.returnToHintsFirstStep), 10) : null;
  const pricing = await loadPricingForTypeSelect(chatId, studentChatId);
  const cur = (pricing && pricing.currency) || (CONSTANTS.PRICING && CONSTANTS.PRICING.DEFAULT_CURRENCY) || 'UAH';
  let backCallback;
  if (hintsFirstStepId && !isNaN(hintsFirstStepId)) {
    backCallback = CONSTANTS.CALLBACK_PREFIXES.HINT_STEP_VIEW + ':' + hintsFirstStepId;
  } else if (returnToProfile) {
    backCallback = CONSTANTS.CALLBACKS.BACK_TO_PROFILE;
  } else {
    backCallback = studentChatId
      ? CONSTANTS.CALLBACK_PREFIXES.VIEW_STUDENT + ':' + studentChatId
      : CONSTANTS.CALLBACKS.COACH_STUDENTS_LIST;
  }
  const keyboard = [
    [{ text: pricingTypeButtonLabel('Персональна', pricing && pricing.pricePersonal, cur), callback_data: CONSTANTS.CALLBACKS.PRICING_TYPE_PERSONAL }],
    [{ text: pricingTypeButtonLabel('Спліт (2)', pricing && pricing.priceSplit, cur), callback_data: CONSTANTS.CALLBACKS.PRICING_TYPE_SPLIT }],
    [{ text: pricingTypeButtonLabel('Тріо (3)', pricing && pricing.priceTrio, cur), callback_data: CONSTANTS.CALLBACKS.PRICING_TYPE_TRIO }],
    [{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: backCallback }]
  ];
  const statusBlock = buildPricingTypeStatusLines(pricing);
  await Helpers.sendKeyboard(
    chatId,
    '💰 Обери вид тренування для введення або зміни вартості:\n\n' +
      statusBlock +
      '\n\n' +
      '• Персональна — ціна за 1 клієнта.\n' +
      '• Спліт — загальна сума за 2 клієнти (у звіті з кожного — половина).\n' +
      '• Тріо — загальна сума за 3 клієнти (у звіті з кожного — третина).',
    keyboard
  );
}

function formatPerStudentShare(total, parts) {
  const per = total / parts;
  return Number.isInteger(per) ? String(per) : per.toFixed(1);
}

function formatPricingSavedMessage(pricingType, amount, currency) {
  const cur = currency || 'UAH';
  if (pricingType === CONSTANTS.TRAINING_TYPES.PERSONAL) {
    return '✅ Вартість персональної встановлено: ' + amount + ' ' + cur + ' (за 1 клієнта).';
  }
  if (pricingType === CONSTANTS.TRAINING_TYPES.SPLIT) {
    return (
      '✅ Вартість спліт встановлено: ' +
      amount +
      ' ' +
      cur +
      ' за двох.\nУ звіті доходу з кожного клієнта: ' +
      formatPerStudentShare(amount, 2) +
      ' ' +
      cur +
      '.'
    );
  ...
  return (
    '✅ Вартість тріо встановлено: ' +
    amount +
    ' ' +
    cur +
    ' за трьох.\nУ звіті доходу з кожного клієнта: ' +
    formatPerStudentShare(amount, 3) +
    ' ' +
    cur +
    '.'
  );
...
async function startDefaultPricingFromHints(chatId, firstStepId) {
  const stepId = parseInt(String(firstStepId), 10);
  await State.set(chatId, {
    step: CONSTANTS.FSM_STATES.PRICING_TYPE_SELECT,
    pricingStudentId: '',
    returnToProfile: false,
    returnToHintsFirstStep: !isNaN(stepId) ? stepId : 8
  });
...
async function showPricingStudentSelect(chatId) {
  const students = await User.getStudentsByCoach(chatId);
  if (!students || students.length === 0) {
    await Helpers.safeSend(chatId, '📋 У тебе поки немає клієнтів. Додай клієнта: **Мої клієнти** → **➕ Додати клієнта**.', { parse_mode: 'Markdown' });
    await showCoachStudentsHub(chatId);
    return;
  }
  ...
  keyboard.push([{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS_LIST }]);
  await Helpers.sendKeyboard(chatId, '💰 Обери клієнта для індивідуальної вартості:', keyboard);
}
...
async function handlePricingAmountInput(chatId, text) {
  ...
  const returnToProfile = !!state.returnToProfile;
  const returnToHintsFirstStep =
    state.returnToHintsFirstStep != null ? parseInt(String(state.returnToHintsFirstStep), 10) : null;
  const ok = await supabase.setPricing(chatId, pricingStudentId || null, current);
  if (ok) {
    await Helpers.safeSend(chatId, formatPricingSavedMessage(pricingType, amount, currency));
  } else {
    await Helpers.safeSend(chatId, '❌ Не вдалося зберегти. Спробуй ще раз.');
  }
  if (returnToHintsFirstStep && !isNaN(returnToHintsFirstStep)) {
    await State.set(chatId, {
      step: CONSTANTS.FSM_STATES.PRICING_TYPE_SELECT,
      pricingStudentId: pricingStudentId || '',
      returnToProfile: false,
      returnToHintsFirstStep
    });
    await showPricingTypeSelect(chatId, pricingStudentId || null);
    return;
  }
  await State.clear(chatId);
  if (returnToProfile) {
...
async function showStudentTrainingTypeSelect(chatId, studentChatId) {
  const keyboard = [
    ...
    [{ text: '🔙 До картки клієнта', callback_data: CONSTANTS.CALLBACK_PREFIXES.VIEW_STUDENT + ':' + studentChatId }]
  ];
  await Helpers.sendKeyboard(chatId, '🎯 Обери тип тренування за замовчуванням для цього клієнта:', keyboard);
}
...
async function showStudentExperienceSelect(chatId, studentChatId) {
  const keyboard = [
    ...
    [{ text: 'Більше року (1–2 роки)', callback_data: CONSTANTS.CALLBACK_PREFIXES.STUDENT_EXPERIENCE_MORE_YEAR + ':' + studentChatId }],
    [{ text: '2+ роки', callback_data: CONSTANTS.CALLBACK_PREFIXES.STUDENT_EXPERIENCE_2_PLUS + ':' + studentChatId }],
    [{ text: '🔙 До меню Обміри/Медицина/Досвід', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_HEALTH_MENU + ':' + studentChatId }]
  ];
  await Helpers.sendKeyboard(chatId, '📅 Встановити рівень досвіду клієнта:', keyboard);
}
    ...
    [
      {
        text: '🔙 Назад',
        callback_data:
          String(chatId) === String(studentChatId)
            ? CONSTANTS.CALLBACKS.MENU_TRAINING_PLANS
            : CONSTANTS.CALLBACK_PREFIXES.VIEW_STUDENT + ':' + studentChatId
      }
    ]
  ];
  await Helpers.sendKeyboard(chatId, '📆 Скільки тренувальних днів на тиждень у клієнта?', keyboard);
}
...
async function setStudentTrainingDays(chatId, studentChatId, days) {
  ...
  if (String(chatId) === String(studentChatId)) {
    const Menu = require('./menu');
...
async function setStudentExperience(chatId, studentChatId, level) {
  ...
  if (level === '0-3') {
    ...
    label = '6-12 м-ців';
  } else if (level === 'more_year') {
    experienceStartDate = new Date(now.getTime() - 400 * 24 * 60 * 60 * 1000);
    label = 'Більше року (1–2 роки)';
  } else {
    experienceStartDate = new Date(now.getTime() - 731 * 24 * 60 * 60 * 1000);
    label = '2+ роки';
  }
  const ok = await supabase.updateUser(studentChatId, { experienceStartDate });
  if (ok) {
    await Helpers.safeSend(chatId, '✅ Досвід клієнта встановлено: ' + label + '. Статус буде оновлюватися автоматично.');
  } else {
    await Helpers.safeSend(chatId, '❌ Не вдалося зберегти. Спробуй ще раз.');
  }
  if (await isStudentsListFromHintsPlan(chatId)) {
    await returnAfterStudentHealthCompletion(chatId, studentChatId);
    return;
  }
  await showStudentHealthMenu(chatId, studentChatId);
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.PVCH && param) {
    await showPublicVenueCoachCard(chatId, param);
    return true;
  }

  if (action === CONSTANTS.CALLBACKS.COACH_STUDENTS) {
    await State.update(chatId, { coachStudentsListFromHintsPlan: false });
    await showCoachStudentsHub(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.COACH_STUDENTS_LIST) {
    const st = await State.getSafe(chatId);
    if (!st || !st.coachStudentsListFromHintsPlan) {
      await State.update(chatId, { coachStudentsListFromHintsPlan: false });
    }
    await showStudentsList(chatId);
    return true;
  ...
  if (action === CONSTANTS.CALLBACKS.PRICING_TYPE_PERSONAL) {
    ...
    await Helpers.safeSend(chatId, '💰 Введіть вартість персонального тренування за 1 клієнта (ціле число, грн):');
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.PRICING_TYPE_SPLIT) {
    ...
    await Helpers.safeSend(
      chatId,
      '💰 Введіть загальну вартість спліт-тренування за 2 клієнти (ціле число, грн).\nПлатформа поділить суму на 2 — у звіті доходу з кожного клієнта буде половина.'
    );
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.PRICING_TYPE_TRIO) {
    ...
    await Helpers.safeSend(
      chatId,
      '💰 Введіть загальну вартість тріо-тренування за 3 клієнти (ціле число, грн).\nПлатформа поділить суму на 3 — у звіті доходу з кожного клієнта буде третина.'
    );
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.STUDENT_EXPERIENCE_2_PLUS && param) {
    await setStudentExperience(chatId, param.trim(), '2-plus');
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACKS.COACH_ADD_STUDENT_CONTINUE_PROFILE) {
    const state = await State.get(chatId);
    if (!state || state.step !== CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_INVITE_READY) return false;
    await askStudentGender(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.COACH_ADD_STUDENT_CREATE_INVITE_NOW) {
    const state = await State.get(chatId);
    if (!state || state.step !== CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_INVITE_READY) return false;
    if (!state.inviteExperienceLevel || !state.inviteBirthDate) {
      await Helpers.safeSend(chatId, '⚠️ Спочатку вкажи рівень досвіду та дату народження клієнта.');
      return true;
    }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.INVITE_GOAL && param) {
    ...
    if (state.inviteExperienceLevel) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.INVITE_MC_SEV && param) {
    ...
    if (severityKey === '__other__') {
      await Helpers.safeSend(chatId, 'У реєстрації клієнта можна обрати лише ступінь з кнопок. Інше можна додати пізніше в картці клієнта.');
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.INVITE_BODY_FAT_SKIP) {
    ...
    if (state.coachMeasurementsStudentChatId) {
      await saveCoachEnteredMeasurementsAndReturn(chatId, { ...state, inviteBodyFatPct: undefined });
      return true;
    }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_ACTIVITY_EXTRA && param) {
    ...
    await clearStateKeepingHintsPlanFlag(chatId);
    await Helpers.safeSend(chatId, '✅ Активність клієнта збережено.');
    await returnAfterStudentHealthCompletion(chatId, studentChatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_AI_ANALYTICS && param) {
    const studentChatId = param.trim();
    const student = await User.getByChatId(studentChatId);
    const studentName = student ? (student.firstName || '') : '';
    const label = studentName ? '🤖 AI-аналітика клієнта ' + studentName : '🤖 AI-аналітика клієнта';
    await bodyAnalysisAI.regenerateAndSendAnalysis(chatId, studentChatId, label, studentName);
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_HEALTH_MENU && param) {
    await showStudentHealthMenu(chatId, param.trim());
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_BODY_MENU && param) {
    await showStudentBodyMenu(chatId, param.trim());
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_MEAS_CURRENT && param) {
    await showStudentCurrentMeasurements(chatId, param.trim());
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_MEAS_HISTORY && param) {
    await showStudentMeasurementsHistory(chatId, param.trim());
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_MEAS_VIEW && param) {
    await showStudentMeasurementByIndex(chatId, param.trim());
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_EDIT_STUDENT_MEASUREMENTS && param) {
    const studentChatId = param.trim();
    const student = await User.getByChatId(studentChatId);
    if (!student || String(student.coachId) !== String(chatId)) {
      await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
    ...
    await showCoachMeasurementsPicker(chatId, studentChatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_MEAS_PICK && param) {
    const parts2 = String(callbackData).split(':');
    const field = parts2[1] ? parts2[1].trim() : '';
    const studentChatId = parts2[2] ? parts2[2].trim() : '';
    if (!field || !studentChatId) return false;
    const student = await User.getByChatId(studentChatId);
    if (!student || String(student.coachId) !== String(chatId)) {
      await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
      return true;
    }
    await State.set(chatId, { step: CONSTANTS.FSM_STATES.COACH_STUDENT_MEAS_EDIT_VALUE, coachMeasurementsStudentChatId: studentChatId, coachMeasEditField: field });
    const cancelKbd = [[{ text: '🔙 До меню обмірів', callback_data: CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_BODY_MENU + ':' + studentChatId }]];
    await Helpers.sendKeyboard(chatId, getCoachMeasAskText(field), cancelKbd);
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_BODY_GOALS && param) {
    ...
    await showCoachGoalsPicker(chatId, studentChatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_BODY_GOALS_PICK && param) {
    const parts2 = String(callbackData).split(':');
    const goalField = parts2[1] ? parts2[1].trim() : '';
    const studentChatId = parts2[2] ? parts2[2].trim() : '';
    if (!goalField || !studentChatId) return false;
    const student = await User.getByChatId(studentChatId);
    if (!student || String(student.coachId) !== String(chatId)) {
      await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
      return true;
    }
    const known = COACH_GOAL_FIELDS.some((f) => f.key === goalField);
    if (!known) return false;
    const st = await State.get(chatId);
    const draft = st && st.coachBodyGoalsDraft
      ? st.coachBodyGoalsDraft
      : mapCoachGoalsDraft(await supabase.getBodyGoals(studentChatId));
    await State.set(chatId, {
      step: CONSTANTS.FSM_STATES.COACH_BODY_GOALS_EDIT_VALUE,
      coachBodyGoalsStudentChatId: studentChatId,
      coachBodyGoalsDraft: draft,
      coachBodyGoalsEditField: goalField
    });
    await askCoachGoalField(chatId, student, goalField);
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_BODY_GOALS_SAVE && param) {
    const studentChatId = param.trim();
    const student = await User.getByChatId(studentChatId);
    if (!student || String(student.coachId) !== String(chatId)) {
      await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
    ...
    const st = await State.get(chatId);
    const goals = mapCoachGoalsDraft((st && st.coachBodyGoalsDraft) || null);
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_BODY_GOALS_SKIP_WEIGHT) {
    ...
    await showCoachGoalsPicker(chatId, state.coachBodyGoalsStudentChatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACKS.COACH_DOCS_VIEW) {
    await showCoachDocumentsToStudent(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_STUDENT_SCHEDULE && param) {
    const Schedule = require('./schedule');
    await Schedule.showCoachStudentSchedule(chatId, param.trim());
    return true;
  }
  return false;
}
...
async function askStudentName(chatId) {
  ...
  await Helpers.safeSend(chatId, "➕ Додавання нового клієнта\n\nВведи ім'я та прізвище клієнта одним повідомленням:\n\nФормат: Ім'я Прізвище\nПриклад: Марія Коваль");
}
...
async function askStudentBirthDate(chatId) {
  ...
  await Helpers.safeSend(
    chatId,
    "!! Обов'язково !!\n\n📅 Введіть дату народження клієнта:\n\nФормат: ДД.ММ.РРРР\nПриклад: " + example
  );
}

async function showInviteCreateOrContinueMenu(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_INVITE_READY });
  const keyboard = [
    [{ text: '🎟️ Створити інвайт-код зараз', callback_data: CONSTANTS.CALLBACKS.COACH_ADD_STUDENT_CREATE_INVITE_NOW }],
    [{ text: '➡️ Продовжити заповнення', callback_data: CONSTANTS.CALLBACKS.COACH_ADD_STUDENT_CONTINUE_PROFILE }]
  ];
  await Helpers.sendKeyboard(
    chatId,
    "✅ Обов'язкові дані заповнено (досвід і дата народження).\n\nСтворити інвайт зараз чи продовжити заповнення профілю?",
    keyboard
  );
}
...
async function askStudentGender(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, 'Оберіть стать клієнта:', keyboard);
}
...
async function askStudentGoal(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, '🎯 Оберіть ціль клієнта:', keyboard);
}
...
async function askStudentExperience(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: 'Більше року (1–2 роки)', callback_data: CONSTANTS.CALLBACK_PREFIXES.INVITE_EXP + ':more_year' }],
    [{ text: '2+ роки', callback_data: CONSTANTS.CALLBACK_PREFIXES.INVITE_EXP + ':2-plus' }]
  ];
  await Helpers.sendKeyboard(chatId, "!! Обов'язково !!\n\n📅 Оберіть рівень досвіду клієнта:", keyboard);
}
...
async function askStudentTrainingTypeForInvite(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, '💪 Оберіть вид тренування за замовчуванням для клієнта:', keyboard);
}
...
async function askStudentTrainingDays(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, '📆 Скільки днів на тиждень клієнт планує тренуватися?\n\n💡 Якщо клієнт змінить кількість днів тренування на тиждень — оновіть ці дані в картці клієнта.', keyboard);
}
...
async function askStudentMedicalChoice(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: 'Пропустити (додати пізніше з картки клієнта)', callback_data: CONSTANTS.CALLBACK_PREFIXES.INVITE_MC_SKIP }]
  ];
  await Helpers.sendKeyboard(chatId, '🩺 Медичний профіль клієнта\n\nЧи є обмеження за станом здоров\'я? Можна заповнити зараз (категорії та ступінь тяжкості) або пропустити і додати пізніше в картці клієнта.', keyboard);
}
...
async function showInviteMeasurementsChoice(chatId) {
  ...
  const keyboard = [
    ...
    [{ text: 'Пропустити (додати пізніше з картки клієнта)', callback_data: CONSTANTS.CALLBACK_PREFIXES.INVITE_MEASUREMENTS_SKIP }]
  ];
  await Helpers.sendKeyboard(chatId, '📏 Параметри клієнта (опційно)\n\nМожна вказати вагу, зріст, талію та сідниці зараз або пропустити і додати пізніше в картці клієнта.', keyboard);
}
...
async function showInviteAccentZones(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, '🎯 На що робимо акцент у тренуваннях клієнта?\nОбери 1–2 зони (або «Все рівномірно»).', keyboard);
}
...
async function askInviteBodyGoalsWeight(chatId) {
  ...
  const state = await State.get(chatId);
  const userCtx = buildInviteGoalsUserCtx(state);
  const hint = buildGoalPromptHint('goal_weight', userCtx);
  await Helpers.sendKeyboard(chatId, '🎯 **Бажані параметри клієнта**\n\n' + hint + 'Введіть бажану вагу клієнта (кг)\nПриклад: 55.0\nАбо натисніть «Пропустити»', keyboard);
}

function buildInviteGoalsUserCtx(state = {}) {
  return {
    height: state.inviteHeight != null ? state.inviteHeight : null,
    age: state.inviteBirthDate ? User.calculateAge(new Date(state.inviteBirthDate)) : null,
    gender: state.inviteGender || 'unknown',
    wrist: state.inviteWrist != null ? state.inviteWrist : null,
    experienceStartDate: state.inviteExperienceStartDate || null,
...
function buildCoachGoalsUserCtx(student) {
  ...
  return {
    ...
    wrist: student.wrist != null ? student.wrist : null,
    experienceStartDate: student.experienceStartDate || null,
    jobType: student.jobType || null,
    transportType: student.transportType || null,
    stepsCategory: student.stepsCategory || null,
    extraActivity: student.extraActivity || null
  };
}

function buildGoalPromptHint(goalField, userCtx) {
  if (!userCtx || userCtx.height == null || !userCtx.gender) return '';
...
function buildGoalRealismText(goalField, goalValue, currentValue, userCtx) {
  ...
  if (!realism) return '';
  const term = realism.weeks != null
    ? `Орієнтовний строк: ~${realism.weeks} тиж.`
    : (realism.months != null ? `Орієнтовний строк: ~${realism.months} міс.` : '');
...
async function askInviteBodyGoalsWaist(chatId) {
  ...
  const state = await State.get(chatId);
  const userCtx = buildInviteGoalsUserCtx(state);
  const hint = buildGoalPromptHint('goal_waist', userCtx);
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажану талію клієнта (см)\nПриклад: 65.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askInviteBodyGoalsHips(chatId) {
  ...
  const state = await State.get(chatId);
  const userCtx = buildInviteGoalsUserCtx(state);
  const hint = buildGoalPromptHint('goal_hips', userCtx);
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажані ягодиці клієнта (см)\nПриклад: 100.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askInviteBodyGoalsShoulders(chatId) {
  ...
  const state = await State.get(chatId);
  const userCtx = buildInviteGoalsUserCtx(state);
  const hint = buildGoalPromptHint('goal_shoulders', userCtx);
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажані плечі клієнта (см)\nПриклад: 100.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askInviteBodyGoalsChest(chatId) {
  ...
  const state = await State.get(chatId);
  const userCtx = buildInviteGoalsUserCtx(state);
  const hint = buildGoalPromptHint('goal_chest', userCtx);
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажані груди клієнта (см)\nПриклад: 90.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askCoachBodyGoalsWeight(chatId) {
  ...
  const state = await State.get(chatId);
  const student = state && state.coachBodyGoalsStudentChatId ? await User.getByChatId(state.coachBodyGoalsStudentChatId) : null;
  const userCtx = buildCoachGoalsUserCtx(student);
  const hint = buildGoalPromptHint('goal_weight', userCtx);
  await Helpers.sendKeyboard(chatId, '🎯 **Бажані параметри клієнта**\n\n' + hint + 'Введіть бажану вагу (кг)\nПриклад: 65.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askCoachBodyGoalsWaist(chatId) {
  ...
  const userCtx = buildCoachGoalsUserCtx(student);
  const hint = buildGoalPromptHint('goal_waist', userCtx);
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажану талію (см)\nПриклад: 70.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askCoachBodyGoalsHips(chatId) {
  ...
  const userCtx = buildCoachGoalsUserCtx(student);
  const hint = buildGoalPromptHint('goal_hips', userCtx);
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажані ягодиці (см)\nПриклад: 95.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askCoachBodyGoalsShoulders(chatId) {
  ...
  const userCtx = buildCoachGoalsUserCtx(student);
  const hint = buildGoalPromptHint('goal_shoulders', userCtx);
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажані плечі (см)\nПриклад: 105.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askCoachBodyGoalsChest(chatId) {
  ...
  const userCtx = buildCoachGoalsUserCtx(student);
  const hint = buildGoalPromptHint('goal_chest', userCtx);
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажані груди (см)\nПриклад: 90.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askInviteWeight(chatId) {
  ...
  await Helpers.safeSend(chatId, '⚖️ Введіть вагу клієнта (кг):\n\nДіапазон: ' + min + '–' + max + '\nПриклад: 72');
}
...
async function askInviteHeight(chatId) {
  ...
  await Helpers.safeSend(chatId, '📏 Введіть зріст клієнта (см):\n\nДіапазон: ' + min + '–' + max + '\nПриклад: 175');
}
...
async function askInviteWaist(chatId) {
  ...
  await Helpers.safeSend(chatId, '📐 Введіть обхват талії клієнта (см):\n\nДіапазон: ' + min + '–' + max + '\nПриклад: 80');
}
...
async function askInviteGlutes(chatId) {
  ...
  await Helpers.safeSend(chatId, '📐 Введіть обхват сідниць клієнта (см):\n\nДіапазон: ' + min + '–' + max + '\nПриклад: 98');
}

async function askInviteNeck(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_NECK });
  const v = CONSTANTS.VALIDATION || {};
  const min = v.NECK_MIN != null ? v.NECK_MIN : 20;
  const max = v.NECK_MAX != null ? v.NECK_MAX : 80;
  await Helpers.safeSend(chatId, '🧣 Введіть обхват шиї клієнта (см):\n\nДіапазон: ' + min + '–' + max + '\nПриклад: 36');
}

async function askInviteWrist(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_WRIST });
  const v = CONSTANTS.VALIDATION || {};
  const min = v.WRIST_MIN != null ? v.WRIST_MIN : 10;
  const max = v.WRIST_MAX != null ? v.WRIST_MAX : 35;
  await Helpers.safeSend(chatId, '⌚ Введіть обхват запʼястя клієнта (см):\n\nДіапазон: ' + min + '–' + max + '\nПриклад: 16');
}

async function askInviteShoulders(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_SHOULDERS });
  await Helpers.safeSend(chatId, "📐 Введіть обхват плечей клієнта (см)\nВимірювати по найширшій точці дельтоподібних м'язів, горизонтально.\nПриклад: 98");
}

async function askInviteChest(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_CHEST });
  await Helpers.safeSend(chatId, '📐 Введіть обхват грудей клієнта (см)\nВимірювати по найширшій точці грудної клітки.\nПриклад: 86');
}
...
async function askInviteBodyFat(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, 'Введіть відсоток жиру клієнта, якщо вимірювали каліпером.\nПриклад: 22.5\nАбо натисніть «Пропустити»', keyboard);
}

/** Зберегти заміри, введені тренером з картки клієнта (після ланцюжка weight→height→waist→glutes→shoulders→chest→bodyFat), і повернути до картки. */
async function saveCoachEnteredMeasurementsAndReturn(chatId, state) {
  const studentChatId = state.coachMeasurementsStudentChatId;
  if (!studentChatId) return;
  try {
    const measurements = {
      weight: state.inviteWeight,
      waist: state.inviteWaist,
      glutes: state.inviteGlutes,
      neck: state.inviteNeck,
      wrist: state.inviteWrist,
      shoulders: state.inviteShoulders,
      chest: state.inviteChest,
      bodyFatPct: state.inviteBodyFatPct
    };
    if (state.inviteHeight != null) {
      await supabase.updateUser(studentChatId, { height: state.inviteHeight });
    }
    await User.updateMeasurements(studentChatId, measurements);
    await clearStateKeepingHintsPlanFlag(chatId);
    await Helpers.safeSend(chatId, '✅ Обміри клієнта збережено.');
    await returnAfterStudentHealthCompletion(chatId, studentChatId);
  } catch (e) {
    console.error('Coach.saveCoachEnteredMeasurementsAndReturn', e.message || e);
    await clearStateKeepingHintsPlanFlag(chatId);
    await Helpers.safeSend(chatId, '❌ Помилка збереження обмірів. Спробуйте ще раз.');
    await returnAfterStudentHealthCompletion(chatId, studentChatId);
  }
}
...
async function askInviteActivityJob(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, '🏃 **Активність клієнта**\n\nЯка у клієнта робота?', keyboard);
}
...
async function askInviteActivityTransport(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, 'Як клієнт добирається до роботи?', keyboard);
}
...
async function askInviteActivitySteps(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, 'Скільки кроків приблизно на день у клієнта?', keyboard);
}
...
async function askInviteActivityExtra(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, 'Чи є у клієнта інша активність поза залом?', keyboard);
}
...
async function finishCreateStudentByInvite(chatId, state, days, conditions) {
  ...
  if (!birthDate || isNaN(birthDate.getTime())) {
    await Helpers.safeSend(chatId, '❌ Вкажи дату народження клієнта перед створенням інвайту.');
    return;
  }
  if (!experienceLevel) {
    await Helpers.safeSend(chatId, '❌ Вкажи рівень досвіду клієнта перед створенням інвайту.');
    return;
  }
  try {
    const accentZones = Array.isArray(state.inviteAccentZones) && state.inviteAccentZones.length > 0 ? state.inviteAccentZones : ['full'];
    ...
    const inviteCode = await User.createStudentByInvite(chatId, first, last, {
      ...
      neck: state.inviteNeck != null ? state.inviteNeck : null,
      wrist: state.inviteWrist != null ? state.inviteWrist : null,
    ...
    // Зберегти стартові заміри в measurements_history (щоб "архів обмірів" не був порожній після створення інвайта)
    const hasAnyMeas = state.inviteWeight != null || state.inviteWaist != null || state.inviteGlutes != null || state.inviteShoulders != null || state.inviteChest != null || state.inviteNeck != null || state.inviteWrist != null || state.inviteBodyFatPct != null;
    if (hasAnyMeas) {
      await supabase.insertMeasurement({
        chatId: String(inviteCode),
        date: new Date(),
        weight: state.inviteWeight != null ? state.inviteWeight : null,
        waist: state.inviteWaist != null ? state.inviteWaist : null,
        ...
        wrist: state.inviteWrist != null ? state.inviteWrist : null,
        bodyFatPct: state.inviteBodyFatPct != null ? state.inviteBodyFatPct : null,
        source: 'coach_invite_create'
      });
    }
    // AI-аналіз тіла за замірами — показати тренеру одразу після створення клієнта
    // Аналітика для тренера; зберігаємо в БД для клієнта (inviteCode) — 4-й аргумент saveForChatId
    ...
    if (hasBodyGoals) {
      ...
      // Будуємо поточні заміри зі state (measurements_history порожня — клієнт щойно створений)
      const inviteCurrent = (state.inviteWaist != null || state.inviteGlutes != null || state.inviteShoulders != null)
        ? {
            weight: state.inviteWeight != null ? state.inviteWeight : null,
            waist: state.inviteWaist != null ? state.inviteWaist : null,
            glutes: state.inviteGlutes != null ? state.inviteGlutes : null,
      ...
      const inviteAnalysis = bodyGoals.analyzeGoalsVsCurrentState(inviteGoals, inviteUserCtx, inviteCurrent, 'goals_save');
      const bgRes = await supabase.upsertBodyGoals(chatId, inviteCode, inviteGoals, inviteAnalysis);
      if (!bgRes || !bgRes.ok) console.error('Coach invite: upsertBodyGoals failed', bgRes && bgRes.error);
      // Показуємо тренеру результат аналізу цілей
      if (inviteAnalysis) {
        const deltaItems = inviteAnalysis.deltaItems || [];
        const warnings = inviteAnalysis.warnings || [];
        const errors = inviteAnalysis.errors || [];
        if (errors.length > 0) {
          await Helpers.safeSend(chatId, '⚠️ Бажані параметри збережено, але є питання:\n' + errors.map((e) => '• ' + e.message).join('\n'));
        } else if (deltaItems.length > 0 || warnings.length > 0) {
          const lines = ['🎯 Аналіз бажаних параметрів:'];
          for (const item of deltaItems) {
            if (item.reached) {
              lines.push(item.label + ': ціль вже досягнута (' + item.current + ' ' + item.unit + ')');
            } else {
              lines.push(item.label + ': ' + item.current + ' → ' + item.goal + ' ' + item.unit + ' (' + item.direction + ' ~' + item.months + ' міс.)');
            }
          }
          for (const w of warnings) {
            lines.push('⚠️ ' + w.message);
          }
          await Helpers.safeSend(chatId, lines.join('\n'));
        }
      }
    }
    ...
    let doneMsg =
      '✅ Клієнта створено!\n\n' +
      'Нижче — **окреме повідомлення** лише з інвайт-кодом. ПЕРЕШЛИ його клієнту.\n\n' +
      'Клієнт у боті: **🎟️ У мене є код** → ввести код. Профіль прив’яжеться до тебе автоматично. Вартість — за обраним видом тренування.';
    if (conditions.length > 0) {
      doneMsg += '\n\nМедичний профіль заповнено (' + conditions.length + ' станів).';
    }
    await Helpers.safeSend(chatId, doneMsg, { parse_mode: 'Markdown' });
    await sendInviteCodeForForwarding(chatId, inviteCode);
    await showAfterInviteCreatedMenu(chatId);
  } catch (err) {
    console.error('Coach.finishCreateStudentByInvite', err.message);
    await State.clear(chatId);
    await Helpers.safeSend(chatId, "❌ Помилка створення клієнта:\n" + err.message + "\n\nСпробуй ще раз.");
    await showAfterInviteCreatedMenu(chatId);
  }
}
...
async function handleTextMessage(chatId, text) {
  ...
  const v = CONSTANTS.VALIDATION || {};
  if (state.step === CONSTANTS.FSM_STATES.COACH_STUDENT_MEAS_EDIT_VALUE && state.coachMeasurementsStudentChatId && state.coachMeasEditField) {
    const studentChatId = String(state.coachMeasurementsStudentChatId);
    const field = String(state.coachMeasEditField);
    const student = await User.getByChatId(studentChatId);
    if (!student || String(student.coachId) !== String(chatId)) {
      await State.clear(chatId);
      await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
    ...
    const parsed = parseAndValidateCoachMeasurement(field, text, v);
    if (!parsed.ok) {
      await Helpers.safeSend(chatId, parsed.error);
      return true;
    }
    await User.updateMeasurements(studentChatId, { [field]: parsed.value });
    await clearStateKeepingHintsPlanFlag(chatId);
    // Залишаємо тренера у підменю вводу обмірів, щоб можна було ввести кілька значень підряд
    await showCoachMeasurementsPicker(chatId, studentChatId, '✅ Обмір оновлено.');
    return true;
  }
  ...
  if (state.step === CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_GLUTES) {
    ...
    await askInviteNeck(chatId);
    return true;
  }
  if (state.step === CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_NECK) {
    const n = parseFloat(String(text).trim().replace(',', '.'));
    const min = v.NECK_MIN != null ? v.NECK_MIN : 20;
    const max = v.NECK_MAX != null ? v.NECK_MAX : 80;
    if (isNaN(n) || n < min || n > max) {
    ...
    await State.update(chatId, { inviteNeck: Math.round(n * 10) / 10 });
    await askInviteWrist(chatId);
    return true;
  }
  if (state.step === CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_WRIST) {
    const n = parseFloat(String(text).trim().replace(',', '.'));
    const min = v.WRIST_MIN != null ? v.WRIST_MIN : 10;
    const max = v.WRIST_MAX != null ? v.WRIST_MAX : 35;
    if (isNaN(n) || n < min || n > max) {
    ...
    await State.update(chatId, { inviteWrist: Math.round(n * 10) / 10 });
  ...
  if (state.step === CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_BODY_FAT) {
    ...
    const stateAfter = await State.get(chatId);
    if (stateAfter && stateAfter.coachMeasurementsStudentChatId) {
      await saveCoachEnteredMeasurementsAndReturn(chatId, stateAfter);
      return true;
    }
    await askInviteActivityJob(chatId);
    return true;
  ...
  if (state.step === CONSTANTS.FSM_STATES.INVITE_BODY_GOALS_WEIGHT) {
    const check = bodyGoals.validateGoalField(
      'goal_weight',
      String(text).trim(),
      state.inviteHeight || null,
      buildInviteGoalsUserCtx(state)
    );
    if (!check.valid) {
    ...
    const realismText = buildGoalRealismText('goal_weight', check.value, state.inviteWeight != null ? state.inviteWeight : null, buildInviteGoalsUserCtx(state));
    if (realismText) await Helpers.safeSend(chatId, realismText);
    await askInviteBodyGoalsWaist(chatId);
    return true;
  }
  if (state.step === CONSTANTS.FSM_STATES.INVITE_BODY_GOALS_WAIST) {
    ...
    const realismText = buildGoalRealismText('goal_waist', check.value, state.inviteWaist != null ? state.inviteWaist : null, buildInviteGoalsUserCtx(state));
    if (realismText) await Helpers.safeSend(chatId, realismText);
    await askInviteBodyGoalsHips(chatId);
    return true;
  }
  if (state.step === CONSTANTS.FSM_STATES.INVITE_BODY_GOALS_HIPS) {
    ...
    const realismText = buildGoalRealismText('goal_hips', check.value, state.inviteGlutes != null ? state.inviteGlutes : null, buildInviteGoalsUserCtx(state));
    if (realismText) await Helpers.safeSend(chatId, realismText);
    await askInviteBodyGoalsShoulders(chatId);
    return true;
  }
  if (state.step === CONSTANTS.FSM_STATES.INVITE_BODY_GOALS_SHOULDERS) {
    ...
    const realismText = buildGoalRealismText('goal_shoulders', check.value, state.inviteShoulders != null ? state.inviteShoulders : null, buildInviteGoalsUserCtx(state));
    if (realismText) await Helpers.safeSend(chatId, realismText);
    await askInviteBodyGoalsChest(chatId);
    return true;
  }
  if (state.step === CONSTANTS.FSM_STATES.INVITE_BODY_GOALS_CHEST) {
    ...
    const realismText = buildGoalRealismText('goal_chest', check.value, state.inviteChest != null ? state.inviteChest : null, buildInviteGoalsUserCtx(state));
    if (realismText) await Helpers.safeSend(chatId, realismText);
    await runFinishCreateStudentByInvite(chatId);
    return true;
  }
  if (state.step === CONSTANTS.FSM_STATES.COACH_BODY_GOALS_EDIT_VALUE && state.coachBodyGoalsStudentChatId && state.coachBodyGoalsEditField) {
    const studentChatId = String(state.coachBodyGoalsStudentChatId);
    const student = await User.getByChatId(studentChatId);
    if (!student || String(student.coachId) !== String(chatId)) {
      await State.clear(chatId);
      await Helpers.safeSend(chatId, '❌ Клієнта не знайдено.');
    ...
    const goalField = String(state.coachBodyGoalsEditField);
    const check = bodyGoals.validateGoalField(goalField, String(text).trim(), student.height != null ? student.height : null);
    if (!check.valid) {
      await Helpers.safeSend(chatId, '⚠️ ' + check.error);
      return true;
    }
    const draft = {
      ...mapCoachGoalsDraft((state && state.coachBodyGoalsDraft) || null),
      [goalField]: check.value
    };
    await State.set(chatId, {
      step: CONSTANTS.FSM_STATES.COACH_BODY_GOALS_EDIT_VALUE,
      coachBodyGoalsStudentChatId: studentChatId,
    ...
    const currentMap = {
      goal_weight: student.weight != null ? student.weight : null,
      goal_waist: student.waist != null ? student.waist : null,
      goal_hips: student.glutes != null ? student.glutes : null,
      goal_shoulders: student.shoulders != null ? student.shoulders : null,
      goal_chest: student.chest != null ? student.chest : null,
      goal_arm: student.arm != null ? student.arm : null
    };
    const realismText = buildGoalRealismText(goalField, check.value, currentMap[goalField], buildCoachGoalsUserCtx(student));
    if (realismText) await Helpers.safeSend(chatId, realismText);
    await showCoachGoalsPicker(chatId, studentChatId, '✅ Бажаний параметр оновлено.');
    return true;
  }
  if (state.step === CONSTANTS.FSM_STATES.COACH_BODY_GOALS_WEIGHT && state.coachBodyGoalsStudentChatId) {
    ...
    const check = bodyGoals.validateGoalField(
      'goal_weight',
      String(text).trim(),
      student ? student.height : null,
      buildCoachGoalsUserCtx(student)
    );
    if (!check.valid) {
      await Helpers.safeSend(chatId, '⚠️ ' + check.error);
      return true;
    ...
    const realismText = buildGoalRealismText('goal_weight', check.value, student && student.weight != null ? student.weight : null, buildCoachGoalsUserCtx(student));
    if (realismText) await Helpers.safeSend(chatId, realismText);
  ...
  if (state.step === CONSTANTS.FSM_STATES.COACH_BODY_GOALS_WAIST && state.coachBodyGoalsStudentChatId) {
    ...
    const realismText = buildGoalRealismText('goal_waist', check.value, student && student.waist != null ? student.waist : null, buildCoachGoalsUserCtx(student));
    if (realismText) await Helpers.safeSend(chatId, realismText);
  ...
  if (state.step === CONSTANTS.FSM_STATES.COACH_BODY_GOALS_HIPS && state.coachBodyGoalsStudentChatId) {
    ...
    const realismText = buildGoalRealismText('goal_hips', check.value, student && student.glutes != null ? student.glutes : null, buildCoachGoalsUserCtx(student));
    if (realismText) await Helpers.safeSend(chatId, realismText);
  ...
  if (state.step === CONSTANTS.FSM_STATES.COACH_BODY_GOALS_SHOULDERS && state.coachBodyGoalsStudentChatId) {
    ...
    const realismText = buildGoalRealismText('goal_shoulders', check.value, student && student.shoulders != null ? student.shoulders : null, buildCoachGoalsUserCtx(student));
    if (realismText) await Helpers.safeSend(chatId, realismText);
  ...
  if (state.step === CONSTANTS.FSM_STATES.COACH_BODY_GOALS_CHEST && state.coachBodyGoalsStudentChatId) {
    ...
    const realismText = buildGoalRealismText('goal_chest', check.value, student && student.chest != null ? student.chest : null, buildCoachGoalsUserCtx(student));
    if (realismText) await Helpers.safeSend(chatId, realismText);
    const result = await bodyGoals.saveBodyGoals(chatId, studentChatId, goals);
    await clearStateKeepingHintsPlanFlag(chatId);
    if (!result.saved) {
  ...
  if (state.step === CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_AGE) {
    ...
    await showInviteCreateOrContinueMenu(chatId);
    return true;
  }
  ...
  if (parts.length < 2) {
    await Helpers.safeSend(chatId, "⚠️ Введи ім'я та прізвище клієнта одним повідомленням.\n\nФормат: Ім'я Прізвище\nПриклад: Марія Коваль\n\nСпробуй ще раз:");
    return true;
  }
  const first = parts[0];
...
module.exports = {
  showStudentsList,
  openStudentsListFromHintsPlan,
  showStudentProfile,
  startDefaultPricingFromHints,
  handleCallback,
  handleTextMessage,
  askStudentName,
  getGoalText,
  showPublicVenueCoachCard,
  showCoachProfilePreviewForCoach,
  sendCoachPublicProfileCard
};
