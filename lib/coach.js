...
async function sendCoachPublicProfileCard(viewerChatId, coachId, options = {}) {
  ...
  const docsCount = await supabase.countCoachDocuments(coachId);
  ...
  const keyboard = [];
  if (!preview && docsCount > 0) {
    keyboard.push([{ text: '📄 Документи тренера', callback_data: CONSTANTS.CALLBACKS.COACH_DOCS_VIEW }]);
  }
  for (const v of activeVenues) {
