  ...
  try {
    ...
    if (coachId) {
      ...
      if (coach) {
        ...
        docsCount = await supabase.countCoachDocuments(coachId);
        if (docsCount > 0) text += 'Документи: ' + docsCount + '\n';
        text += '\n';
      }
    }
    const keyboard = [
      ...
      ...(coachId && docsCount > 0 ? [[{ text: '📄 Документи тренера', callback_data: CONSTANTS.CALLBACKS.COACH_DOCS_VIEW }]] : []),
      [{ text: CONSTANTS.EMOJI.HOME + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]
    ];
