...
async function showStudentProfile(chatId, studentChatId) {
  try {
    ...
    const kbd = [
      [{ text: '🎯 Змінити тип тренування', callback_data: CONSTANTS.CALLBACK_PREFIXES.STUDENT_TRAINING_TYPE + ':' + studentChatId }, { text: '💰 Встановити індивідуальну вартість', callback_data: CONSTANTS.CALLBACK_PREFIXES.PRICING_STUDENT + ':' + studentChatId }],
      [{ text: '📅 Досвід', callback_data: CONSTANTS.CALLBACK_PREFIXES.STUDENT_EXPERIENCE + ':' + studentChatId }, { text: '📆 Тренувальних днів', callback_data: CONSTANTS.CALLBACK_PREFIXES.STUDENT_DAYS + ':' + studentChatId }],
      [{ text: '🩺 Медичний профіль', callback_data: CONSTANTS.CALLBACK_PREFIXES.MC_PROFILE + ':' + studentChatId }],
      [{ text: '📋 Програма тренувань', callback_data: CONSTANTS.CALLBACK_PREFIXES.PLAN_LIST + ':' + studentChatId }],
...
async function showAIChecklists(chatId) {
  const me = await User.getByChatId(chatId);
  if (!me || me.role !== CONSTANTS.ROLES.COACH) {
    await Helpers.safeSend(chatId, '🤖 Чеклісти AI доступні тільки тренерам.');
    return;
  }
  const text =
    '🤖 **Чеклісти роботи AI**\n\n' +
    '**Що робить AI у боті** (якщо увімкнено):\n' +
    '• **Коментарі тренера в планах** — при генерації плану (Авто-підбір) до кожної вправи додається короткий персональний коментар (💬 Тренер).\n' +
    '• **Розумні нагадування** — учні отримують персоналізовані нагадування перед тренуванням замість фіксованого тексту.\n' +
    '• **Аналіз невиконання вправ** — після тренування, якщо учень виконав менше 80% підходів, AI дає пораду учню та за потреби сповіщає тебе.\n\n' +
    '**Чекліст підключення AI:**\n' +
    '□ API-ключ OpenAI (platform.openai.com) створено та додано в Railway\n' +
    '□ У Supabase виконано міграцію supabase_migration_ai_content.sql (таблиця ai_generated_content)\n' +
    '□ У Railway задано змінні: OPENAI_API_KEY та AI_ENABLED=true\n' +
    '□ Коментарі з\'являються тільки в нових планах, створених через Авто-підбір після підключення AI\n\n' +
    'Детально: документ Підключення_AI_покроково.md в проєкті.';
  const keyboard = [
  ...
  try {
    ...
    console.error('Coach.showAIChecklists sendKeyboard', e.message);
    await Helpers.safeSend(chatId, '🤖 Чеклісти AI. Помилка відправки — спробуй пізніше або звернись до підтримки.');
  }
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.COACH_AI_CHECKLISTS) {
    await showAIChecklists(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.INVITE_DAYS && param) {
    ...
    const age = state.inviteAge != null ? state.inviteAge : null;
    ...
    try {
      const inviteCode = await User.createStudentByInvite(chatId, first, last, { age, gender, goal, experienceLevel, defaultTrainingType, trainingDaysPerWeek: days });
      await State.clear(chatId);
      await Helpers.safeSend(chatId, "✅ Учня створено!\n\nПередай йому цей код доступу:\n" + inviteCode + "\n\nКоли він введе його у боті (У мене є код), його профіль автоматично прив'яжеться до тебе. Вартість підтягнута до обраного виду тренування.");
      await showStudentsList(chatId);
    } catch (err) {
...
async function askStudentAge(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_AGE });
  await Helpers.safeSend(chatId, "📅 Введіть вік учня (цифрами):\n\nПриклад: 23");
}
...
async function askStudentTrainingDays(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, '📆 Скільки днів на тиждень учень планує тренуватися?', keyboard);
}
...
async function handleTextMessage(chatId, text) {
  ...
  if (state.step === CONSTANTS.FSM_STATES.COACH_ADD_STUDENT_AGE) {
    ...
    if (isNaN(age) || age < ageMin || age > ageMax) {
      await Helpers.safeSend(chatId, '⚠️ Введіть вік цифрами (від ' + ageMin + ' до ' + ageMax + ').\n\nПриклад: 23');
      return true;
    }
    await State.update(chatId, { inviteAge: age });
  ...
  await askStudentAge(chatId);
  return true;
}

module.exports = { showStudentsList, showStudentProfile, showAIChecklists, handleCallback, handleTextMessage, askStudentName, getGoalText };
