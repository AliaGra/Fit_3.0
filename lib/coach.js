...
async function showStudentBodyMenu(chatId, studentChatId) {
  const student = await User.getByChatId(studentChatId);
  ...
  await Helpers.sendKeyboard(chatId, '📏 **Обміри/Активність**\n\nУчень: ' + name.trim(), keyboard, { parse_mode: 'Markdown' });
}
...
async function handleTextMessage(chatId, text) {
  ...
  if (state.step === CONSTANTS.FSM_STATES.COACH_STUDENT_MEAS_EDIT_VALUE && state.coachMeasurementsStudentChatId && state.coachMeasEditField) {
    ...
    await State.clear(chatId);
    await Helpers.safeSend(chatId, '✅ Обмір оновлено.');
    await showStudentBodyMenu(chatId, studentChatId);
    return true;
  }
