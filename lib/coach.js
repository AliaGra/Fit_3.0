...
async function finishCreateStudentByInvite(chatId, state, days, conditions) {
  ...
  try {
    ...
    if (hasBodyGoals) {
      ...
      const inviteStudent = await User.getByChatId(inviteCode);
      const inviteCurrent = await supabase.getLatestMeasurementsForGoals(inviteCode);
      const inviteAnalysis = bodyGoals.analyzeGoalsVsCurrentState(inviteGoals, inviteStudent || { height: state.inviteHeight || null, age: null, gender: state.inviteGender || 'unknown', teenMode: null }, inviteCurrent, 'goals_save');
