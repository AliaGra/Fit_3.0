...
/** Режим роботи тренера для картки учня (з «Налаштування розкладу»). */
...
/** Окреме повідомлення лише з кодом — зручно копіювати та переслати учню в Telegram. */
...
/** Підменю «Мої учні»: список чи архів. */
async function showCoachStudentsHub(chatId) {
  const me = await User.getByChatId(chatId);
  if (!User.isCoach(me)) {
    await Helpers.safeSend(chatId, '👥 Розділ «Мої учні» доступний тільки тренерам.');
    return;
  }
  const keyboard = [
  ...
  await Helpers.sendKeyboard(chatId, '👥 **Мої учні**\n\nОбери розділ:', keyboard, { parse_mode: 'Markdown' });
}
  ...
  try {
    ...
    if (!students || students.length === 0) {
      ...
      await Helpers.sendKeyboard(
        chatId,
        '📋 У тебе поки немає учнів.\n\nДодай учня: **Мої учні** → **➕ Додати учня**.',
        keyboard,
        { parse_mode: 'Markdown' }
      );
      return;
    ...
    keyboard.push([{ text: '🔙 Розділ «Мої учні»', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS }]);
...
async function showStudentProfile(chatId, studentChatId) {
  try {
    ...
    const kbd = [];
    if (!isInvite) {
      const studentTgBtn = Helpers.telegramContactButton('💬 Написати учню в Telegram', student);
...
async function showStudentHealthMenu(chatId, studentChatId, notice = null) {
  ...
  if (fromHintsPlan) {
    keyboard.push([{ text: '🔙 До списку учнів', callback_data: CONSTANTS.CALLBACKS.COACH_STUDENTS_LIST }]);
  } else {
...
function getCoachMeasAskText(field) {
  // той самий текст, але “учня”
  switch (field) {
    case 'height': return '📏 Введіть зріст учня (см):\n\nПриклад: 175';
    case 'weight': return '⚖️ Введіть вагу учня (кг):\n\nПриклад: 72';
    case 'waist': return '⭕ Введіть обхват талії учня (см):\n\nПриклад: 80';
    case 'hip': return '⭕ Введіть обхват стегна учня (см):\n\nПриклад: 95';
    case 'glutes': return '⭕ Введіть обхват ягодиць учня (см):\n\nПриклад: 98';
    case 'arm': return '💪 Введіть обхват руки учня (см):\n\nПриклад: 32';
    case 'neck': return '🧣 Введіть обхват шиї учня (см):\n\nПриклад: 36';
    case 'wrist': return '⌚ Введіть обхват запʼястя учня (см):\n\nПриклад: 16';
    case 'shoulders': return "📐 Введіть обхват плечей учня (см)\nПриклад: 98";
    case 'chest': return '📐 Введіть обхват грудей учня (см)\nПриклад: 86';
    case 'bodyFatPct': return '📊 Введіть відсоток жиру учня (%):\n\nПриклад: 22.5';
...
async function archiveStudent(chatId, studentChatId) {
  try {
    ...
    const name = (student.firstName || '') + ' ' + (student.lastName || '').trim();
    await Helpers.safeSend(chatId, '✅ Профіль учня **' + name + '** архівовано. Він більше не відображатиметься в списку «Мої учні». Усі дані збережено. Переглянути можна в **Мої учні** → **Архів учнів**.', { parse_mode: 'Markdown' });
...
/** Картка тренера для учня або попереднього перегляду тренером. */
async function sendCoachPublicProfileCard(viewerChatId, coachId, options = {}) {
  ...
  if (preview) {
    text =
      ...
      '<i>Так бачать учні та користувачі, які шукають тренера:</i>\n\n' +
      text;
  }
...
function buildPricingTypeStatusLines(pricing) {
  ...
  if (pricingAmountIsSet(personal)) {
    lines.push('• Персональна: ' + formatStoredPriceLabel(personal, cur) + ' (за 1 учня)');
  } else {
...
async function showPricingTypeSelect(chatId, studentChatId) {
  ...
  await Helpers.sendKeyboard(
    ...
    '💰 Обери вид тренування для введення або зміни вартості:\n\n' +
      statusBlock +
      '\n\n' +
      '• Персональна — ціна за 1 учня.\n' +
      '• Спліт — загальна сума за 2 учні (у звіті з кожного — половина).\n' +
      '• Тріо — загальна сума за 3 учні (у звіті з кожного — третина).',
    keyboard
  );
}
...
function formatPricingSavedMessage(pricingType, amount, currency) {
  ...
  if (pricingType === CONSTANTS.TRAINING_TYPES.PERSONAL) {
    return '✅ Вартість персональної встановлено: ' + amount + ' ' + cur + ' (за 1 учня).';
  }
...
async function showPricingStudentSelect(chatId) {
  const students = await User.getStudentsByCoach(chatId);
  if (!students || students.length === 0) {
    await Helpers.safeSend(chatId, '📋 У тебе поки немає учнів. Додай учня: **Мої учні** → **➕ Додати учня**.', { parse_mode: 'Markdown' });
...
async function showStudentExperienceSelect(chatId, studentChatId) {
  ...
  await Helpers.sendKeyboard(chatId, '📅 Встановити рівень досвіду учня:', keyboard);
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.PRICING_TYPE_PERSONAL) {
    ...
    await Helpers.safeSend(chatId, '💰 Введіть вартість персонального тренування за 1 учня (ціле число, грн):');
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.PRICING_TYPE_SPLIT) {
    ...
    await Helpers.safeSend(
      chatId,
      '💰 Введіть загальну вартість спліт-тренування за 2 учні (ціле число, грн).\nПлатформа поділить суму на 2 — у звіті доходу з кожного учня буде половина.'
    );
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.PRICING_TYPE_TRIO) {
    ...
    await Helpers.safeSend(
      chatId,
      '💰 Введіть загальну вартість тріо-тренування за 3 учні (ціле число, грн).\nПлатформа поділить суму на 3 — у звіті доходу з кожного учня буде третина.'
    );
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACKS.COACH_ADD_STUDENT_CREATE_INVITE_NOW) {
    ...
    if (!state.inviteExperienceLevel || !state.inviteBirthDate) {
      await Helpers.safeSend(chatId, '⚠️ Спочатку вкажи рівень досвіду та дату народження учня.');
      return true;
    }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_AI_ANALYTICS && param) {
    ...
    const label = studentName ? '🤖 AI-аналітика учня ' + studentName : '🤖 AI-аналітика учня';
...
async function askInviteBodyGoalsWaist(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажану талію учня (см)\nПриклад: 65.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askInviteBodyGoalsHips(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажані ягодиці учня (см)\nПриклад: 100.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askInviteBodyGoalsShoulders(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажані плечі учня (см)\nПриклад: 100.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askInviteBodyGoalsChest(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, hint + 'Введіть бажані груди учня (см)\nПриклад: 90.0\nАбо натисніть «Пропустити»', keyboard);
}
...
async function askInviteNeck(chatId) {
  ...
  await Helpers.safeSend(chatId, '🧣 Введіть обхват шиї учня (см):\n\nДіапазон: ' + min + '–' + max + '\nПриклад: 36');
}
...
async function askInviteWrist(chatId) {
  ...
  await Helpers.safeSend(chatId, '⌚ Введіть обхват запʼястя учня (см):\n\nДіапазон: ' + min + '–' + max + '\nПриклад: 16');
}
...
/** Зберегти заміри, введені тренером з картки учня (після ланцюжка weight→height→waist→glutes→shoulders→chest→bodyFat), і повернути до картки. */
async function saveCoachEnteredMeasurementsAndReturn(chatId, state) {
  ...
  try {
    ...
    await clearStateKeepingHintsPlanFlag(chatId);
    await Helpers.safeSend(chatId, '✅ Обміри учня збережено.');
    await returnAfterStudentHealthCompletion(chatId, studentChatId);
  } catch (e) {
...
async function finishCreateStudentByInvite(chatId, state, days, conditions) {
  ...
  if (!birthDate || isNaN(birthDate.getTime())) {
    await Helpers.safeSend(chatId, '❌ Вкажи дату народження учня перед створенням інвайту.');
    return;
  }
  if (!experienceLevel) {
    await Helpers.safeSend(chatId, '❌ Вкажи рівень досвіду учня перед створенням інвайту.');
    return;
  }
  try {
    const accentZones = Array.isArray(state.inviteAccentZones) && state.inviteAccentZones.length > 0 ? state.inviteAccentZones : ['full'];
    ...
    // Аналітика для тренера; зберігаємо в БД для учня (inviteCode) — 4-й аргумент saveForChatId
    ...
    if (hasBodyGoals) {
      ...
      // Будуємо поточні заміри зі state (measurements_history порожня — учень щойно створений)
    ...
    let doneMsg =
      '✅ Учня створено!\n\n' +
      'Нижче — **окреме повідомлення** лише з інвайт-кодом. ПЕРЕШЛИ його учню.\n\n' +
      'Учень у боті: **🎟️ У мене є код** → ввести код. Профіль прив’яжеться до тебе автоматично. Вартість — за обраним видом тренування.';
    if (conditions.length > 0) {
