...
function formatShortMeasurementValue(m, field) {
  if (!m) return null;
  const v = m[field];
  if (v == null || v === '') return null;
  // bodyFatPct зберігається як відсоток (без % в БД зазвичай)
...
function buildCoachMeasurementsPickerKeyboard(studentChatId, latest = null, fallback = null) {
  ...
  // пріоритет: останні обміри -> fallback з профілю
  const m = latest || fallback || null;
  const withVal = (baseText, field) => {
    const sv = formatShortMeasurementValue(m, field);
