 ...
 * Три точки входу: клієнт (своя), тренер (своя), тренер (за клієнта).
 */
...
async function showHistoryMenu(chatId, targetChatId, origin) {
  ...
  let title = '📊 Історія моїх тренувань';
  let keyboard = [
  ...
  if (origin === 'coach_student') {
    ...
    const name = user ? [(user.firstName || user.first_name || '').trim(), (user.lastName || user.last_name || '').trim()].filter(Boolean).join(' ') : 'Клієнт';
    ...
    if (origin === 'coach_own') {
      keyboard.push([{ text: '📈 Кількість тренувань', callback_data: CONSTANTS.CALLBACKS.REPORTS_TRAININGS }]);
    }
    keyboard.push(
      [{ text: (CONSTANTS.EMOJI && CONSTANTS.EMOJI.HOME ? CONSTANTS.EMOJI.HOME : '🏠') + ' Головне меню', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]
    );
...
/** Список вправ клієнта по вибраній групі (для фільтру «За вправою»). */
