...
function groupRowsByExercise(rows) {
  ...
  for (const row of rows) {
    ...
    if (!exerciseMap.has(exId)) {
      exerciseMap.set(exId, { name: row.exercise_name || 'Вправа', groupLevel2: gl2, sets: [] });
    }
...
function formatDetailText(dateStr, rows, groups) {
  ...
  for (const [, exercises] of Object.entries(groups)) {
    for (const ex of exercises) {
      ...
      for (const s of ex.sets) {
        text += `   Підхід ${s.setNum}: ${s.weight != null ? s.weight : '—'}кг × ${s.reps != null ? s.reps : '—'} повторів\n`;
        totalSets++;
      }
      text += '\n';
      exerciseNum++;
...
async function showHistoryMenu(chatId, targetChatId, origin) {
  ...
  let backCb = CONSTANTS.CALLBACK_PREFIXES.HIST_BACK_MENU;
  if (origin === 'coach_student') {
    ...
    backCb = CONSTANTS.CALLBACK_PREFIXES.HIST_BACK_STUDENT;
  }
  const keyboard = [
...
async function showHistoryDetail(chatId) {
  ...
  const text = formatDetailText(dateStr, rows, groups);
  const keyboard = [];
  const nav = [];
