...
async function showSecondLevelGroups(chatId, topLevel) {
  ...
  if (groups.length === 0) {
    await Helpers.safeSend(chatId, '❌ Групу не знайдено.');
    await askMuscleGroup(chatId);
    return;
  }
  const state = await State.get(chatId);
  await State.set(chatId, { ...state, step: CONSTANTS.FSM_STATES.TRAINING_GROUP });
  const keyboard = groups.map((g) => [{ text: g, callback_data: CONSTANTS.CALLBACK_PREFIXES.GROUP + ':' + g }]);
  keyboard.push([{ text: '🔎 Ввести назву', callback_data: CONSTANTS.CALLBACKS.TRAINING_SEARCH_NAME }]);
...
async function showGroupOrExercises(chatId, groupLevel1) {
  const subgroups = await supabase.getSubgroups(groupLevel1);
  if (subgroups && subgroups.length > 0) {
    const state = await State.get(chatId);
    await State.set(chatId, { ...state, step: 'training_exercise', selectedGroup: groupLevel1 });
    const keyboard = subgroups.map((sub) => [
      { text: sub, callback_data: CONSTANTS.CALLBACK_PREFIXES.GROUP2 + ':' + groupLevel1 + ':' + sub }
    ]);
    keyboard.push([{ text: '📋 Всі вправи', callback_data: CONSTANTS.CALLBACK_PREFIXES.GROUP2 + ':' + groupLevel1 + ':__all__' }]);
    keyboard.push([{ text: '🔎 Ввести назву', callback_data: CONSTANTS.CALLBACKS.TRAINING_SEARCH_NAME }]);
    keyboard.push([{ text: CONSTANTS.EMOJI.BACK + " До груп", callback_data: CONSTANTS.CALLBACKS.TRAINING_BACK_TO_GROUP }]);
    await Helpers.sendKeyboard(chatId, '📋 ' + groupLevel1 + '\n\nОбери підкатегорію або вправу:', keyboard);
    return;
  }
  await showExercises(chatId, groupLevel1, null);
}

async function showExercises(chatId, groupLevel1, groupLevel2) {
  const state = await State.get(chatId);
  await State.set(chatId, { ...state, step: 'training_exercise', selectedGroup: groupLevel1 });
  const groupLevel2Arg = groupLevel2 === '__all__' || groupLevel2 === '' ? null : groupLevel2;
  const exercises = await supabase.getExercisesByGroup(groupLevel1, groupLevel2Arg);
  if (!exercises || exercises.length === 0) {
    await Helpers.safeSend(chatId, '❌ У групі "' + groupLevel1 + '" немає вправ.');
    await askMuscleGroup(chatId);
    return;
  }
  const header = groupLevel2Arg ? groupLevel1 + ' → ' + groupLevel2Arg : groupLevel1;
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.GROUP2 && rest) {
    ...
    const level2 = parts.slice(2).join(':').trim() || null;
