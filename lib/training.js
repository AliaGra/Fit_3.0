...
/** Поточний тиждень прогресивного плану (ТЗ 7.1). */
function getCurrentPlanWeek(activatedAt, totalWeeks) {
  if (!activatedAt || !totalWeeks) return 1;
...
async function applyProgressionAfterWorkout(chatId, planId, dayNumber, exercises, planMeta, currentWeek) {
  ...
  try {
    const medList = await supabase.getActiveMedicalConditions(chatId);
    hasModerateSeverity = (medList || []).some((m) => {
      const s = (m.severity || '').toLowerCase();
      return s.indexOf('moderate') >= 0 || s === 'moderate-severe';
    });
  } catch (_) {}
  ...
  for (const ex of exercises) {
    ...
    if (allMet) {
      if ((ex.medicalStatus || '').toUpperCase() === 'ALLOWED_WITH_MOD') continue;
      ...
      let newTarget = maxWeight + step;
      if (hasModerateSeverity) {
        const current = ex.targetWeight != null && ex.targetWeight > 0 ? ex.targetWeight : maxWeight;
        newTarget = Math.min(newTarget, current * 1.5);
      }
      if (ex.planExerciseId) {
        await supabase.updatePlanExerciseTargetWeight(ex.planExerciseId, newTarget);
      } else if (planMeta && (planMeta.generationMode || '').toLowerCase() === 'progressive' && currentWeek != null) {
        await supabase.updateProgressivePlanExerciseWeight(planId, ex.exerciseId, newTarget, currentWeek);
      }
      continue;
    }
    ...
    if (!prevMet && prevDates.length > 0) {
      ...
      if (baseForReduce != null && baseForReduce > 0) {
      ...
      if (coachChatId) {
        // VETO 6: без parse_mode для динамічного тексту (ім'я, назва вправи)
        await Helpers.safeSend(
          coachChatId,
          '⚠️ Деавтоматизація\n\nКлієнт ' + studentName + ' двічі поспіль не виконав мінімум підходів у вправі «' + (ex.exerciseName || 'вправа') + '». Рекомендовану вагу зменшено на 10%. Рекомендуй переглянути план.'
        );
      }
    }
  }
...
/** Підказка з минулими результатами перед першим підходом вправи. */
async function buildExerciseHint(chatId, exerciseId) {
  ...
  if (lastWorkout && lastWorkout.rows && lastWorkout.rows.length > 0) {
    const dateStr = formatHintDate(lastWorkout.date);
    const setsText = lastWorkout.rows
      .map((r) => `${r.weight != null ? r.weight : 0}кг x ${r.reps != null ? r.reps : 0}`)
      .join(', ');
    hint += `Минулого разу (${dateStr}): ${setsText}`;
  }

  if (bestSet && bestSet.weight != null) {
    const lastMaxWeight = lastWorkout && lastWorkout.rows && lastWorkout.rows.length
      ? Math.max(...lastWorkout.rows.map((r) => (r.weight != null ? r.weight : 0)))
      : 0;
    if (bestSet.weight > lastMaxWeight) {
      if (hint) hint += '\n';
      hint += `Рекорд: ${bestSet.weight}кг x ${bestSet.reps != null ? bestSet.reps : 0}`;
    }
  }
...
// ——— ENTRY: Тренування клієнтів (меню) ———
async function startCoachTrainingFlow(chatId) {
  ...
  if (list.length === 0) {
    await Helpers.safeSend(chatId, '👥 У тебе немає клієнтів. Додай клієнта в «Мої клієнти».');
    await Menu.show(chatId);
    return;
  }
  ...
  await showCoachTrainingInitialScreen(chatId);
}

/** Початковий екран: оберіть клієнта (персональна) або Спліт/Тріо. Якщо клієнт з тарифом «Персональна» — пропускаємо вибір типу. */
async function showCoachTrainingInitialScreen(chatId) {
  ...
  const keyboard = ids.map((id, i) => [
    { text: (displayNames[i] || id).slice(0, 50), callback_data: CONSTANTS.CALLBACKS.TRAINING_PICK_STUDENT + ':' + id }
  ]);
  if (ids.length >= 2) {
    keyboard.push([{ text: 'Спліт (2 клієнти)', callback_data: CONSTANTS.CALLBACKS.TRAINING_COACH_TYPE_SPLIT }]);
    if (ids.length >= 3) {
      keyboard.push([{ text: 'Тріо (3 клієнти)', callback_data: CONSTANTS.CALLBACKS.TRAINING_COACH_TYPE_TRIO }]);
    }
  }
  keyboard.push([{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]);
  await Helpers.sendKeyboard(chatId, '💪 Обери клієнта для тренування або тип групового заняття:', keyboard);
}

// ——— ENTRY: Клієнт виконує план (Почати тренування) ———
async function startStudentPlanWorkout(chatId) {
  ...
  const isProgressive = (plan.generationMode || '').toLowerCase() === 'progressive';
  let days = [];
  let byDay = {};
  if (isProgressive && plan.daysPerWeek) {
    days = Array.from({ length: plan.daysPerWeek }, (_, i) => i + 1);
    const splitConfig = plan.splitConfig || [];
    byDay = days.reduce((acc, d) => {
      acc[d] = [{ dayLabel: (splitConfig[d - 1] && splitConfig[d - 1].day_label) || ('День ' + d) }];
      return acc;
    }, {});
  } else {
    ...
    for (const ex of plan.exercises) {
      const d = Number(ex.dayNumber);
      if (!byDay[d]) byDay[d] = [];
      byDay[d].push(ex);
    }
    days = Object.keys(byDay).map(Number).sort((a, b) => a - b);
  }
  const keyboard = days.map((d) => {
    const label = byDay[d] && byDay[d][0] && byDay[d][0].dayLabel ? byDay[d][0].dayLabel : 'День ' + d;
...
async function showStudentPlanExercise(chatId) {
  ...
  if (idx >= exercises.length) {
    if (exercises.length === 0) {
      await Helpers.safeSend(chatId, '❌ Вправи для цього дня не знайдено. Обери інший день.');
      await Menu.show(chatId);
  ...
  const studentChatId = state.targetUserId || chatId;
  const currentSet = state.studentPlanCurrentSet || 1;
  let hintLine = '';
  if (currentSet === 1 && ex.exerciseId != null) {
    hintLine = await buildExerciseHint(studentChatId, ex.exerciseId);
  }
...
async function finishStudentPlanWorkout(chatId) {
  ...
  if (state?.studentPlanPlan?.planId && state.studentPlanExercises?.length) {
    await applyProgressionAfterWorkout(
      ...
      state.studentPlanPlan,
      state.studentPlanWeek
    );
    ...
    if (failedExercises.length > 0) {
      try {
        ...
        if (analysis && analysis.notify_coach && analysis.coach_message) {
          ...
          const coachChatId = student?.coachId ? String(student.coachId) : null;
          if (coachChatId) {
            const studentName = (student?.firstName || '') + ' ' + (student?.lastName || '').trim() || 'Клієнт';
            await Helpers.safeSend(coachChatId, '📋 Аналіз тренування клієнта — ' + studentName + '\n\n' + analysis.coach_message);
          }
        }
      } catch (e) {
  ...
  let closedScheduleSlotInPlan = false;
  if (isCoachPlan && state?.scheduleSlotIds?.[studentChatId]) {
    try {
      ...
      const okSlot = await supabase.updateScheduleSlotStatus(slotId, CONSTANTS.SCHEDULE_STATUS.COMPLETED);
      if (okSlot) closedScheduleSlotInPlan = true;
    } catch (e) {
      console.error('Training.finishStudentPlanWorkout slot', e.message);
    }
  }
  await State.clear(chatId);
  if (!closedScheduleSlotInPlan) {
    try {
      await supabase.recordExperienceTrainingSession(String(applyChatId), 'plan_finish:' + crypto.randomUUID());
    } catch (e) {
      console.error('Training.finishStudentPlanWorkout experience', e.message);
    }
  }
  if (isCoachPlan) {
    await Helpers.safeSend(chatId, '✅ Тренування за планом для клієнта завершено! 💪');
  } else {
    await Helpers.safeSend(chatId, '✅ Тренування за планом завершено! Гарна робота! 💪');
  }
  try {
    const Subscription = require('./subscription');
    await Subscription.notifyIfLastTrainingInSubscription(applyChatId);
  } catch (e) {
    console.error('Training.finishStudentPlanWorkout subscription notify', e.message);
  }
  await Menu.show(chatId);
}

// ——— ENTRY: З профілю клієнта (1 клієнт) ———
async function startCoachTrainingForStudent(chatId, studentChatId) {
  ...
  const hasPlan = plan && (
    ...
    ((plan.generationMode || '').toLowerCase() === 'progressive' && plan.daysPerWeek)
  );
  if (hasPlan) {
...
async function showCoachTrainingPlanChoice(chatId, studentChatId, plan) {
  ...
  if (isProgressive && plan.daysPerWeek) {
    ...
    const splitConfig = plan.splitConfig || [];
    days.forEach((d) => {
      byDay[d] = [{ dayLabel: (splitConfig[d - 1] && splitConfig[d - 1].day_label) || ('День ' + d) }];
    });
  } else {
    for (const ex of plan.exercises || []) {
      const d = Number(ex.dayNumber);
  ...
  await State.set(chatId, {
    ...
    coachPlanWorkout: undefined,
    studentPlanExercises: undefined,
    studentPlanExerciseIndex: undefined
  });
...
/** Старт вільного тренування (без плану) для одного клієнта. */
...
async function askCoachTrainingType(chatId) {
  ...
  await Helpers.sendKeyboard(
    chatId,
    '💪 Обери тип тренування:\n\n💡 Це потрібно для коректного звіту «Сума доходів».',
    keyboard
  );
}
...
async function showStudentPickKeyboard(chatId, required, alreadySelectedCount) {
  ...
  const text = required === 1
    ? '👥 Обери клієнта (натисни прізвище):'
    : '👥 Обери ' + which + ' клієнта (натисни прізвище):';
...
/** Показати вибір кількості кругів перед стартом сету (тренування без плана). */
async function showSetRoundsChoice(chatId) {
  const keyboard = [
    [{ text: '3 круги', callback_data: CONSTANTS.CALLBACKS.TRAINING_PLANNED_ROUNDS + ':3' }],
    [{ text: '4 круги', callback_data: CONSTANTS.CALLBACKS.TRAINING_PLANNED_ROUNDS + ':4' }],
    [{ text: '5 кругів', callback_data: CONSTANTS.CALLBACKS.TRAINING_PLANNED_ROUNDS + ':5' }],
    [{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: 'TRAINING_SET_BUILD_BACK' }]
  ];
  await Helpers.sendKeyboard(chatId, '🔄 Скільки кругів виконаєте?', keyboard);
}

/** Показати вибір кількості підходів для одиночної вправи (тренування без плана). */
async function showSingleSetsChoice(chatId) {
  const keyboard = [
    [{ text: '3 підходи', callback_data: CONSTANTS.CALLBACKS.TRAINING_PLANNED_SETS + ':3' }],
    [{ text: '4 підходи', callback_data: CONSTANTS.CALLBACKS.TRAINING_PLANNED_SETS + ':4' }],
    [{ text: '5 підходів', callback_data: CONSTANTS.CALLBACKS.TRAINING_PLANNED_SETS + ':5' }],
    [{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: CONSTANTS.CALLBACKS.TRAINING_BACK_TO_GROUP }]
  ];
  await Helpers.sendKeyboard(chatId, '1️⃣ Скільки підходів виконаєте?', keyboard);
}
...
async function askTrainingInputForSetCircuit(chatId) {
  ...
  const plannedRounds = state?.plannedRoundsCount;
  ...
  const roundLabel = plannedRounds ? ('Круг ' + round + ' з ' + plannedRounds) : ('Круг ' + round);
  const hintChatId = state.trainingTargetChatId || state.targetUserId || chatId;
  let hintLine = '';
  if (round === 1 && ex.id != null) {
    hintLine = await buildExerciseHint(hintChatId, ex.id);
  }
  let text = '🔄 **Сет** · Вправа ' + (idx + 1) + '/' + total + ' · ' + roundLabel + '\n\n';
  text += '💪 ' + (ex.name || 'Вправа') + '\n\n';
  if (hintLine) text += hintLine + '\n\n';
  if (targetName) text += 'Для: ' + targetName + '\n\n';
  text += 'Введи вагу і повтори через пробіл (один підхід).\nПриклад: 20 12';
  const keyboard = (plannedRounds != null && plannedRounds > 0)
    ? []
...
async function showSecondLevelGroups(chatId, topLevel) {
  ...
  await State.set(chatId, {
    ...state,
    step: CONSTANTS.FSM_STATES.TRAINING_GROUP,
    selectedGroupLevel1: topLevel,
    selectedGroupLevel2: null,
    selectedGroupLevel3: null
  });
...
async function showGroupOrExercises(chatId, topLevel, group) {
  ...
  if (subgroups && subgroups.length > 0) {
    ...
    await State.set(chatId, {
      ...
      selectedGroup: level2 || level1,
      selectedGroupLevel1: level1,
      selectedGroupLevel2: level2,
      selectedGroupLevel3: null
    });
...
async function askTargetStudentSelection(chatId, pendingAction) {
  ...
  if (!names.length) {
    await Helpers.safeSend(chatId, '⚠️ Список клієнтів порожній.');
    return;
  }
  ...
  await Helpers.sendKeyboard(chatId, '👤 Обери клієнта для запису підходу:', keyboard);
}
...
/** Запит одного підходу (одиночне виконання, без плану — з фіксованою кількістю підходів). */
...
/** Запит підходу для одиночної вправи з обраною кількістю підходів (без «Додати підхід»). Кнопку «Завершити тренування» не показуємо, поки не виконано всі підходи. */
async function askTrainingInputDataWithPlannedSets(chatId) {
  const state = await State.get(chatId);
  const planned = state?.plannedSetsCount || 3;
  ...
  const targetName = isSelf ? '' : (state?.coachSelectedNames && state?.coachSelectedIds ? state.coachSelectedNames[state.coachSelectedIds.indexOf(state.targetUserId)] || '' : '');
  let hintLine = '';
  if (currentSet === 1 && state?.exerciseId != null) {
    const hintChatId = state.trainingTargetChatId || state.targetUserId || chatId;
    hintLine = await buildExerciseHint(hintChatId, state.exerciseId);
  }
  let text = '💪 ' + exName + ' · підхід ' + currentSet + ' з ' + planned + '\n\n';
  if (isSelf) text += 'Моє тренування\n\n';
  else if (targetName) text += 'Для: ' + targetName + '\n\n';
  if (hintLine) text += hintLine + '\n\n';
  text += 'Введи вагу і повтори через пробіл.\nПриклад: 20 12';
...
async function finishWorkout(chatId) {
  ...
  const soloExperienceKey =
    !wasCoach && state?.mode === CONSTANTS.TRAINING_MODES.SELF
      ? 'free:' + String(chatId) + ':' + (state.trainingStartedAt ? String(state.trainingStartedAt) : crypto.randomUUID())
      : null;
  await State.clear(chatId);
  ...
  } else if (soloExperienceKey) {
    try {
      await supabase.recordExperienceTrainingSession(String(chatId), soloExperienceKey);
    } catch (e) {
      console.error('Training.finishWorkout experience', e.message);
    }
  }
  await Helpers.safeSend(chatId, '✅ Тренування завершено!\n\nГарна робота! 💪');
  try {
    const Subscription = require('./subscription');
    await Subscription.notifyIfLastTrainingInSubscription(chatId);
  } catch (e) {
    console.error('Training.finishWorkout subscription notify', e.message);
  }
  await Menu.show(chatId);
}
...
async function finishExercise(chatId) {
  ...
  await State.update(chatId, { trainingExecutionType: undefined });
  await askExecutionTypeThenMuscleGroup(chatId);
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.COACH_PLAN_DAY && parts[1] && parts[2]) {
    ...
    if (!plan) {
      await Helpers.safeSend(chatId, '❌ План не знайдено. Обери клієнта знову.');
      return true;
    }
    let exercises = [];
    let currentWeek = null;
    if ((plan.generationMode || '').toLowerCase() === 'progressive' && plan.planId && plan.totalWeeks) {
      currentWeek = getCurrentPlanWeek(plan.activatedAt, plan.totalWeeks);
      const weekRows = await supabase.getPlanWeekDay(plan.planId, currentWeek, dayNum);
      exercises = (weekRows || []).map((r) => ({
      ...
      exercises = (plan.exercises || []).filter((e) => Number(e.dayNumber) === Number(dayNum));
    }
    if (!exercises.length) {
  ...
  if (action === CONSTANTS.CALLBACKS.TRAINING_PICK_STUDENT && rest) {
    ...
    if (newSelectedIds.length >= required) {
      const trainingType = required === 1 ? CONSTANTS.TRAINING_TYPES.PERSONAL
        : (required === 2 ? CONSTANTS.TRAINING_TYPES.SPLIT : CONSTANTS.TRAINING_TYPES.TRIO);
      const baseState = {
        ...state,
        ...
        coachTrainingType: trainingType,
        trainingStartedAt: new Date(),
        scheduleSlotIds: {},
  ...
  if (action === CONSTANTS.CALLBACKS.TRAINING_SELECT_STUDENT && rest) {
    ...
    const execType = state?.trainingExecutionType || 'SET';
    if (execType === 'SINGLE') {
      if (state?.plannedSetsCount) {
        await askTrainingInputDataWithPlannedSets(chatId);
      } else {
  ...
  if (action.startsWith(CONSTANTS.CALLBACK_PREFIXES.EXERCISE + ':') || (action === CONSTANTS.CALLBACK_PREFIXES.EXERCISE && rest)) {
    ...
    if (!isPlan) {
      await State.update(chatId, { step: CONSTANTS.FSM_STATES.TRAINING_CHOOSE_SETS });
      await showSingleSetsChoice(chatId);
      return true;
    }
  ...
  if (action === CONSTANTS.CALLBACKS.TRAINING_PLANNED_SETS && rest) {
    const state = await State.get(chatId);
    if (state?.coachPlanWorkout || state?.mode === 'STUDENT_PLAN') return false;
    const num = parseInt(rest.trim(), 10);
    if (isNaN(num) || num < 1 || num > 20) return true;
    await State.update(chatId, { plannedSetsCount: num, step: CONSTANTS.FSM_STATES.TRAINING_INPUT_DATA, singleExecution: true });
    const st = await State.get(chatId);
    if (st?.coachSelectedIds && st.coachSelectedIds.length > 1) {
      await askTargetStudentSelection(chatId, 'ASK_TRAINING_INPUT_SINGLE');
    } else {
      await askTrainingInputDataWithPlannedSets(chatId);
    }
    return true;
  ...
  if (action === CONSTANTS.CALLBACKS.TRAINING_PLANNED_ROUNDS && rest) {
    const state = await State.get(chatId);
    if (state?.coachPlanWorkout || state?.mode === 'STUDENT_PLAN') return false;
    const num = parseInt(rest.trim(), 10);
    if (isNaN(num) || num < 1 || num > 20) return true;
    await State.update(chatId, {
      plannedRoundsCount: num,
      setCurrentRound: 1,
  ...
  if (action === 'TRAINING_SET_BUILD_BACK') {
    await showSetBuildScreen(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACKS.TRAINING_SET_GO) {
    ...
    await State.update(chatId, { step: CONSTANTS.FSM_STATES.TRAINING_CHOOSE_SETS });
    await showSetRoundsChoice(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACKS.TRAINING_SET_FINISH) {
    ...
    await State.update(chatId, {
      ...
      exerciseId: undefined,
      exerciseName: undefined,
      plannedRoundsCount: undefined,
      trainingExecutionType: undefined
    });
    await Helpers.safeSend(chatId, '✅ Сет завершено. Обери тип наступного тренування:');
    await askExecutionTypeThenMuscleGroup(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACKS.TRAINING_EXEC_TYPE_SINGLE) {
    ...
    if (state?.pendingAction === 'SEARCH') {
      ...
      await Helpers.safeSend(chatId, "🔎 Введи мінімум 3 літери для пошуку вправи (в т.ч. за своєю назвою):\n\nПриклад: жим, блок зверху");
      return true;
    }
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.STUDENT_PLAN_DAY && rest) {
    ...
    const plan = state.studentPlanPlan;
    let exercises = [];
    let currentWeek = null;
    if ((plan.generationMode || '').toLowerCase() === 'progressive' && plan.planId && plan.totalWeeks) {
      currentWeek = getCurrentPlanWeek(plan.activatedAt, plan.totalWeeks);
...
async function handleTextMessage(chatId, text) {
  ...
  if (step === CONSTANTS.FSM_STATES.TRAINING_COACH_SELECT_STUDENTS) {
    await Helpers.safeSend(chatId, '👆 Оберіть клієнта кнопкою з прізвищем вище.');
    return true;
  }
  ...
  if (step === CONSTANTS.FSM_STATES.TRAINING_COACH_SELECT_TARGET) {
    ...
    if (!numbers) {
      await Helpers.safeSend(chatId, '⚠️ Введи номер клієнта зі списку (1–' + ids.length + ').');
      return true;
    }
    const idx = numbers[0] - 1;
    await State.update(chatId, { targetUserId: ids[idx], pendingAction: undefined });
    const st = await State.get(chatId);
    if (st?.plannedSetsCount) {
      await askTrainingInputDataWithPlannedSets(chatId);
    } else {
      await askTrainingInputData(chatId);
    }
  ...
  if (step === CONSTANTS.FSM_STATES.TRAINING_CHOOSE_SETS) {
    await Helpers.safeSend(chatId, '👆 Оберіть кількість підходів або кругів кнопкою вище.');
    return true;
  }
  ...
  if (step === CONSTANTS.FSM_STATES.TRAINING_INPUT_DATA) {
    ...
    if (Array.isArray(state.setExerciseList) && state.setCurrentRound != null && state.setCurrentExerciseIndex != null) {
      ...
      const plannedRounds = state.plannedRoundsCount || 0;
      const round = state.setCurrentRound || 1;
      if (plannedRounds && round >= plannedRounds) {
        await State.update(chatId, {
          setExerciseList: undefined,
          setCurrentRound: undefined,
          setCurrentExerciseIndex: undefined,
        ...
        await Helpers.safeSend(chatId, '✅ Всі ' + plannedRounds + ' круги виконано. Сет завершено.');
        await askExecutionTypeThenMuscleGroup(chatId);
        return true;
      }
      await State.update(chatId, { setCurrentRound: round + 1, setCurrentExerciseIndex: 0 });
      await Helpers.safeSend(chatId, '✅ Круг ' + round + ' завершено. Наступний круг:');
      await askTrainingInputForSetCircuit(chatId);
      return true;
    }

    if (state.singleExecution) {
      const planned = state.plannedSetsCount || 0;
      await State.update(chatId, { currentSet: setNum + 1 });
      if (planned && setNum >= planned) {
        await Helpers.safeSend(chatId, '✅ Підхід №' + setNum + ' записано. Обрану кількість підходів виконано.');
        await finishExercise(chatId);
        return true;
      }
      if (planned) {
        await Helpers.safeSend(chatId, '✅ Підхід №' + setNum + ' записано.');
        await askTrainingInputDataWithPlannedSets(chatId);
        return true;
      }
      const kbd = [
  ...
  if (step === CONSTANTS.FSM_STATES.TRAINING_SEARCH_NAME_INPUT) {
    ...
    if (query.length < 3) {
      await Helpers.safeSend(chatId, '⚠️ Введи мінімум 3 літери (в т.ч. можна за своєю назвою).');
      return true;
    }
    const user = await User.getByChatId(chatId);
    const coachId = user && user.coach_id ? user.coach_id : null;
    let exercises = await Alias.searchExercisesWithAliases(query, chatId, coachId);
    const level1 = state?.selectedGroupLevel1;
    const level2 = state?.selectedGroupLevel2;
    const level3 = state?.selectedGroupLevel3;
    if (level1 != null && String(level1).trim() !== '') {
      const inGroup = await supabase.getExercisesByGroup(level1, level2 || null, level3 || null);
      const idsInGroup = new Set((inGroup || []).map((ex) => Number(ex.id)));
      exercises = (exercises || []).filter((ex) => idsInGroup.has(Number(ex.id)));
    }
    if (!exercises || exercises.length === 0) {
      const groupLabel = (level1 != null && String(level1).trim() !== '')
        ? ([level1, level2, level3].filter(Boolean).join(' → ') || level1)
        : '';
      const msg = groupLabel
        ? ('❌ Нічого не знайдено в групі «' + groupLabel + '». Спробуй інший запит або обери групу.')
    ...
    const keyboard = exercises.slice(0, 15).map((ex) => {
      ...
      return [{ text: label.slice(0, 60), callback_data: CONSTANTS.CALLBACK_PREFIXES.EXERCISE + ':' + ex.id }];
    });
    keyboard.push([{ text: CONSTANTS.EMOJI.BACK + " До груп", callback_data: CONSTANTS.CALLBACKS.TRAINING_BACK_TO_GROUP }]);
    const groupLabel = (level1 != null && String(level1).trim() !== '')
      ? ([level1, level2, level3].filter(Boolean).join(' → ') || level1)
      : '';
    const groupHint = groupLabel ? (' (група: ' + groupLabel + ')') : '';
    await Helpers.sendKeyboard(chatId, '🔎 Результати пошуку «' + query + '»' + groupHint + ':\n\nОбери вправу:', keyboard);
    return true;
  }
