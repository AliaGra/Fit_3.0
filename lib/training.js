...
async function showExercises(chatId, groupName) {
  const state = await State.get(chatId);
  await State.set(chatId, { ...state, step: 'training_exercise', selectedGroup: groupName });
  const exercises = await supabase.getExercisesByGroup(groupName);
  if (!exercises || exercises.length === 0) {
    await Helpers.safeSend(chatId, '❌ У групі "' + groupName + '" немає вправ.');
    await askMuscleGroup(chatId);
    return;
  }
  ...
  await Helpers.sendKeyboard(chatId, '📋 ' + groupName + '\n\nОбери вправу:', keyboard);
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action.startsWith(CONSTANTS.CALLBACK_PREFIXES.GROUP + ':') || (action === CONSTANTS.CALLBACK_PREFIXES.GROUP && rest)) {
    ...
    await showExercises(chatId, groupName);
    return true;
  }
