...
/** Початковий екран: оберіть учня (персональна) або Спліт/Тріо. Якщо учень з тарифом «Персональна» — пропускаємо вибір типу. */
async function showCoachTrainingInitialScreen(chatId) {
  ...
  if (ids.length >= 2) {
    keyboard.push([{ text: 'Спліт (2 учні)', callback_data: CONSTANTS.CALLBACKS.TRAINING_COACH_TYPE_SPLIT }]);
    if (ids.length >= 3) {
      keyboard.push([{ text: 'Тріо (3 учні)', callback_data: CONSTANTS.CALLBACKS.TRAINING_COACH_TYPE_TRIO }]);
    }
  }
  keyboard.push([{ text: CONSTANTS.EMOJI.BACK + ' Назад', callback_data: CONSTANTS.CALLBACKS.BACK_TO_MAIN }]);
  await Helpers.sendKeyboard(chatId, '💪 Обери учня для тренування або тип групового заняття:', keyboard);
}
