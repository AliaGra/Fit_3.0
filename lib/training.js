...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.TRAINING_FINISH_EXERCISE) {
    ...
    if (Array.isArray(state?.setExerciseList) && state.setExerciseIndex != null) {
      const list = state.setExerciseList;
      const nextIdx = state.setExerciseIndex + 1;
      if (nextIdx < list.length) {
        await State.update(chatId, {
          setExerciseIndex: nextIdx,
          exerciseId: list[nextIdx].id,
          exerciseName: list[nextIdx].name,
          currentSet: 1
        });
        const st = await State.get(chatId);
        if (st?.coachSelectedIds && st.coachSelectedIds.length > 1) {
      ...
      await State.update(chatId, { setExerciseList: undefined, setExerciseIndex: undefined, exerciseId: undefined, exerciseName: undefined });
