...
async function operatorGenerateInvite(opChatId, requestId) {
  ...
  // Auto-finish user dialog after providing invite code
  await supabase.supportCloseRequest(String(requestId), { closedBy: 'operator', operatorChatId: String(opChatId) });
  await supabase.setStateRow(String(r.user_chat_id), { step: null, help_request_id: null, help_topic: null, updated_at: nowIso() });
  await showMenu(String(r.user_chat_id));
