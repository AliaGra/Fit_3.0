...
async function showMenu(chatId) {
  const keyboard = [
    [{ text: '🎟️ Попросити інвайт-код', callback_data: `${CALLBACKS.TOPIC}:${TOPICS.INVITE}` }],
  ...
  if (isOp) {
    keyboard.push([{ text: '🛠 Панель оператора', callback_data: CALLBACKS.OP_PANEL }]);
  }
  const text =
...
async function startTopic(chatId, topic) {
  ...
  if (t === TOPICS.INVITE) {
    intro += '\nПідказка: щоб отримати інвайт, напиши коротко хто ти і навіщо потрібен доступ.';
  } else if (t === TOPICS.DELETE_DATA) {
...
async function userAppendMessage(chatId, text) {
  const state = await supabase.getStateRow(String(chatId));
...
async function operatorSendReply(opChatId, text) {
  if (!HelpHelpers.isHelpOperator(opChatId)) return;
  const st = await supabase.getStateRow(String(opChatId));
...
async function operatorGenerateInvite(opChatId, requestId) {
  ...
  await HelpTelegram.sendMessage(String(r.user_chat_id), '🎟️ Твій інвайт-код:\n\n' + code + '\n\nПовернись у платформу FIT 3.0 і введи його у реєстрації.');
...
async function handleText(chatId, text) {
  ...
  if (HelpHelpers.isHelpOperator(chatId)) {
    const st = await supabase.getStateRow(String(chatId));
