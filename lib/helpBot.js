...
const HelpInviteIntake = require('./helpInviteIntake');
...
async function showMenu(chatId) {
  ...
  const text =
    ...
    'Обери тему звернення. Після вибору ти зможеш написати повідомлення(я), а оператор відповість у цьому чаті.';
...
function mainBotUsername() {
  return String(process.env.TELEGRAM_BOT_USERNAME || process.env.MAIN_BOT_USERNAME || process.env.BOT_USERNAME || '')
    .replace(/^@/, '')
    .trim();
}

function mainBotStartLink() {
  const uname = mainBotUsername();
  if (!uname) return '';
  return `https://t.me/${uname}?start`;
}
...
async function startTopic(chatId, topic) {
  ...
  if (t === TOPICS.INVITE) {
    await HelpInviteIntake.startInviteIntake(chatId, requestId);
  } else {
    ...
    await HelpTelegram.sendMessage(chatId, intro);
  }
...
async function userAppendMessage(chatId, text) {
  ...
  const topic = state && state.help_topic ? String(state.help_topic) : '';
  if (!requestId) {
    await showMenu(chatId);
    return;
  }
  ...
  if (topic === TOPICS.INVITE) {
    await HelpTelegram.sendMessage(chatId, '✅ Прийнято.\n\nОчікуй відповідь оператора.');
  } else {
    await HelpTelegram.sendMessage(chatId, '✅ Прийнято.\n\nМожеш написати ще повідомлення.');
  }
...
async function showRequestCard(opChatId, requestId) {
  ...
  let prefillLine = '';
  if (r.topic === TOPICS.INVITE) {
    const tech = r.tech_json && typeof r.tech_json === 'object' ? r.tech_json : {};
    const pf = tech.invite_prefill;
...
async function operatorGenerateInvite(opChatId, requestId) {
  ...
  if (!r) return showOperatorPanel(opChatId);
  const tech = r.tech_json && typeof r.tech_json === 'object' ? r.tech_json : {};
  const prefill = tech.invite_prefill && typeof tech.invite_prefill === 'object' ? tech.invite_prefill : null;
  const result = await supabase.supportGenerateUniversalInvite({
    operatorChatId: String(opChatId),
    prefill
  });
  const code = result && result.code ? result.code : null;
  if (!code) {
    const tech = result && result.error ? `\n\nТехнічно: ${result.error}` : '';
    await HelpTelegram.sendMessage(
      opChatId,
      '❌ Не вдалося згенерувати інвайт.' + tech + '\n\nПеревір логи Railway та міграції Supabase (users).'
    );
    return;
  }
  ...
  // Auto-close invite thread for user after code is delivered.
  await supabase.supportCloseRequest(String(requestId), { closedBy: 'operator', operatorChatId: String(opChatId) });
  await supabase.setStateRow(String(r.user_chat_id), {
    step: null,
  ...
  await HelpTelegram.sendKeyboard(
    ...
    '✅ Інвайт-код надіслано.\n\nСкопіюй інвайт-код, повернись через кнопку «🏠 Відкрити FIT 3.0» на платформу, натисни «🎟️ У мене є інвайт код» і введи скопійований код.',
    (() => {
      const mainLink = mainBotStartLink();
      return [
        [
          mainLink
            ? { text: '🏠 Відкрити FIT 3.0', url: mainLink }
            : { text: '🏠 Меню підтримки', callback_data: CALLBACKS.MENU }
        ]
      ];
    })()
  );
...
async function handleText(chatId, text) {
  ...
  const st = await getBotStateData(chatId);
  if (st && HelpInviteIntake.isInviteIntakeStep(st.step)) {
    await HelpInviteIntake.handleInviteIntakeText(chatId, t, st);
    return;
  }
...
async function handleCallback(chatId, callbackData, callbackQueryId) {
  ...
  const param = String(callbackData).slice(action.length + 1);
  const st = await getBotStateData(chatId);
  if (st && (await HelpInviteIntake.handleInviteIntakeCallback(chatId, action, param || p1, st))) return;
