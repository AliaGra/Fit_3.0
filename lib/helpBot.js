...
const { parseInviteRequestText } = require('./invitePrefillParse');
...
async function startTopic(chatId, topic) {
  ...
  if (t === TOPICS.INVITE) {
    intro =
      'Одним повідомленням вкажи: імʼя, прізвище та місто.\n\n' +
      'Приклад: Іван Коваль Київ';
  } else {
    intro =
...
async function userAppendMessage(chatId, text) {
  ...
  if (topic === TOPICS.INVITE) {
    const existingReq = await supabase.supportGetRequestById(requestId);
    const threadBefore = Array.isArray(existingReq?.thread_json) ? existingReq.thread_json : [];
    const isFirstUserMsg = !threadBefore.some((m) => m && m.from === 'user');
    if (isFirstUserMsg) {
      const parsed = parseInviteRequestText(clean);
      if (parsed) {
        await supabase.supportMergeTechJson(requestId, { invite_prefill: parsed });
      } else {
        await HelpTelegram.sendMessage(
          chatId,
          '⚠️ Краще одним рядком: Імʼя Прізвище Місто\n\nПриклад: Іван Коваль Київ\n\nПовідомлення збережено — оператор все одно побачить текст.'
        );
      }
    }
  }
...
async function operatorGenerateInvite(opChatId, requestId) {
  ...
  const code = await supabase.supportGenerateUniversalInvite({
