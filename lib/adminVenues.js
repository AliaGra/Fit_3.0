...
const CB = Object.freeze({
  ...
  /** Зняти власника: ADM_VOWN_RM:<venueId>:<managerChatId> */
  OWNER_REMOVE: 'ADM_VOWN_RM',
  /** Підтвердити зняття власника: ADM_VOWN_OK:<venueId>:<managerChatId> */
  OWNER_REMOVE_OK: 'ADM_VOWN_OK',
...
async function askRemoveVenueOwnerConfirm(chatId, venueId, managerChatId) {
  const vid = String(venueId || '').trim();
  const mid = String(managerChatId || '').trim();
  const v = await supabase.getVenueById(vid);
  const u = await supabase.getUserByChatId(mid);
  if (!v) {
    await AdminTelegram.sendMessage(chatId, '❌ Заклад не знайдено.');
    return listVenues(chatId);
  }
  const name = u ? [u.firstName, u.lastName].filter(Boolean).join(' ').trim() || '(без імені)' : '(користувача не знайдено)';
  const text =
    '⚠️ <b>Зняти власника з закладу?</b>\n\n' +
    `👤 ${escHtml(name)} · <code>${escHtml(mid)}</code>\n` +
    `🏢 ${escHtml(v.nameUa || 'Заклад')}\n\n` +
    '<i>Заклад залишиться в системі. Користувач втратить доступ «Мій заклад».</i>';
  const keyboard = [
    [{ text: '✅ Так, зняти', callback_data: `${CB.OWNER_REMOVE_OK}:${vid}:${mid}` }],
    [{ text: '❌ Ні, назад', callback_data: `${CB.VIEW}:${vid}` }]
  ];
  await AdminTelegram.sendKeyboard(chatId, text, keyboard, { parse_mode: 'HTML' });
  return true;
}

async function execRemoveVenueOwner(adminChatId, venueId, managerChatId) {
  const vid = String(venueId || '').trim();
  const mid = String(managerChatId || '').trim();
  const res = await supabase.adminUnassignVenueOwner(mid, vid);
  if (!res.ok) {
    await AdminTelegram.sendMessage(adminChatId, '❌ Не вдалося зняти власника.');
    return showVenueDetails(adminChatId, vid);
  }
  await supabase.adminInsertLog({
    adminChatId: String(adminChatId),
    action: 'venue_owner_remove',
    targetUserChatId: mid,
    payloadJson: { venueId: vid, roleDemoted: !!res.roleDemoted }
  });
  const extra = res.roleDemoted ? ' Роль користувача змінено на <b>учень</b>.' : '';
  await AdminTelegram.sendMessage(
    adminChatId,
    `✅ Власника знято з закладу. Заклад не видалено.${extra}`,
    { parse_mode: 'HTML' }
  );
  return showVenueDetails(adminChatId, vid);
}
...
async function showVenueDetails(chatId, venueId) {
  ...
  const owners = await supabase.listVenueManagers(vid, 'owner');
  ...
  const text =
    ...
    (owners.length
      ? (
          await Promise.all(
            owners.map(async (o) => {
              const ou = await supabase.getUserByChatId(o.managerChatId);
              const full = ou
                ? [ou.firstName, ou.lastName].filter(Boolean).join(' ').trim() || '(без імені)'
                : '(користувача не знайдено)';
              return `• ${escHtml(full)} · <code>${escHtml(o.managerChatId)}</code>`;
            })
          )
        ).join('\n') + '\n\n'
      : '—\n\n') +
  ...
  const keyboard = [
    ...
    ...owners.slice(0, 5).map((o) => {
      const label = `👑 Зняти власника (${o.managerChatId})`;
      return [
        {
          text: label.length > 64 ? label.slice(0, 61) + '…' : label,
          callback_data: `${CB.OWNER_REMOVE}:${vid}:${o.managerChatId}`
        }
      ];
    }),
  ...
  if (action === CB.OWNER_ASSIGN) {
    ...
    await AdminTelegram.sendMessage(
      ...
      '👑 Надішли <b>chat_id</b> користувача, якого потрібно призначити власником цього закладу.\n\n' +
        '<i>Користувач має бути зареєстрований у основному боті (зʼявляється в «Користувачі»).</i>',
      { parse_mode: 'HTML' }
    );
    return true;
  }
  if (action === CB.OWNER_REMOVE) {
    const parts = String(param || '').split(':');
    const vid = String(parts[0] || '').trim();
    const managerChatId = String(parts[1] || '').trim();
    if (!vid || !managerChatId) return true;
    return askRemoveVenueOwnerConfirm(chatId, vid, managerChatId);
  }
  if (action === CB.OWNER_REMOVE_OK) {
    const parts = String(param || '').split(':');
    const vid = String(parts[0] || '').trim();
    const managerChatId = String(parts[1] || '').trim();
    if (!vid || !managerChatId) return true;
    return execRemoveVenueOwner(chatId, vid, managerChatId);
  }
...
async function handleText(chatId, text) {
  ...
  if (d.step === STEPS_EXTRA.OWNER_ASSIGN_INPUT) {
    ...
    const prevOwners = await supabase.listVenueManagers(vid, 'owner');
    for (const po of prevOwners) {
      if (po.managerChatId && po.managerChatId !== ownerChatId) {
        await supabase.adminUnassignVenueOwner(po.managerChatId, vid);
      }
    }
