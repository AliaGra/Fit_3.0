...
async function showVenuePricesMenu(chatId, d) {
  ...
  const keyboard = [
    [{ text: '🏷 Групові заняття', callback_data: CB.PRICE_G }],
    [{ text: '🏋 Абонементи в зал', callback_data: CB.PRICE_M }],
    [{ text: '🧾 Інші послуги', callback_data: CB.PRICE_A }],
...
async function showGroupClassPricesEditor(chatId, d) {
  ...
  const lines = rows.length
    ...
    : '—';
  const keyboard = [];
  keyboard.push([{ text: '➕ Додати / оновити ціну', callback_data: CB.PRICE_GC_ADD }]);
  for (const r of rows.slice(0, 20)) {
    const lab = r.labelUa || gcMap.get(r.groupClassCode) || r.groupClassCode;
    const short = (`🗑 ${lab}`).length > 60 ? (`🗑 ${lab}`).slice(0, 57) + '…' : `🗑 ${lab}`;
    keyboard.push([{ text: short, callback_data: `${CB.PRICE_GC_DEL}:${r.id}` }]);
  }
  keyboard.push([{ text: '⬅️ До цін закладу', callback_data: CB.PRICES_MENU }]);
  await AdminTelegram.sendKeyboard(
    chatId,
...
async function showMembershipPricesEditor(chatId, d) {
  ...
  const lines = rows.length
    ? rows
        ...
        .join('\n')
    : '—';
  const keyboard = [[{ text: '➕ Додати абонемент', callback_data: CB.PRICE_M_ADD }]];
  for (const r of rows.slice(0, 20)) {
...
async function showAncillaryPricesEditor(chatId, d) {
  ...
  const keyboard = [
    [
      { text: '🧴 Рушник', callback_data: `${CB.PRICE_A_PRE}:towel_rental` },
      { text: '🧊 Заморозка', callback_data: `${CB.PRICE_A_PRE}:membership_freeze` }
    ],
    [{ text: '➕ Інша послуга (свій код)', callback_data: CB.PRICE_A_ADD }]
  ];
  for (const r of rows.slice(0, 20)) {
    const short = (`🗑 ${r.labelUa}`).length > 60 ? (`🗑 ${r.labelUa}`).slice(0, 57) + '…' : `🗑 ${r.labelUa}`;
...
async function showVenueDetails(chatId, venueId) {
  ...
  const keyboard = [
    ...
    [{ text: '💰 Ціни (довідник)', callback_data: CB.PRICES_MENU }],
...
async function handleCallback(chatId, data) {
  ...
  if (action === CB.PRICE_GC_ADD) {
    ...
    keyboard.push([{ text: '⬅️ Назад', callback_data: CB.PRICE_G }]);
  ...
  if (action === CB.PRICE_GC_DEL) {
    ...
    if (!id) return true;
    await supabase.deleteVenueGroupClassPrice(d.viewVenueId, id);
    await showGroupClassPricesEditor(chatId, d);
    return true;
  }
  ...
  if (action === CB.PRICE_M_DEL) {
    ...
    const id = String(param || '').trim();
    if (!id) return true;
    await supabase.deleteVenueGymMembershipOffer(d.viewVenueId, id);
    await showMembershipPricesEditor(chatId, d);
    return true;
  }
  ...
  if (action === CB.PRICE_A_DEL) {
    ...
    const id = String(param || '').trim();
    if (!id) return true;
    await supabase.deleteVenueAncillaryService(d.viewVenueId, id);
    await showAncillaryPricesEditor(chatId, d);
    return true;
  }
