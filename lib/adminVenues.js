...
async function showScheduleEditor(chatId, d) {
  ...
  const keyboard = [];
  keyboard.push([{ text: '➕ Додати заняття', callback_data: CB.SCHED_ADD }]);
  for (const x of (sched || []).slice(0, 10)) {
    const label = `${WD[x.weekday] || x.weekday} ${x.timeStart || ''} ${x.title || x.groupClassCode || ''}`.trim();
    keyboard.push([
      {
...
async function handleText(chatId, text) {
  ...
  if (d.step === STEPS_EXTRA.SCHED_TIME_INPUT) {
    ...
    await supabase.addVenueScheduleItem({
      ...
      groupClassCode: String(d.tmpSchedule.groupClassCode)
    });
    d.step = STEPS_EXTRA.VENUE_VIEW;
    d.tmpSchedule = null;
    await showScheduleEditor(chatId, d);
