...
const STATUS = Object.freeze({
  ...
  MENOPAUSE: 'menopause',
  MENOPAUSE_CONFIRMED: 'menopause_confirmed',
...
const CYCLE_LENGTH_MAX = 99;

function parseCycleLengthDays(text) {
  const raw = String(text || '').trim();
  if (!/^\d+$/.test(raw)) return { ok: false, value: null };
  const n = parseInt(raw, 10);
  if (n < CYCLE_LENGTH_MIN || n > CYCLE_LENGTH_MAX) return { ok: false, value: null };
  return { ok: true, value: n };
}

function cycleLengthRangeHintUa() {
  return CYCLE_LENGTH_MIN + '–' + CYCLE_LENGTH_MAX + ' днів';
}
...
function dayInCycle(lastStart, now, cycleLen) {
  const L = Math.max(CYCLE_LENGTH_MIN, Math.min(CYCLE_LENGTH_MAX, cycleLen || DEFAULT_CYCLE_LEN));
...
function calcSymptomLoad(symptomLog) {
  if (!symptomLog) return { score: 0, level: 'none' };
  const vals = [
    symptomLog.hotFlashes,
    symptomLog.sleepQuality,
    symptomLog.fatigue,
    symptomLog.jointPain,
    symptomLog.moodStress,
    symptomLog.recoveryScore
  ].map((x) => (x != null && isFinite(Number(x)) ? Number(x) : 0));
  const score = vals.reduce((s, v) => s + v, 0);
  const level = score >= 9 ? 'high' : (score >= 5 ? 'moderate' : (score >= 1 ? 'low' : 'none'));
  return { score, level };
}

function applySymptomOverlay(modifiers, symptomLoad) {
  const base = { ...(modifiers || {}) };
  const load = symptomLoad || { score: 0, level: 'none' };
  if (load.level === 'high') {
    base.setsMultiplier = Math.min(base.setsMultiplier != null ? base.setsMultiplier : 1, 0.8);
    base.excludeInversion = true;
    base.excludeHighImpact = true;
    base.hint = (base.hint ? base.hint + '\n' : '') + 'Симптоми виражені: тимчасово знижуємо обсяг, без high-impact.';
  } else if (load.level === 'moderate') {
    base.setsMultiplier = Math.min(base.setsMultiplier != null ? base.setsMultiplier : 1, 0.9);
    base.excludeHighImpact = !!base.excludeHighImpact;
    base.hint = (base.hint ? base.hint + '\n' : '') + 'Є симптомне навантаження: помірно зменшуємо обсяг.';
  }
  return base;
}
...
function resolveTrainingContext(settings, logs, now = new Date()) {
  ...
  if (
    g === STATUS.MENOPAUSE ||
    g === STATUS.MENOPAUSE_CONFIRMED ||
    g === STATUS.POSTMENOPAUSE ||
    g === 'menopause' ||
    g === 'menopause_confirmed' ||
    g === 'postmenopause'
  ) {
...
module.exports = {
  ...
  CYCLE_LENGTH_MIN,
  CYCLE_LENGTH_MAX,
  parseCycleLengthDays,
  cycleLengthRangeHintUa,
