...
async function showInviteVenuesPicker(chatId, page = 0) {
  ...
  for (const v of slice) {
    ...
    const label = `${mark}${v.nameUa || 'Заклад'}${v.city ? ' — ' + v.city : ''}`;
    keyboard.push([{ text: label.length > 64 ? label.slice(0, 61) + '…' : label, callback_data: CONSTANTS.CALLBACK_PREFIXES.REG_VENUES_TOGGLE + ':' + id }]);
  }
  if (pages > 1) {
    const nav = [];
...
async function handleTextMessage(chatId, text) {
  ...
  if (step === CONSTANTS.FSM_STATES.REG_BODY_FAT) {
    ...
    const st2 = await State.get(chatId);
    if (isInviteStudentOnboarding(st2)) await askRegBodyGoalsChoice(chatId);
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_MEAS_BODY_FAT_SKIP) {
    ...
    if (isInviteStudentOnboarding(state)) await askRegBodyGoalsChoice(chatId);
    else await askRegActivityJob(chatId);
    return true;
  }
