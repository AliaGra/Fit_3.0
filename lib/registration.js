...
async function buildGoalRangeLine(chatId, goalField) {
  ...
  if (goalField === 'goal_waist' && model.waist) {
    return `Орієнтирна вилка для талії: ${model.waist.min}–${model.waist.max} см.\n`;
  }
  if (goalField === 'goal_hips' && model.hips) {
    return `Орієнтирна вилка для ягідниць: ${model.hips.min}–${model.hips.max} см.\n`;
  }
  if (goalField === 'goal_shoulders' && model.shoulders) {
    return `Орієнтирна вилка для плечей: ${model.shoulders.min}–${model.shoulders.max} см.\n`;
  }
