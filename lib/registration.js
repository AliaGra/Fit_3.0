...
function buildRegAiIntroHintsText(role) {
  if (role === CONSTANTS.ROLES.COACH) {
    return (
      ...
      '1. **Розклад.** Головне меню → **📅 Розклад** → **⚙️ Налаштування розкладу**.\n' +
      '2. **Вільні слоти.** **📅 Розклад** → **🕒 Слоти — праця з ними** → **📆 Створити слоти**.\n' +
      '3. **Перший учень.** **👥 Мої учні** — сформуй інвайт-код і передай учню для входу та прив’язки до тебе.\n' +
      '4. **Перенесення.** Учень у Telegram обирає вільний слот — ти отримаєш запит; після підтвердження час зміниться в обох.\n\n'
    );
  }
