...
/** Підказки перед «Продовжити / Заповнити пізніше» (залежить від обраної ролі). */
function buildRegAiIntroHintsText(role) {
  if (role === CONSTANTS.ROLES.COACH) {
    const home = CONSTANTS.EMOJI.HOME;
    return (
      '🚀 **Що зробити в першу чергу**\n\n' +
      '1. **Налаштування розкладу.** Налаштуй розклад своєї робочої тижні: ' +
      home + ' **Головне меню** → **📅 Розклад** → **⚙️ Налаштування розкладу**.\n' +
      '2. **Вільні слоти.** Створи вільні слоти («вікна») для запису часу тренувань: ' +
      home + ' **Головне меню** → **📅 Розклад** → **🕒 Слоти — праця з ними** → **📆 Створити слоти**.\n' +
      '3. **Учні.** Додай учнів на платформу: ' +
      home + ' **Головне меню** → **👥 Мої учні** — сформуй інвайт-код і передай учню для входу та прив’язки до тебе.\n' +
      '4. **Перенесення.** Поясни учню, що за потреби перенести тренування йому більше не потрібно телефонувати чи писати тобі, намагаючись узгодити твоє і своє вільний час.\n\n'
    );
  }
  // Учень: текст підказок доповнимо окремо; поки — коротко про наступний крок анкети.
  return (
...
async function handleTextMessage(chatId, text) {
  ...
  if (step === CONSTANTS.FSM_STATES.REG_LAST_NAME) {
    ...
    if (isInviteOnboarding(st2)) await askInviteLocationIfNeeded(chatId);
