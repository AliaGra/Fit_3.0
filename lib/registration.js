...
const { buildRegAiIntroHintsText: buildVenueOwnerIntroHintsText } = require('./venueOwnerFirstSteps');
...
async function startWebOnboarding(chatId, rawToken) {
  const token = String(rawToken || '').trim();
  const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
  const request = await supabase.getWebCabinetOnboarding(tokenHash);
  if (!request || request.status !== 'pending' || new Date(request.expires_at).getTime() <= Date.now()) {
    await Helpers.safeSend(chatId, '⚠️ Посилання на реєстрацію вже недійсне. Повернись на сайт і створи новий запит.');
    return true;
  }

  await State.set(chatId, {
    step: CONSTANTS.FSM_STATES.REG_WEB_CONTACT,
    webCabinetTokenHash: tokenHash,
    webCabinetPhone: normalizePhone(request.phone)
  });
  await Helpers.safeSend(
    chatId,
    '🌐 Реєстрація через сайт MAYAG\n\n' +
      'Щоб підтвердити номер телефону, натисни кнопку нижче та надішли свій контакт. ' +
      'Після підтвердження продовжимо заповнення профілю тут, у Telegram.',
    {
      reply_markup: {
        keyboard: [[{ text: '📱 Підтвердити номер у Telegram', request_contact: true }]],
...
async function handleWebContactMessage(chatId, contact) {
  const st = await State.get(chatId);
  if (!st || st.step !== CONSTANTS.FSM_STATES.REG_WEB_CONTACT) return false;
  const contactUserId = String(contact?.user_id || '');
  if (contactUserId && contactUserId !== String(chatId)) {
    await Helpers.safeSend(chatId, '⚠️ Надішли саме свій контакт, а не контакт іншої людини.');
    return true;
  }
  const phone = normalizePhone(contact?.phone_number);
  if (!phonesMatch(phone, st.webCabinetPhone)) {
    await Helpers.safeSend(
      chatId,
      '⚠️ Номер не збігається з номером, який вказаний на сайті.\n\n' +
        'Надішли контакт із цим самим номером або повернись на сайт і створи новий запит.'
    );
    return true;
  }
  const confirmed = await supabase.confirmWebCabinetOnboarding(st.webCabinetTokenHash, chatId, phone);
  if (!confirmed) {
    await Helpers.safeSend(chatId, '⚠️ Запит уже використано або його термін дії завершився. Створи новий запит на сайті.');
    await State.clear(chatId);
    return true;
  }
  await Helpers.safeSend(chatId, '✅ Номер підтверджено.', {
    reply_markup: { remove_keyboard: true }
  });
  const existing = await User.getByChatId(String(chatId));
  if (existing) {
    await supabase.updateUser(String(chatId), { phone });
    await supabase.completeWebCabinetOnboarding(st.webCabinetTokenHash, chatId, phone);
    await State.clear(chatId);
    await Helpers.safeSend(chatId, 'Профіль уже існує. Відкрий кабінет на сайті ще раз — він буде доступний.');
    return true;
  }
  await State.update(chatId, { webCabinetPhone: phone });
  await showWebCoachRoleStep(chatId);
  return true;
}

async function showWebCoachRoleStep(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_ROLE });
  await Helpers.sendKeyboard(
    chatId,
    '🌐 Номер підтверджено. Обери роль для кабінету:',
    [[{ text: '💪 Тренер', callback_data: CONSTANTS.CALLBACKS.REG_ROLE_COACH }]]
  );
}

function buildRegIntroHintsText(role) {
  if (role === CONSTANTS.ROLES.COACH) {
    return buildRegAiIntroHintsText(CONSTANTS.ROLES.COACH);
  }
  if (role === CONSTANTS.ROLES.VENUE_OWNER) {
    return buildVenueOwnerIntroHintsText(CONSTANTS.ROLES.VENUE_OWNER);
  }
  return buildRegAiIntroHintsText(CONSTANTS.ROLES.STUDENT);
}

/** Після імені/прізвища: власник закладу без анкети клієнта; invite — далі місце/роль. */
async function proceedAfterRegNameStep(chatId) {
  const st = await State.get(chatId);
  if (!isInviteOnboarding(st) && st?.role === CONSTANTS.ROLES.VENUE_OWNER) {
    await finishRegistration(chatId);
    return;
  }
  if (isInviteOnboarding(st)) {
    const st2 = await State.get(chatId);
    if (hasRegLocationFilled(st2)) {
      await proceedAfterInvitePrefillApplied(chatId);
      return;
    }
...
async function proceedAfterCycleRegistration(chatId) {
  await proceedAfterRegGenderStep(chatId);
}

/** Після статі (або циклу): клієнт — мета; тренер — без мети, одразу дата народження. */
async function proceedAfterRegGenderStep(chatId) {
  const st = await State.get(chatId);
  if (isInviteStudentOnboarding(st)) {
    await askBirthDate(chatId);
    return;
  }
  if (st?.role === CONSTANTS.ROLES.COACH) {
    await askBirthDate(chatId);
    return;
  }
...
async function askRegCycleIntro(chatId) {
  ...
  await Helpers.sendKeyboard(
    chatId,
    '🌸 **Дані про менструальний цикл і менопаузу**\n\n' +
...
async function showRoleStep(chatId) {
  ...
  const keyboard = [
    [{ text: '💪 Тренер', callback_data: CONSTANTS.CALLBACKS.REG_ROLE_COACH }],
    [{ text: '🎯 Клієнт', callback_data: CONSTANTS.CALLBACKS.REG_ROLE_STUDENT }],
    [{ text: '🏢 Власник закладу', callback_data: CONSTANTS.CALLBACKS.REG_ROLE_VENUE_OWNER }]
  ];
  await Helpers.sendKeyboard(
    chatId,
    '👤 Обери свою роль:\n\n' +
      '💪 Роль “тренер” — ти маєш освіту у сфері фітнесу та є фітнес‑тренером або інструктором у тренажерному залі.\n\n' +
      '🎓 Роль “клієнт” — ти займаєшся розвитком свого тіла і не тренуєш інших людей.\n\n' +
      '🏢 Роль “власник закладу” — керуєш сторінкою свого клубу/студії на платформі.',
    keyboard
  );
}
...
async function askCoachServiceModes(chatId) {
  const state = await State.get(chatId);
  const M = CONSTANTS.COACH_SERVICE_MODES;
  const selected = Array.isArray(state?.regCoachServiceModes) ? state.regCoachServiceModes : [];
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_COACH_SERVICE_MODES, regCoachServiceModes: selected });
  const hasOffline = selected.includes(M.OFFLINE);
  const hasOnline = selected.includes(M.ONLINE);
  const hasPrograms = selected.includes(M.PROGRAMS);
  const keyboard = [
    [{ text: `${hasOffline ? '✅' : '☐'} Офлайн`, callback_data: CONSTANTS.CALLBACKS.REG_COACH_SERVICE_TOGGLE_OFFLINE }],
    [{ text: `${hasOnline ? '✅' : '☐'} Онлайн`, callback_data: CONSTANTS.CALLBACKS.REG_COACH_SERVICE_TOGGLE_ONLINE }],
    [{ text: `${hasPrograms ? '✅' : '☐'} Програми тренувань`, callback_data: CONSTANTS.CALLBACKS.REG_COACH_SERVICE_TOGGLE_PROGRAMS }],
    [{ text: '✅ Готово', callback_data: CONSTANTS.CALLBACKS.REG_COACH_SERVICE_DONE }]
  ];
  await Helpers.sendKeyboard(
    chatId,
    'Як ти надаєш послуги тренера?\n\n' +
      'Обери один або кілька варіантів (натисни, щоб відмітити):\n\n' +
      '1. Офлайн — проводиш тренування фізично у фітнес-клубах і студіях\n' +
      '2. Онлайн — проводиш заняття через відеозвʼязок\n' +
      '3. Програми — складаєш програми тренувань, клієнт займається самостійно',
    keyboard
  );
}
...
async function askAccentAvoidChoice(chatId) {
  const st = await State.get(chatId);
  if (st?.role === CONSTANTS.ROLES.COACH) {
    await proceedAfterCoachBirthDateStep(chatId);
    return;
  }
  if (isInviteStudentOnboarding(st)) {
...
async function askCity(chatId) {
  ...
  await Helpers.safeSend(chatId, CONSTANTS.REG_LOCATION_PROMPTS.OBLAST);
}
...
async function proceedFromRegDistrictStep(chatId) {
  ...
  if (!st) return;
  if (isInviteOnboarding(st)) {
    await proceedAfterInviteLocation(chatId);
    return;
  }
...
async function proceedAfterCoachInstagramStep(chatId) {
  const st = await State.get(chatId);
  if (st?.role === CONSTANTS.ROLES.COACH) {
    await finishRegistration(chatId);
    return;
...
async function skipInstagram(chatId) {
  await State.update(chatId, { instagram: '' });
  await proceedAfterCoachInstagramStep(chatId);
}

async function askInstagram(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_INSTAGRAM });
  const keyboard = [[{ text: '⏭️ Пропустити', callback_data: CONSTANTS.CALLBACKS.REG_INSTAGRAM_SKIP }]];
  await Helpers.sendKeyboard(
    chatId,
    '📸 Надішли посилання на свій Instagram:\n\n' +
      'Приклад: https://www.instagram.com/yourname\n\n' +
      'Можна пропустити зараз і додати пізніше в «Мій профіль» → «Редагувати дані» → Instagram.',
    keyboard
  );
}
...
function isInviteOnboarding(state) {
  return !!(state && state.inviteOnboarding === true);
}

/** Інвайт від тренера (є coach_id у заготовці) — роль клієнта без питання. */
function isInviteCoachBound(state) {
  return isInviteOnboarding(state) && !!String(state.inviteCoachId || '').trim();
}

/** Універсальний інвайт (help / система) — роль обирає користувач після місця проживання. */
function isUniversalInviteOnboarding(state) {
  return isInviteOnboarding(state) && !isInviteCoachBound(state);
}

function isInviteStudentOnboarding(state) {
  return isInviteOnboarding(state) && state.role === CONSTANTS.ROLES.STUDENT;
}

function isInviteCoachOnboarding(state) {
  return isInviteOnboarding(state) && state.role === CONSTANTS.ROLES.COACH;
}

function coachHasOfflineService(state) {
  const modes = Array.isArray(state?.regCoachServiceModes) ? state.regCoachServiceModes : [];
  return modes.includes(CONSTANTS.COACH_SERVICE_MODES.OFFLINE);
}

const COACH_REG_VENUE_OBL_MIN = 2;
const COACH_REG_VENUE_CITY_MIN = 3;

function shouldAskCoachMoreVenuesAfterVenues(st) {
  return isInviteCoachOnboarding(st) && coachHasOfflineService(st);
}

function coachRegVenuePickPlace(st) {
  const city = String(st?.regVenuePickCity || st?.regVenueCity || st?.city || '').trim();
  const oblast = String(st?.regVenuePickOblast || st?.regVenueOblast || st?.regOblast || '').trim();
  return { city, oblast };
}

async function askCoachMoreVenuesQuestion(chatId) {
  const st = await State.get(chatId);
  const count = Array.isArray(st?.regVenueSelectedIds) ? st.regVenueSelectedIds.length : 0;
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_COACH_MORE_VENUES_ASK });
  let msg = 'Чи є ще клуби чи студії, де ти тренуєш клієнтів?';
  if (count > 0) msg = `Обрано закладів: ${count}.\n\n${msg}`;
  await Helpers.sendKeyboard(chatId, msg, [
    [{ text: '🏢 Так, заклади в іншому місті', callback_data: CONSTANTS.CALLBACKS.REG_COACH_MORE_VENUES_YES }],
    [{ text: '✅ Ні, це всі заклади', callback_data: CONSTANTS.CALLBACKS.REG_COACH_MORE_VENUES_NO }]
  ]);
}

async function startCoachRegOtherCityVenues(chatId) {
  await State.update(chatId, {
    step: CONSTANTS.FSM_STATES.REG_COACH_VENUE_OTHER_OBLAST,
    regVenuePickOblast: '',
    regVenuePickCity: ''
  });
  await Helpers.safeSend(
    chatId,
    '🏢 <b>Заклади в іншому місті</b>\n\n' +
      'Обери область і населений пункт з довідника.\n\n' +
      `Введи назву <b>області</b> (від ${COACH_REG_VENUE_OBL_MIN} літер):`,
    { parse_mode: 'HTML' }
  );
}

async function finalizeCoachRegVenuesAndProceed(chatId) {
  const st = await State.get(chatId);
  const ids = Array.isArray(st?.regVenueSelectedIds) ? st.regVenueSelectedIds : [];
  await State.update(chatId, { regVenueSelectedIds: ids });
  await Helpers.safeSend(chatId, ids.length ? '✅ Заклади збережено для профілю.' : 'ℹ️ Заклади не обрано.');
  await proceedAfterInviteVenuesStep(chatId);
}

async function handleCoachRegVenueOblastText(chatId, text) {
  const q = String(text || '').trim();
  if (q.length < COACH_REG_VENUE_OBL_MIN) {
    await Helpers.safeSend(chatId, `⚠️ Введи щонайменше ${COACH_REG_VENUE_OBL_MIN} літери назви області:`);
    return true;
  }
  const oblasts = await supabase.searchOblasts(q, 12);
  if (!oblasts.length) {
    await Helpers.safeSend(chatId, '❌ Область не знайдено. Спробуй інше написання:');
    return true;
  }
  const keyboard = oblasts.map((o) => [
    {
      text: o.length > 64 ? o.slice(0, 61) + '…' : o,
      callback_data: CONSTANTS.CALLBACK_PREFIXES.REG_COACH_VEN_OBL + ':' + o
    }
  ]);
  await Helpers.sendKeyboard(chatId, 'Обери <b>область</b> зі списку:', keyboard, { parse_mode: 'HTML' });
  return true;
}

async function handleCoachRegVenueCityText(chatId, text) {
  const st = await State.get(chatId);
  const oblast = String(st?.regVenuePickOblast || '').trim();
  if (!oblast) {
    await startCoachRegOtherCityVenues(chatId);
    return true;
  }
  const q = String(text || '').trim();
  if (q.length < COACH_REG_VENUE_CITY_MIN) {
    await Helpers.safeSend(chatId, `⚠️ Введи щонайменше ${COACH_REG_VENUE_CITY_MIN} літери назви населеного пункту:`);
    return true;
  }
  const cities = await supabase.searchCitiesInOblast(oblast, q, 12);
  if (!cities.length) {
    await Helpers.safeSend(chatId, `❌ Не знайдено. Спробуй інші ${COACH_REG_VENUE_CITY_MIN}+ літери:`);
    return true;
  }
  const keyboard = cities.map((c) => [
    {
      text: c.length > 64 ? c.slice(0, 61) + '…' : c,
      callback_data: CONSTANTS.CALLBACK_PREFIXES.REG_COACH_VEN_CIT + ':' + c
    }
  ]);
  keyboard.push([
    { text: '⬅️ Змінити область', callback_data: CONSTANTS.CALLBACK_PREFIXES.REG_COACH_VEN_OBL + ':__BACK__' }
  ]);
  await Helpers.sendKeyboard(
    chatId,
    `Обери <b>населений пункт</b> (область: ${Helpers.escapeHtml(oblast)}):`,
    keyboard,
    { parse_mode: 'HTML' }
  );
  return true;
...
async function handleCoachRegVenueLocationPick(chatId, action, params) {
  const param = (params && params.length ? params.join(':') : '').trim();
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_COACH_VEN_OBL) {
    if (param === '__BACK__') {
      await startCoachRegOtherCityVenues(chatId);
      return true;
    }
    await State.update(chatId, {
      regVenuePickOblast: param,
      regVenuePickCity: '',
      step: CONSTANTS.FSM_STATES.REG_COACH_VENUE_OTHER_CITY
    });
    await Helpers.safeSend(
      chatId,
      `Область: <b>${Helpers.escapeHtml(param)}</b>\n\n` +
        `Введи назву <b>населеного пункту</b> (від ${COACH_REG_VENUE_CITY_MIN} літер):`,
      { parse_mode: 'HTML' }
    );
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_COACH_VEN_CIT) {
    const st = await State.get(chatId);
    const oblast = String(st?.regVenuePickOblast || '').trim();
    if (!oblast) {
      await startCoachRegOtherCityVenues(chatId);
    ...
    await State.update(chatId, {
      regVenuePickCity: param,
      step: CONSTANTS.FSM_STATES.REG_INVITE_VENUES_PICK,
      regVenuePage: 0
    });
    const place = [param, oblast].filter(Boolean).join(', ');
    await Helpers.safeSend(chatId, `Шукаю заклади: <b>${Helpers.escapeHtml(place)}</b>…`, { parse_mode: 'HTML' });
...
/** Клієнт або тренер на invite-onboarding — вибір закладів однаковим сценарієм. */
function needsInviteVenuePick(state) {
  return isInviteStudentOnboarding(state) || isInviteCoachOnboarding(state);
}
...
function hasRegLocationFilled(st) {
  if (!st) return false;
  const city = String(st.city || st.regVenueCity || '').trim();
  const oblast = String(st.regOblast || st.regVenueOblast || '').trim();
  return isFilled(city) && isFilled(oblast);
}

/** Після дати народження тренера: акцент-зони пропускаємо; місто — лише якщо ще не вказано. */
async function proceedAfterCoachBirthDateStep(chatId) {
  try {
    await State.update(chatId, { regAccentZones: [], regAvoidZones: [] });
    const st = await State.get(chatId);
    if (hasRegLocationFilled(st)) {
      if (isInviteOnboarding(st)) {
        await askInstagram(chatId);
        return;
      }
      const Venues = require('./venues');
      const ob = String(st.regOblast || st.regVenueOblast || '').trim();
      const ci = String(st.city || st.regVenueCity || '').trim();
      await Venues.showRegistrationVenueOffer(chatId, ob, ci);
      return;
    }
    await askCity(chatId);
  } catch (e) {
    console.error('Registration.proceedAfterCoachBirthDateStep', chatId, e && e.message ? e.message : e);
    await Helpers.safeSend(
      chatId,
      '❌ Не вдалося перейти до наступного кроку реєстрації. Спробуй ще раз або /start.'
    );
  }
}
...
async function ensureInvitePrefillSession(chatId, state) {
  if (!state || !isInviteOnboarding(state)) return state;
  if (isInviteCoachBound(state) || state.role) return state;
  await State.update(chatId, {
...
/** Дані з рядка INVITE_* / користувача після активації (help-bot prefill у first_name, city, oblast). */
function invitePrefillFromUserRow(u) {
  if (!u) return { firstName: '', lastName: '', oblast: '', city: '' };
  const first = String(u.firstName || '').trim();
  const last = String(u.lastName || '').trim();
  const city = String(u.city || '').trim();
  const oblast = String(u.oblast != null ? u.oblast : '').trim();
  const techFirst = first.toLowerCase() === 'інвайт' || first.toLowerCase() === 'invite';
  return {
    firstName: techFirst ? '' : first,
    lastName: last,
    city,
    oblast
  };
...
function inviteProfileFromUser(st, u) {
  const pf = invitePrefillFromUserRow(u);
  return {
    firstName: (st?.firstName && String(st.firstName).trim()) || pf.firstName || '',
    lastName: (st?.lastName && String(st.lastName).trim()) || pf.lastName || '',
    oblast: st?.regVenueOblast || st?.regOblast || pf.oblast || '',
    city: st?.city || pf.city || ''
  };
}

async function seedInviteStateFromUserProfile(chatId) {
  const st = await State.get(chatId);
  if (!st || st.inviteOnboarding !== true) return st;
  const u = await User.getByChatId(chatId);
  const prof = inviteProfileFromUser(st, u);
  const patch = {};
  if (!isFilled(st.firstName) && isFilled(prof.firstName)) patch.firstName = prof.firstName;
  if (!isFilled(st.lastName) && isFilled(prof.lastName)) patch.lastName = prof.lastName;
  if (!isFilled(st.city) && isFilled(prof.city)) patch.city = prof.city;
  const ob = String(st.regOblast || st.regVenueOblast || '').trim();
  if (!ob && isFilled(prof.oblast)) {
    patch.regOblast = prof.oblast;
    patch.regVenueOblast = prof.oblast;
  }
  if (!isFilled(st.regVenueCity) && isFilled(prof.city)) patch.regVenueCity = prof.city;
  if (!Object.keys(patch).length) return st;
  await State.update(chatId, patch);
  return State.get(chatId);
}

/** Після оферти: застосувати prefill з users (help-bot) у state без повторних питань. */
async function applyInvitePrefillFromProfile(chatId) {
  await seedInviteStateFromUserProfile(chatId);
  const st = await State.get(chatId);
  const u = await User.getByChatId(chatId);
  const prof = inviteProfileFromUser(st, u);
  const patch = { regDistrict: '' };
  if (isFilled(prof.firstName)) patch.firstName = prof.firstName;
  if (isFilled(prof.lastName)) patch.lastName = prof.lastName;
  if (isFilled(prof.city)) {
    patch.city = prof.city;
    patch.regVenueCity = prof.city;
  }
  if (isFilled(prof.oblast)) {
    patch.regOblast = prof.oblast;
    patch.regVenueOblast = prof.oblast;
  }
  await State.update(chatId, patch);
...
/** Після prefill: роль (універсальний інвайт) або заклади (інвайт тренера). */
async function proceedAfterInvitePrefillApplied(chatId) {
  const st = await State.get(chatId);
  if (!isInviteOnboarding(st)) {
  ...
  const u = await User.getByChatId(chatId);
  const prof = inviteProfileFromUser(st, u);
  if (!isFilled(prof.firstName)) {
    await askFirstName(chatId);
    return;
  }
  if (!isFilled(prof.lastName)) {
  ...
  if (isInviteCoachBound(st2)) {
    await askInviteVenuesIfNeeded(chatId);
    return;
  }
  if (isUniversalInviteOnboarding(st2)) {
    await showRoleStep(chatId);
    return;
  }
  await askInviteVenuesIfNeeded(chatId);
...
function prefillValueText(field, value) {
  ...
  if (field === 'role') {
    if (value === CONSTANTS.ROLES.COACH) return 'Тренер';
    if (value === CONSTANTS.ROLES.VENUE_OWNER) return 'Власник закладу';
    return 'Клієнт';
  }
...
/** Після «Приймаю умови»: prefill з help-bot → одразу вибір ролі (без підтвердження імені/міста/району). */
async function startInviteOnboardingAfterAccept(chatId) {
  const st = await State.get(chatId);
  if (!isInviteOnboarding(st)) {
    await showRoleStep(chatId);
    return;
  }
  if (isInviteCoachBound(st)) {
    await State.update(chatId, {
      role: CONSTANTS.ROLES.STUDENT
    });
    await syncChosenRoleToDb(chatId);
  }
  await applyInvitePrefillFromProfile(chatId);
  await proceedAfterInvitePrefillApplied(chatId);
}

async function askInviteRoleAfterLocation(chatId) {
  const st = await State.get(chatId);
  if (!isUniversalInviteOnboarding(st)) return;
  await showRoleStep(chatId);
}

/** Після області/міста/району в invite-onboarding. */
async function proceedAfterInviteLocation(chatId) {
  const st = await State.get(chatId);
  if (!isInviteOnboarding(st)) return;
  if (isUniversalInviteOnboarding(st)) {
    await askInviteRoleAfterLocation(chatId);
    return;
  }
  await askInviteVenuesIfNeeded(chatId);
}

/** Після вибору ролі (універсальний інвайт): клієнт/тренер → заклади; власник закладу → інтро. */
async function proceedAfterInviteRoleChosen(chatId) {
  const st = await State.get(chatId);
  if (!isUniversalInviteOnboarding(st) || !st.role) {
    await askInviteAiIntro(chatId);
    return;
  }
  await syncChosenRoleToDb(chatId);
  if (st.role === CONSTANTS.ROLES.STUDENT) {
    await askInviteVenuesIfNeeded(chatId);
  } else if (st.role === CONSTANTS.ROLES.VENUE_OWNER) {
    await askInviteVenueOwnerIntro(chatId);
  } else if (st.role === CONSTANTS.ROLES.COACH) {
    await askCoachAfterRoleIntroAndServiceModes(chatId);
  } else {
    await askInviteAiIntro(chatId);
  }
}

async function askCoachAfterRoleIntroAndServiceModes(chatId) {
  const text = buildRegIntroHintsText(CONSTANTS.ROLES.COACH);
  await Helpers.safeSend(chatId, text, { parse_mode: 'Markdown' });
  await askCoachServiceModes(chatId);
}

async function proceedAfterCoachServiceModesDone(chatId) {
  const st = await State.get(chatId);
  const modes = Array.isArray(st?.regCoachServiceModes) ? st.regCoachServiceModes : [];
  try {
    await supabase.updateUser(String(chatId), { coachServiceModes: modes });
  } catch (e) {
    console.error('Registration.proceedAfterCoachServiceModesDone save', chatId, e && e.message ? e.message : e);
  }
  if (coachHasOfflineService(st) && isInviteCoachOnboarding(st)) {
    await askInviteVenuesIfNeeded(chatId);
    return;
  }
  if (isUniversalInviteOnboarding(st) && st.role === CONSTANTS.ROLES.COACH) {
    await finishRegistration(chatId);
    return;
  }
...
async function proceedAfterInviteVenuesStep(chatId) {
  const st = await State.get(chatId);
  if (st?.role === CONSTANTS.ROLES.COACH) {
    if (isUniversalInviteOnboarding(st)) {
      await finishRegistration(chatId);
...
async function showInviteVenuesPicker(chatId, page = 0) {
  const st = await State.get(chatId);
  const { city, oblast } = coachRegVenuePickPlace(st);
  ...
  const isCoachOfflineVenues = isInviteCoachOnboarding(st) && coachHasOfflineService(st);
  const listHeader = isCoachOfflineVenues
    ? `🏢 Обери клуби та студії, у яких ти тренуєш клієнтів (${place || 'з профілю'}):`
...
async function showInviteVenueCard(chatId, venueId) {
  ...
  const gcLine = await coachGroupPublic.buildVenueGroupClassesCoachesHtml(vid, gcFacets, dirGroupMap);
...
async function askInviteVenuesIfNeeded(chatId) {
  const st = await State.get(chatId);
  if (!needsInviteVenuePick(st)) return;
  const city = String(st?.regVenueCity || st?.city || '').trim();
  const oblast = String(st?.regVenueOblast || '').trim();
  const place = [city, oblast].filter(Boolean).join(', ');
  const isCoachOfflineVenues = isInviteCoachOnboarding(st) && coachHasOfflineService(st);
  const hubText = isCoachOfflineVenues
    ? `🏢 Обери клуби та студії, у яких ти тренуєш клієнтів.\nНаселений пункт: ${place || '—'}\n\nОбери дію нижче:`
    : `🏢 Спортивні та фітнес організації міста: ${place || '—'}\n\nОбери дію нижче:`;
  const pickPatch = { step: CONSTANTS.FSM_STATES.REG_INVITE_VENUES_PICK, inviteEditField: null, regVenuePickMode: null };
  if (isCoachOfflineVenues) {
    pickPatch.regVenuePickCity = city;
    pickPatch.regVenuePickOblast = oblast;
  }
  await State.update(chatId, pickPatch);
  const keyboard = [];
  const coachId = String(st?.inviteCoachId || '').trim();
  if (coachId && isInviteStudentOnboarding(st)) {
    const coachVenues = await supabase.getCoachVenuesWhereTeach(coachId);
    if (coachVenues && coachVenues.length > 0) {
      keyboard.push([{ text: '🏢 Де тренує тренер', callback_data: CONSTANTS.CALLBACKS.REG_INVITE_COACH_VENUES }]);
    }
  }
  keyboard.push([{ text: '🔎 Показати заклади міста', callback_data: CONSTANTS.CALLBACKS.REG_INVITE_VENUES_OPEN }]);
  keyboard.push([{ text: '⏭️ Пропустити', callback_data: CONSTANTS.CALLBACKS.REG_INVITE_VENUES_SKIP }]);
  await Helpers.sendKeyboard(chatId, hubText, keyboard);
}

async function showInviteCoachVenuesPicker(chatId) {
  const st = await State.get(chatId);
  const coachId = String(st?.inviteCoachId || '').trim();
  if (!coachId) {
    await askInviteVenuesIfNeeded(chatId);
  ...
  if (!venues || !venues.length) {
    await Helpers.safeSend(chatId, 'ℹ️ Тренер ще не вказав заклади для тренувань.');
    await askInviteVenuesIfNeeded(chatId);
    return;
  }
  const selected = Array.isArray(st?.regVenueSelectedIds) ? st.regVenueSelectedIds : [];
  ...
  for (const v of venues) {
    const id = String(v.id);
    const base = (v.nameUa || 'Заклад') + (v.city ? ' — ' + v.city : '');
    const label = (selected.includes(id) ? '✓ ' : '') + base;
    keyboard.push([
      {
        text: label.length > 64 ? label.slice(0, 61) + '…' : label,
        callback_data: CONSTANTS.CALLBACK_PREFIXES.REG_INVITE_COACH_VEN_TOGGLE + ':' + id
      }
    ]);
  }
  keyboard.push([{ text: '✅ Готово', callback_data: CONSTANTS.CALLBACKS.REG_INVITE_COACH_VENUES_DONE }]);
  keyboard.push([{ text: '🔙 Назад', callback_data: CONSTANTS.CALLBACKS.REG_INVITE_VENUES_HUB }]);
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_INVITE_VENUES_PICK, regVenuePickMode: 'coach' });
  const picked = venues.filter((v) => selected.includes(String(v.id))).length;
  await Helpers.sendKeyboard(
    chatId,
    '🏢 **Де тренує тренер**\n\nОбери заклад(и), де тренуєшся з тренером. Можна кілька.\n\nОбрано: ' + picked,
    keyboard,
    { parse_mode: 'Markdown' }
  );
}
...
async function askInviteVenueOwnerIntro(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_INVITE_AI_INTRO });
  const keyboard = [[{ text: '✅ Завершити реєстрацію', callback_data: CONSTANTS.CALLBACKS.REG_AI_LATER }]];
  const text =
    buildVenueOwnerIntroHintsText(CONSTANTS.ROLES.VENUE_OWNER) +
    '\n\nНатисни **Завершити реєстрацію**. Далі адміністратор призначить тебе власником закладу.\n' +
    'Детальні підказки — у головному меню → **💡 Підказки** та **🏢 Мій заклад**.';
  await Helpers.sendKeyboard(chatId, text, keyboard, { parse_mode: 'Markdown' });
}

async function askInviteAiIntro(chatId) {
  const st = await State.get(chatId);
  const role = st?.role || CONSTANTS.ROLES.STUDENT;
  if (role === CONSTANTS.ROLES.VENUE_OWNER) {
    await askInviteVenueOwnerIntro(chatId);
    return;
  }
  ...
  const text =
    buildRegIntroHintsText(role) +
    'Можеш **продовжити заповнення** зараз або **заповнити пізніше** (у профілі).';
  await Helpers.sendKeyboard(chatId, text, keyboard, { parse_mode: 'Markdown' });
}
...
/**
 * Після «Приймаю умови» по інвайту: не затирати весь bot_state (прибираємо лише pendingInviteCode).
 * Роль: клієнт одразу лише для інвайту тренера; універсальний — роль після prefill (без підтвердження імені/міста).
 */
async function mergeStateAfterInviteOfferAccept(chatId, inviteCoachId) {
  const prev = (await State.get(chatId)) || {};
  const rest = { ...prev };
  delete rest.pendingInviteCode;
  delete rest.role;
  const hasCoachOnInvite = !!String(inviteCoachId || '').trim();
  const next = {
    ...rest,
    step: CONSTANTS.FSM_STATES.REG_INVITE_PREFILL_REVIEW,
    inviteOnboarding: true,
    inviteCoachId: String(inviteCoachId || '')
  };
  if (hasCoachOnInvite) {
    next.role = CONSTANTS.ROLES.STUDENT;
  }
  await State.set(chatId, next);
}

/** Записати обрану роль у `users` (після вибору ролі під час реєстрації / інвайту). */
async function syncChosenRoleToDb(chatId) {
  const st = await State.get(chatId);
  if (!st?.role) return;
  const id = String(chatId);
  const user = await User.getByChatId(id);
  if (!user) return;
  try {
    const ok = await supabase.updateUser(id, { role: st.role });
    if (!ok) {
      console.error('Registration.syncChosenRoleToDb: update failed', id, st.role);
    }
  } catch (e) {
    console.error('Registration.syncChosenRoleToDb', e.message);
  }
}

async function askAdsOptIn(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_ADS_OPT_IN });
  await Helpers.sendKeyboard(
    chatId,
    '📣 Отримувати новини та пропозиції від тренерів і закладів у моєму місті?\n\n' +
      'Це не стосується повідомлень вашого тренера/закладу та нагадувань про тренування.\n\nОбов’язковий вибір:',
    [
      [{ text: 'Так', callback_data: CONSTANTS.CALLBACKS.REG_ADS_YES }],
      [{ text: 'Ні', callback_data: CONSTANTS.CALLBACKS.REG_ADS_NO }]
    ]
  );
}
...
async function finishRegistration(chatId) {
  try {
    ...
    if (stateData.adsOptIn !== true && stateData.adsOptIn !== false) {
      await askAdsOptIn(chatId);
      return;
    }
    ...
    const userData = {
      ...
      phone: stateData.webCabinetPhone || '',
      coachTrainingTypes: Array.isArray(stateData.regCoachTrainingTypes) ? stateData.regCoachTrainingTypes : [],
      coachServiceModes: Array.isArray(stateData.regCoachServiceModes) ? stateData.regCoachServiceModes : [],
      ...
      adsOptIn: stateData.adsOptIn !== false
    };
    ...
    if (existing) {
      const ok = await supabase.updateUser(String(chatId), {
        ...
        coachServiceModes: userData.coachServiceModes,
    ...
    if (!User.isCoach(await User.getByChatId(String(chatId))) && User.normalizeRole(userData.role) === CONSTANTS.ROLES.COACH) {
      await supabase.updateUser(String(chatId), { role: userData.role });
      await supabase.syncUserIdToChatId(String(chatId));
    }
    const registrationVenueIds = Array.isArray(stateData.regVenueSelectedIds)
      ? [...new Set(stateData.regVenueSelectedIds.map((id) => String(id || '').trim()).filter(Boolean))]
      : [];
    if (registrationVenueIds.length > 0) {
      if (userData.role === CONSTANTS.ROLES.COACH) {
        const currentTeachVenues = await supabase.getCoachVenuesWhereTeach(String(chatId));
        for (let i = 0; i < registrationVenueIds.length; i += 1) {
          await supabase.linkCoachVenue(String(chatId), registrationVenueIds[i], {
            isPrimary: (!currentTeachVenues || currentTeachVenues.length === 0) && i === 0,
            teachesHere: true
          });
        }
      } else {
        await supabase.setUserVenues(String(chatId), registrationVenueIds);
      }
    }
    ...
    if (stateData.webCabinetTokenHash) {
      await supabase.completeWebCabinetOnboarding(
        stateData.webCabinetTokenHash,
        chatId,
        userData.phone
      );
    }
    const sawVenueOwnerIntroAtReg =
      userData.role === CONSTANTS.ROLES.VENUE_OWNER &&
      stateData.step === CONSTANTS.FSM_STATES.REG_INVITE_AI_INTRO;
    await State.clear(chatId);
    const roleText =
      userData.role === CONSTANTS.ROLES.COACH
        ? 'тренере'
        : userData.role === CONSTANTS.ROLES.VENUE_OWNER
          ? 'власнику закладу'
          : 'учне';
    if (userData.role === CONSTANTS.ROLES.COACH) {
      await Helpers.safeSend(
        ...
        'Вітаю, ' +
          ...
          'Ласкаво просимо в FIT 3.0, тренере!\n\n' +
          'Для початку зайди в меню «Підказки».'
      );
    } else if (userData.role === CONSTANTS.ROLES.STUDENT) {
      const registeredStudent = await User.getByChatId(String(chatId));
      const coachId = String(
        registeredStudent?.coachId || stateData.inviteCoachId || existing?.coachId || ''
      ).trim();
      let welcome =
        '🎉 Вітаю, ' +
        userData.firstName +
        '!\n\n' +
        'Реєстрацію завершено. Ласкаво просимо в FIT 3.0!\n\n' +
        'Для початку зайди в меню «Підказки».';
      const keyboard = [];
      if (coachId) {
        const coach = await User.getByChatId(coachId);
        const coachLabel = coach
          ? ((coach.firstName || '') + ' ' + (coach.lastName || '')).trim() || 'тренер'
          : 'тренер';
        welcome += '\n\nТвій тренер: ' + coachLabel + ' — натисни кнопку нижче, щоб відкрити картку.';
        keyboard.push([{ text: '👨‍🏫 Картка тренера', callback_data: CONSTANTS.CALLBACKS.COACH_MY_COACH_MENU }]);
      }
      if (keyboard.length) {
        await Helpers.sendKeyboard(chatId, welcome, keyboard);
      } else {
        await Helpers.safeSend(chatId, welcome);
      }
    } else {
      await Helpers.safeSend(
        chatId,
        '🎉 Вітаю, ' + userData.firstName + '!\n\nРеєстрацію завершено. Ласкаво просимо в FIT 3.0, ' + roleText + '!'
      );
    }
    if (userData.role === CONSTANTS.ROLES.VENUE_OWNER) {
      if (!sawVenueOwnerIntroAtReg) {
        await Helpers.safeSend(
          chatId,
          buildVenueOwnerIntroHintsText(CONSTANTS.ROLES.VENUE_OWNER) +
            '\n\nПісля призначення закладу адміністратором відкрий **💡 Підказки** та **🏢 Мій заклад**.',
          { parse_mode: 'Markdown' }
        );
      } else {
        await Helpers.safeSend(
          chatId,
          'Після призначення закладу адміністратором відкрий **💡 Підказки** та **🏢 Мій заклад** у головному меню.',
          { parse_mode: 'Markdown' }
        );
      }
    }
...
async function handleTextMessage(chatId, text) {
  ...
  if (step === CONSTANTS.FSM_STATES.REG_ADS_OPT_IN) {
    await Helpers.safeSend(chatId, '⚠️ Обери «Так» або «Ні» кнопкою вище.');
    return true;
  }
  if (step === CONSTANTS.FSM_STATES.REG_COACH_MORE_VENUES_ASK) {
    await Helpers.safeSend(chatId, '⚠️ Обери варіант кнопкою нижче.');
    return true;
  }
  if (step === CONSTANTS.FSM_STATES.REG_COACH_VENUE_OTHER_OBLAST) {
    return handleCoachRegVenueOblastText(chatId, text);
  }
  if (step === CONSTANTS.FSM_STATES.REG_COACH_VENUE_OTHER_CITY) {
    return handleCoachRegVenueCityText(chatId, text);
  }
  ...
  if (step === CONSTANTS.FSM_STATES.REG_LAST_NAME) {
    ...
    await proceedAfterRegNameStep(chatId);
    return true;
  }
  ...
  if (step === CONSTANTS.FSM_STATES.REG_CYCLE_LEN) {
    ...
    await State.update(chatId, { regAvgCycleLength: parsed.value });
    await askRegCycleBleeding(chatId);
    return true;
  }

  if (step === CONSTANTS.FSM_STATES.REG_CYCLE_BLEED) {
    await Helpers.safeSend(chatId, '⚠️ Обери тривалість місячних кнопками вище.');
    return true;
  }

  if (step === CONSTANTS.FSM_STATES.REG_CYCLE_STATUS) {
    await Helpers.safeSend(chatId, '⚠️ Обери репродуктивний статус кнопкою вище.');
    return true;
  }
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.REG_ADS_YES || action === CONSTANTS.CALLBACKS.REG_ADS_NO) {
    const st = await State.get(chatId);
    if (!st || st.step !== CONSTANTS.FSM_STATES.REG_ADS_OPT_IN) return false;
    await State.update(chatId, { adsOptIn: action === CONSTANTS.CALLBACKS.REG_ADS_YES });
    await finishRegistration(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_KEEP) {
    ...
    if (!st || !isInviteOnboarding(st)) return false;
    if (field === 'role') {
      await askInviteRoleAfterLocation(chatId);
      return true;
    }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_COACH_VEN_OBL || action === CONSTANTS.CALLBACK_PREFIXES.REG_COACH_VEN_CIT) {
    const handled = await handleCoachRegVenueLocationPick(chatId, action, parts.slice(1));
    if (handled) return true;
  }
  if (action === CONSTANTS.CALLBACKS.REG_COACH_MORE_VENUES_YES) {
    const st = await State.get(chatId);
    if (!st || st.step !== CONSTANTS.FSM_STATES.REG_COACH_MORE_VENUES_ASK) return false;
    await startCoachRegOtherCityVenues(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.REG_COACH_MORE_VENUES_NO) {
    const st = await State.get(chatId);
    if (!st || st.step !== CONSTANTS.FSM_STATES.REG_COACH_MORE_VENUES_ASK) return false;
    await finalizeCoachRegVenuesAndProceed(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACKS.REG_INVITE_COACH_VENUES) {
    await showInviteCoachVenuesPicker(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.REG_INVITE_VENUES_HUB) {
    await askInviteVenuesIfNeeded(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_INVITE_COACH_VEN_TOGGLE && param) {
    const st = await State.get(chatId);
    if (!st || st.step !== CONSTANTS.FSM_STATES.REG_INVITE_VENUES_PICK) return false;
    const vid = String(param).trim();
    const cur = Array.isArray(st.regVenueSelectedIds) ? [...st.regVenueSelectedIds] : [];
    const idx = cur.indexOf(vid);
    if (idx >= 0) cur.splice(idx, 1);
    else cur.push(vid);
    await State.update(chatId, { regVenueSelectedIds: cur });
    await showInviteCoachVenuesPicker(chatId);
  ...
  if (action === CONSTANTS.CALLBACKS.REG_INVITE_COACH_VENUES_DONE) {
    const st = await State.get(chatId);
    const ids = Array.isArray(st?.regVenueSelectedIds) ? st.regVenueSelectedIds : [];
    if (isInviteStudentOnboarding(st) && ids.length > 0) {
      await supabase.setUserVenues(chatId, ids);
    }
    await Helpers.safeSend(chatId, ids.length ? '✅ Заклад(и) тренера додано до твого профілю.' : 'ℹ️ Заклади тренера не обрано.');
    await askInviteVenuesIfNeeded(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.REG_INVITE_VENUES_SKIP) {
    const stSkip = await State.get(chatId);
    if (shouldAskCoachMoreVenuesAfterVenues(stSkip)) {
      await askCoachMoreVenuesQuestion(chatId);
      return true;
    }
    const ids = Array.isArray(stSkip?.regVenueSelectedIds) ? stSkip.regVenueSelectedIds : [];
    if (isInviteStudentOnboarding(stSkip) && ids.length > 0) {
      await supabase.setUserVenues(chatId, ids);
    }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_VENUES_DONE) {
    const st = await State.get(chatId);
    const ids = Array.isArray(st?.regVenueSelectedIds) ? st.regVenueSelectedIds : [];
    if (shouldAskCoachMoreVenuesAfterVenues(st)) {
      await Helpers.safeSend(
        chatId,
        ids.length ? `✅ У цьому місті обрано закладів: ${ids.length}.` : 'ℹ️ У цьому місті заклади не обрано.'
      );
      await askCoachMoreVenuesQuestion(chatId);
      return true;
    }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_VENUES_SKIP) {
    const stSkip2 = await State.get(chatId);
    if (shouldAskCoachMoreVenuesAfterVenues(stSkip2)) {
      await askCoachMoreVenuesQuestion(chatId);
      return true;
    }
    await Helpers.safeSend(chatId, 'ℹ️ Можна додати заклади пізніше у меню «Клуби, студії».');
  ...
  if (action === CONSTANTS.CALLBACKS.REG_AI_CONTINUE) {
    ...
    if (st.role === CONSTANTS.ROLES.VENUE_OWNER || st.role === CONSTANTS.ROLES.COACH) {
      await finishRegistration(chatId);
      return true;
    }
    await askGender(chatId);
  ...
  if (action === CONSTANTS.CALLBACKS.REG_ROLE_STUDENT) {
    ...
    await syncChosenRoleToDb(chatId);
    const st = await State.get(chatId);
    if (isUniversalInviteOnboarding(st)) await proceedAfterInviteRoleChosen(chatId);
    else if (isInviteOnboarding(st)) {
      await syncChosenRoleToDb(chatId);
  ...
  if (action === CONSTANTS.CALLBACKS.REG_ROLE_COACH) {
    ...
    await syncChosenRoleToDb(chatId);
    const stCoach = await State.get(chatId);
    if (isUniversalInviteOnboarding(stCoach)) {
      await proceedAfterInviteRoleChosen(chatId);
      return true;
    }
    await askCoachServiceModes(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.REG_ROLE_VENUE_OWNER) {
    await State.update(chatId, { role: CONSTANTS.ROLES.VENUE_OWNER });
    await syncChosenRoleToDb(chatId);
    const stOwner = await State.get(chatId);
    if (isUniversalInviteOnboarding(stOwner)) {
      await proceedAfterInviteRoleChosen(chatId);
      return true;
    }
    await askFirstName(chatId);
  ...
  if (action === CONSTANTS.CALLBACKS.REG_INSTAGRAM_SKIP) {
    const state = await State.get(chatId);
    if (!state || state.step !== CONSTANTS.FSM_STATES.REG_INSTAGRAM) return false;
    await skipInstagram(chatId);
    return true;
  }
  ...
  if (
    action === CONSTANTS.CALLBACKS.REG_COACH_SERVICE_TOGGLE_OFFLINE ||
    action === CONSTANTS.CALLBACKS.REG_COACH_SERVICE_TOGGLE_ONLINE ||
    action === CONSTANTS.CALLBACKS.REG_COACH_SERVICE_TOGGLE_PROGRAMS
  ) {
    const state = await State.get(chatId);
    if (!state || state.step !== CONSTANTS.FSM_STATES.REG_COACH_SERVICE_MODES) return false;
    const cur = Array.isArray(state.regCoachServiceModes) ? state.regCoachServiceModes : [];
    const set = new Set(cur);
    const M = CONSTANTS.COACH_SERVICE_MODES;
    const key =
      action === CONSTANTS.CALLBACKS.REG_COACH_SERVICE_TOGGLE_OFFLINE
    ...
    await State.update(chatId, { regCoachServiceModes: Array.from(set) });
    await askCoachServiceModes(chatId);
    return true;
  }
  if (action === CONSTANTS.CALLBACKS.REG_COACH_SERVICE_DONE) {
    const state = await State.get(chatId);
    if (!state || state.step !== CONSTANTS.FSM_STATES.REG_COACH_SERVICE_MODES) return false;
    const arr = Array.isArray(state.regCoachServiceModes) ? state.regCoachServiceModes : [];
    if (!arr.length) {
      await Helpers.safeSend(chatId, '⚠️ Обери хоча б один варіант надання послуг.');
      await askCoachServiceModes(chatId);
      return true;
    }
    await proceedAfterCoachServiceModesDone(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_ACCENT_SKIP) {
    ...
    else if (st?.role === CONSTANTS.ROLES.COACH) await proceedAfterCoachBirthDateStep(chatId);
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_AVD_SKP || action === CONSTANTS.CALLBACK_PREFIXES.REG_AVD_NXT) {
    ...
    else if (state.role === CONSTANTS.ROLES.COACH) await proceedAfterCoachBirthDateStep(chatId);
    else await askCity(chatId);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.CITY_OBLAST) {
    ...
    await Helpers.safeSend(chatId, CONSTANTS.REG_LOCATION_PROMPTS.CITY);
    return true;
  }
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.CITY_PICK) {
    ...
    if (isInviteOnboarding(stateBefore)) {
  ...
  if (action === CONSTANTS.CALLBACKS.REG_INVITE_OFFER_ACCEPT) {
    ...
    const inviteCode = st?.pendingInviteCode ? String(st.pendingInviteCode).trim().toUpperCase() : '';
    if (!inviteCode) {
      // Telegram may deliver a second click from the old offer message after
      // the first click already moved the user into invite onboarding.
      if (st?.inviteOnboarding === true && st?.step !== CONSTANTS.FSM_STATES.REG_INVITE_OFFER) {
        return true;
      }
    ...
    try {
      ...
      try {
        ...
        // Клієнт уже має рядок у users (раніше почав реєстрацію) — не можна «замінити» інвайт на chatId через activateInvite
        if (e1 && e1.message === 'This Telegram account is already registered') {
          ...
          if (!inviteCoachId) {
            ...
            await mergeStateAfterInviteOfferAccept(chatId, '');
            if (invRow) {
              const pf = invitePrefillFromUserRow(invRow);
              const merge = {};
              if (isFilled(pf.firstName)) merge.firstName = pf.firstName;
              if (isFilled(pf.lastName)) merge.lastName = pf.lastName;
              if (isFilled(pf.city)) merge.city = pf.city;
              if (isFilled(pf.oblast)) merge.oblast = pf.oblast;
              if (Object.keys(merge).length) await supabase.updateUser(chatId, merge);
            }
            await supabase.touchUserInviteRegistrationStarted(chatId);
            await startInviteOnboardingAfterAccept(chatId);
            return true;
          }
      ...
      await mergeStateAfterInviteOfferAccept(chatId, inviteCoachId);
      await supabase.touchUserInviteRegistrationStarted(chatId);
      await startInviteOnboardingAfterAccept(chatId);
    } catch (err) {
      const msg = (err && err.message) ? String(err.message) : '';
