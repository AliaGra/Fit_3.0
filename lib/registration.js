...
async function askInviteRoleIfNeeded(chatId) {
  ...
  const inviteCoach = String(st.inviteCoachId || '').trim();
  // Універсальний інвайт (без coach_id): не затирати вже обрану роль «тренер».
  if (!inviteCoach && st.role === CONSTANTS.ROLES.COACH) {
    await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_INVITE_PREFILL_REVIEW });
    await seedInviteStateFromUserProfile(chatId);
    await askInviteNameIfNeeded(chatId);
    return;
  }
  // Інвайт прив’язаний до тренера або роль ще не «тренер» — сценарій учня.
  await State.update(chatId, {
    role: CONSTANTS.ROLES.STUDENT,
    step: CONSTANTS.FSM_STATES.REG_INVITE_PREFILL_REVIEW
  });
 ...
 * Роль: завжди учень, якщо інвайт прив’язаний до тренера; інакше зберігаємо вже обрану «тренер».
 */
async function mergeStateAfterInviteOfferAccept(chatId, inviteCoachId) {
  ...
  const role = hasCoachOnInvite
    ? CONSTANTS.ROLES.STUDENT
    : prev.role === CONSTANTS.ROLES.COACH
      ? CONSTANTS.ROLES.COACH
      : CONSTANTS.ROLES.STUDENT;
  await State.set(chatId, {
    ...rest,
    step: CONSTANTS.FSM_STATES.REG_INVITE_PREFILL_REVIEW,
...
/** Після активації універсального інвайту: одразу виставити `coach` у `users`, якщо в state вже обрано тренера. */
async function syncCoachRoleToDbIfUniversalInvite(chatId, inviteCoachId) {
  if (String(inviteCoachId || '').trim()) return;
  const st = await State.get(chatId);
  if (st?.role !== CONSTANTS.ROLES.COACH) return;
  try {
    await supabase.updateUser(String(chatId), { role: CONSTANTS.ROLES.COACH });
  } catch (e) {
    console.error('Registration.syncCoachRoleToDbIfUniversalInvite', e.message);
  }
}
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.REG_INVITE_OFFER_ACCEPT) {
    ...
    try {
      ...
      try {
        ...
        if (e1 && e1.message === 'This Telegram account is already registered') {
          ...
          if (!inviteCoachId) {
            ...
            await supabase.touchUserInviteRegistrationStarted(chatId);
            await syncCoachRoleToDbIfUniversalInvite(chatId, '');
      ...
      await supabase.touchUserInviteRegistrationStarted(chatId);
      await syncCoachRoleToDbIfUniversalInvite(chatId, inviteCoachId);
      await askInviteRoleIfNeeded(chatId);
    } catch (err) {
      const msg = (err && err.message) ? String(err.message) : '';
      let hint = '';
