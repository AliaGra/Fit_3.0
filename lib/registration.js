...
/** Після імені/прізвища: власник закладу без анкети учня; invite — далі місце/роль. */
...
/** Після статі (або циклу): учень — мета; тренер — без мети, одразу дата народження. */
...
async function askCoachServiceModes(chatId) {
  ...
  await Helpers.sendKeyboard(
    ...
    'Як ти надаєш послуги тренера?\n\n' +
      ...
      '3. Програми — складаєш програми тренувань, учень займається самостійно',
    keyboard
  );
}
...
/** Інвайт від тренера (є coach_id у заготовці) — роль учня без питання. */
...
async function askCoachMoreVenuesQuestion(chatId) {
  ...
  let msg = 'Чи є ще клуби чи студії, де ти тренуєш учнів?';
...
/** Учень або тренер на invite-onboarding — вибір закладів однаковим сценарієм. */
...
/** Після вибору ролі (універсальний інвайт): учень/тренер → заклади; власник закладу → інтро. */
...
async function showInviteVenuesPicker(chatId, page = 0) {
  ...
  const listHeader = isCoachOfflineVenues
    ? `🏢 Обери клуби та студії, у яких ти тренуєш учнів (${place || 'з профілю'}):`
...
async function askInviteVenuesIfNeeded(chatId) {
  ...
  const hubText = isCoachOfflineVenues
    ? `🏢 Обери клуби та студії, у яких ти тренуєш учнів.\nНаселений пункт: ${place || '—'}\n\nОбери дію нижче:`
 ...
 * Роль: учень одразу лише для інвайту тренера; універсальний — роль після prefill (без підтвердження імені/міста).
 */
