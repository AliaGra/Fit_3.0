...
async function finishRegistration(chatId) {
  try {
    ...
    const hasMeasurements = stateData.regWeight != null || stateData.regWaist != null || stateData.regHip != null || stateData.regGlutes != null || stateData.regArm != null || stateData.regNeck != null || stateData.regWrist != null || stateData.regShoulders != null || stateData.regChest != null || stateData.regBodyFatPct != null;
    if (hasMeasurements) {
      const updates = {};
...
async function handleTextMessage(chatId, text) {
  ...
  if (step === CONSTANTS.FSM_STATES.REG_INVITE_INPUT) {
    ...
    try {
      ...
      if (!inviteUser.coachId) throw new Error('Invite code has no coach');
      ...
      else if (msg === 'Invite code has no coach') hint = '\n\nУ коду немає прив’язки до тренера. Попроси тренера створити учня знову.';
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACKS.REG_INVITE_OFFER_READ) {
    ...
    await Helpers.safeSend(chatId, offer);
    return true;
  }
