...
async function proceedAfterRegNameStep(chatId) {
  ...
  if (isInviteOnboarding(st)) await askInviteLocationIfNeeded(chatId);
...
async function askCity(chatId) {
  ...
  await Helpers.safeSend(
    ...
    '🗺️ Укажи місце свого проживання в Україні:\n\n' +
      'Введи область (напиши перші 2–3 букви області, де проживаєш, і потім вибери з появившогося списку):'
  );
}
...
/** Після «Приймаю умови»: ім’я → місце; роль — лише для універсального інвайту (після міста). */
 ...
 * Роль: учень одразу лише для інвайту тренера; універсальний — роль після місця проживання.
 */
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_KEEP) {
    ...
    if (field === 'location' || field === 'city') {
      ...
      else await proceedAfterInviteLocation(chatId);
      return true;
    }
    if (field === 'venues') {
      await askInviteAiIntro(chatId);
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.CITY_OBLAST) {
    ...
    await Helpers.safeSend(
      chatId,
      '🏙️ Введи назву населеного пункту (напиши перші 2–3 букви міста чи села, де проживаєш, і потім вибери з появившогося списку):'
    );
    return true;
  }
