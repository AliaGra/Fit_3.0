...
async function askInviteAiIntro(chatId) {
  ...
  await Helpers.sendKeyboard(
    chatId,
    '🤖 Дані профілю потрібні для AI-аналітики тілобудови та коректного плану тренувань для максимального результату.\n\n' +
...
async function sendGoalWeightRangeHint(chatId) {
  try {
    const st = await State.get(chatId);
    const user = await User.getByChatId(chatId);
    if (!user) return;
    ...
    const analysis = bodyGoals.analyzeGoalsVsCurrentState({ goal_weight: current.weight != null ? current.weight : null }, userCtx, current, 'registration_goal_weight_hint');
    const iw = analysis && analysis.snapshot ? analysis.snapshot.idealWeight : null;
    if (!iw || iw.optMin == null || iw.optMax == null) return;
    const activity = getActivitySummaryForGoals(st || {}, user || {});
    await Helpers.safeSend(
      chatId,
      '🤖 Орієнтир перед вводом бажаної ваги:\n' +
        `За фактичними даними (вік, зріст, запʼястя, активність) рекомендована вилка ваги: <b>${iw.optMin}–${iw.optMax} кг</b>.\n\n` +
        'Твоя активність:\n' +
    ...
    console.error('Registration.sendGoalWeightRangeHint', e.message);
  }
}
...
async function handleTextMessage(chatId, text) {
  ...
  if (step === CONSTANTS.FSM_STATES.REG_BODY_GOALS_CHEST) {
    ...
    await State.update(chatId, { regGoalChest: check.value });
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_BODY_GOALS_FILL) {
    await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_BODY_GOALS_WEIGHT });
    await sendGoalWeightRangeHint(chatId);
