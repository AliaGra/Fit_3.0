...
async function askCoachDocs(chatId) {
  ...
  await Helpers.sendKeyboard(
    chatId,
    '📄 Документи тренера\n\n' +
      'Надішли, будь ласка, документи про освіту (фото або файл).\n' +
...
async function askRegNeck(chatId) {
  ...
  await Helpers.safeSend(chatId, '🧣 Введи обхват шиї (см):\n\nПриклад: 36');
}

async function askRegWrist(chatId) {
  await State.update(chatId, { step: CONSTANTS.FSM_STATES.REG_WRIST });
  await Helpers.safeSend(chatId, '⌚ Введи обхват запʼястя (см):\n\nПриклад: 16');
}
...
async function finishRegistration(chatId) {
  try {
    ...
    const hasMeasurements = stateData.regHeight != null || stateData.regWeight != null || stateData.regWaist != null || stateData.regHip != null || stateData.regGlutes != null || stateData.regArm != null || stateData.regNeck != null || stateData.regWrist != null || stateData.regShoulders != null || stateData.regChest != null || stateData.regBodyFatPct != null;
    if (hasMeasurements) {
      const updates = {};
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.CITY_PICK) {
    ...
    if (stateData && stateData.role === CONSTANTS.ROLES.COACH) {
      ...
      // During onboarding we must collect at least height+weight before showing main menu
      ...
      try {
        ...
        if (e1 && e1.message === 'This Telegram account is already registered') {
          ...
          if (!coachId) {
            ...
            await Helpers.sendKeyboard(chatId, '✅ Код прийнято.\n\nПродовжимо реєстрацію.\n\n👤 Обери свою роль:', keyboard);
            return true;
          }
