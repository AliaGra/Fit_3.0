...
async function startInviteOnboardingAfterAccept(chatId) {
  ...
  if (isInviteCoachBound(st)) {
    ...
    await syncInviteChosenRoleToDb(chatId);
  }
...
/** Записати обрану роль у `users` (після кроку вибору ролі або для інвайту тренера). */
async function syncInviteChosenRoleToDb(chatId) {
  const st = await State.get(chatId);
  if (!st?.role || !isInviteOnboarding(st)) return;
  try {
    await supabase.updateUser(String(chatId), { role: st.role });
  } catch (e) {
    console.error('Registration.syncInviteChosenRoleToDb', e.message);
  }
}
