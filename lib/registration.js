...
async function buildRegBodyGoalsWeightPromptText(chatId) {
  ...
  let hintBlock = '';
  try {
    const analysis = bodyGoals.analyzeGoalsVsCurrentState(
      { goal_weight: current.weight != null ? current.weight : null },
      userCtx,
      current,
      'registration_goal_weight_hint'
    );
    const iw = analysis && analysis.snapshot ? analysis.snapshot.idealWeight : null;
    if (iw && iw.optMin != null && iw.optMax != null) {
      const activity = getActivitySummaryForGoals(st || {}, user || {});
      const comfortLine =
        iw.comfort != null ? `комфортна орієнтирна вага — ${iw.comfort} кг.\n` : '';
      hintBlock =
        '🤖 Орієнтир за віком, зростом, запʼястям і досвідом тренувань:\n' +
        `рекомендована вилка ваги — ${iw.optMin}–${iw.optMax} кг.\n` +
        comfortLine +
        '\n' +
...
const REG_GOAL_RANGE_FACTORS = Object.freeze({
  goal_waist: { min: 0.35, max: 0.65, unit: 'см' },
  goal_hips: { min: 0.45, max: 0.80, unit: 'см' },
  goal_shoulders: { min: 0.50, max: 0.85, unit: 'см' },
  goal_chest: { min: 0.45, max: 0.75, unit: 'см' },
  goal_arm: { min: 0.12, max: 0.30, unit: 'см' }
});

async function buildGoalRangeLine(chatId, goalField) {
  const cfg = REG_GOAL_RANGE_FACTORS[goalField];
  if (!cfg) return '';
  const st = await State.get(chatId);
  const user = await User.getByChatId(chatId);
  const h = st?.regHeight != null ? st.regHeight : user?.height || null;
  if (!h || !isFinite(Number(h))) return '';
  const min = Math.round(Number(h) * cfg.min);
  const max = Math.round(Number(h) * cfg.max);
  return `Орієнтирна вилка: ${min}–${max} ${cfg.unit}.\n`;
}
...
async function askRegBodyGoalsChest(chatId) {
  ...
  await Helpers.sendKeyboard(chatId, `${range}Бажані груди (см)\nПриклад: 90.0\nАбо натисніть «Пропустити»`, keyboard);
}
...
function isInvitePlaceholderUser(u) {
  ...
  return isTechFirstName && !isFilled(last) && !isFilled(city) && !hasRealProfileData;
}
...
async function askInviteNameIfNeeded(chatId) {
  ...
  const u = await User.getByChatId(chatId);
  const first = st?.firstName || u?.firstName || '';
  const last = st?.lastName || u?.lastName || '';
  if (isFilled(first) && isFilled(last)) {
    await askInviteKeepOrEdit(chatId, 'name', 'Імʼя та прізвище', `${first} ${last}`.trim());
    return;
  }
  await askFirstName(chatId);
}
...
async function askInviteCityIfNeeded(chatId) {
  const st = await State.get(chatId);
  if (!isInviteStudentOnboarding(st)) {
    await askCity(chatId);
    return;
  }
  if (st?.inviteEditField === 'city') {
    await State.update(chatId, { inviteEditField: null });
    await askCity(chatId);
    return;
  }
  const u = await User.getByChatId(chatId);
  const city = st?.city || u?.city || '';
  if (isFilled(city)) {
    await askInviteKeepOrEdit(chatId, 'city', 'Місто', city);
    return;
  }
  await askCity(chatId);
}
  ...
  try {
    ...
    if (analysis.errors && analysis.errors.length) {
      ...
      if (bodyGoals.shouldShowAIComment(analysis)) {
        const block = bodyGoals.buildAIInputBlock(analysis, userCtx);
...
async function handleTextMessage(chatId, text) {
  ...
  if (step === CONSTANTS.FSM_STATES.REG_LAST_NAME) {
    ...
    if (isInviteStudentOnboarding(st2)) await askInviteCityIfNeeded(chatId);
  ...
  if (step === CONSTANTS.FSM_STATES.REG_BODY_GOALS_WEIGHT) {
    ...
    const user = await User.getByChatId(chatId);
    const h = st?.regHeight != null ? st.regHeight : user?.height || null;
    const check = bodyGoals.validateGoalField('goal_weight', String(text).trim(), h);
    if (!check.valid) {
...
async function handleCallback(chatId, callbackData) {
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.REG_KEEP) {
    ...
    if (field === 'name') {
      await State.update(chatId, { firstName: u?.firstName || st.firstName || '', lastName: u?.lastName || st.lastName || '' });
      await askInviteCityIfNeeded(chatId);
      return true;
    }
    if (field === 'city') {
      await State.update(chatId, { city: u?.city || st.city || '', regVenueCity: u?.city || st.city || '', regVenueOblast: st.regVenueOblast || '' });
    ...
    if (field === 'gender') {
      await State.update(chatId, { gender: u?.gender || st.gender || '' });
  ...
  if (action === CONSTANTS.CALLBACK_PREFIXES.CITY_PICK) {
    ...
    const city = param || '';
    if (isInviteStudentOnboarding(stateData)) {
      await State.update(chatId, { regVenueOblast: oblast, regVenueCity: city });
      await askInviteVenuesIfNeeded(chatId);
    } else {
  ...
  if (action === CONSTANTS.CALLBACKS.REG_SKIP_LASTNAME) {
    await State.update(chatId, { lastName: '' });
    const st = await State.get(chatId);
    if (isInviteStudentOnboarding(st)) await askInviteCityIfNeeded(chatId);
    else await askGender(chatId);
    return true;
  }
