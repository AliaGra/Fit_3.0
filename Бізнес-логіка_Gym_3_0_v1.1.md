...
**Оновлення:** 17.02.2026. **Мої учні:** у картці учня (профіль за прізвищем) кнопку **«Почати тренування»** прибрано; старт тренування для учня — **тільки** через меню «Тренування» → «Тренування учнів» → вибір учня за прізвищем (lib/coach.js showStudentProfile — ряд кнопок без COACH_TRAIN). **Мій тренер:** у головному меню (учень і тренер) пункт «Мій тренер» → блок з даними тренера (ім'я, місто, Instagram), якщо заповнений `users.coach_id` — для **будь-якої** ролі (раніше тільки для учня); кнопки «Пошук тренера», «Ввести інвайт код», «Головне меню». Після введення інвайт-коду — `User.linkCoachByInviteCode`, оновлення `coach_id`, потім знову екран «Мій тренер» з даними тренера. Тренер може прив'язати собі тренера так само, як учень (та сама колонка `coach_id`). **Розклад:** біля дати скрізь день тижня (нд/пн/…); у «Мій розклад»/календар — ім'я учня (не invite), через `getStudentDisplayName`; вихідний (червоний) тільки за `rest_days` та відпусткою; кнопка «Додати слоти на день» + `createSlotsForCoachForDate`. **Тренування (моє / по плану):** вибір типу «Одиночне (1 вправа)» / «Сет (мін. 2 вправи)» **до** вибору вправ; логіка в `training_plan_exercises` (sets, reps, rest_sec). Раніше: меню тренера (EMOJI.HOME), тренування учнів (план/вільне, запрошені), план вручну (PLAN_EXERCISE_ADD).
