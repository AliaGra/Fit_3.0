...
| 3 | **Мої учні** | ✅ Реалізовано | Список учнів, профіль учня, **Додати учня** (ім'я+прізвище → createStudentByInvite, показ коду). **Записати** — календар → дата → тільки вільні слоти. Історія / Почати тренування — заглушки |
...
| 11 | **Вартість тренувань (Pricing)** | ✅ Реалізовано | Мої учні → «💰 Вартість тренувань» (тариф за замовчуванням / індивідуально для учня), профіль учня → вартість + тип за замовчуванням; при COMPLETED записується PriceCharged (з 1/2/3 за типом). Звіт «Сумма доходів» — заглушка |
