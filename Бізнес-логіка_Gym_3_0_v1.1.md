...
**Оновлення:** 17.03.2026 — **розклад тренера:** календар (легенда 🟡 неділя/відпустка/вихідний), «Мій розклад» — лічильники та списки на **21 день** (без приставки «7 днів» у назвах), пункт «Всі слоти» прибрано; **«Відмітити тренування»** винесено в меню «Розклад»; у «Зайняті» без кнопок «Скасувати/Перенести» + підказка про відмітку; після підтвердження/відхилення в **«Чекають підтвердження»** залишаємось у цьому перегляді (`State.afterCoachConfirmDecline`).  
**Попереднє:** 21.03.2026 — календар 21 день, перенос через календар, «Хочу перерву», зони акценту авто-перехід, BMI-ліміти по статі/віку, fallback floor ваги, конфіденційність підлітків, fix invite FK-constraint, SCH_CR_OK/NO; у календарі учня (запис/перенос): 🟡 REQUESTED, 🔵 BOOKED, легенда кольорів.  
**Попереднє:** 15.02.2026. Тренування учнів: реалізовано (тип Персональна/Спліт/Тріо, вибір учнів кнопками за прізвищем, групи→вправи→вага/повтори; при завершенні — запис price_charged з Pricing → звіт «Сума доходів»). Самостійне тренування тренера: Тренування → «Моє тренування». Бібліотека вправ: у картці вправи медичні поля показуються розшифровано (повні назви MC + ступінь тяжкості) через lib/medicalDecode.js. Раніше: 12.02 — розклад (слоти, скасування/перенос), запис на тренування.
...
| 3 | **Мої учні** | ✅ Реалізовано | Список учнів, профіль учня (в т.ч. **досвід** — 0–3/4–6/6–12 м-ців, більше року), **Додати учня**, **Записати**, вартість, тип тренування. **Досвід** встановлюється тренером вручну, оновлення автоматично. Історія / Почати тренування — заглушки |
| 4 | **Тренування учнів** | ✅ Реалізовано | Тренер: Тренування → «Тренування учнів» → тип (Персональна/Спліт/Тріо) → вибір учнів **кнопками за прізвищем** → групи м'язів → вправи → вага/повтори. Слоти створюються при першому записі підходу (findOrCreateSlotForCoachSession); при [Завершити тренування] — price_charged з Pricing, статус COMPLETED. Звіт «Сума доходів» включає ці тренування. |
| 5 | **Інвайт-код («У мене є код»)** | ✅ Реалізовано | Учень вводить код → activateInvite (user_id/chat_id INVITE_XXX → real ChatID). **Fallback:** якщо учень вже зареєстрований → linkCoachByInviteCode (прив'язка без заміни профілю). FK-fix: replaceInviteWithChatId спочатку INSERT нового user, потім переносить user_body_goals/medical. Нові: getUserByUserId, syncUserChatIdToUserId. |
...
| 7a | **Бажані параметри тіла (user_body_goals)** | ✅ Реалізовано | Тренер: при invite (після зон уникнення) — 5 питань (вага, талія, ягодиці, плечі, груди) з Пропустити; у картці учня — кнопка «Бажані параметри». Реєстрація: перед фінішем — «Вказати бажані параметри?» (Пропустити/Заповнити). Таблиця user_body_goals (Supabase). lib/bodyGoals.js: валідація, аналіз (analyzeGoalsVsCurrentState), блокування/попередження, кеш goals_analysis. |
| 7b | **AI-аналітика тіла (збереження та перегляд)** | ✅ Реалізовано | Текст аналізу тіла зберігається в ai_generated_content (contentType='body_analysis') після реєстрації та при оновленні замірів. Кнопка «🤖 AI-аналітика»: у головному меню учня — показує збережений текст; у картці учня (тренер) — показує аналіз учня. При активації invite — entity_id оновлюється з inviteCode на realChatId. lib/ai/bodyAnalysis.js: generateAndSave, getStoredAnalysis, sendStoredAnalysis. |
| 7c | **Аналіз цілей vs поточний стан** | ✅ Реалізовано | analyzeGoalsVsCurrentState() (lib/bodyGoals.js): Група 1 (без поточних замірів): ІМТ цілі, WH цілі, goal_waist≥goal_hips, teen-рестрикції. Група 2 (з поточними замірами): дельти, великі відхилення, тип фігури, естетичний вектор, apple/pear/v_shape тощо. Результат кешується в user_body_goals.goals_analysis (jsonb). При оновленні замірів — перерахунок + сповіщення тренеру при зміні статусу. lib/ai/goalsVsCurrent.js — AI-переказ для учня. |
| 8 | **Розклад тренувань** | ✅ Реалізовано | Тренер: налаштування шаблону, створення слотів, **Календар 21 день** (легенда кнопок днів: 🟢 сьогодні, 🟡 неділя або відпустка / вихідний у налаштуваннях, ⬜ без слотів), **Мій розклад** (Календар 1-й, далі фільтри з **лічильниками** майбутніх слотів у вікні **21 день**), **«Відмітити тренування»** у меню **«Розклад»** (підтверджені слоти, ✔️ → COMPLETED). Перенос через **календар**; «Хочу перерву»; «Створити відпустку». Учень: запис (календар 21 день: 🟡/🔵), «Мій розклад», «Змінити запис». Перенос: REQUESTED у БД, SCH_CR_OK/NO. **Нагадування** — `/cron/reminders` |
...
**Крок 2:** Показ календаря (**21 день**, сітка 7×3). Позначення: 🟢 сьогодні, 🔴 неділя, ⬜ день без вільних слотів; біля дати — кількість вільних слотів у дужках. Дата без року.
...
**Крок 3:** Показ календаря (**21 день**, сітка 7×3). **Легенда:** 🟢 сьогодні; 🔴 неділя; 🟡 запис очікує підтвердження тренера (REQUESTED); 🔵 запис підтверджено (BOOKED); ⬜ немає вільних слотів. Біля дати — кількість вільних слотів у дужках. Дата без року.
...
**Крок 1:** Учень → `[📅 Розклад]` → `[🔄 Змінити запис]` → обирає слот → `[🔄 Перенести]`

**Крок 2:** Система показує учню **календар 21 день** (та сама легенда, що в «Записатись»: 🟢/🔴/🟡/🔵/⬜) → учень обирає дату → список вільних слотів тренера.

**Крок 3:** Учень обирає новий слот → система **спочатку зберігає** запит у БД (новий слот → REQUESTED, student_id = chatId), потім надсилає запит тренеру. Callback кнопки тренеру: `SCH_CR_OK:{newSlotId}` / `SCH_CR_NO:{newSlotId}` (короткі, щоб не перевищити 64-байт ліміт Telegram).
```
UI Тренера:
📩 **Запит на перенос**
...
[❌ Відхилити (залишити запис)]
```
Учень підтверджує → слот звільняється (у БД статус повертається до вільного для запису). Учень відхиляє → запис залишається. **UI (Mar 2026):** після підтвердження або відхилення учень бачить повідомлення та **підменю «Розклад»** (кнопки «Записатись», «Мій розклад», «Змінити запис»), а не головне меню; текст після підтвердження скасування: «Дякую, скасування підтверджено».

**Перенос:** Тренер обирає слот з учнем → [🔄 Перенести] → **календар 21 день** → обирає дату → слоти дня → обирає новий слот. Перед відправкою учню новий слот зберігається як **REQUESTED** + `studentId` (щоб запит не загубився). У рядку нового слота в БД зберігається **`reschedule_from_slot_id`** = id старого BOOKED-слота (інакче при кількох записах у того ж тренера можна було б звільнити не той час). Inline-кнопки учня — **короткі** `SCH_SR_OK:{newSlotId}` / `SCH_SR_NO:{newSlotId}` (Telegram обмежує `callback_data` 64 байтами). Учень підтверджує або відхиляє; після відповіді — підменю «Розклад». Після вибору слота тренер повертається до **календаря**. Міграція: `supabase_migration_reschedule_from_slot.sql`.
...
**Таблиця:** `coach_schedule_settings` (coach_id PK, rest_days jsonb, workout_duration_min, work_start, work_end, **work_hours_by_weekday** jsonb nullable — інтервал `start`/`end` для кожного дня 0=Пн…6=Нд; якщо NULL — один інтервал work_start/work_end для всіх робочих днів), updated_at.
...
3. **Робочий день:** «За замовчуванням (09:00–21:00)», або **один інтервал для всіх днів** (ввести вручну), або **«Різний час по днях тижня»** — по черзі для кожного дня вводиться інтервал; у підсумку показується таблиця Пн–Нд.
...
**Міграція:** `supabase_migration_coach_schedule_work_hours_by_weekday.sql`.

**Код (Node):** `getWorkHoursForWeekday(settings, dow)` — інтервал `start`/`end` для дня тижня **0=Пн … 6=Нд** (як `rest_days`). Якщо `work_hours_by_weekday` відсутній або для дня немає запису — використовуються `work_start` / `work_end`. У тексті запиту «Введи робочий час для …» при налаштуванні **різного часу по днях** використовується масив **`WEEKDAY_LONG_UA_MON0`** (повна назва дня в порядку Пн–Нд); **`WEEKDAY_LONG_UA`** у коді відповідає **`getDay()`** (неділя=0) — для індексу кнопки **не** застосовувати `WEEKDAY_LONG_UA[dayIdx]`.
...
- Для кожного дня з діапазону: пропускаються дні з rest_days; для робочих днів генеруються слоти від **робочого інтервалу цього дня тижня** (див. `work_hours_by_weekday` або `work_start`/`work_end`) з кроком workout_duration_min. Кожен слот — один ряд у workout_schedule (status = AVAILABLE).
...
### 4.4.6a Додати слоти на один день (тренер)

**Вхід:** Розклад тренувань → **«Додати слоти на день»** (після заповненого шаблону 4.4.5).

**Логіка:** `showAddSlotsForDayCalendar` показує календар на **21 день** (сітка 7 рядків × 3 стовпці, як «Мій календар»). Після вибору дати викликається **`createSlotsForCoachForDate`** — створюються лише відсутні слоти **AVAILABLE** на обрану дату згідно з робочим інтервалом цього дня тижня та `rest_days` / відпусткою (як у 4.4.5–4.4.6).

**Callbacks:** `SCH_ADD_SLOTS_FOR_DAY`, `SCH_ADD_SLOTS_DAY_PICK:{dateKey}`.

**Реалізовано в Node:** так (`lib/schedule.js`).
...
- **Оновлено (Mar 2026):**
  - «📆 Календар (21 день)» — **перший пункт** меню
  - «Мої резерви» → **«Мої перерви»**
  - **Вікно даних:** фільтри **Зайняті / Вільні / Мої перерви / Чекають підтвердження** показують **майбутні** слоти в **21 календарний день** від сьогодні (константа `COACH_MY_SCHEDULE_WINDOW_DAYS`, у коді — «сьогодні + 20 днів»). У назвах кнопок **немає** приставки «7 днів»; на кнопках у **дужках** — кількість слотів за цим вікном.
  - Пункт **«Всі слоти»** прибрано з меню (старий callback `SCH_7_ALL` веде назад у «Мій розклад» для сумісності).
  - **«Зайняті слоти»:** у списку **немає** кнопок «Скасувати» / «Перенести» лише для цього фільтра; показується **максимум зайнятих слотів на один день**; підказка відмітити виконання — через **«Розклад» → «Відмітити тренування»**.
  - **«Вільні слоти»:** **лише текстовий список** у тій самій логіці компоновки, що й **«Зайняті слоти»** — секція **«🕐 Вільні слоти»**, далі заголовки **день тижня + дата (дд.мм)**, під ними рядки **час — Вільний**; **немає інлайн-кнопок по слотах** і **немає пагінації**; показується **максимум вільних слотів на один день**. Дії зі слотом (запис учня, перерва тощо) — через **Календар** або інші фільтри «Мій розклад». Код: `showCoach7DaysView` (`lib/schedule.js`), `filter === 'available'`. Док. проєкту: `README.md`, `CALLBACK_FSM_MODULE_MATRIX.md`, цей підрозділ.
  - **«Чекають підтвердження»:** після **підтвердження** або **відхилення** запиту (`SCH_CONF` / `SCH_DECLINE`) екран **залишається** у цьому перегляді (`State.afterCoachConfirmDecline`). При інших переходах контекст скидається.
  - Слот статусу RESERVED позначається кнопкою **«🍔 Хочу перерву»** (раніше «Резерв»)
  - При позначенні перерви: повідомлення «позначено як перерва» + **1.5 сек пауза** перед оновленням
  - Дати слотів відображаються **без року** (формат `04.05, сб 10:00`)

### 4.4.8 Меню «Розклад» (тренер)

- «📌 Створити резерв» → **«🍔 Створити перерву»**
- «🏖 Відпустка» → **«🏖 Створити відпустку»**
- **«✔️ Відмітити тренування»** (`SCH_MARK_TRAINING` / `SCH_MARK_TRAINING:{page}`): список підтверджених записів на **21 день** з кнопками ✔️ → `SCH_COMPLETE` (завершення тренування, COMPLETED + ціна); після відмітки з цього екрана повторно **«Відмітити тренування»** (`State.afterCompleteSlot=mark_training`); при виході в інші екрани розкладу — скидання.
- **Календар тренера** (21 день з «Мій розклад»): легенда кнопок днів — 🟢 сьогодні; 🟡 **неділя** або **відпустка** або **вихідний день за налаштуваннями**; ⬜ немає слотів; число в дужках — зайняті слоти на день; на кнопці дня — **дд.мм** + скорочений день тижня (`formatDateShortWithWeekday`). У тексті календаря: рядок **⏳ На підтвердження:** — кількість слотів REQUESTED у вікні **21 день** або **—** якщо немає. Неділя візуально **жовта** (🟡). У календарі відпустки неділя також **🟡** (не червона).
- **Слоти обраного дня** (`showCoachDaySlots`): у **тексті** — рядки для **зайнятих** (BOOKED — учень), **перерви** (RESERVED), **на підтвердженні** (REQUESTED); рядків «Вільний» у тексті **немає**. Далі «Обери слот:» — у **інлайн-кнопках** усі майбутні слоти дня; на кнопках використовується **`formatSlotTimeOnly`** (лише час), без `formatSlotDateTime`. Без пагінації.
...
### 5.4 Таблиця user_body_goals (бажані параметри тіла)

**Призначення:** Зберігання цільових (бажаних) параметрів тіла учня: вага, талія, ягодиці, плечі, груди. Використовується для відображення прогресу до цілі та розрахунку орієнтовного терміну досягнення.

**Структура (Supabase):**

| Колонка | Тип | Опис |
|---------|-----|------|
| chat_id | text PK | ChatID учня (REFERENCES users(chat_id) ON DELETE CASCADE) |
| goal_weight | decimal(5,2) | Бажана вага (кг) |
| goal_waist | decimal(5,1) | Бажана талія (см) |
| goal_hips | decimal(5,1) | Бажані ягодиці (см) |
| goal_shoulders | decimal(5,1) | Бажані плечі (см) |
| goal_chest | decimal(5,1) | Бажані груди (см) |
| set_by_coach | text | ChatID тренера (NULL якщо вказав сам користувач при реєстрації) |
| updated_at | timestamptz | Час останнього оновлення |

**Міграція:** `supabase_migration_user_body_goals.sql`

**Потоки введення:**

1. **Тренер при invite (додавання учня):** Після блоку «Є зони, які НЕ розвиваємо?» — 5 питань (вага → талія → ягодиці → плечі → груди), кожне з кнопкою «Пропустити». Без кнопки «Пропустити весь блок». FSM: `invite_body_goals_weight` … `invite_body_goals_chest`. Router: кроки `invite_*` маршрутизуються в Coach.handleTextMessage.

2. **Тренер у картці учня:** Кнопка «🎯 Бажані параметри» → 5 питань з Пропустити. Валідація: обов'язково зріст учня; якщо немає — «Спочатку вкажіть зріст учня». FSM: `coach_body_goals_weight` … `coach_body_goals_chest`.

3. **Реєстрація (самореєстрація):** Перед `finishRegistration` — «Вказати бажані параметри тіла?» [Пропустити] [Заповнити]. Заповнити → 5 питань з Пропустити. Без валідації зросту; після збереження — нагадування «Для точнішої валідації заповніть зріст у профілі». FSM: `reg_body_goals_choice`, `reg_body_goals_weight` … `reg_body_goals_chest`.

**Модуль:** `lib/bodyGoals.js` — `validateGoalField`, `calcDeltaAndTimeline`, `formatGoalsSummaryForCoach`, `formatGoalsSummaryForStudent`, `saveBodyGoals`, `showGoalsToStudent`. `lib/supabase.js` — `upsertBodyGoals`, `getBodyGoals`, `getLatestMeasurementsForGoals`.

================================================================================================

## 5A. ІДЕАЛЬНА ВАГА ТА BMI-ОБМЕЖЕННЯ (Mar 2026)

### 5A.1 Формула ідеальної ваги

Базова формула Devine + три коефіцієнти:

```
base       = 45.5 (ж) або 50.0 (ч)
inches     = max(0, (зріст_см − 152.4) / 2.54)
idealBase  = base + 2.3 × inches
adjIdeal   = max(idealBase × buildFactor × ageFactor × expFactor, heightFloor)
```

**Fallback floor при зрості < 152.4 см:** `heightFloor` = 43.0 кг (ж) / 47.0 кг (ч)
...
|--------|---------|
| min | adjIdeal × 0.95 |
| comfort | adjIdeal × 1.00 |
| athletic | adjIdeal × 1.05 |
| max | adjIdeal × 1.10 |

### 5A.2 Коефіцієнти

**buildFactor (тілобудова):** asthenic 0.93 / normosthenic 1.00 / hypersthenic 1.07
...
**expFactor (стаж тренувань):**

| Стаж | Жінки | Чоловіки | Підлітки ≤17 (кеп) |
|------|-------|---------|-------------------|
| < 90 днів | 1.000 | 1.000 | — |
| 90–365 | 1.040 | 1.060 | ж: 1.030, ч: 1.040 |
| 366–730 | 1.065 | 1.090 | ж: 1.030, ч: 1.040 |
| > 730 | 1.090 | 1.120 | ж: 1.030, ч: 1.040 |

### 5A.3 Обмеження ціль-ваги (goal_weight)

| Правило | Значення | Файл |
|---------|---------|------|
| BMI мін (жінки) | **17.5** | `getBMIMinForGoal('female', false)` |
| BMI мін (чоловіки) | **18.0** | `getBMIMinForGoal('male', false)` |
| BMI мін (підлітки ≤17) | **16.5** | `getBMIMinForGoal(*, true)` |
| BMI макс | **29.9** | `BMI_RANGES.goal_max` |

Порушення → `result.errors` (блокування збереження цілі).

### 5A.4 Конфіденційність підлітків

- Учень-підліток (age ≤ 17): у AI-аналізі **не показуються числові діапазони** ваги (тільки «розраховано, цифри доступні тренеру»)
- Тренер при перегляді аналізу учня-підлітка: **бачить** повні діапазони min/comfort/athletic/max

**Реалізовано в:** `lib/bodyMetrics.js` (`getBMIMinForGoal`, `BMI_RANGES.goal_max`), `lib/bodyGoals.js`, `lib/ai/bodyAnalysis.js` (`calcIdealWeight` → floor, `buildBodyAnalysisBlock` → teen check).
