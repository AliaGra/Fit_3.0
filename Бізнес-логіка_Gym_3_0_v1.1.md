...
**Оновлення:** 17.02.2026. Меню тренера: на всіх кнопках «Головне меню» — іконка домика (🏠 EMOJI.HOME). Тренування учнів: при наявності активного плану у учня — показ вибору дня плану або «Вільне тренування» (без підбору вправ по групах); у списку учнів показуються також запрошені (INVITE_*) з позначкою «(запрошення)», запис тренування для них зберігається в bot_training_data з chat_id=INVITE_xxx; при активації інвайту (replaceInviteWithChatId) оновлюється й bot_training_data.chat_id. План вручну: при додаванні вправи — вибір типу виконання: «Кілька підходів (сети)» або «Одиночне виконання» (callback PLAN_EXERCISE_ADD). Раніше: 15.02 — тренування учнів (тип Персональна/Спліт/Тріо, групи→вправи), бібліотека вправ (медичні поля розшифровано).
