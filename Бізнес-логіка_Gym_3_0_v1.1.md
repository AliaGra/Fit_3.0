...
**Оновлення:** 03.2026. **Історія тренувань:** модуль реалізовано (замість заглушки): три точки входу (учень, тренер своя, тренер за учня), фільтри (всі / за групою / за вправою), підфільтри (попереднє тренування, останні N), список дат, деталі тренування з навігацією; lib/history.js, нові функції в supabase.js. **Акцент-зони:** підпис glutes змінено на «Ягодиці»; у меню зон уникнення зони з акценту не показуються. — 02.2026: **План тренувань** — акцент-зони, зони уникнення, превʼю спліту (авто та вручну); пресети сетів (sets/reps/rest за goal/level); ACCENT_MAP, training_plans (accent_zones, avoid_zones, split_config). **Тренування без плану:** вибір підходів/кругів 3/4/5, автозавершення, «Завершити тренування» після вправи/сету. **Пошук вправ** — тільки з обраної групи. **Тренування учнів** — список учнів + Спліт/Тріо, при виборі учня тренування одразу. **План (інше):** рівень з картки, картки вправ, градація group_level3, без Markdown. Деталі — **Зміни_логіки_та_функціоналу.md**, розд. 7, 8, 9.
...
| 3 | **Мої учні** | ✅ Реалізовано | Список учнів, профіль учня (в т.ч. **досвід** — 0–3/4–6/6–12 м-ців, більше року), **Додати учня**, **Записати**, вартість, тип тренування. **Досвід** встановлюється тренером вручну, оновлення автоматично. У картці учня **немає** кнопки «Почати тренування» — вона лише в меню «Тренування» → «Тренування учнів» → вибір учня. Кнопка **«📊 Історія»** відкриває історію тренувань учня (модуль Історія тренувань). |
| 4 | **Тренування учнів** | ✅ Реалізовано | Тренер: Тренування → «Тренування учнів» → **список учнів** + «Спліт (2 учні)»/«Тріо (3 учні)»; при виборі учня — тренування стартує одразу (без вибору типу Персональна). При виборі Спліт/Тріо — вибір 2 або 3 учнів, далі тренування. У списку також **запрошені** — прізвище + «(запрошення)»; для них запис у bot_training_data з chat_id=INVITE_xxx; слоти розкладу не створюються. Якщо обрано **одного** учня і в нього є **активний план** — показ вибору дня плану або «Вільне тренування»; інакше — групи м'язів → вправи → вага/повтори. Слоти (для активних учнів) при першому записі (findOrCreateSlotForCoachSession); при [Завершити тренування] — price_charged, COMPLETED. Звіт «Сума доходів» включає ці тренування. |
| 5 | **Інвайт-код («У мене є код»)** | ✅ Реалізовано | Учень вводить код → activateInvite (replaceInviteWithChatId: user_id/chat_id INVITE_XXX → real ChatID; також оновлення workout_schedule.student_id та **bot_training_data.chat_id** для записів, зроблених до активації), потім головне меню |
| 5а | **Мій тренер** | ✅ Реалізовано | У головному меню (учень і тренер): «Мій тренер» → якщо є `coach_id` — блок ім'я/місто/Instagram тренера (для будь-якої ролі); кнопки «Пошук тренера», «Ввести інвайт код», «Головне меню». Введення інвайт-коду → linkCoachByInviteCode (оновлення users.coach_id, markInviteAsUsed) → знову showMyCoachMenu. Тренер може мати свого тренера (той самий users.coach_id). lib/coach.js showMyCoachMenu, lib/menu.js COACH_INVITE_INPUT, lib/user.js linkCoachByInviteCode. |
| 6 | **Бібліотека вправ** | ✅ Реалізовано | Таблиця в Supabase: group_level1/2/3, name_ua, name_ru, vid, difficulty, medical_contraindications, medical_limitations, safe_for, modifications, alternatives, safety_notes. Ієрархія Верх/Низ/Кор/Кардіо → групи → підкатегорії. У **картці вправи** медичні поля показуються **розшифровано** (повні назви станів + ступінь тяжкості) через lib/medicalDecode.js; підтримка форматів MC001, MC001/mild, MC003 (moderate-severe), кириличні МС→латиниця. Експорт з Sheets → імпорт. |
...
| 8 | **Розклад тренувань** | ✅ Реалізовано | Тренер: налаштування шаблону (дні відпочинку, тривалість, робочий день), створення слотів (авто на 7–30 днів, + кнопка **«Додати слоти на день»** — календар → дата → createSlotsForCoachForDate), Мій розклад з позначкою «Вільний», підтвердити/відхилити/завершити, записати учня (календар → дата → вільні слоти), скасування та перенос (запит учню). У всіх датах — **день тижня** біля дати (нд/пн/…); у «Мій розклад»/календар — **ім'я учня** (не invite, getStudentDisplayName); **червоний (вихідний)** тільки за `rest_days` та відпусткою. Учень: записатись (календар → дата → вільні слоти), Мій розклад, скасування та перенос (запит тренеру). **Нагадування учням** — endpoint `/cron/reminders`. lib/schedule.js: formatSlotDateTime, formatDateShortWithWeekday, showCoachCalendar. |
| 9 | **Історія тренувань** | ✅ Реалізовано | Три точки входу: учень (Головне меню → «📊 Історія»), тренер своя («📊 Моя історія»), тренер за учня (картка учня → «📊 Історія»). Фільтри: всі тренування / за групою м'язів / за вправою. Підфільтри: попереднє тренування, останні N (1–100). Список дат у форматі «📅 дата — групи (N вправ, M підходів)». Деталі тренування з навігацією Попереднє/Наступне. Джерело даних: bot_training_data. lib/history.js, getWorkoutDates, getWorkoutByDate, getWorkoutDatesByMuscleGroup, getWorkoutDatesByExercise, getExercisesTrainedByStudent. Деталі — Зміни_логіки_та_функціоналу.md, розд. 9; ТЗ_Історія_тренувань_v1_1_FIT3.md. |
...
| 12 | **Самостійне тренування (тренер)** | ✅ Реалізовано | Тренер: Тренування → «Моє тренування» → вибір типу «Одиночне»/«Сет» → групи м'язів → вправи → **вибір кількості підходів (3/4/5) або кругів (3/4/5)** → вага/повтори. Без кнопок «Додати круг»/«Додати підхід»; автозавершення після обраної кількості; «Завершити тренування» тільки після вправи/сету. Пошук «Ввести назву» — тільки в межах обраної групи. lib/training.js: plannedSetsCount, plannedRoundsCount, showSetRoundsChoice, showSingleSetsChoice, askTrainingInputDataWithPlannedSets. |
| 13 | **Почати тренування (учень)** | ✅ Реалізовано | Учень: «Моє тренування» → перевірка активного плану → вибір дня → вправи з sets/reps/rest (тип одиночне/сет з training_plan_exercises), ввід ваги/повторів → запис у bot_training_data; кнопки «Завершити вправу» / «Завершити тренування». lib/training.js: startStudentPlanWorkout, STUDENT_PLAN_DAY, TRAINING_STUDENT_PLAN_INPUT. План вручну: при додаванні вправи — **пресети сетів** (sets/reps/rest за goal та level з SET_PRESETS) або «Свій варіант» (PLAN_SETS_PRESET, PLAN_SETS_CUSTOM); зберігання в training_plan_exercises (sets, reps, rest_sec). Акцент-зони та зони уникнення — перед збереженням плану (авто/вручну); превʼю спліту з підтвердженням. |
...
- **Етап 3:** SQL training_plans (uuid), training_plan_exercises (11.2, 11.3). planGenerator.js: generateTrainingPlan(). trainingPlan.js: «Програма тренувань» → список → «Новий план» → «Авто-підбір» або «Вручну» (назва → ціль → **рівень з картки учня** → термін ревізії → дні → збереження). **Після Авто-підбір** план відображається одразу з деталями (дні, вправи) та кнопками Додати вправи, Картки вправ, Активувати, Видалити, До списку планів; відображення без Markdown (надійність Telegram API). У перегляді плану — **«Картки вправ»** → список вправ → відкриття картки з бібліотеки, **«Назад до плану»**. При додаванні вправ — **градація по group_level3** (підкатегорії 3-го рівня). Активувати/Видалити.
...
- **Ревізія плану (9.1):** термін ревізії **revision_weeks** за рівнем: beginner 10 тиж., intermediate 7 тиж., advanced 5 тиж. (константа `REVISION_WEEKS_BY_LEVEL` у lib/supabase.js). При активації плану: `valid_until = now + revision_weeks * 7` днів, `activated_at = now`; при створенні плану зберігаються `revision_weeks` та опційно `parent_plan_id`. Cron GET /cron/plan-revision — нагадування тренеру «Час оновити план для [Ім'я]». `getPlansDueForRevision`, `markPlanRevisionReminderSent`, planRevisionReminders.js. Допоміжні функції для генерації плану: `getPlanExerciseIdsWithVid(planId)`, `getPreviousPlanIsolationExerciseIds(studentChatId)` (правило Б Anti-Repeat), `getExerciseUsageCountForStudent(studentChatId)` (правило В — зважений рандом). **Anti-Repeat (5.5):** правило А — одна вправа не повторюється в різних днях одного плану для тієї самої group_level2; правило Б — ізоляційні вправи попереднього плану учня виключаються; правило В — зважений вибір за кількістю появ в історії (вага 1/(count+1)). **Відбір для жінок (5.3):** `getSplitSchemeAndDays(level, daysPerWeek, gender)` — для female інший порядок груп (Ноги, Сідниці вище), більше вправ на низ. lib/planGenerator.js: `weightedSample`, `pickExercisesForDay(..., options)` з excludeExerciseIdsByGroup, previousPlanIsolationIds, usageCountByExerciseId.

**AI (реалізовано, опційно):** при увімкненому `AI_ENABLED` та налаштованому `OPENAI_API_KEY`: персоналізовані коментарі тренера в планах (planGenerator + lib/ai/planComments.js), розумні нагадування (reminders.js + lib/ai/smartReminder.js), аналіз невиконання вправ після тренування (lib/ai/failureAnalysis.js). Таблиця `ai_generated_content`, міграція `supabase_migration_ai_content.sql`. Деталі — **Підключення_AI_покроково.md**, **AI_Integration_FIT3_Basic.md**.
...
**Змінні середовища (Railway):** `BOT_TOKEN`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `REMINDER_CRON_SECRET`, `REMINDER_HOURS_BEFORE`. Для AI (опційно): `OPENAI_API_KEY`, `AI_ENABLED=true`, `AI_MODEL`, `AI_MAX_TOKENS`, `AI_TIMEOUT_MS` — див. **Підключення_AI_покроково.md** та **DEPLOY.md**.

**Нагадування учням:** Endpoint `GET /cron/reminders?secret=XXX` — надсилає учням повідомлення за N годин до тренування (lib/reminders.js). При `AI_ENABLED=true` текст нагадування може бути персоналізований (lib/ai/smartReminder.js). Запускати через cron-job.org (кожну годину). Таблиця `reminders_sent` — запобігає дублікатам. **Ревізія плану:** `GET /cron/plan-revision?secret=XXX` — нагадування тренеру про оновлення плану (lib/planRevisionReminders.js).

### Архітектура: Railway + Supabase; GAS лише для бібліотеки вправ

**Продакшен-бот:** працює на **Node.js**, розгорнутий на **Railway** ([railway.com](https://railway.com/)). Webhook Telegram приймає саме цей сервіс. Уся бізнес-логіка (реєстрація, розклад, профіль, звіти тощо) виконується в Node (каталог `lib/`).

**Дані:** майже всі дані зберігаються в **Supabase** (користувачі, розклад, ціни, логи, нагадування, заміри, бібліотека вправ тощо). Жодні робочі дані бота не зберігаються в Google Таблицях для продакшену.

**Google Apps Script (GAS):** використовується **тільки для бібліотеки вправ**. В GAS є одна Google Таблиця (лист) з вправами — її використовують, коли вносять або редагують нові вправи. Після змін дані експортують з Sheets і імпортують у Supabase (таблиця `exercise_library`). Інші модулі в GAS (Router.gs, Schedule.gs, Menu.gs, Registration.gs тощо) **не використовуються** для робочого бота і не обробляють webhook; бот у продакшені не працює на GAS.

Підсумок: **Railway (Node) + Supabase** — робоча архітектура; **GAS + один лист Google Sheets** — лише інструмент для ведення бібліотеки вправ з подальшим імпортом у Supabase.
...
**Актуальна реалізація (Node.js, 03.2026):** після введення дати народження — стать, ціль, досвід, тип тренування, дні на тиждень; потім медичний профіль (або пропуск); потім **параметри учня** (вага, зріст, талія, сідниці — або пропуск); потім **зони акценту та уникнення** (на що робимо акцент, що не розвиваємо). Ці зони зберігаються в `users.accent_zones`, `users.avoid_zones` і використовуються при створенні авто-плану для учня. Деталі — розд. 11 у «Зміни_логіки_та_функціоналу.md».

---

### 1.4 Вибір тренера тренером (реалізовано)

**Мета:** дозволити тренеру обрати свого тренера (як у студента). У таблиці `users` для тренера використовується та сама колонка `coach_id`, що й для учня.

**UI (головне меню — учень і тренер):** пункт «👨‍🏫 Мій тренер» → підменю:
- Якщо є `coach_id`: блок з даними тренера (ім'я, місто, Instagram) — **для будь-якої ролі** (учень або тренер).
- Кнопки: «Пошук тренера», «Ввести інвайт код», «Головне меню».

**Алгоритм прив'язки по інвайт-коду:**
1. Користувач (учень або тренер) обирає «Ввести інвайт код» → FSM стан `coach_invite_input`.
2. Введення коду → `User.linkCoachByInviteCode(realChatId, code)`: оновлення `users.coach_id`, `markInviteAsUsed(code)`.
3. Показати знову екран «Мій тренер» з даними прив'язаного тренера.

**Реалізація:** lib/coach.js (showMyCoachMenu — показ блоку тренера за умови `if (coachId)` без перевірки ролі), lib/menu.js (COACH_INVITE_INPUT, стан coach_invite_input), lib/user.js (linkCoachByInviteCode).
...
**У Node/Supabase** (`supabase.replaceInviteWithChatId`): оновлюються також таблиці `workout_schedule` (student_id) та **bot_training_data** (chat_id), щоб записи тренувань, зроблені для запрошеного учня до активації коду, прив’язати до реального ChatID.
...
**Додаткові колонки (Supabase, з 03.2026):** у таблиці `users` можуть бути додані **accent_zones** (TEXT[]), **avoid_zones** (TEXT[]) — зони акценту та уникнення для підбору плану (ключі: glutes, legs, thighs, abs, arms, back, shoulders, full). Міграція: `supabase_migration_users_accent_avoid_zones.sql`. Заповнюються при реєстрації (інвайт тренером — обов’язковий блок після параметрів; самореєстрація — опційно з можливістю пропустити і заповнити в профілі) та в профілі («Редагувати дані» → «Зони акценту та уникнення»). Використовуються при авто-генерації плану (перевірка повноти профілю і передача в generateTrainingPlan).
...
### 2.3 Досвід учня (Experience)

**Призначення:** Відображення рівня досвіду учня (0–3 міс., 4–6 міс., 6–12 міс., більше року). Встановлюється тренером вручну; статус обчислюється автоматично від дати встановлення.

**Supabase:** У таблиці `users` поле `experience_start_date` (timestamptz, nullable) — дата, від якої рахується досвід. Встановлює тренер при виборі рівня (0–3 / 4–6 / 6–12 м-ців).

**Правила відображення статусу (за днями):**

| Дні (від experience_start_date) | Статус на екрані   |
|----------------------------------|--------------------|
| 0–90                             | 0–3 м-ців          |
| 91–180                           | 4–6 м-ців          |
| 181–365                          | 6–12 м-ців         |
| 366+ днів **від реєстрації** (created_at) | Більше року |

**Як встановлюється:** Тренер у картці учня (Мої учні → учень → «📅 Досвід») обирає: 0–3 м-ців, 4–6 м-ців, 6–12 м-ців або **Більше року**. Система зберігає `experience_start_date`:
- **0–3 м-ців:** `experience_start_date = now` (0 днів).
- **4–6 м-ців:** нижня межа 91 день — `experience_start_date = now - 91 днів`.
- **6–12 м-ців:** нижня межа 181 день — `experience_start_date = now - 181 день`.
- **Більше року:** `experience_start_date = now - 366 днів` (відображається як «Більше року»).

**Автоматичне оновлення:** Від дня встановлення досвіду тренером дні рахуються автоматично. Після досягнення **366 днів від реєстрації учня** (created_at) статус завжди показується як **«Більше року»**, незалежно від `experience_start_date`.

**Приклад:** Тренер встановив учню 6–12 м-ців (зберігається дата «now − 181 день»). Поки від реєстрації не минуло 366 днів, відображається «6–12 м-ців». Після 366 днів від реєстрації — автоматично «Більше року».

---

### 2.4 Поле CoachID (зв'язок Тренер-Учень)
...
### 2.5 Instagram та CalendarId (тільки для тренерів)
...
**Спосіб 2 — із режиму тренування (без попереднього запису):** тренер веде учня (картка учня → Почати тренування або Тренування учнів → тип → вибір учнів). У списку учнів показуються також **запрошені** (прізвище + «(запрошення)»); для них запис тренування зберігається в bot_training_data з chat_id = INVITE_xxx, слоти розкладу не створюються; при активації інвайту (replaceInviteWithChatId) оновлюється bot_training_data.chat_id на реальний ChatID. Якщо обрано **одного** учня і в нього є **активний план** — показується вибір дня плану або «Вільне тренування»; при виборі дня тренування ведуться за планом (вправи з плану). При старті сесії в State зберігаються `trainingStartedAt` та `scheduleSlotIds`. При **першому** записі підходу по кожному **активному** учню (не INVITE_*) система створює слот у WorkoutSchedule за потреби. При [🏁 Завершити тренування] для слотів виставляється COMPLETED. Для спліт/тріо: один слот на кожного учня, одна дата/час.
...
**Сет (мін. 2 вправи) — потік формування списку, потім тренування (круговий сет):**
```
1. Вибір типу: [Сет (мін. 2 вправи)]
2. Вибір першої вправи (група м'язів → вправа)
3. Програма пропонує: [Додати ще вправу]
4. Користувач додає другу (і далі) вправу через групи/вправи
5. Коли в списку уже ≥ 2 вправи — подвійний вибір: [Додати ще вправу] або [Перейти до тренування]
6. [Перейти до тренування] → на кожному кругу: виконується по 1 підходу кожної вправи зі списку (вправа 1 → вправа 2 → … → круг завершено)
7. Після круга: [Додати круг N+1] або [Завершити сет]
```
Тобто спочатку список вправ, потім введення даних **кругами**: у кожному крузі — по одному підходу кожної вправи (як у п. 3.6 Круговий сет).
...
**Крок 2:** Показ календаря (28 днів, сітка 7×4). Позначення: 🟢 сьогодні, 🔴 неділя, ⬜ день без вільних слотів; біля дати — кількість вільних слотів у дужках.

**Крок 3:** Тренер обирає дату → система показує **тільки вільні (AVAILABLE)** слоти на обрану дату.

**Крок 4:** Тренер обирає слот → Status = REQUESTED, StudentID = studentChatId (оновлення існуючого слоту).

**Крок 5:** Відправка запиту учню
```
UI Учня:
...
**Крок 6:** Підтвердження учнем → Оновлення статусу на BOOKED
...
**Крок 2:** Перевірка наявності тренера (user.coachId). Якщо немає — «У тебе немає призначеного тренера».

**Крок 3:** Показ календаря (28 днів, сітка 7×4). Позначення: 🟢 сьогодні, 🔴 неділя, ⬜ день без вільних слотів; біля дати — кількість вільних слотів.

**Крок 4:** Учень обирає дату → система показує **тільки вільні (AVAILABLE)** слоти тренера на обрану дату.

**Крок 5:** Учень обирає слот → Status = REQUESTED, StudentID = chatId учня; відправка запиту тренеру на підтвердження.
...
### 6.4 Медичні поля та розшифровка в картці вправи

**Колонки exercise_library:** medical_contraindications, medical_limitations, safe_for, modifications, alternatives, safety_notes (текст з кодами MC001…MC025 та опційно severity: mild, stage1, тощо).

**Відображення в боті:** У картці вправи (lib/library.js → showExerciseDetail) значення цих полів **розшифровуються** через lib/medicalDecode.js: коди MC замінюються на повні назви українською (наприклад MC003 → «Плечі (імпінджмент, ротаторна манжета»), ступінь тяжкості — на підпис (mild → «Легка ступінь», stage1 → «Стадія 1»). Підтримуються формати: окремий код (MC001), код/ступінь (MC001/mild), ступінь у дужках (MC003 (moderate-severe)), примітки після двокрапки (MC003 (mild): кут 30°). Кириличні «МС» у даних автоматично приводяться до латинських «MC». Джерело маппінгу: документ «Расшифровка тяжести заболевания» та таблиця MC-кодів.
...
**Реалізація в Node/Supabase:** таблиці `training_plans` (plan_id uuid, coach_id, student_id, plan_name, goal, level, split_scheme, days_per_week, is_active, **revision_weeks**, **parent_plan_id**, **activated_at**, valid_until, revision_reminder_sent_at, generation_type, **accent_zones** TEXT[], **avoid_zones** TEXT[], **split_config** JSONB) та `training_plan_exercises` (plan_id, exercise_id, …). Міграція: supabase_migration_accent_zones.sql. **lib/constants.js:** ACCENT_MAP — ключі зон (glutes, legs, thighs, abs, arms, back, shoulders, full) змаповані на масиви **group_level2** з exercise_library (лише 7 груп: Спина, Груди, Плечі, Сідниці, Ноги, Прес, Руки); ACCENT_LABELS, ACCENT_ZONES_ORDER, AVOID_ZONES_ORDER, ACCENT_DAYS_COUNT, SET_PRESETS (пресети сетів за goal/level). **lib/supabase.js:** `insertTrainingPlan` приймає accent_zones, avoid_zones, split_config; `getPlanWithExercises` повертає splitConfig для підписів днів; REVISION_WEEKS_BY_LEVEL, setPlanActiveForStudent, getPlanExerciseIdsWithVid, getPreviousPlanIsolationExerciseIds, getExerciseUsageCountForStudent — без змін. **lib/planGenerator.js:** `generateSplitWithAccent(daysPerWeek, accentZones, avoidZones, gender, level)` — формує splitConfig з урахуванням акцентів/уникнення; `generateTrainingPlan` приймає options.splitConfig, options.accentZones, options.avoidZones; getSplitSchemeAndDays, weightedSample, pickExercisesForDay, generateTrainingPlan — як раніше. **lib/trainingPlan.js:** askAccentZones, askAvoidZones, showSplitPreview, showSetsPreset; callback PLAN_ACC_*, PLAN_AVD_*, PLAN_SPL_*, PLAN_SETS_*. Модулі: lib/trainingPlan.js (UI тренера, акцент/спліт/пресети), lib/training.js, lib/medicalFilter.js, lib/planRevisionReminders.js. Медичний профіль: lib/medicalProfile.js, user_medical_conditions. Таблиця `plan_adjustments` (міграція) — для логування змін плану.
...
**Структура (Supabase):** plan_id, coach_id, student_id, plan_name, goal, level, split_scheme, days_per_week, is_active, revision_weeks, parent_plan_id, activated_at, valid_until, revision_reminder_sent_at, generation_type, **accent_zones** (TEXT[]), **avoid_zones** (TEXT[]), **split_config** (JSONB). Еталон A–G — історична звірка; у продакшені використовуються всі колонки вище.
...
| — | accent_zones | Зони акценту (ключі: glutes, legs, …) |
| — | avoid_zones | Зони уникнення |
| — | split_config | Розподіл груп по днях (підтверджений тренером) |
...
**Авто-підбір (реалізація Node.js, 03.2026):** перед показом екрану авто-підбору та при натисканні «Генерувати за профілем учня» перевіряється повнота профілю учня: мета (goal), дата народження (birthDate), зони акценту (accent_zones не порожні). Якщо щось відсутнє — показується повідомлення з кнопками «До картки учня», «План вручну», «До списку планів»; генерація не запускається. Якщо профіль повний — у generateTrainingPlan передаються accentZones та avoidZones з профілю учня (users). Деталі — розд. 11 у «Зміни_логіки_та_функціоналу.md».
