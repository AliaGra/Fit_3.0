...
- **Календар тренера** (21 день з «Мій розклад»): легенда кнопок днів — 🟢 сьогодні; 🟡 **неділя** або **відпустка** або **вихідний день за налаштуваннями**; ⬜ немає слотів; число в дужках — зайняті слоти на день; на кнопці дня **немає календарної дати** (лише скорочений день тижня). Підпис екрана — короткий (без текстових списків дат). Неділя візуально **жовта** (🟡). У календарі відпустки неділя також **🟡** (не червона).
- **Слоти обраного дня** (`showCoachDaySlots`): у **тексті** повідомлення — **повний перелік** усіх майбутніх слотів дня (час, учень / Вільний / перерва / очікує); у **інлайн-кнопках** — лише **час** (без дати); усі слоти дня на одному екрані (без пагінації за `SLOTS_PER_DAY_PAGE`).
