...
| 14 | **Довідник закладів (venues)** | ✅ Реалізовано (MVP + розширення 2026-04) | **Основний бот:** «Клуби, студії» — гео + текстовий пошук (транслит), фільтр типу організації; хаб з «Мої заклади», пошук нового закладу, deep link `venue_<id>`; картка закладу з тренерами (`PVCH`), deep link `pvch_<chatId>`; прив’язка тренера в профілі; реєстрація — пропозиція обрати заклад. **Адмін-бот:** створення/картка закладу, години, розклад групових, amenities, локальні назви групових, довідник цін (без оплат у основному боті). Деталі та кроки: **`VENUES_DIRECTORY_IMPLEMENTATION_PLAN.md`**, зміни по датах — **`Зміни_логіки_та_функціоналу.md`**. |
