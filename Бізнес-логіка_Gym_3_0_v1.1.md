...
**Оновлення:** 12.02.2026. Розклад: скасування/перенос (4.4.1–4.4.4), налаштування шаблону слотів (4.4.5), автоматичне створення слотів (4.4.6), меню Мій розклад (4.4.7) — реалізовано. **Записати/Записатись:** спочатку календар (28 днів), обрання дати, потім показ тільки вільних (AVAILABLE) слотів на обрану дату — реалізовано.
...
| 10 | **Звіти тренера** | ✅ Реалізовано | **Кількість тренувань** — період 1–365 днів, по учнях, середнє/день. **Сума доходів** — період 1–365 днів, сума PriceCharged (COMPLETED), розбивка по учнях і по валютах |
...
| 12 | **Самостійне тренування (тренер)** | ✅ Реалізовано | Тренер: Тренування → «Моя тренування» → групи м'язів → вправи → вага/повтори; записи в bot_training_data з ChatID тренера (mode SELF) |
