...
**Оновлення:** 02.2026. **Тренування без плану:** після вибору вправ — вибір кількості підходів (одиночна) або кругів (сет) 3/4/5; під час запису немає кнопок «Додати круг»/«Додати підхід»; вправа/сет завершуються автоматично після обраної кількості; «Завершити тренування» показується тільки після завершення вправи/сету; сет: спочатку всі вправи круга, потім наступний круг; після вправи/сету — кнопка «Завершити тренування» замість «Скасувати». **Пошук вправ:** при «Ввести назву» результати тільки з обраної групи (рівень 1 або 1→2, 1→2→3); у повідомленні вказується група. **Тренування учнів:** спочатку список учнів + «Спліт (2 учні)»/«Тріо (3 учні)»; при виборі учня — тренування стартує одразу (без вибору типу Персональна). **План тренувань:** при ручному створенні не питаємо рівень — він береться з картки учня (experience_start_date → getLevelFromExperienceDays); після **Авто-підбір** план відображається одразу з деталями (дні, вправи) та кнопками Додати вправи, Картки вправ, Активувати, Видалити, До списку планів; у перегляді плану — «Картки вправ» → список вправ → відкриття картки вправи з бібліотеки, «Назад до плану»; при додаванні вправ — градація по group_level3. Відображення плану без Markdown (надійність Telegram API). Деталі — **Зміни_логіки_та_функціоналу.md**, розд. 7. — Раніше (21.02.2026): **План тренувань:** ревізія за рівнем, Anti-Repeat, відбір для жінок. — (17.02.2026): Мої учні, Мій тренер, Розклад, Тренування (тип Одиночне/Сет).
...
| 13 | **Почати тренування (учень)** | ✅ Реалізовано | Учень: «Моє тренування» → перевірка активного плану → вибір дня → вправи з sets/reps/rest (тип одиночне/сет з training_plan_exercises), ввід ваги/повторів → запис у bot_training_data; кнопки «Завершити вправу» / «Завершити тренування». lib/training.js: startStudentPlanWorkout, STUDENT_PLAN_DAY, TRAINING_STUDENT_PLAN_INPUT. План вручну: при додаванні вправи — вибір «Кілька підходів (сети)» / «Одиночне виконання» (PLAN_EXERCISE_ADD); зберігання в training_plan_exercises (sets, reps, rest_sec). |
...
**Реалізація в Node/Supabase:** таблиці `training_plans` (plan_id uuid, coach_id, student_id, plan_name, goal, level, split_scheme, days_per_week, is_active, **revision_weeks**, **parent_plan_id**, **activated_at**, valid_until, revision_reminder_sent_at, generation_type) та `training_plan_exercises` (plan_id, exercise_id, …). **lib/supabase.js:** `REVISION_WEEKS_BY_LEVEL` (beginner 10, intermediate 7, advanced 5); `insertTrainingPlan` — проставляє revision_weeks за рівнем або з plan.revisionWeeks, опційно parent_plan_id; `setPlanActiveForStudent` — читає revision_weeks з плану, ставить valid_until = now + revision_weeks*7, activated_at = now; `getPlanExerciseIdsWithVid(planId)` — масив { exerciseId, groupLevel2, vid }; `getPreviousPlanIsolationExerciseIds(studentChatId)` — ID ізоляційних вправ останнього плану (Anti-Repeat Б); `getExerciseUsageCountForStudent(studentChatId)` — { [exerciseId]: count } для зваженого вибору (Anti-Repeat В). **lib/planGenerator.js:** `getSplitSchemeAndDays(level, daysPerWeek, gender)` — для female інший порядок груп і кількість вправ (Ноги, Сідниці); `weightedSample(arr, count, getWeight)`; `pickExercisesForDay(..., options)` — правила А (excludeExerciseIdsByGroup), Б (previousPlanIsolationIds), В (usageCountByExerciseId); `generateTrainingPlan` викликає ці допоміжні функції та передає options. Модулі: lib/trainingPlan.js (UI тренера), lib/training.js (учень, автопрогресія, деавтоматизація), lib/medicalFilter.js (filterExerciseForUser), lib/planRevisionReminders.js + GET /cron/plan-revision. Медичний профіль: lib/medicalProfile.js, user_medical_conditions. Таблиця `plan_adjustments` (міграція) — для логування змін плану.
