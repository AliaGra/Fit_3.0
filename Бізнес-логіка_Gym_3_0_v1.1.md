...
**Оновлення:** 19.05.2026 — **адмін «Користувачі» без дубля після інвайту** (`8ed0a12`); **intro циклу/менопаузи в реєстрації + «Цикл і менопауза» в профілі** (`336510c`); **«Мої вправи»** (`fd40667`); **notify при прив’язці тренера до закладу** (`e92e07a`).  
...
| 14 | **Довідник закладів (venues)** | ✅ Реалізовано (MVP + розширення 2026-05) | **Основний бот:** «Клуби, студії» — гео + текстовий пошук (транслит), фільтр типу організації; хаб з «Мої заклади», пошук нового закладу, deep link `venue_<id>`; картка закладу з тренерами (`PVCH`), deep link `pvch_<chatId>`; прив’язка тренера в профілі; реєстрація — пропозиція обрати заклад. **Нове (05.2026):** notify про новий заклад за `oblast+city(+district)`; **notify при першій прив’язці тренера до закладу** — користувачам з `user_venues` (`COACH_VENUE_NOTIFY_DISABLED=1`); у профілі/реєстрації зберігається `oblast/district`; у картці закладу показується район. **Адмін-бот:** створення/картка закладу, …, **👥 Користувачі** без технічних `INVITE_*`. Деталі: **`VENUES_DIRECTORY_IMPLEMENTATION_PLAN.md`**, **`Зміни_логіки_та_функціоналу.md`**. |
