...
|---|---------------------|--------|----------|
| 1 | **Реєстрація (повна)** | ✅ Реалізовано | Role → (Coach: документи) → FirstName → Continue/Skip → LastName → Gender → Goal → BirthDate → City → (Coach: Instagram) → Finish, Menu |
...
| 7 | **Профіль (перегляд/редагування, заміри)** | ✅ Реалізовано | Перегляд, «Оновити заміри» (вага → талія → стегно → ягодиці → біцепс (розслаблено) → біцепс (напруга), додатково шия/запʼястя/плечі/груди/% жиру), «Редагувати дані» (ім'я, прізвище, місто, зріст, дата народження), історія замірів у measurements_history |
...
| 14 | **Довідник закладів (venues)** | ✅ Реалізовано (MVP + розширення 2026-05) | **Основний бот:** «Клуби, студії» — гео + текстовий пошук (транслит), фільтр типу організації; хаб з «Мої заклади», пошук нового закладу, deep link `venue_<id>`; картка закладу з тренерами (`PVCH`), deep link `pvch_<chatId>`; прив’язка тренера в профілі; реєстрація — пропозиція обрати заклад. **Нове (05.2026):** notify про новий заклад за `oblast+city(+district)`; у профілі/реєстрації зберігається `oblast/district`; у картці закладу показується район. **Адмін-бот:** створення/картка закладу, години, розклад групових, amenities, локальні назви групових, довідник цін (без оплат у основному боті), поле «район». Деталі та кроки: **`VENUES_DIRECTORY_IMPLEMENTATION_PLAN.md`**, зміни по датах — **`Зміни_логіки_та_функціоналу.md`**. |
