...
**Оновлення:** 21.02.2026. **План тренувань:** ревізія за рівнем — `revision_weeks` (beginner 10, intermediate 7, advanced 5 тиж.), при активації плану `valid_until = now + revision_weeks*7`, `activated_at = now`; при створенні плану зберігаються `revision_weeks` та опційно `parent_plan_id`. Anti-Repeat: правило А (вправа не дублюється в різних днях за group_level2), Б (виключення ізоляційних вправ попереднього плану), В (зважений вибір за історією появ). Відбір для жінок: `getSplitSchemeAndDays(..., gender)` — інший порядок груп і кількість вправ. Допоміжні функції: getPlanExerciseIdsWithVid, getPreviousPlanIsolationExerciseIds, getExerciseUsageCountForStudent. — Раніше (17.02.2026): **Мої учні:** кнопку «Почати тренування» прибрано з картки учня; старт — тільки через «Тренування» → «Тренування учнів». **Мій тренер:** блок даних тренера для будь-якої ролі при заповненому coach_id; кнопки «Пошук тренера», «Ввести інвайт код». **Розклад:** день тижня біля дати, ім'я учня в календарі, «Додати слоти на день». **Тренування:** вибір типу «Одиночне»/«Сет» до вибору вправ.
...
| 12 | **Самостійне тренування (тренер)** | ✅ Реалізовано | Тренер: Тренування → **«Моє тренування»** → **спочатку вибір типу:** «Одиночне (1 вправа)» / «Сет (мін. 2 вправи)» → потім групи м'язів → вправи → вага/повтори (одиночне — один підхід, сет — кілька підходів; для сету при «Завершити вправу» перевірка мін. 2 вправ). Записи в bot_training_data з ChatID тренера (mode SELF). Слоти розкладу не створюються. lib/training.js: showExercises, trainingExecutionType SINGLE/SET, askTrainingInputDataSingle / askTrainingInputData, finishExercise. |
