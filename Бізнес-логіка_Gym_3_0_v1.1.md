...
**Оновлення:** 13.08.2026 — **розсилка тренера/закладу (v1):** свої клієнти / потенційні в обраному місті; ліміт 1/добу (Київ); opt-in «Новини у місті» на реєстрації та в профілі; міграція `supabase_migration_broadcast_ads.sql`.  
**Попереднє:** 03.06.2026 — **роль `venue_owner`** (фаза 0, `7c71f39`); **адмін зняття власника без видалення закладу** (`537b296`); **Умови користування** в головному меню (`1207ab4`); **пошук закладів за `city_list`** (`b35ff46`).  
**Попереднє:** 19.05.2026 — **адмін «Користувачі» без дубля після інвайту** (`8ed0a12`); **intro циклу/менопаузи в реєстрації + «Цикл і менопауза» в профілі** (`336510c`); **«Мої вправи»** (`fd40667`); **notify при прив’язці тренера до закладу** (`e92e07a`).  
**Попереднє:** 11.05.2026 — **локація користувача/закладу + розсилка нових закладів:** додано `users.oblast`, `users.district`, `venues.district`; після збереження нового закладу в адмін-боті запускається notify в основному боті з таргетингом за `oblast+city` (та `district`, якщо задано); у реєстрації й профілі після вибору НП — опційний крок «район».  
**Попереднє:** 17.03.2026 — **розклад тренера:** календар (легенда 🟡 неділя/відпустка/вихідний), «Мій розклад» — лічильники та списки на **21 день** (без приставки «7 днів» у назвах), пункт «Всі слоти» прибрано; **«Відмітити тренування»** винесено в меню «Розклад»; у «Зайняті» без кнопок «Скасувати/Перенести» + підказка про відмітку; після підтвердження/відхилення в **«Чекають підтвердження»** залишаємось у цьому перегляді (`State.afterCoachConfirmDecline`).  
...
|---|---------------------|--------|----------|
| 1 | **Реєстрація (повна)** | ✅ Реалізовано | Role (**Тренер** → **Учень**) → (Coach: документи) → FirstName → … → Gender → для **жінки**: intro циклу (**зараз** / **пізніше**) → Goal → … → Finish. Після «пізніше» — дані циклу в **Профіль → Цикл і менопауза** |
| 2 | **Старт / меню** | ✅ Реалізовано | /start, привітання, «Нова реєстрація» / «У мене є код», головне меню тренера, учня та **власника закладу**; **📜 Умови користування** (`MENU_TERMS_OF_USE`) |
...
| 5 | **Інвайт-код («У мене є код»)** | ✅ Реалізовано | Учень вводить код → гейт оферти → activateInvite. **Node:** `replaceInviteWithChatId` — INSERT реального user + перенос FK-даних; заготовка `INVITE_*` → `USED_INVITE_*` (технічний рядок). **Адмін-бот:** у **👥 Користувачі** лише реальні `chat_id` (без `user_id INVITE_%`). **Fallback:** linkCoachByInviteCode. |
...
| 7 | **Профіль (перегляд/редагування, заміри)** | ✅ Реалізовано | Перегляд, «Оновити заміри», «Редагувати дані» (ім'я, прізвище, місто, зріст, дата народження). **Жінка:** **🌸 Цикл і менопауза** — статус, довжина циклу, тривалість, дата, symptom-check. Історія замірів у measurements_history |
| 7d | **Документи тренера** | ✅ Реалізовано | Тренер завантажує документи про освіту у сфері фітнесу (фото/файл); зберігаємо `telegram_file_id`; учень може переглянути у картці тренера; тренер може переглянути/додати/видалити в `Профіль → Мої документи` |
...
| 8 | **Розклад тренувань** | ✅ Реалізовано | Тренер: налаштування шаблону, **зберегти і створити слоти на 21 день**, створення слотів, **автопродовження** cron `/cron/slot-auto-extend` (+7 днів, якщо залишилось <14), **Календар 21 день** (легенда кнопок днів: 🟢 сьогодні, 🟡 неділя або відпустка / вихідний у налаштуваннях, ⬜ без слотів), **Мій розклад** (Календар 1-й, далі фільтри з **лічильниками** майбутніх слотів у вікні **21 день**), **«Відмітити тренування»** у меню **«Розклад»** (підтверджені слоти, ✔️ → COMPLETED). Перенос через **календар**; «Хочу перерву»; «Створити відпустку». Учень: запис (календар 21 день: 🟡/🔵), «Мій розклад», «Змінити запис». Перенос: REQUESTED у БД, SCH_CR_OK/NO. **Нагадування** — `/cron/reminders` |
...
| 14 | **Довідник закладів (venues)** | ✅ Реалізовано (MVP + розширення 2026-05–06) | **Основний бот:** «Клуби, студії» — гео-пошук; **пошук за областю/НП з `city_list`** (`VEN_LOC_OBL`, `VEN_LOC_CIT`); **пошук за назвою**; фільтри studio/section/group; хаб «Мої заклади»; deep link `venue_<id>`; картка з тренерами (`PVCH`) та **груповими** (`coachGroupPublic`); notify нового закладу / тренера в закладі. **Адмін-бот:** картка закладу, **👑 Призначити власника** (`ADM_VOWN`), **👑 Зняти власника** (`ADM_VOWN_RM` — заклад не видаляється). Деталі: **`CHANGELOG.md`**, **`Зміни_логіки_та_функціоналу.md`**. |
| 15 | **Мої вправи (персональна бібліотека)** | ✅ Реалізовано | `💪 Тренування → ⭐ Мої вправи`: додати/переглянути власні вправи (`user_custom_exercises`). Ручний план — підбір каталогу + custom (`custom_exercise_id`). Міграція `supabase_migration_user_custom_exercises.sql`. Імпорт з каталогу в бібліотеку — ще не в UI. |
| 16 | **Власник закладу (`venue_owner`)** | ✅ Фаза 0 | Реєстрація **🏢 Власник закладу**; прив’язка через адмін (`venue_managers`). Меню: **Мій заклад** (публічна картка), **Тренери закладу**, **Клуби, студії**, **Зв’язок з розробником**. Редагування контактів/групових (service role). Тренери: `listing_visible`. Міграція `supabase_migration_venue_managers.sql`. |
| 17 | **Умови користування** | ✅ Реалізовано | **📜 Умови користування** у головному меню всіх ролей; текст за роллю (`lib/termsOfUse.js`), інформативно (без оплати в боті). |
| 18 | **Розсилка (тренер / заклад)** | ✅ Реалізовано (v1) | Меню **📣 Розсилка**. Тренер: 1/акаунт/добу (Київ); свої (усім / активним / сплячим) без фільтра міста; потенційні = solo + архівовані чужі, **одне місто** (профіль або клуб). Заклад: 1/заклад/добу; свої = `user_venues`; потенційні = учні міста (навіть з тренером), хто ще не додав заклад. Текст ≤400 + кнопка картки тренера (`PVCH`) / закладу (`VENUES_CARD`). Opt-out лише на потенційних (`users.ads_opt_in`). Реєстрація: обов’язкове Так/Ні; профіль — зміна. Архів і `INVITE_*` не слати. `lib/broadcast.js`, `broadcast_log`. |
...
- Для тренера додатково: документи про освіту у сфері фітнесу (опційно), Instagram
- **Результат:** UserID = ChatID = Telegram ChatID (одразу реальний)
- **CoachID:** порожній (учень без тренера)

> **Примітка (тестовий режим / closed beta):** на етапі закритого бета‑тестування кнопка **«✅ Нова реєстрація»** може бути **заблокована**. У цьому режимі бот показує повідомлення про тестування та посилання на підтримку/розробника, а доступ надається лише через **інвайт‑код** (Тип Б).
...
- **Перед активацією профілю** учень має прийняти умови оферти (кнопка **«✅ Приймаю умови»**). Текст оферти зберігається у файлі `OFERTA.md` і доступний через кнопку **«📋 Читати угоду»**.
- Система активує профіль **лише після акцепту** (заміна INVITE_ на реальний ChatID)
...
**Крок 1a (Node / closed beta): гейт оферти перед активацією**
- Після того як код `INVITE_XXXX` **валідний** і **існує в БД**, бот показує екран з діями:
  - **`📋 Читати угоду`** → надсилає текст з `OFERTA.md` (в одному повідомленні) + кнопки виходу (**💬 Написати розробнику**, **🏠 Головне меню**)
  - **`✅ Приймаю умови`** → лише після цього виконується активація інвайту
  - **`❌ Відмовитись`** → “Доступ не активовано.” і повернення на старт

> Примітка: натискання “✅ Приймаю умови” вважається акцептом, навіть якщо користувач не натискав “Читати угоду”.
...
**Крок 4: Заміна INVITE_ на реальний ChatID (Node / Supabase)**

> **Node (актуально):** `replaceInviteWithChatId` **не** перезаписує рядок інвайту на місці (через FK). Створюється **новий** user з `user_id=chat_id=Telegram ID`, залежні таблиці переносяться з `INVITE_*`, заготовка позначається `chat_id=USED_INVITE_*`. У **адмін-боті → Користувачі** технічні рядки `user_id INVITE_%` не показуються; активні коди — у **Інвайти**.

```javascript
// Функція: Sheets.replaceInviteWithChatId(inviteCode, realChatId) — історичний GAS-еталон
...
### 2.X Зв’язок з розробником (підтримка)

**Мета:** дати користувачу (тренеру/учню) простий канал для зв’язку з розробником і доступ до тексту оферти.

**UI (головне меню тренера та учня):**
- Кнопка: `Зв’язок з розробником`

**Підменю:**
- `💬 Написати розробнику` → відкриває support-бот: `https://t.me/FitHad_helpbot`
- `📄 Читати оферту` → показує текст з `OFERTA.md` (одним повідомленням) + кнопки виходу:
  - `💬 Написати розробнику`
  - `🏠 Головне меню`
...
4. **Ваші дані:** підсумок, кнопки [Змінити] [Зберегти і створити слоти на найближчі 21 день] [Підказки]. Змінити — підменю: день відпочинку / тривалість тренування / початок і кінець робочого дня. Після збереження+створення слотів — пункт 1 чекліста «Підказки» (розклад і слоти) відмічається. Окремого кроку «Створити слоти» в Підказках немає. **Автопродовження:** cron `GET /cron/slot-auto-extend` (раз на добу): якщо до останнього слота залишилось менше 14 днів — додає ще 7 днів і повідомляє тренера.
...
> **Статус:** не використовується (історичний розділ). Запис на тренування повністю реалізовано у FIT 3.0 всередині бота, без Google Calendar.
