...
**Перенос:** Тренер обирає слот з учнем → [🔄 Перенести] → **календар 21 день** → обирає дату → слоти дня → обирає новий слот. Перед відправкою учню новий слот зберігається як **REQUESTED** + `studentId` (щоб запит не загубився). Inline-кнопки учня — **короткі** `SCH_SR_OK:{newSlotId}` / `SCH_SR_NO:{newSlotId}` (Telegram обмежує `callback_data` 64 байтами; довгий варіант з двома UUID не відправлявся). Учень підтверджує або відхиляє; після відповіді — підменю «Розклад». Після вибору слота тренер повертається до **календаря**.
