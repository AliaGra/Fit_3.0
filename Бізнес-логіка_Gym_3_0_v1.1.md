...
| 14 | **Довідник закладів (venues)** | ✅ Реалізовано (MVP + розширення 2026-05–06) | **Основний бот:** «Клуби, студії» — гео-пошук; **пошук за областю/НП з `city_list`** (`VEN_LOC_OBL`, `VEN_LOC_CIT`); **пошук за назвою**; фільтри studio/section/group; хаб «Мої заклади»; deep link `venue_<id>`; картка з тренерами (`PVCH`) та **груповими** (`coachGroupPublic`); notify нового закладу / тренера в закладі. **Адмін-бот:** картка закладу, **👑 Призначити власника** (`ADM_VOWN`), **👑 Зняти власника** (`ADM_VOWN_RM` — заклад не видаляється). Деталі: **`WORKLOG.md`**, **`Зміни_логіки_та_функціоналу.md`**. |
