...
**Оновлення:** 21.03.2026 — календар 21 день, перенос через календар, «Хочу перерву», зони акценту авто-перехід, BMI-ліміти по статі/віку, fallback floor ваги, конфіденційність підлітків, fix invite FK-constraint, SCH_CR_OK/NO; у календарі учня (запис/перенос): 🟡 REQUESTED, 🔵 BOOKED, легенда кольорів.  
...
| 8 | **Розклад тренувань** | ✅ Реалізовано | Тренер: налаштування шаблону, створення слотів, **Календар 21 день** (без року у датах), Мій розклад (Календар 1-й, потім фільтри 7 днів), перенос через **календар** (не плоский список), «Хочу перерву» (раніше Резерв), «Створити відпустку». Учень: записатись (календар 21 день: 🟡 очікує підтвердження, 🔵 підтверджено, легенда), «Мій розклад» — чистий список, «🔄 Змінити запис» — окреме підменю. Перенос: спочатку записується в БД (REQUESTED), потім сповіщення. Короткі callbacks SCH_CR_OK/NO щоб не перевищити 64б. **Нагадування учням** — `/cron/reminders` |
...
- **Оновлено (Mar 2026):**
  ...
  - Фільтри «7 днів (зайняті)» і «7 днів (резерв)» — підказки прибрані
