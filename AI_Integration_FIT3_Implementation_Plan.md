...
## ЧАСТИНА 6. Функція 4 — Аналіз замірів тіла та типу фігури

Мета: після заповнення замірів (вага, зріст, талія, стегна/ягодиці, плечі, груди, % жиру) надати учню / тренеру пояснення стану тіла, типу фігури та безпечні рекомендації.

| Крок | Дія | Деталі | Файли |
|------|-----|--------|--------|
| 6.1 | Обчислення WH та ІМТ | На основі полів height, weight, waist порахувати WH-індекс (талія/зріст) та ІМТ. Повернути об’єкти `whStatus` і `bmiStatus` з рівнем, прапорцями blocked/notifyCoach/requireDoctorConfirmation та текстовими підказками. | `lib/bodyMetrics.js`, `lib/ai/bodyAnalysis.js` |
| 6.2 | Класифікація типу фігури | На основі shoulders, chest, waist, glutes, height порахувати коефіцієнти (плечі/ягодиці, ягодиці/талія, груди/талія тощо) та визначити тип фігури окремо для жінок і чоловіків (apple, pear, hourglass, v_shape тощо). Повернути `bodyType` з label, description, priority (upper/lower/chest, phase) та ratios. Якщо бракує хоча б одного ключового заміру — type = null, reason = 'insufficient_data'. | `lib/bodyType.js`, `lib/ai/bodyAnalysis.js` |
| 6.3 | Формування bodyStatus | Побудувати об’єкт `bodyStatus` з полями: профіль (gender, age, goal), заміри (heightCm, weightKg, waistCm, hip/glutes/shoulders/chest/bodyFatPct), `whStatus`, `bmiStatus`, `bodyType`, `missingCore` (список відсутніх ключових замірів). | `lib/ai/bodyAnalysis.js` |
| 6.4 | Промпт для AI | Додати шаблон `BODY_ANALYSIS` у `SYSTEM_PROMPTS` та `USER_TEMPLATES.BODY_ANALYSIS` в `aiPrompts.js`. У промпті описати правила: не змінювати числа, не ставити діагнозів, при missingCore — чесно сказати, що аналіз обмежений; при наявному bodyType — описати тип фігури та пріоритети тренувань. | `lib/ai/aiPrompts.js` |
| 6.5 | Тригери виклику | Викликати `bodyAnalysis.generateAndSend(chatId, scenario, measurements)` у трьох сценаріях: (1) після `finishCreateStudentByInvite` (coach.js) — аналітика тренеру; (2) після `activateInvite` у `REG_INVITE_INPUT` — аналітика учню при вході з кодом; (3) після `finishRegistration` для самореєстрації учня — аналітика учню, якщо є достатньо даних. | `lib/coach.js`, `lib/registration.js` |
| 6.6 | Fallback та безпека | Якщо AI вимкнено або є помилка — нічого не відправляти, бот працює як раніше. Якщо бракує критичних замірів (height/weight/waist) — замість AI-відповіді надсилати підказку типу «Для детальної аналітики тіла потрібно вказати: …». | `lib/ai/bodyAnalysis.js` |

**Критерій завершення частини 6:** після введення замірів у підтримуваних сценаріях бот надсилає учню/тренеру зрозумілий аналіз тіла, тип фігури та рекомендації; при вимкненому AI або нестачі даних — відправляється лише пояснювальне повідомлення без виклику моделі.

---

## ЧАСТИНА 7. Кеш, бюджет, моніторинг (опційно)

| Крок | Дія | Деталі |
|------|-----|--------|
| 7.1 | Кеш по prompt_hash | Перед викликом AI обчислити hash промпту (наприклад SHA256 від system+user). Перевірити ai_generated_content по prompt_hash; якщо є запис за останні N годин — повернути збережену відповідь. Корисно для однакових коментарів до типових планів. |
| 7.2 | Місячний бюджет | Змінна AI_MONTHLY_BUDGET (наприклад 50). Перед кожним викликом (або раз на день) підраховувати SUM(cost_usd) за поточний місяць; якщо >= бюджету — не викликати API, fallback. |
| 7.3 | Алерти | Опційно: при щоденних витратах > $2 або error rate > 5% писати в лог або відправляти повідомлення адміну. |
