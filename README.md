...
**Оновлення документації:** 17.03.2026
...
| **Зміни_логіки_та_функціоналу.md** | Хронологія змін логіки та функціоналу по проєкту |
...
**Розклад тренера — «Мій розклад» → «Вільні слоти»:** за замовчуванням показуються **вільні слоти на 3 дні** (від поточної дати) як **текстовий список** (групування по днях, як у **«Зайняті слоти»**); інлайн-кнопок по кожному вільному слоту **немає**. Якщо є слоти далі — доступна кнопка **«Інші вільні слоти»**, яка відкриває решту вільних слотів у вікні 21 день. Деталі: **Бізнес-логіка_Gym_3_0_v1.1.md** §4.4.7; матриця callback: **CALLBACK_FSM_MODULE_MATRIX.md** (блоки «Мій розклад», «Налаштування шаблону», «Розклад»); код: `lib/schedule.js` → `showCoach7DaysView`, `filter === 'available'`.

**Налаштування шаблону — різний час по днях:** §4.4.5 у **Бізнес-логіка_Gym_3_0_v1.1.md**, таблиця callback у **CALLBACK_FSM_MODULE_MATRIX.md**.
